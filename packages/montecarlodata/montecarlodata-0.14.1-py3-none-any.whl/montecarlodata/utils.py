...
class Wrapper:
    def __init__(self, abort_on_error: Optional[bool] = True, disable_handle_errors: Optional[bool] = False):
        self._abort_on_error = abort_on_error
        self._disable_handle_errors = disable_handle_errors


class GqlWrapper(Wrapper):
    def __init__(self, mcd_id: str, mcd_token: str, **kwargs):
        self._mcd_id = mcd_id
    ...
    def make_request_v2(self, query: str, operation: str, variables: Optional[Dict] = None) -> MonolithResponse:
        """
        Make a GraphQl request to the MCD API and parse out the results into data and errors. Aborts on error if set.
        """
        response = self._make_request(query=query, variables=self.convert_snakes_to_camels(variables) if variables else variables)
...
class AwsClientWrapper(Wrapper):
    ...
    @manage_errors
    def create_stack(self, stack_name: str, template_link: str, termination_protection: bool,
        ...
        return self._wait_on_stacks(stack_id=stack_id, waiter=self._CF_CREATE_WAITER,
    ...
    def upgrade_stack(self, stack_id: str, template_link: str, parameters: List[Dict]) -> bool:
        ...
        return self._wait_on_stacks(stack_id=stack_id, waiter=self._CF_UPDATE_WAITER,
    ...
    @manage_errors
    def get_emr_cluster_details(self, created_after: Optional[str] = None, states: Optional[List] = None
                                ) -> Iterable[Dict]:
        """
        ...
        if states:
            params['ClusterStates'] = [mapped for state in states for mapped in self._EMR_STATES[state]]
    ...
    def get_bucket_location(self, bucket_name: str) -> str:
        ...
        """
        region = self._session.client('s3').get_bucket_location(Bucket=bucket_name)['LocationConstraint']
        if region is None:
            return settings.DEFAULT_AWS_REGION  # Buckets in us-east-1 have a LocationConstraint of null
        return region
    ...
    def create_sns_topic(self, name: str) -> str:
        ...
        """
        return self._session.client('sns').create_topic(Name=name)['TopicArn']  # This action is idempotent
    ...
    def set_topic_attributes(self, arn: str, name: str, value: str) -> None:
        ...
        """
        return self._session.client('sns').set_topic_attributes(TopicArn=arn, AttributeName=name, AttributeValue=value)
    ...
    def get_q_attributes(self, name: str, attributes: List[str]) -> Box:
        ...
        """
        return self._session.client('sqs').get_queue_attributes(QueueUrl=self.get_queue_url(name), AttributeNames=attributes)

    def set_q_attributes(self, name: str, attributes: List[str]) -> Box:
        """
        Set attributes for a sqs
        """
        return self._session.client('sqs').set_queue_attributes(QueueUrl=self.get_queue_url(name), Attributes=attributes)
    ...
    @boxify
    def get_bucket_event_config(self, name: str, ) -> Box:
        """
    ...
    def list_topic_subscriptions(self, arn: str) -> List:
        ...
        """
        for page in self._session.client('sns').get_paginator('list_subscriptions_by_topic').paginate(TopicArn=arn):
            if page and page.get('Subscriptions'):
                yield page['Subscriptions']

    def subscribe_to_topic(self, arn: str, endpoint: str, protocol: Optional[str] = 'sqs',
                           attributes: Optional[Dict] = None) -> None:
        """
        Subscribe to a topic
        """
        self._session.client('sns').subscribe(TopicArn=arn, Endpoint=endpoint, Protocol=protocol, Attributes=attributes)
    ...
    def _wait_on_stacks(self, stack_id: str, waiter: str, success_state: str) -> bool:
        ...
        """
        self._session.client('cloudformation').get_waiter(waiter).wait(StackName=stack_id,
