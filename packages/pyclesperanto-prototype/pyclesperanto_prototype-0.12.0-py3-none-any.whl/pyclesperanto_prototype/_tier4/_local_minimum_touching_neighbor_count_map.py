...
def local_minimum_touching_neighbor_count_map(input : Image, destination : Image = None):
    ...
    local_minimum_vector = minimum_of_touching_neighbors(number_of_touching_neighbors_vector, touch_matrix)

    # ignore how many objects touch the background
    set_column(number_of_touching_neighbors_vector, 0, 0)
    replace_intensities(input, local_minimum_vector, destination)
    return destination
