...
from typing import Type, Generator, Optional, Dict, Callable, Union
...
class RED:
    ...
    def add_in(self,
        ...
        :param handler: awaitable which will be called when event will be received
        ...
        # # get or create necessary consumer
        # if via == Via.LIST:
        #     consumer = RedisListConsumer(red=self, **kwargs)
        #     self.in_registry[Via.LIST][kwargs['list_name']] = consumer
        #
        # elif via == Via.CHANNELS:
        #     consumer = RedisChannelConsumer(red=self, **kwargs)
        #     self.in_registry[Via.CHANNELS][kwargs['channel_name']] = consumer
        #
        # elif via == Via.STREAMS:
        #     stream_name = kwargs['stream_name']
        #     group_name = kwargs['group_name']
        #     consumer_name = kwargs['consumer_name']
        #
        #     consumer = RedisStreamsConsumer(red=self, **kwargs)
        #     self.in_registry[Via.STREAMS][(stream_name, group_name, consumer_name)] = consumer
        #
        # else:
        #     raise RedError(f"Can't register incoming event. Unknown `Via`: {via}")
