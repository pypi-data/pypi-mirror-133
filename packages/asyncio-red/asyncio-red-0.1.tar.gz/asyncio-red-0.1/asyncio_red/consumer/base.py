...
from asyncio_red.errors import InvalidEventError, EventHandlingError, MalformedEventError
...
class BaseConsumer:
    ...
    async def _handle_event(self, header: Header, event: BaseEvent) -> Any:
        ...
        event_name = header.name
        handler = self._registry[event_name]["handler"]
        if not handler:
...
class ListChannelConsumer(BaseConsumer):

    async def _process_message(self, redis_message):
        # get raw event
        try:
            raw_event = self._get_raw_event(redis_message)
