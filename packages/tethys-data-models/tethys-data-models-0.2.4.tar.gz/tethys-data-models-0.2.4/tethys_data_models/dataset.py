...
class ChunkParams(BaseModel):
    geometry_count: conint(gt=0) = Field(..., description='The number of geometries that should be chunked together. If the result_type is grid, then the geometry_count must be the square of another number. I.e. it must represent the area calc of a square (length * length).')
    time_interval: conint(gt=0) = Field(None, description='The chunk time interval is the number of days the time chunk should cover (e.g. 365 would be about a year). A value of None indicates that there is no chunking along the time axis.')
    height_interval: conint(gt=0) = Field(None, description='The chunk height interval is the interval of height indeces that the height should be chunked. A value of None indicates that there is no chunking along the height axis.')
...
class ChunkID(BaseModel):
    ...
    """
    heights_index: List[conint(ge=0)] = Field(None, description='The index locations (starting with 0) of the heights array of the results. Should be omitted if the chunk should include all heights.')
