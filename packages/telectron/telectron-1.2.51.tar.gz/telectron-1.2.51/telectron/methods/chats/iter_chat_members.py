...
class IterChatMembers(Scaffold):
    async def iter_chat_members(
        ...
        This convenience method does the same as repeatedly calling :meth:`~telectron.Client.get_chat_members` in a loop,
        ...
        Example:
            .. code-block:: python
                ...
                for member in app.iter_chat_members("telectronchat"):
                    print(member.user.first_name)

                # Iterate though all administrators
                for member in app.iter_chat_members("telectronchat", filter="administrators"):
                    print(member.user.first_name)

                # Iterate though all bots
                for member in app.iter_chat_members("telectronchat", filter="bots"):
                    print(member.user.first_name)
        """
        current = 0
        yielded = set()
