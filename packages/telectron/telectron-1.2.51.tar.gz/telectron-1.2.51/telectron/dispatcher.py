...
class Dispatcher:
    ...
    def __init__(self, client: "telectron.Client"):
        ...
        async def message_parser(update, users, chats):
            return await telectron.types.Message._parse(
        ...
        async def callback_query_parser(update, users, chats):
            return await telectron.types.CallbackQuery._parse(self.client, update, users), CallbackQueryHandler

        async def user_status_parser(update, users, chats):
            return telectron.types.User._parse_user_status(self.client, update), UserStatusHandler

        async def inline_query_parser(update, users, chats):
            return telectron.types.InlineQuery._parse(self.client, update, users), InlineQueryHandler

        async def poll_parser(update, users, chats):
            return telectron.types.Poll._parse_update(self.client, update), PollHandler

        async def chosen_inline_result_parser(update, users, chats):
            return telectron.types.ChosenInlineResult._parse(self.client, update, users), ChosenInlineResultHandler

        async def chat_member_updated_parser(update, users, chats):
            return telectron.types.ChatMemberUpdated._parse(self.client, update, users, chats), ChatMemberUpdatedHandler
            ...
            try:
                ...
                async with lock:
                    for group in self.groups.values():
                        for handler in group:
                            ...
                            try:
                                ...
                                raise
                            except telectron.ContinuePropagation:
                                continue
                            except Exception as e:
                                log.error(e, exc_info=True)
                    ...
                    for key, event in self.waiting_events.items():
                        if event.handler_type == handler_type:
                            try:
                                filters_passed = await event.handler_type.execute_filters(
                                    event.filters, self.client, parsed_update
                                )
                                if filters_passed:
                                    async with self.waiting_events[key].lock:
                                        if not self.waiting_events[key].update:
                                            self.waiting_events[key].update = update
                                            self.waiting_events[key].event.set()
                            except Exception as e:
                                log.error(e, exc_info=True)
                                continue

                        elif event.handler_type == RawUpdateHandler:
                            async with self.waiting_events[key].lock:
                                if not self.waiting_events[key].update:
                                    self.waiting_events[key].update = update
                                    self.waiting_events[key].event.set()
