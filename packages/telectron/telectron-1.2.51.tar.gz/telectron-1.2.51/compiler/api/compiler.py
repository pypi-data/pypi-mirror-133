...
DESTINATION_PATH = Path("telectron/raw")
NOTICE_PATH = "NOTICE"
...
def get_docstring_arg_type(t: str, is_list: bool = False, is_telectron_type: bool = False):
    if t in CORE_TYPES:
        if t == "long":
            return "``int`` ``64-bit``"
        elif "int" in t:
        ...
        return "Any object from :obj:`~telectron.raw.types`"
    elif t == "!X":
        return "Any method from :obj:`~telectron.raw.functions`"
    elif t.lower().startswith("vector"):
        return "List of " + get_docstring_arg_type(t.split("<", 1)[1][:-1], True)
    else:
        return f":obj:`{t} <telectron.raw.base.{t}>`"
...
def get_references(t: str, kind: str):
    ...
    if t:
        return "\n            ".join(
            f"- :obj:`{i} <telectron.raw.functions.{i}>`"
            for i in t
        ), len(t)
...
def start(format: bool = False):
    ...
    for qualtype in types_to_constructors:
        ...
        items = "\n            ".join([f"- :obj:`{c} <telectron.raw.types.{c}>`" for c in constructors])
        ...
        for i, arg in enumerate(sorted_args):
            ...
            docstring_args.append(
                "{}{}: {}".format(
                    ...
                    get_docstring_arg_type(arg_type, is_telectron_type=c.namespace == "telectron")
                )
            )

        if c.section == "types":
            docstring += f"This object is a constructor of the base type :obj:`~telectron.raw.base.{c.qualtype}`.\n\n"
        else:
    ...
    with open(DESTINATION_PATH / "all.py", "w", encoding="utf-8") as f:
        ...
        for c in combinators:
            f.write(f'\n    {c.id}: "telectron.raw.{c.section}.{c.qualname}",')
