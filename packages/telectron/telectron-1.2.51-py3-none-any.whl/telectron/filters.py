...
import telectron
from telectron.types import Message, CallbackQuery, InlineQuery, InlineKeyboardMarkup, ReplyKeyboardMarkup, Update


class Filter:
    async def __call__(self, client: "telectron.Client", update: Update):
...
def create(func: Callable, name: str = None, **kwargs) -> Filter:
    ...
    Parameters:
        func (``callable``):
            ...
            The *client* argument refers to the :obj:`~telectron.Client` that received the update.
            The *update* argument type will vary depending on which `Handler <handlers>`_ is coming from. 
            For example, in a :obj:`~telectron.handlers.MessageHandler` the *update* argument will be a :obj:`~telectron.types.Message`; in a :obj:`~telectron.handlers.CallbackQueryHandler` the *update* will be a :obj:`~telectron.types.CallbackQuery`.
        ...
        **kwargs (``any``, *optional*):
            ...
            :meth:`~telectron.filters.command` or :meth:`~telectron.filters.regex`.
    """
    return type(
...
"""Filter messages that contain :obj:`~telectron.types.Audio` objects."""
...
"""Filter messages that contain :obj:`~telectron.types.Document` objects."""
...
"""Filter messages that contain :obj:`~telectron.types.Photo` objects."""
...
"""Filter messages that contain :obj:`~telectron.types.Sticker` objects."""
...
"""Filter messages that contain :obj:`~telectron.types.Animation` objects."""
...
"""Filter messages that contain :obj:`~telectron.types.Game` objects."""
...
"""Filter messages that contain :obj:`~telectron.types.Video` objects."""
...
"""Filter messages that contain :obj:`~telectron.types.Voice` note objects."""
...
"""Filter messages that contain :obj:`~telectron.types.VideoNote` objects."""
...
"""Filter messages that contain :obj:`~telectron.types.Contact` objects."""
...
"""Filter messages that contain :obj:`~telectron.types.Location` objects."""
...
"""Filter messages that contain :obj:`~telectron.types.Venue` objects."""
...
"""Filter messages that contain :obj:`~telectron.types.Poll` objects."""
...
"""Filter messages that contain :obj:`~telectron.types.Dice` objects."""
...
def command(commands: Union[str, List[str]], prefixes: Union[str, List[str]] = "/", case_sensitive: bool = False):
    ...
    Parameters:
        commands (``str`` | ``list``):
            ...
            field of the :obj:`~telectron.types.Message`.
    ...
    async def func(flt, client: telectron.Client, message: Message):
        # Username shared among all commands; used for mention commands, e.g.: /start@username
        global username

        if username is None:
            try:
                username = client.cached_me.username or ""
