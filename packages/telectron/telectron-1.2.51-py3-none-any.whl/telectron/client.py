...
from telectron.storage import Storage, FileStorage, MemoryStorage
from telectron.types import User, TermsOfService
from telectron.utils import ainput
from .dispatcher import Dispatcher
from .file_id import FileId, FileType, ThumbnailSource
from .scaffold import Scaffold
from telectron.raw.types.updates import State
...
class Client(Methods, Scaffold):
    ...
    Parameters:
        ...
        app_version (``str``, *optional*):
            Application version. Defaults to "telectron |version|".
            This is an alternative way to set it if you don't want to use the *config.ini* file.
        ...
        workdir (``str``, *optional*):
            Define a custom working directory. The working directory is the location in your filesystem where telectron
    ...
    def __init__(
        ...
        self.state = self.parse_state(last_state or '')
        self.cached_me = None
    ...
    def parse_state(state: str):
        if len(state) == 0:
            return
        string = base64.b64decode(state).decode()
        version, other = string.split(':', 1)
        if version == 'v1':
            pts, qts, date, seq, unread_count = other.split(':')
            return State(
                pts=int(pts), 
                qts=int(qts), 
                date=int(date), 
            ...
            raise NotImplementedError(f'State version {version} not implemented')

    @staticmethod
    def unparse_state(s: State):
        return base64.b64encode(f'v1:{s.pts}:{s.qts}:{s.date}:{s.seq}:{s.unread_count}'.encode()).decode()

    def update_state(self, new_state: State):
        if self.state is None or new_state.pts > self.state.pts:
            self.state = new_state
            self.on_state_update(self.unparse_state(new_state))

    def update_state_args(self, pts=None, date=None):
        if self.state is not None:
            edited = False
            if pts is not None and pts > self.state.pts:
                self.state.pts = pts
                edited = True
            if date is not None and date > self.state.date:
                self.state.date = date
                edited = True
            if edited:
                self.on_state_update(self.unparse_state(self.state))

    @staticmethod
    def on_state_update(new_state: str):
        pass
    ...
    def load_config(self):
        ...
        if self.bot_token:
            ...
            self.bot_token = parser.get("telectron", "bot_token", fallback=None)

        for option in ["api_id", "api_hash", "app_version", "device_model", "system_version", "lang_code"]:
            if getattr(self, option):
                pass
            else:
                if parser.has_section("telectron"):
    ...
    async def get_file(
        ...
        try:
            ...
            if not isinstance(e, telectron.StopTransmission):
                log.error(e, exc_info=True)
