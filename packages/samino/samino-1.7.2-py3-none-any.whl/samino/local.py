...
class Local:
    ...
    def claim_video_rep(self, chatId: str):
        ...
        if int(reputation) < 1: return CheckExceptions \
            (f"reputation should be more than 1 (Your rep Now {int(reputation)}")
    ...
    def get_chat_threads(self, start: int = 0, size: int = 25):
        ...
        if req.status_code != 200: return CheckExceptions(req.json())
        else: return ThreadList(req.json()["threadList"])
    ...
    def get_chat_messages(self, chatId: str, size: int = 25):
        ...
        if req.status_code != 200: return CheckExceptions(req.json())
        return MessageList(req.json()["messageList"])
    ...
    def get_wall_comments(self, userId: str, sorting: str, start: int = 0, size: int = 25):
        ...
        req = requests.get \
            (api(f"/x{self.comId}/s/user-profile/{userId}/comment?sort={sorting}&start={start}&size={size}"), headers=self.headers)
        if req.status_code != 200: return CheckExceptions(req.json())
    ...
    def unfollow(self, userId: str):
        req = requests.post(api(f"/x{self.comId}/s/user-profile/{userId}/member/{self.uid}"), headers=self.headers)
        if req.status_code != 200: return CheckExceptions(req.json())
        return Json(req.json())
    ...
    def get_blogs_all(self, start: int = 0, size: int = 25, pagingType: str = "t"):
        ...
        if req.status_code != 200: return CheckExceptions(req.json())
        else: return RecentBlogs(req.json()["blogList"])
