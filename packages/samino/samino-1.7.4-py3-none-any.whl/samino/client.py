...
class Client:
    ...
    def get_from_link(self, link: str):
        ...
        if "OK" not in req.json()["api:message"]: return CheckExceptions(req.json())
        return FromCode(req.json()["linkInfoV2"]["extensions"])
    ...
    def get_member_following(self, userId: str, start: int = 0, size: int = 25):
        ...
        if "OK" not in req.json()["api:message"]: return CheckExceptions(req.json())
        return UserProfileList(req.json()["userProfileList"])
    ...
    def get_member_visitors(self, userId: str, start: int = 0, size: int = 25):
        ...
        if "OK" not in req.json()["api:message"]: return CheckExceptions(req.json())
        return VisitorsList(req.json()["visitors"])
    ...
    def get_wall_comments(self, userId: str, sorting: str, start: int = 0, size: int = 25):
        ...
        if "OK" not in req.json()["api:message"]: return CheckExceptions(req.json())
        return CommentList(req.json()["commentList"])

    def send_message(self, chatId: str, message: str = None, messageType: int = 0, replyTo: str = None, stickerId: str = None, mentionUserIds: list = None, embedId: str = None, embedType: int = None, embedLink: str = None, embedTitle: str = None, embedContent: str = None, embedImage: BinaryIO = None, file: BinaryIO = None, fileType: str = None):
    ...
    def get_community_info(self, comId: str):
        req = requests.get \
            (api(f"/g/s-x{comId}/community/info?withInfluencerList=1&withTopicList=true&influencerListOrderStrategy=fansCount"), headers=self.headers)
        if "OK" not in req.json()["api:message"]: return CheckExceptions(req.json())
    ...
    def get_chat_messages(self, chatId: str, start: int = 0, size: int = 25):
        ...
        if "OK" not in req.json()["api:message"]: return CheckExceptions(req.json())
        return GetMessages(req.json()["messageList"])
    ...
    def get_user_info(self, userId: str):
        ...
        if "OK" not in req.json()["api:message"]: return CheckExceptions(req.json())
        return UserProfile(req.json()["userProfile"])
    ...
    def get_chat_members(self, start: int = 0, size: int = 25, chatId: str = None):
        ...
        if "OK" not in req.json()["api:message"]: return CheckExceptions(req.json())
        return UserProfileList(req.json()["memberList"])
    ...
    def get_from_id(self, id: str, comId: str = None, objectType: int = 2):  # never tried
        ...
        **Returns**
        ...
        req = requests.post(url, headers=headers.Headers(data=data).headers, data=data)
        if "OK" not in req.json()["api:message"]: return CheckExceptions(req.json())
        return FromCode(req.json()["linkInfoV2"]["extensions"]["linkInfo"])
    ...
    def watch_ad(self, uid: str = None):
        ...
        req = requests.post(tapjoy, json=data, headers=headers.AdHeaders().headers)
