...
class Acm:
    ...
    def promote(self, userId: str, rank: str):
        ...
        req = requests.post(api(f"/x{self.comId}/s/user-profile/{userId}/{rank}"), headers=self.headers)
        if req.status_code != 200: return CheckExceptions(req.json())
        else: return Json(req.json())

    def accept_join_request(self, userId: str):
        req = requests.post(api(f"/x{self.comId}/s/community/membership-request/{userId}/accept"), headers=self.headers)
        if req.status_code != 200: return CheckExceptions(req.json())
        else: return Json(req.json())

    def reject_join_request(self, userId: str):
        req = requests.post(api(f"/x{self.comId}/s/community/membership-request/{userId}/reject"), headers=self.headers)
        if req.status_code != 200: return CheckExceptions(req.json())
        else: return Json(req.json())
    ...
    def change_welcome_message(self, message: str, enabled: bool = True):
        ...
        req = requests.post(api(f"/x{self.comId}/s/community/configuration"), data=data, headers=self.headers)
        if req.status_code != 200: return CheckExceptions(req.json())
        else: return Json(req.json())

    def change_guidelines(self, content: str):
        data = json.dumps({"content": content})
        req = requests.post(api(f"/x{self.comId}/s/community/guideline"), headers=self.headers, data=data)
        if req.status_code != 200: return CheckExceptions(req.json())
        else: return Json(req.json())
    ...
    def edit_community(self, name: str = None, description: str = None, aminoId: str = None, lang: str = None, themePackUrl: str = None):
        ...
        req = requests.post(api(f"/x{self.comId}/s/community/settings"), data=data, headers=self.headers)
        if req.status_code != 200: return CheckExceptions(req.json())
        else: return Json(req.json())

    def get_community_stats(self):
        req = requests.get(api(f"/x{self.comId}/s/community/stats"), headers=self.headers)
        if req.status_code != 200: return CheckExceptions(req.json())
        else: return CommunityStats(req.json()["communityStats"])
    ...
    def get_admin_stats(self, type: str, start: int = 0, size: int = 25):
        ...
        req = requests.get(api(f"/x{self.comId}/s/community/stats/moderation?type={type}&start={start}&size={size}"), headers=self.headers)
        if req.status_code != 200: return CheckExceptions(req.json())
        else: return Json(req.json())

    def get_join_requests(self, start: int = 0, size: int = 25):
        req = requests.get(api(f"/x{self.comId}/s/community/membership-request?status=pending&start={start}&size={size}"), headers=self.headers)
        if req.status_code != 200: return CheckExceptions(req.json())
        else: return JoinRequest(req.json())
    ...
    def get_all_members(self, type: str, start: int = 0, size: int = 25):
        ...
        if req.status_code != 200: return CheckExceptions(req.json())
        else: return UserProfileList(req.json()["userProfileList"])
