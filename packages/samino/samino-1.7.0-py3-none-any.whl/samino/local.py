...
class Local:
    ...
    def get_chat_members(self, start: int = 0, size: int = 25, chatId: str = None):
        req = requests.get \
            (api(f"/x{self.comId}/s/chat/thread/{chatId}/member?start={start}&size={size}&type=default&cv=1.2"), headers=self.headers)
        if req.status_code != 200: return CheckExceptions(req.json())
    ...
    def get_online_users(self, start: int = 0, size: int = 25):
        req = requests.get \
            (api(f"/x{self.comId}/s/live-layer?topic=ndtopic:x{self.comId}:online-members&start={start}&size={size}"), headers=self.headers)
        if req.status_code != 200: return CheckExceptions(req.json())
        return UserList(req.json()["userProfileList"])

    def get_public_chats(self, type: str = "recommended", start: int = 0, size: int = 50):
        req = requests.get \
            (api(f"/x{self.comId}/s/chat/thread?type=public-all&filterType={type}&start={start}&size={size}"), headers=self.headers)
        if req.status_code != 200: return CheckExceptions(req.json())
        else: return MyChats(req.json()["threadList"])
