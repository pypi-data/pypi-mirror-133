...
_eval_func = _typing.Union[_typing.Callable[[_T], _typing.Any], str]
_multi_eval_func = _typing.Union[_typing.Callable[[_T], _typing.Iterable], str]
_aggregate_func = _typing.Callable[[_typing.Iterable[_T]], _typing.Any]


class Writer(_typing.Generic[_T]):
    def __init__(self, f: _typing.TextIO):
        self._writer = _csv.writer(f)
        self._fields = []
        self._to_aggregate = _collections.defaultdict(list)
        self._row_count = 0
    ...
    def write_all(self, items: _typing.Iterable[_T]):
        """
...
class _Field(_abc.ABC, _typing.Generic[_T, _V]):
    ...
    def normalise_evaluator(evaluator):
        if isinstance(evaluator, str):
            return _operator.attrgetter(evaluator)
        return evaluator
