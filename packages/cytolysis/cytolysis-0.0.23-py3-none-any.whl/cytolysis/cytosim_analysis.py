...
class Frame():
    ...
    def __init__(self, *args, number=None, **kwargs):
        ...
        self.fibers_analysis = None
        self.couples_analysis = None
        self.objects_analysis = None

    def initialize(self, *args,
                   fibers_lines=None, couples_lines=None, objects_lines=None,
                   config=None, options=None, dim=__N_DIM__, **kwargs):

        if fibers_lines is None: fibers_lines = {}
        if couples_lines is None: couples_lines = {}
        if objects_lines is None: objects_lines = {}
        if options is None: options = {}

        # Fibers factory :
        for name, lines_dict in fibers_lines.items():
            opt = an.get_options(options, name)
            self.fibers[name] = Fiber_set(name=name, dim=dim, reports=lines_dict, config=config, **opt)

        # Couples factory
        for name, lines_dict in couples_lines.items():
            opt = an.get_options(options, name)
            self.couples[name] = Couple_set(name=name, dim=dim, reports=lines_dict, config=config, **opt)

        # Custom objects factory
        for name, lines_dict in objects_lines.items():
            opt = an.get_options(options, name)
            self.objects[name] = Object_set(name=name, dim=dim, reports=lines_dict, config=config, **opt)
...
class Simulation(list):
    ...
    # Initialization step
    def initialize(self, *args,
                   fibers_report=None, couples_report=None, objects_report=None,
                   config=None, **kwargs):
        ...
        if fibers_report is None: fibers_report = {}
        if couples_report is None: couples_report = {}
        if objects_report is None: objects_report = {}
        ...
        an.make_block_dict(fibers_iter)
        an.make_block_dict(couples_iter)
        an.make_block_dict(objects_iter)
        ...
        while keep:
            ...
            if not f_flag and not c_flag and not o_flag:
                ...
                self.append(Frame(*args, number=len(self),
                                  ...
                                  config=self.config, dim=self.dim, **kwargs))
