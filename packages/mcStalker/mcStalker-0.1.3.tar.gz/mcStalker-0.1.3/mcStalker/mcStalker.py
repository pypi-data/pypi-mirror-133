...
class Server(MCStalker):
    ...
    def convertAuthStatusToString(authStatus:int):
        aDict = {0: 'Cracked', 1: 'OnlineMode', 4: 'Unknown', 5: 'Whitelisted_Cracked', 6: 'Whitelisted_Online', 7: 'Open_Cracked', 8: 'Open_OnlineMode', 9: 'Error'}
...
class Player(MCStalker):
    ...
    def returnPlayerObject(self, player: dict):
        ...
        _player.servers = [whyDoIEvenDoThis.returnServerObject(whyDoIEvenDoThis.returnCleanServerDict(p)) for p in player["servers"]]
        _player.raw = player
        return _player
