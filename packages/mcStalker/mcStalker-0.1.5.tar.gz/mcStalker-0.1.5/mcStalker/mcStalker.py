...
class Server(MCStalker):
    ...
    def returnPlayerObject(self, player: dict):
        ...
        try:
            _player.servers = [
                self.returnServerObject(
                    self.returnCleanServerDict(p)
                )
    ...
    def returnServerObject(self, server: dict) -> _Server:
        ...
        try:
            _server.players = [
                self.returnPlayerObject(
                    self.returnCleanPlayerDict(player)
                )
    ...
    async def requestServer(self, ip: str):
        ...
        """
        async with aiohttp.ClientSession() as session, session.get(
            f"https://backend.mcstalker.com/api/searchserver/{ip}", headers={"Authorization": f"Bearer {self.key}"}
        ) as response:
...
class Player(MCStalker):
    ...
    async def requestPlayer(self, name: str) -> _Player:
        ...
        """
        async with aiohttp.ClientSession() as session, session.get(
            f"https://backend.mcstalker.com/api/searchusername/{name}", headers={"Authorization": f"Bearer {self.key}"}
        ) as resp:
