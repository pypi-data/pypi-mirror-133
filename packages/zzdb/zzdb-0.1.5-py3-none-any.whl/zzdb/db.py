...
class ZzDb:
    ...
    def delete(self, table_name="", record={}):
        ...
        for x in self.db_schema.get_child_tables(table_name, record):
            x["escape_char"] = self.escape_char
            rez = self._cursor(
                "select 1 from {child_table} where {escape_char}{child_column}{escape_char}='{parent_value}'".format(**x)
            )
            if {} != rez:
                self.last_sql_error = (
                    "Foreign key error for delete:"
                    + f" Value in {self.escape_char}{table_name}{self.escape_char}"
                    + ".{escape_char}{parent_column}{escape_char}".format(**x)
                    + "={parent_value}".format(**x)
                    + " can not to be deleted, beacause "
