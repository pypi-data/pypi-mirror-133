...
after installing this tool via `pip install aedev_git_repo_manager` the `grm` command will be available in your OS
console.
...
.. hint::
    ...
    that don't need authentication (and not using `gitToken`), like e.g. :func:`_git_fetch`. alternatively
...
in order to synchronize the local :data:`MAIN_BRANCH` branch with any changes done to same branch on the 'origin'
remote, execute `grm` with the `update-project` action.


file patching helper functions
==============================

this portion of the 'aedev' namespace is also providing some helper functions to patch code and documentation files.
...
from typing import (
    Any, AnyStr, Callable, Dict, Generator, Iterable, Iterator, List, Optional, Sequence, Set, Tuple, Union, cast)

from github import Github
from gitlab import Gitlab, MAINTAINER_ACCESS, GitlabError
...
from aedev.setup_project import (                                                       # type: ignore
    APP_PRJ, MODULE_PRJ, NO_PRJ, PACKAGE_PRJ, PARENT_PRJ, ROOT_PRJ, VERSION_PREFIX, VERSION_QUOTE,
...
def deploy_template(tpl_file_path: str, dst_path: str, patcher: str, f_vars: PdvType,
                    logger: Callable = print,
                    replacer: Optional[Dict[str, Replacer]] = None,
                    dst_files: Optional[set] = None) -> bool:
    ...
    :param dst_path:            destination path (relative in project root folder) without the project file name.
    ...
    if dst_file.startswith(SKIP_PRJ_TYPE_FILE_NAME_PREFIX):
        ...
        if project_type == f_vars['project_type']:
    ...
    deployed = False
    dst_path = os.path.join(f_vars.get('project_path', ""), dst_path)   # project path ignored if dst_path is absolute
    dst_file = norm_path(os.path.join(dst_path, patch_string(dst_file, f_vars)))
...
def find_extra_modules(project_path: str, namespace_name: str, portion_name: str) -> List[str]:
    ...
    :param project_path:        file path of the local namespace project root directory/folder.
    :param namespace_name:      namespace name.
    :param portion_name:        name of the portion.
    :return:                    list of extra/additional module names without extension (empty if portion is a module).
    """
    modules_dir = norm_path(os.path.join(project_path, namespace_name, portion_name))
    if os.path.isdir(modules_dir):
        base = os.path.basename
        item_mask = os.path.join(modules_dir, "**", '*' + PY_EXT)
        return path_items(item_mask, selector=lambda _: base(_) != PY_INIT, creator=lambda _: base(_)[:-len(PY_EXT)])
    return []
...
def install_requirements(req_file: str, project_path: str = ""):
    ...
    with in_wd(project_path):
        sh_err = _cl(12, f"pip install -r {req_file}", exit_on_err=False)
...
def main_file_path(project_path: str, project_type: str, namespace_name: str) -> str:
    ...
    main_name = ('main' if project_type in (APP_PRJ, ROOT_PRJ) else main_stem) + PY_EXT
    if project_type == PACKAGE_PRJ:
        main_path = join(main_path, namespace_name, main_stem)
        main_name = PY_INIT
    elif is_portion:
        main_path = join(main_path, namespace_name)
    main_file = join(main_path, main_name)
...
def next_package_version(pdv: PdvType, increment_part: int = 3) -> str:
    ...
    :return:                    latest published package version as string
                                or "0.0.1" if project never published a version tag to remote/origin
...
def package_project_path(import_name: str) -> str:
    ...
    #     if os.path.basename(parent_path) in pdv_val({}, 'PARENT_FOLDERS') and os.path.isdir(sister_path):
    #         return sister_path
...
def project_dev_vars(project_path: str = "") -> PdvType:
    ...
    if project_type == ROOT_PRJ:
        ...
        for package_name_version in cast(List[str], pdv_val(pdv, 'portions_packages')):
            ...
            for e_mod in find_extra_modules(portion_path, namespace_name, portion_name):
    ...
    elif tpl_projects and tpl_projects[0]['import_name'] == namespace_name:  # and not is root|parent and namespace_name
...
def pypi_versions(package_name: str) -> List[str]:
    """ determine package version of the releases at PyPI.

    :param package_name:        package name to get release versions from.
    :return:                    list of released versions or list with a single empty string item on error.
    """
    start_marker = "(from versions: "

    output: List[str] = []
    _cl(32, f"pip install {package_name}==non_existing_version_number", exit_on_err=False, lines_output=output)

    if len(output) != 4 or start_marker not in output[1]:
        return [""]
    return output[1][output[1].index(start_marker) + len(start_marker): -1].split(", ")


def refresh_templates(f_vars: PdvType, *exclude_folders: str, logger: Callable = print, **replacer: Replacer
                      ) -> Set[str]:
    ...
    :param f_vars:              project env/dev variables dict of the destination project to patch/refresh,
                                ...
                                * `project_path`: project path of the destination project.
    ...
    """
    f_vars['pypi_versions'] = pypi_versions(pdv_val(f_vars, 'package_name'))
    ...
    for tpl_prj in tpl_projects:
        ...
        for tpl_file_path in [file for file in glb(join(tpl_path, "**/.*")) + glb(join(tpl_path, "**/*")) if isf(file)]:
            ...
            if not any(rel_path.startswith(folder_prefix) for folder_prefix in exclude_folders):
                deploy_template(cast(str, tpl_file_path), rel_path, patcher, f_vars,
...
def _check_children_not_exist(parent_or_root_pdv: PdvType, *package_versions: str):
    for package_version in package_versions:
        parent_path, package_version = _get_parent_package(parent_or_root_pdv, package_version)
        project_path = os.path.join(parent_path, package_version.split(PACKAGE_VERSION_SEP)[0])
                ...
                if isinstance(text, dict):
                    for sub_key, sub_txt in text.items():
                        ...
                        if sub_key in ('app_flow_delay', 'fade_out_app', 'next_page_delay', 'page_update_delay',
                            ...
                            _chk_if(69, isinstance(sub_txt, float), f"sub-dict-values of {sub_key} must be floats")
                        else:
                            _chk_if(69, isinstance(sub_txt, str), f"sub-dict-values of {sub_key} must be strings")
...
def _check_templates(pdv: PdvType):
    ...
    with _record_calls(ae.base, 'write_file', _block_and_log_file_writes,
        ...
        checked = refresh_templates(
            pdv, *((pdv_str(pdv, 'DOCS_FOLDER'), )
                   if pdv_str(pdv, 'namespace_name') and pdv_str(pdv, 'project_type') != ROOT_PRJ else ()),
            logger=cae.po if verbose else cae.vpo)
    ...
    if missing or outdated:
        ...
        _chk_if(40, False, "***** check cancelled. update outsourced files first via the 'renew' action")
...
def _check_types_linting_tests(pdv: PdvType):
    ...
    with in_wd(project_path):
        ...
        _cl(61, f"mypy {exclude_options} --lineprecision-report mypy_report"
            ...
            extra_args=(["--namespace-packages"] if is_portion else []) + args)
        ...
        out: List[str] = []
        _cl(62, f"pylint --output-format=text --max-line-length={mll}" + "".join(" --ignore=" + _ for _ in excludes),
            extra_args=args, exit_on_err=False, lines_output=out)   # _exit_on_err ~= run pylint with option --exit-zero
        ...
        _cl(63, f"python -m pytest --cov-report html --cov={namespace_name if is_portion else 'main'}"
...
def _children_desc(pdv: PdvType) -> str:
    return f"{pdv_str(pdv, 'namespace_name')} portions" if pdv_str(pdv, 'project_type') == ROOT_PRJ else "children"
...
def _cl(err_code: int, command_line: str, extra_args: Sequence = (), lines_output: Optional[List[str]] = None,
        exit_on_err: bool = True, exit_msg: str = "") -> int:
    ...
    sh_err = sh_exec(command_line, extra_args=extra_args, lines_output=lines_output, cae=cae)
...
def _get_package(ini_pdv: PdvType, project_type: str = NO_PRJ) -> str:
    ...
    if namespace_name and project_type != ROOT_PRJ and not package_name.startswith(namespace_name + '_'):
...
def _get_parent_package(ini_pdv, package_or_portion: str) -> Tuple[str, str]:
    if package_or_portion:
        package_version = package_or_portion
        parent_path = _get_parent_path(ini_pdv)
    else:
        parent_path, _project_path, package_version = _get_path_package(ini_pdv)

    return parent_path, package_version
...
def _get_path_package(ini_pdv: PdvType, project_type: str = NO_PRJ) -> Tuple[str, str, str]:
    ...
    project_path = cae.get_option('path')       # if specified then cae.get_option('package') will be ignored
    if project_path:
        project_path = norm_path(project_path)
...
def _get_renamed_path_package(ini_pdv: PdvType, is_portion: bool, namespace_name: str, project_type: str
                              ) -> Tuple[str, str]:
    _parent_path, project_path, package_name = _get_path_package(ini_pdv, project_type=project_type)
    import_name = namespace_name + '.' + package_name[len(namespace_name) + 1:] if is_portion else package_name
    ...
    if old_prj_type != project_type and ROOT_PRJ in (old_prj_type, project_type):
        package_name = _get_package(ini_pdv) if old_prj_type == ROOT_PRJ else namespace_name
...
def _git_clone(repo_root: str, package_name: str, branch_or_tag: str = "", parent_path: str = "") -> str:
    ...
    with in_wd(parent_path):
        _cl(40, "git clone", extra_args=args)

    return norm_path(os.path.join(parent_path, package_name))
...
def _git_fetch(pdv: PdvType, *extra_args: str) -> List[str]:
    ...
    if not os.path.isdir(os.path.join(project_path, GIT_FOLDER_NAME)):
        return [f"missing git repository - there is no git folder {GIT_FOLDER_NAME} in project root {project_path}"]
...
def _git_init_if_needed(pdv: PdvType) -> bool:
    ...
    if os.path.isdir(os.path.join(pdv_str(pdv, 'project_path'), GIT_FOLDER_NAME)):
        return False
...
def _git_merge(pdv: PdvType, from_branch: str) -> bool:
    args = [f"--file={_check_commit_msg_file(pdv)}", "--log"]
    if _debug_or_verbose():
        args.append("-v")
    args.append(from_branch)
...
def _git_tag_list(pdv: PdvType, tag_pattern: str = "v*") -> List[str]:
    ...
    if os.path.isdir(os.path.join(project_path, GIT_FOLDER_NAME)):
        with in_wd(project_path):
            _cl(89, "git tag --list", extra_args=(tag_pattern, ), exit_on_err=False, lines_output=output)
    return output or [""]
...
def _init_act_args_check(ini_pdv: PdvType, act_spec: Any, act_name: str, act_args: ActionArgs):
    ...
    project_type = pdv_str(ini_pdv, 'project_type')
    cae.dpo(f"    - detected project type '{project_type}' in {pdv_str(ini_pdv, 'project_path')}")
...
def _init_act_exec_args() -> Tuple[PdvType, str, tuple]:
    ...
    * run optional pre_action.
    ...
    act_args = cae.get_argument('arguments')
    ...
    while act_name not in actions:
        if not act_args:
            ...
            if found_act_name:
                ...
                break
            _exit_error(36, f"****  invalid action '{act_name.replace('_', '-')}'. valid actions: {actions}")
    ...
    if 'children_pdv' in act_spec['annotations']:   # and '_children' in act_name
        extra_arg_count = len(act_spec['annotations']) - (2 + (1 if 'return' in act_spec['annotations'] else 0))
        if extra_arg_count:
            extra_children_args = " <" + " ".join(_ for _ in act_args[:extra_arg_count]) + ">"
        act_args[extra_arg_count:] = _init_children_pdv_args(ini_pdv, act_args[extra_arg_count:])
        extra_msg += f" :: {len(act_args) - extra_arg_count} {_children_desc(ini_pdv)}: "
        extra_msg += " ".join(_['package_name'] for _ in act_args[extra_arg_count:])
...
def _init_children_pdv_args(ini_pdv: PdvType, act_args: ActionArgs) -> List[PdvType]:
    ...
    chi_vars: Dict[str, PdvType] = pdv_val(ini_pdv, 'children_project_vars')

    if act_args == [ARG_ALL]:
        pkg_names = list(chi_vars)
    else:
        pkg_names = try_eval(" ".join(act_args), (Exception, ), glo_vars=_init_children_presets(chi_vars).copy())
    ...
    if pdv_str(ini_pdv, 'project_type') == ROOT_PRJ:
        ...
        pkg_names = [("" if por_name.startswith(pkg_prefix) else pkg_prefix) + por_name for por_name in pkg_names]

    return [chi_vars.get(p_name, {'package_name': p_name}) for p_name in pkg_names]


def _init_children_presets(chi_vars: Dict[str, PdvType]) -> Dict[str, Set[str]]:
    ...
    for chi_pdv in chi_vars.values():
        ...
        if _git_current_branch(chi_pdv) == MAIN_BRANCH:
            ps_dev.add(package_name)
        if branch and _git_current_branch(chi_pdv) == branch:
            chi_ps['filterBranch'].add(package_name)
        if expr and try_eval(expr, ignored_exceptions=(Exception, ), glo_vars=chi_pdv.copy()):
...
def _print_pdv(pdv: PdvType):
    if not cae.get_option('verbose'):
        ...
        pdv['data_files'] = [f"{sub_dir or '.': <15} == {', '.join(file[len(sub_dir):].strip('/') for file in files)}"
                             for sub_dir, files in pdv_val(pdv, 'data_files')]
        ...
        pdv['project_resources'] = ", ".join(pdv_val(pdv, 'project_resources'))
    ...
    if not cae.verbose:
        ...
        for name, val in list(pdv.items()):
            if not val or name in (
                    name.upper(), 'children_project_vars', 'data_files', 'dev_require', 'docs_require', 'import_name',
...
def _register_project_templates(project_templates_packages_versions: List[str], namespace_name: str):
    ...
    for package_sep_version in project_templates_packages_versions:
        ...
        if ver and not any(norm_name(reg['import_name']) == pkg_name and reg['version'] == ver[0]
            ...
            # tpl_root=pdv_str(project_env_vars(project_path=package_project_path('aedev.setup_project')), 'repo_root')
            # .. would break ae-group because it is patching setup_project.REPO_GROUP_SUFFIX in the grm runtime module.
            _register_template_project(import_name, tpl_ver=ver[0], tpl_root='https://gitlab.com/aedev-group')
...
def _renew_prj_dir(new_pdv: PdvType):
    ...
    import_name = namespace_name + '.' + portion_name if is_portion else package_name
    ...
    file_name = join(project_path, pdv_str(new_pdv, 'REQ_DEV_FILE_NAME'))
    ...
    if not is_file(file_name) and not is_file(file_name + LOCK_EXT):
        ...
        tpl = join(os.path.dirname(project_path), 'aedev_tpl_project', tpl_folder, 'de_otf_de_tpl_dev_requirements.txt')
        if is_file(tpl):
            deploy_template(tpl, "", "grm._renew_prj_dir", new_pdv)
        else:
...
def _renew_project(ini_pdv: PdvType, project_type: str) -> PdvType:
    namespace_name = _get_namespace(ini_pdv, project_type)
    is_portion = bool(namespace_name and project_type != ROOT_PRJ)

    project_path, package_name = _get_renamed_path_package(ini_pdv, is_portion, namespace_name, project_type)
    ...
    if req_branch or _git_current_branch(new_pdv) == MAIN_BRANCH:
        renew_branch = req_branch if req_branch and req_branch != MAIN_BRANCH else \
            f"{'new' if new_repo else 'renew'}_{project_type}_{package_name}"
    ...
    dst_files = refresh_templates(new_pdv, *((pdv_str(new_pdv, 'DOCS_FOLDER'),) if is_portion else ()),
                                  logger=cae.po if cae.get_option('verbose') else cae.vpo)
    ...
    if is_portion:
        _renew_root_req_file(new_pdv)

    return new_pdv


def _renew_root_req_file(pdv: PdvType):
    namespace_name = pdv_str(pdv, 'namespace_name')
    package_name = pdv_str(pdv, 'package_name')

    root_prj_path = REGISTERED_TPL_PROJECTS.get(namespace_name, {}).get('path', "")
    if not root_prj_path:
        cae.po(f"   ## namespace root {namespace_name} is not registered as template in {PPF(REGISTERED_TPL_PROJECTS)}")
        root_prj_path = os.path.join(os.path.dirname(pdv_str(pdv, 'project_path')), namespace_name)
        if not os.path.isdir(root_prj_path):
            cae.po(f"  ### ensure {package_name} gets renewed in {pdv_str(pdv, 'REQ_DEV_FILE_NAME')} of namespace root")
            return

    root_req = os.path.join(root_prj_path, pdv_str(pdv, 'REQ_DEV_FILE_NAME'))
    if os.path.isfile(root_req):
        req_content = read_file(root_req)
    else:
        cae.po(f"   ## {root_req} not found in {namespace_name} namespace root project path: creating ...")
        req_content = ""
...
def _required_package(import_or_package_name: str, packages_versions: List[str]) -> bool:
    package_name = norm_name(import_or_package_name)
...
def _template_projects(pdv: PdvType) -> List[RegisteredTemplateProject]:
    ...
    tpl_pkg_name_prefix = norm_name(TPL_IMPORT_NAME_PREFIX)
    tpl_packages_versions = [_ for _ in dev_require if _ == namespace_name or _.startswith(tpl_pkg_name_prefix)]
    ...
    tpl_projects = []   # collect projects of namespace, project type and generic project (the highest priority first)
    tpl_candis = [namespace_name, TPL_IMPORT_NAME_PREFIX + norm_name(project_type), TPL_IMPORT_NAME_PREFIX + 'project']
    for candidate in tpl_candis:
        if candidate in REGISTERED_TPL_PROJECTS and (forced or _required_package(candidate, tpl_packages_versions)):
            tpl_projects.append(REGISTERED_TPL_PROJECTS[candidate])
        else:
            cae.dpo(debug_msg.format(name=candidate))
...
def _write_commit_message(pdv: PdvType, package_version: str = "{package_version}", title: str = ""):
    sep = os.linesep
    file_name = os.path.join(pdv_str(pdv, 'project_path'), COMMIT_MSG_FILE_NAME)
    write_file(file_name, f"V{package_version}: {title}{sep}{sep}{os.linesep.join(_git_status(pdv))}{sep}")
...
class GithubCom(_RemoteHost):
    ...
    def fork_project(self, _ini_pdv: PdvType, fork_repo_remote_url: str):
...
class GitlabCom(_RemoteHost):
    ...
    def group_from_name(self, group_name: str) -> Optional[Group]:
        ...
        """
        try:
            return cast(Group, self.connection.groups.get(group_name))
        except GitlabError:
    ...
    def project_from_name(self, err_code: int, err_msg: str, project_name: str, owned: bool = True) -> Project:
        """ convert group/package_name or a endswith-fragment if it to a project object/instance of the remote repo api.
        ...
        """
        try:
            # to determine namespace root package via .get() the group name has to be specified in project_name; e.g.
            # .. get('aedev') raises GitLabError (404 project not found), but is then as last item in the list() return
    ...
    def clean_releases(self, ini_pdv: PdvType) -> List[str]:
        ...
        :param ini_pdv:             vars of the project to clean up.
        :return:                    list of all deleted tags and branches.
        """
        project_path = pdv_str(ini_pdv, 'project_path')
        ...
        pypi_releases = pypi_versions(pdv_val(ini_pdv, 'package_name'))
    ...
    def fork_children(self, ini_pdv: PdvType, *children_pdv: PdvType):
        ...
        :param ini_pdv:         project variables of the parent/namespace-root project.
        :param children_pdv:    project variables of the children to be forked.
        """
        _children_path_package_option_reset()
        for chi_pdv in children_pdv:
            self.fork_project(chi_pdv)
        cae.po(f"  === {len(children_pdv)} {_children_desc(ini_pdv)} forked")

    @_action(PARENT_PRJ, *ANY_PRJ_TYPE, shortcut='fork')
    def fork_project(self, ini_pdv: PdvType):
        """ create/renew fork of a remote repo into our user/group namespace. """
    ...
    def push_children(self, ini_pdv: PdvType, *children_pdv: PdvType):
        """ push specified children projects to remote/origin. """
        for chi_pdv in children_pdv:
            self.push_project(chi_pdv)
        cae.po(f"  === pushed {len(children_pdv)} {_children_desc(ini_pdv)}")

    @_action(shortcut='push')
    def push_project(self, ini_pdv: PdvType, branch_name: str = ''):
        ...
        pkg_version = next_package_version(ini_pdv)
        ...
        if parse_version(pkg_version) > parse_version(file_version):     # and cae.get_option('force')
            ...
            _write_commit_message(ini_pdv, package_version=pkg_version,
    ...
    def release_children(self, ini_pdv: PdvType, *children_pdv: PdvType):
        """ release specified children projects to remote/origin. """
        for chi_pdv in children_pdv:
            self.release_project(chi_pdv, 'LATEST')
        cae.po(f"  === {len(children_pdv)} {_children_desc(ini_pdv)} released")
    ...
    def release_project(self, ini_pdv: PdvType, push_tag: str):
        ...
        :param ini_pdv:         project dev vars.
        :param push_tag:        push version tag in the format 'v<version-number>' to release.
        """
        errors = _git_fetch(ini_pdv)
        _chk_if(86, not bool(errors), f" **** git fetch errors:{_pp(errors)}" + _hint(
            self.release_project, " later to retry if server is currently unavailable, or check remote configuration"))

        if push_tag == 'LATEST':
            package_version = next_package_version(ini_pdv, increment_part=0)
            push_tag = f"v{package_version}"
        else:
            _chk_if(85, push_tag[0] == "v" and push_tag.count(".") == 2, f"   ** push tag format error {push_tag}")
            package_version = push_tag[1:]
        ...
        release_branch = f"release{package_version}"
    ...
    def request_merge(self, ini_pdv: PdvType):
        """ request the merge of the origin (fork) repository into the upstream (forked) repository.

        :param ini_pdv:         development vars of the local project repository.
        """
    ...
    def show_children_repos(self, ini_pdv: PdvType, *children_pdv: PdvType):
        """ display remote properties of children repos. """
        for chi_pdv in children_pdv:
            self.show_repo(chi_pdv)
        cae.po(f"  === {len(children_pdv)} {_children_desc(ini_pdv)}")
...
def add_children_file(ini_pdv: PdvType, file_name: str, rel_path: str, *children_pdv: PdvType) -> bool:
    ...
    :return:                    True if the file got added to the parent and to all children, else False.
    """
    added = []
    if add_file(ini_pdv, file_name, rel_path):
        added.append(pdv_str(ini_pdv, 'package_name'))
        cae.po(f"   == renamed {file_name} to {rel_path} in {pdv_str(ini_pdv, 'project_desc')}")

    for chi_pdv in children_pdv:
        if add_file(chi_pdv, file_name, rel_path):
            added.append(pdv_str(chi_pdv, 'package_name'))

    cae.po(f"  === renamed {len(added)}/{len(children_pdv)} times {file_name} to {rel_path} in: {added}")
    return len(added) == 1 + len(children_pdv)
...
def add_file(ini_pdv: PdvType, file_name: str, rel_path: str) -> bool:
    ...
    dst_dir = os.path.join(pdv_str(ini_pdv, 'project_path'), rel_path)
    ...
    if any(os.path.basename(file_name).startswith(_) for _ in (
        ...
        ret = deploy_template(file_name, rel_path, "grm.add_file", ini_pdv)
        if not ret:
            cae.dpo(f"    # template {file_name} could not be added (already exists?)")
        return ret
...
def check_children_integrity(parent_pdv: PdvType, *children_pdv: PdvType):
    ...
    cae.po(f"  === {len(children_pdv)} {_children_desc(parent_pdv)} passed integrity checks")
...
def check_integrity(ini_pdv: PdvType):
    ...
    if not running_on_ci_server():  # skip check of missing/outdated templates on CI server w/o dev environment
        _check_templates(ini_pdv)
    _check_resources(ini_pdv)
    _check_types_linting_tests(ini_pdv)
    cae.dpo(f"   == {pdv_str(ini_pdv, 'project_name')} successfully passed integrity checks")
...
def clone_project(ini_pdv: PdvType, package_or_portion: str = "") -> str:
    ...
    """
    parent_path, package_version = _get_parent_package(ini_pdv, package_or_portion)

    repo_root = f"{pdv_str(ini_pdv, 'REPO_HOST_PROTOCOL')}{_get_domain(ini_pdv)}/{_get_group(ini_pdv)}"
    package_name, *ver = package_version.split(PACKAGE_VERSION_SEP)
    req_branch = cae.get_option('branch')
    ...
    if ver and req_branch:
        ...
        package_version += f" (branch: {req_branch})"

    cae.po(f"   == project {package_version} cloned from {repo_root} into project path {project_path}")
...
def commit_children(ini_pdv: PdvType, *children_pdv: PdvType):
    """ commit changes to children of a namespace/parent using the prepared message in :data:`COMMIT_MSG_FILE_NAME`. """
    for chi_pdv in children_pdv:
        cae.po(f" ---  {pdv_str(chi_pdv, 'package_name')}  ---  {pdv_str(chi_pdv, 'project_desc')}")
        commit_project(chi_pdv)
    cae.po(f"  === committed {len(children_pdv)} {_children_desc(ini_pdv)}:{(_['package_name'] for _ in children_pdv)}")
...
def delete_children_file(ini_pdv: PdvType, file_name: str, *children_pdv: PdvType) -> bool:
    ...
    :return:                    True if the file got delete from the parent and all children projects, else False.
    """
    del_in = []
    if delete_file(ini_pdv, file_name):
        del_in.append(pdv_str(ini_pdv, 'package_name'))
        cae.po(f"   == deleted {file_name} from {pdv_str(ini_pdv, 'project_desc')}")

    for chi_pdv in children_pdv:
        if delete_file(chi_pdv, file_name):
            del_in.append(pdv_str(chi_pdv, 'package_name'))

    cae.po(f"  === deleted {file_name} {len(del_in)} times in {len(children_pdv)} {_children_desc(ini_pdv)}: {del_in}")
    return len(del_in) == 1 + len(children_pdv)
...
def install_children_editable(ini_pdv: PdvType, *children_pdv: PdvType):
    """ install namespace root and its portions as editable on local machine. """
    install_editable(ini_pdv)
    cae.po(f"   == installed {pdv_str(ini_pdv, 'project_desc')} as editable")
    for chi_pdv in children_pdv:
        install_editable(chi_pdv)
    cae.po(f"  === installed {len(children_pdv)} {_children_desc(ini_pdv)}:{[_['package_name'] for _ in children_pdv]}")
...
def install_editable(ini_pdv: PdvType):
    ...
    project_path = pdv_str(ini_pdv, 'project_path')
    _cl(90, f"pip install -e {project_path}", exit_msg=f"installation from local portions path {project_path} failed")
...
def new_children(ini_pdv: PdvType, *children_pdv: PdvType) -> List[PdvType]:
    ...
    cae.po(f"  === renewed {len(new_vars)} {_children_desc(ini_pdv)}: {(_['package_name'] for _ in new_vars)}")
    return new_vars
...
@_action(PARENT_PRJ, ROOT_PRJ, shortcut='prepare')
def prepare_children_commit(ini_pdv: PdvType, *children_pdv: PdvType):
    """ run code checks and prepare :data:`COMMIT_MSG_FILE_NAME` for the commit of children of a namespace/parent. """
    for chi_pdv in children_pdv:
        cae.po(f" ---  {pdv_str(chi_pdv, 'package_name')}  ---  {pdv_str(chi_pdv, 'project_desc')}")
        prepare_commit(chi_pdv)
    cae.po(f"  === prepared commit of {len(children_pdv)} {_children_desc(ini_pdv)}:"
           f" {(_['package_name'] for _ in children_pdv)}")


@_action(shortcut='prepare')
def prepare_commit(ini_pdv: PdvType):
...
def refresh_outsourced(ini_pdv: PdvType):
    ...
    :param ini_pdv:             specifying the project to refresh.
    """
    is_portion = pdv_str(ini_pdv, 'namespace_name') and pdv_str(ini_pdv, 'project_type') != ROOT_PRJ
    project_path = pdv_str(ini_pdv, 'project_path')

    dst_files = refresh_templates(ini_pdv, *((pdv_str(ini_pdv, 'DOCS_FOLDER'),) if is_portion else ()),
                                  logger=cae.po if cae.get_option('verbose') else cae.vpo)

    dbg_msg = ": " + " ".join(os.path.relpath(_, project_path) for _ in dst_files) if _debug_or_verbose() else ""
    cae.po(f"    - refreshed {len(dst_files)} outsourced files{dbg_msg}")
...
def rename_children_file(ini_pdv: PdvType, old_file_name: str, new_file_name: str, *children_pdv: PdvType) -> bool:
    ...
    """
    ren = []
    if rename_file(ini_pdv, old_file_name, new_file_name):
        ren.append(pdv_str(ini_pdv, 'package_name'))
        cae.po(f"   == renamed {old_file_name} to {new_file_name} in {pdv_str(ini_pdv, 'project_desc')}")
...
def rename_file(ini_pdv: PdvType, old_file_name: str, new_file_name: str) -> bool:
    ...
    if not os.path.exists(old_file_name) or os.path.exists(new_file_name):
        cae.dpo(f"    # either source file {old_file_name} not found or destination {new_file_name} already exists")
        return False

    os.rename(old_file_name, new_file_name)     # using os.remove because git mv is too picky
    return not os.path.exists(old_file_name) and os.path.exists(new_file_name)


@_action(PARENT_PRJ, ROOT_PRJ, arg_names=tuple(tuple(('command', ) + _) for _ in ARGS_CHILDREN_DEFAULT), shortcut='run')
def run_children_command(_ini_pdv: PdvType, command: str, *children_pdv: PdvType):
    """ run console command for the specified portions/children of a namespace/parent.

    :param _ini_pdv:            parent/root project dev vars.
    ...
    for chi_pdv in children_pdv:
        cae.po(f"---   {pdv_str(chi_pdv, 'package_name')}   ---   {pdv_str(chi_pdv, 'project_desc')}")
        output: List[str] = []
        with in_wd(pdv_str(chi_pdv, 'project_path')):
...
def show_status(ini_pdv: PdvType):
    ...
    if project_type not in (NO_PRJ, PARENT_PRJ):
        ...
        if _git_current_branch(ini_pdv) != MAIN_BRANCH:
            cae.po(f"  --- git diff to {MAIN_BRANCH} branch:{_pp(_git_diff(ini_pdv, MAIN_BRANCH))}")
        ...
        cae.po("   -- git branches: " + ", ".join(_git_branches(ini_pdv)))
        cae.po("   -- git remotes: " + ", ".join(f"{name}={url}" for name, url in _git_remotes(ini_pdv).items()))
        cae.po("   -- git uncommitted files: " + ", ".join(_git_uncommitted(ini_pdv)))
...
def show_versions(ini_pdv: PdvType):
    ...
    pyp = pypi_versions(pdv_val(ini_pdv, 'package_name'))
...
def main():                                                                                 # pragma: no cover
    ...
    cae.add_option('filterExpression', "Python expression creating the 'filterExpression' children-set, to be used in"
                                       " the children-set-expression argument", "", short_opt='F')
    cae.add_option('filterBranch', "branch name creating the 'filterBranch' children-set, to be used in the"
                                   " children-set-expression argument", "", short_opt='B')
    ...
    cae.add_option('namespaces', "namespaces to register (for templates)", ['ae', 'aedev', 'aetst'], short_opt='s')
