...
class InfimaClient:
    ...
    def get_predictions(
        ...
        return infima_client.extras.get_predictions(
            client=self,
            symbols=symbols,
            as_of=as_of,
            col=col,
    ...
    def check_cohort_coverage(self, cohorts: List[str]) -> Optional[pd.DataFrame]:
        return infima_client.extras.check_cohort_coverage(client=self, cohorts=cohorts)

    def check_coverage(self, cusips: List[str]) -> Optional[pd.DataFrame]:
        return infima_client.extras.check_coverage(client=self, cusips=cusips)
    ...
    def get_pool_attributes(self, cusips: List[str]) -> Optional[pd.DataFrame]:
        return infima_client.extras.get_pool_attributes(client=self, cusips=cusips)
