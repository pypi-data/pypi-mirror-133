...
def handle_factor_date(dt: DateT) -> CoreFactorDate:
    ...
    _dt = pd.Timestamp(dt).date()
    if _dt.day != 1:
        raise ValueError(f"Factor dates are expected to have day=1")
...
def model_to_frame(
    model: Any, record_path: List[str], meta: List[str], index_cols: List[str]
) -> pd.DataFrame:
    ...
    df = (
        df.pipe(maybe_parse_known_date_columns)
...
def response_mapping_to_frame(
    ...
    frames = [
        model_to_frame(mapping[key], record_path, meta, index_cols)
    ...
    if frames:
        ...
        if out.empty:
            ...
            return out.pipe(maybe_get_column, col=col).pipe(maybe_wide, on=wide_on)
    else:
        return None
