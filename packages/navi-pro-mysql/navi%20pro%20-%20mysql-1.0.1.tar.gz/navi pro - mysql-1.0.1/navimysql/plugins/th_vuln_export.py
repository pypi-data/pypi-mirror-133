...
def worker():
    ...
    while True:
        ...
        print('print data from worker function the th_vul')
        print(data)
...
def vuln_export(days, ex_uuid, threads):
   ...
   # database = r"navi.db"
   # database = r"mydatabase"
    #drop_conn = new_db_connection()
    #update below  
    #drop_conn = drop_conn.cursor()
   ...
   # new_db_connection()
    print('recreat vuln table function')
    create_vulns_table()
    ...
    pay_load = {"num_assets": 50, "filters": {'last_found': int(day_limit), "state": ['open', 'reopened'],"severity":['critical','high','medium','low']}}
    print('setting payload in th_vul_export')
    print(pay_load)
    try:
