...
def grid(
        ...
        dst_path,
        field,
        where=None, left=_left, bottom=_bottom, right=_right, top=_top, cell_size=_cell_size,
        crs='EPSG:5179',
    ...
    cmd.append('-where')
    where_expr = f'({field} is not null)'
    if where:
        where_expr += f' and ({where})'
    cmd.append(where_expr)
    ...
    for date in dates:
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            if data.empty:
                ...
                continue
            if stn_ids:
                data = data[data.stn_id.isin(stn_ids)]
