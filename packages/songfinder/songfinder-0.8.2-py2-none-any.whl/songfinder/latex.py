...
class LatexParam(tk.Frame, object):
	def __init__(self, fenetre, chants_selection, papa, noCompile, \
					screens=None, exportSettings=None, **kwargs):
		...
		with guiHelper.SmoothWindowCreation(fenetre, screens=screens):
			...
			self.deps = {'reorder':[('list', False)], \
						...
						'keep_first':[('chords', True)], \
						'keep_last':[('chords', True), ('keep_first', False)], \
						'diapo':[('reorder', False), ('alphabetic_list', False), \
								('one_song_per_page', False), ('transpose', False), \
								('list', False), ('booklet', False), \
								('two_columns', False), ('capo', False), \
								('simple_chords', False), ('keep_first', False), \
								('keep_last', False), ('sol_chords', False), \
								('one_chorus', False), ('affiche_liste', False), \
								('chords', False), ('keep_first', False)], \
						}
	...
	def save(self, identifyer, event=0): # pylint: disable=unused-argument
		...
		for param, valeur in self.dictValeurs.items():
			self._exportSettings.set('Export_Parameters', self.dictParam[param], bool(valeur.get()))
			...
			if pressedValue:
				if self.pressed in self.deps:
					for condition in self.deps[self.pressed]:
						self._exportSettings.set('Export_Parameters', condition[0], condition[1])
						if condition[1]:
				...
				for condition in conditions:
					if condition[0] == self.pressed:
						if condition[1] != pressedValue:
							self.dictButton[param].deselect()
							self._exportSettings.set('Export_Parameters', param, condition[1])
		...
		if self._exportSettings.get('Export_Parameters', 'affiche_liste'):
			...
			for title in listTitres:
				...
				text = '%s\\contentsline{section}{%s \\dotfill}{%s}{section.%s}\n'\
							%(text, elem.escape(title), dicoTitres[title], dicoTitres[title])
...
\\end{document}"""%(numPage, self.__tmpName.replace('\\', '/'))
	...
	def __getOutFile(self):
		...
		if not self._exportSettings.get('Export_Parameters', 'reorder') and \
			not self._exportSettings.get('Export_Parameters', 'alphabetic_list'):
	...
	def __isOutput(self):
		if not os.path.isfile(self.__tmpName):
			tkMessageBox.showerror('Attention', 'Error while '
						'generating latex files. Output file %s does not exist'%(self.__tmpName))
			return False
		return True
