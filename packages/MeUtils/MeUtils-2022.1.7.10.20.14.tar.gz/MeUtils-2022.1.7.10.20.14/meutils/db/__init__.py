...
class Redis(object):
    ...
    def __init__(self, ips=None, password=None, decode_responses=True):
        from rediscluster import RedisCluster
        ...
        self.rc = RedisCluster(
            startup_nodes=startup_nodes,
            decode_responses=decode_responses,  # get对象设置False
            skip_full_coverage_check=True,
            password=password,
        )
    ...
    def set(self, key, obj):
        self.rc.set(key, pickle.dumps(obj))

    def get(self, key):
        try:
            return pickle.loads(self.rc.get(key))
