...
from libkdv.utils import GPS_bound_to_XY, GPS_to_XY, XY_to_GPS, is_pandas_df, shift_GPS, shift_time, unshift_GPS,unshift_time
import pandas as pd
...
class kdv:
    def __init__(self, data=None,bound=[],middle_lat=None,GPS =True,KDV_type=1,num_threads=8,
    ...
    def plot(self):
        if self.GPS:
            map_1 = KeplerGl(height=600,show_docs=False)
            map_1.add_data(data=self.result,name='data_1')
            filter = [{'dataId': ['data_1'], 'id': 'tzxwxnfag', 'name': ['t'], 'type': 'range', 'value': [self.t_bound[0], self.t_bound[1]], 'enlarged': False, 'plotType': 'histogram', 'animationWindow': 'free', 'yAxis': None, 'speed': 1}]
            return map_1
        else:                
