...
class PythonTask(BaseTask):
    ...
    **Parameters**
    ...
    def __init__(
        ...
        requirements: Optional[List[str]] = None,
        additional_download_sources: Optional[List[str]] = None,
    ):
        super().__init__(name=name, parents=parents, owner=owner, requirements=requirements)
...
class BranchTask(OwnerTask):
    ...
    branch_on : list or str, optional
        Data or files to be used as basis for branching. Accepts a query-expression or a list of
        query-expressions. The queries must evaluate to list or dict. If the queries evaluates to a
    ...
    def __init__(
        ...
        if branch_data or branch_files:
            if branch_data is not None:
                ...
                warnings.warn("For BranchTask, the 'branch_data' argument is deprecated, use 'branch_on' instead.",
                              FutureWarning)
            if branch_files is not None:
                self.branch_files = (
                    [branch_files] if isinstance(branch_files, (str, Parameterization)) else branch_files
                )
                warnings.warn("For BranchTask, the 'branch_files' argument is deprecated, use 'branch_on' instead.",
                              FutureWarning)
        ...
        if not branch_on:
            ...
            self.branch_on = [branch_on] if isinstance(branch_on, str) else branch_on
