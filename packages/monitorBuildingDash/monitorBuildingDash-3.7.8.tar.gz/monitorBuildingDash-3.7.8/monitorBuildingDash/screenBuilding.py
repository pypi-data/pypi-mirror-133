...
# # COMPUTER DEPENDENT  #
# #######################
import re, os, time, datetime as dt,importlib,pickle,glob
...
class ConfigScreenBuilding(ModeBusConfigMaster):
    def build_dfPLC_fromXML(self):
        print('building PLC configuration file .csv from xml file')
        units     = {'kW':'JTW','kWh':'JTWH','kVA':'JTVA','kvar':'JTVar','kvarh':'JTVarH','kVAh':'JTVAH'}
        compteurs = pd.read_csv(self.confFolder+'compteurs.csv',index_col=0)
        variables = pd.read_excel(self.confFolder+'variables.ods',sheet_name='vmuc').set_index('Variable')

        tags = {}
        allCompteurs = list(compteurs.index)
        for t in self.dfInstr.index:
            currentTag =self.dfInstr.loc[t]
            fullCompteurName = [c for c in allCompteurs if currentTag['point de comptage'] in c]
            variable = currentTag['description']
            if len(fullCompteurName)==0:
                print(currentTag['point de comptage'], ' not found in compteurs')
            elif len(fullCompteurName)>1:
                print(currentTag['point de comptage'],' found multiple times in compteurs.csv')
            elif variable not in list(variables.index):
                print('variable ',variable, 'not describe in variables.ods')
            else:
                fullCompteurName = fullCompteurName[0]
                # print(fullCompteurName)
                unit = units[currentTag['unit']]
                tag  = fullCompteurName + '-' + currentTag['description'] + '-' + unit
                description = compteurs.loc[fullCompteurName,'description Compteurs'] + ' ' + variables.loc[variable,'description']
        ...
        dfplc['VAL_DEF'] = 0
        dfplc['DEVICE']='vmuc'
        dfplc.to_csv(self.confFolder + 'screeningBuilding_PLC_v_' + re.findall('\d[\d_]+',self.xmlfile)[0] + '.csv')
        dfInstr = self.dfInstr[~self.dfInstr['tag'].isna()].set_index('tag')
        pickle.dump(dfInstr,open(self.confFolder + 'dfInstr.pkl','wb'))

    def __init__(self):
        self.confFolder=CONFFOLDER
        self.loaddfInstr()
        self.compteurs = pd.read_csv(self.confFolder+'/compteurs.csv')
        ## build PLC file if necessary
        if len(glob.glob(self.confFolder+'*PLC*.csv'))==0:
            self.build_dfPLC_fromXML()
