...
:py:class:`~pikav0.adapters.blocking_connection.BlockingConnection`
and the :class:`~pikav0.adapters.blocking_connection.BlockingChannel`
classes.
...
import pikav0.channel
from pikav0 import compat
from pikav0 import exceptions
import pikav0.spec
...
class _ConnectionBlockedUnblockedEvtBase(object):
    ...
    def __init__(self, callback, method_frame):
        ...
        :param pikav0.frame.Method method_frame: with method_frame.method of type
          `pikav0.spec.Connection.Blocked` or `pikav0.spec.Connection.Unblocked`
        """
        self._callback = callback
...
class BlockingConnection(object):
    ...
    :class:`Channel <pikav0.channel.Channel>` based communication for the
    ...
    `pikav0.connection.InternalCloseReasons.BLOCKED_CONNECTION_TIMEOUT`. See
    `pikav0.connection.ConnectionParameters` documentation to learn more about
    ...
    def __init__(self, parameters=None, _impl_class=None):
        ...
        :param pikav0.connection.Parameters parameters: Connection parameters
    ...
    def _on_connection_blocked(self, user_callback, method_frame):
        ...
        :param pikav0.frame.Method method_frame: method frame having `method`
            member of type `pikav0.spec.Connection.Blocked`
        """
    ...
    def add_on_connection_blocked_callback(self, callback_method):
        ...
        :param method callback_method: Callback to call on `Connection.Blocked`,
            having the signature `callback_method(pikav0.frame.Method)`, where the
            method frame's `method` member is of type
            `pikav0.spec.Connection.Blocked`
    ...
    def add_on_connection_unblocked_callback(self, callback_method):
        ...
        :param method callback_method: Callback to call on
            ...
            `callback_method(pikav0.frame.Method)`, where the method frame's
            `method` member is of type `pikav0.spec.Connection.Unblocked`
    ...
    def close(self, reply_code=200, reply_text='Normal shutdown'):
        ...
        for impl_channel in pikav0.compat.dictvalues(self._impl._channels):
    ...
    def process_data_events(self, time_limit=0):
        ...
        :param float time_limit: suggested upper bound on processing time in
            ...
            compatibility. This parameter is NEW in pikav0 0.10.0.
        """
        with self._acquire_event_dispatch() as dispatch_acquired:
    ...
    def channel(self, channel_number=None):
        ...
        :rtype: pikav0.adapters.blocking_connection.BlockingChannel
        """
...
class _ConsumerCancellationEvt(_ChannelPendingEvt):
    ...
    def __init__(self, method_frame):
        """
        :param pikav0.frame.Method method_frame: method frame with method of type
            `spec.Basic.Cancel`
        """
    ...
    def __init__(self, callback, channel, method, properties, body):
        ...
        :param callable callback: user's callback, having the signature
            callback(channel, method, properties, body), where
                                channel: pikav1.Channel
                                method: pikav1.spec.Basic.Return
                                properties: pikav1.spec.BasicProperties
                                body: str, unicode, or bytes (python 3.x)

        :param pikav1.Channel channel:
        :param pikav0.spec.Basic.Return method:
        :param pikav0.spec.BasicProperties properties:
...
class BlockingChannel(object):
    ...
    :py:class:`~pikav0.channel.Channel` class. In addition,
    ...
    Example of creating a BlockingChannel::
        ...
        connection = pikav0.BlockingConnection()
    ...
    _RxMessageArgs = namedtuple(
        ...
        [
            'channel',  # implementation pikav0.Channel instance
            'method',   # Basic.GetOk
            'properties',  # pikav0.spec.BasicProperties
            'body'      # str, unicode, or bytes (python 3.x)
        ])
    ...
    def __init__(self, channel_impl, connection):
        ...
        self._impl.add_callback(
            ...
            replies=[pikav0.spec.Basic.ConsumeOk],
            one_shot=False)

        self._impl.add_callback(
            self._on_channel_closed,
            replies=[pikav0.spec.Channel.Close],
            one_shot=True)

        self._impl.add_callback(
            self._basic_getempty_result.set_value_once,
            replies=[pikav0.spec.Basic.GetEmpty],
            one_shot=False)
    ...
    def _on_puback_message_returned(self, channel, method, properties, body):
        ...
        :param pikav1.Channel channel: our self._impl channel
        :param pikav0.spec.Basic.Return method:
        :param pikav0.spec.BasicProperties properties: message properties
        ...
        assert isinstance(method, pikav0.spec.Basic.Return), method
        assert isinstance(properties, pikav0.spec.BasicProperties), (
            properties)
    ...
    def _on_channel_closed(self, method_frame):
        ...
        :param pikav0.Channel channel: channel closed by the
    ...
    def _on_consumer_cancelled_by_broker(self, method_frame):
        ...
        :param pikav0.frame.Method method_frame: method frame with the
    ...
    def _cancel_all_consumers(self):
        ...
        if self._consumer_infos:
            ...
            for consumer_tag in pikav0.compat.dictkeys(self._consumer_infos):
        ...
        try:
            with _CallbackResult() as close_ok_result:
                self._impl.add_callback(callback=close_ok_result.signal_once,
                                        replies=[pikav0.spec.Channel.CloseOk],
                                        one_shot=True)
    ...
    def add_on_cancel_callback(self, callback):
        ...
        :param callable callback: a callable for handling broker's Basic.Cancel
            ...
            where method_frame is of type `pikav0.frame.Method` with method of
    ...
    def add_on_return_callback(self, callback):
        ...
        :param callable callback: The method to call on callback with the
            ...
            channel: pikav0.Channel
            method: pikav0.spec.Basic.Return
            properties: pikav0.spec.BasicProperties
            body: str, unicode, or bytes (python 3.x)
    ...
    def basic_consume(self,
        ...
        :raises pikav0.exceptions.DuplicateConsumerTag: if consumer with given
    ...
    def basic_cancel(self, consumer_tag):
        ...
        :returns: (NEW IN pikav0 0.10.0) empty sequence for a no_ack=False
    ...
    def start_consuming(self):
        ...
        pikav0 callback, because dispatching `basic_consume` callbacks from this
        context would constitute recursion.

        :raises pikav0.exceptions.RecursionError: if called from the scope of a
    ...
    def basic_publish(self, exchange, routing_key, body,
        ...
        :returns: True if delivery confirmation is not enabled (NEW in pikav1
    ...
    def publish(self, exchange, routing_key, body,
        ...
        if self._delivery_confirmation:
            ...
            with self._message_confirmation_result:
                ...
                if isinstance(conf_method, pikav0.spec.Basic.Nack):
                    ...
                    assert isinstance(conf_method, pikav0.spec.Basic.Ack), (
                        conf_method)
    ...
    def confirm_delivery(self):
        ...
        with _CallbackResult() as select_ok_result:
            self._impl.add_callback(callback=select_ok_result.signal_once,
                                    replies=[pikav0.spec.Confirm.SelectOk],
                                    one_shot=True)
    ...
    def exchange_declare(self, exchange=None,
        ...
        :rtype: `pikav0.frame.Method` having `method` attribute of type
