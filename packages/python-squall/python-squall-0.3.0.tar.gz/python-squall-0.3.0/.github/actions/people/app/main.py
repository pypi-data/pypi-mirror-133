...
query Q($after: String) { 
  repository(name: "squall", owner: "stas-dubrovskyi") {
...
if __name__ == "__main__":
    ...
    maintainers_logins = {"stas-dubrovskyi"}
    ...
    if people_old_content == new_people_content and github_sponsors_old_content == new_github_sponsors_content:
        logging.info("The Squall People data hasn't changed, finishing.")
        sys.exit(0)
    ...
    branch_name = "squall-people"
    logging.info(f"Creating a new branch {branch_name}")
    subprocess.run(["git", "checkout", "-b", branch_name], check=True)
    logging.info("Adding updated file")
    subprocess.run(["git", "add", str(people_path)], check=True)
    logging.info("Committing updated file")
    message = "👥 Update Squall People"
