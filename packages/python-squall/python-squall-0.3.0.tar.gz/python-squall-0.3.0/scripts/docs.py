...
def get_base_lang_config(lang: str):
    ...
    squall_url_base = "https://squall.mtag.dev/"
    new_config = en_config.copy()
    new_config["site_url"] = en_config["site_url"] + f"{lang}/"
    new_config["theme"]["logo"] = squall_url_base + en_config["theme"]["logo"]
    new_config["theme"]["favicon"] = squall_url_base + en_config["theme"]["favicon"]
    ...
    for css in en_config["extra_css"]:
        if css.startswith("http"):
            ...
            extra_css.append(squall_url_base + css)
    ...
    for js in en_config["extra_javascript"]:
        if js.startswith("http"):
            ...
            extra_js.append(squall_url_base + js)
...
def build_lang(
    ...
    # Exclude first 2 entries Squall and Languages, for custom handling
    use_nav = nav[2:]
