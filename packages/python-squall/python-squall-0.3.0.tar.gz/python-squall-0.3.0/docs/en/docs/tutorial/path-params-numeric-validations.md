...
It doesn't matter for **Squall**. It will detect the parameters by their names, types and default declarations (`Query`, `Path`, etc), it doesn't care about the order.
...
!!! note "Technical Details"
    When you import `Query`, `Path` and others from `squall`, they are actually functions.
