...
!!! info
    ...
    Sending a body with a `GET` request has an undefined behavior in the specifications, nevertheless, it is supported by Squall, only for very complex/extreme use cases.
...
With just that Python type declaration, **Squall** will:
...
**Squall** will recognize that the function parameters that match path parameters should be **taken from the path**, and that function parameters that are declared to be Pydantic models should be **taken from the request body**.
...
**Squall** will recognize each of them and take the data from the correct place.
