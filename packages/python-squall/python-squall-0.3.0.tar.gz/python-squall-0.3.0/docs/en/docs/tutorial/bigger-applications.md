...
**Squall** provides a convenience tool to structure your application while keeping all the flexibility.
...
But it's still part of the same **Squall** application/web API (it's part of the same "Python Package").

You can create the *path operations* for that module using `APIRouter`.

### Import `APIRouter`

You import it and create an "instance" the same way you would with the class `Squall`:
...
Use it the same way you would use the `Squall` class:

```Python hl_lines="6  11  16"
{!../../../docs_src/bigger_applications/app/routers/users.py!}
```

You can think of `APIRouter` as a "mini `Squall`" class.
...
We are going to include this `APIRouter` in the main `Squall` app, but first, let's check the dependencies and another `APIRouter`.
...
!!! check
    The `prefix`, `tags`, `responses`, and `dependencies` parameters are (as in many other cases) just a feature from **Squall** to help you avoid code duplication.
...
Here's where you import and use the class `Squall`.
...
You import and create a `Squall` class as normally.
...
With `app.include_router()` we can add each `APIRouter` to the main `Squall` application.
...
We can also add *path operations* directly to the `Squall` app.
...
The same way you can include an `APIRouter` in a `Squall` application, you can include an `APIRouter` in another `APIRouter` using:

```Python
router.include_router(other_router)
```

Make sure you do it before including `router` in the `Squall` app, so that the *path operations* from `other_router` are also included.
