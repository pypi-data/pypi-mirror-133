...
We are going to use **Squall** security utilities to get the `username` and `password`.
...
Now let's use the utilities provided by **Squall** to handle this.
...
!!! info
    The `OAuth2PasswordRequestForm` is not a special class for **Squall** as is `OAuth2PasswordBearer`.

    `OAuth2PasswordBearer` makes **Squall** know that it is a security scheme. So it is added that way to OpenAPI.

    But `OAuth2PasswordRequestForm` is just a class dependency that you could have written yourself, or you could have declared `Form` parameters directly.

    But as it's a common use case, it is provided by **Squall** directly, just to make it easier.
...
!!! tip
    ...
    For the rest, **Squall** handles it for you.
