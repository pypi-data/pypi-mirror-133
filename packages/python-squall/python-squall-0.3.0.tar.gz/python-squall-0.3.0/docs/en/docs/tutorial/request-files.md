...
Import `File` and `UploadFile` from `squall`:
...
!!! info
    ...
    But remember that when you import `Query`, `Path`, `File` and others from `squall`, those are actually functions that return special classes.
...
If you declare the type of your *path operation function* parameter as `bytes`, **Squall** will read the file for you and you will receive the contents as `bytes`.
...
!!! note "`async` Technical Details"
    When you use the `async` methods, **Squall** runs the file methods in a threadpool and awaits for them.

!!! note "Starlette Technical Details"
    **Squall**'s `UploadFile` inherits directly from **Starlette**'s `UploadFile`, but adds some necessary parts to make it compatible with **Pydantic** and the other parts of Squall.
...
!!! note "Technical Details"
    ...
    But when the form includes files, it is encoded as `multipart/form-data`. If you use `File`, **Squall** will know it has to get the files from the correct part of the body.
