...
**Squall** allows you to declare additional information and validation for your parameters.
...
The query parameter `q` is of type `Optional[str]`, that means that it's of type `str` but could also be `None`, and indeed, the default value is `None`, so Squall will know it's not required.

!!! note
    Squall will know that the value of `q` is not required because of the default value `= None`.

    The `Optional` in `Optional[str]` is not used by Squall, but will allow your editor to give you better support and detect errors.
...
To achieve that, first import `Query` from `squall`:
...
!!! info
    Have in mind that Squall cares about the part:
...
But whenever you need them and go and learn them, know that you can already use them directly in **Squall**.
...
!!! note
    Have in mind that in this case, Squall won't check the contents of the list.
