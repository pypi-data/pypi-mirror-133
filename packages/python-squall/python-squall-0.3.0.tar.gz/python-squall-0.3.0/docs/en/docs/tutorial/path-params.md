...
!!! check
    ...
    So, with that type declaration, **Squall** gives you automatic request <abbr title="converting the string that comes from an HTTP request into Python data">"parsing"</abbr>.
...
!!! check
    So, with the same Python type declaration, **Squall** gives you data validation.
...
!!! check
    Again, just with that same Python type declaration, **Squall** gives you automatic, interactive documentation (integrating Swagger UI).
...
Because of this, **Squall** itself provides an alternative API documentation (using ReDoc), which you can access at <a href="http://127.0.0.1:8000/redoc" class="external-link" target="_blank">http://127.0.0.1:8000/redoc</a>:
...
Nevertheless, you can still do it in **Squall**, using one of the internal tools from Starlette.
...
With **Squall**, by using short, intuitive and standard Python type declarations, you get:
...
That's probably the main visible advantage of **Squall** compared to alternative frameworks (apart from the raw performance).
