...
!!! tip
    ...
    They are handled automatically by **Squall** and converted to JSON.
...
And you want to handle this exception globally with Squall.
...
**Squall** has some default exception handlers.
...
#### Squall's `HTTPException` vs Starlette's `HTTPException`

**Squall** has its own `HTTPException`.

And **Squall**'s `HTTPException` error class inherits from Starlette's `HTTPException` error class.

The only difference, is that **Squall**'s `HTTPException` allows you to add headers to be included in the response.

This is needed/used internally for OAuth 2.0 and some security utilities.

So, you can keep raising **Squall**'s `HTTPException` as normally in your code.
...
### Re-use **Squall**'s exception handlers

You could also just want to use the exception somehow, but then use the same default exception handlers from **Squall**.

You can import and re-use the default exception handlers from `squall.exception_handlers`:
