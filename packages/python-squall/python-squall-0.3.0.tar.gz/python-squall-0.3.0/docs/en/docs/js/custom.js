...
async function getDataBatch(page) {
    const response = await fetch(`https://api.github.com/search/repositories?q=topic:squall&per_page=100&page=${page}`, { headers: { Accept: 'application/vnd.github.mercy-preview+json' } })
...
async function showRandomAnnouncement(groupId, timeInterval) {
    const announceSquall = document.getElementById(groupId);
    if (announceSquall) {
        let children = [].slice.call(announceSquall.children);
...
async function main() {
    if (div) {
        ...
        data.forEach(v => {
            if (v.full_name === 'mtag-dev/squall') {
                return
            }
