...
**Squall** will use that *temporal* response to extract the cookies (also headers and status code), and will put them in the final response that contains the value you returned, filtered by any `response_model`.
...
!!! tip
    Have in mind that if you return a response directly instead of using the `Response` parameter, Squall will return it directly.
