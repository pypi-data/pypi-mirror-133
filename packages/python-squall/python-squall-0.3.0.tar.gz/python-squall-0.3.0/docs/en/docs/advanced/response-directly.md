...
And when you return a `Response`, **Squall** will pass it directly.
...
Because **Squall** doesn't do any change to a `Response` you return, you have to make sure it's contents are ready for it.
...
The example above shows all the parts you need, but it's not very useful yet, as you could have just returned the `item` directly, and **Squall** would put it in a `JSONResponse` for you, converting it to a `dict`, etc. All that by default.
