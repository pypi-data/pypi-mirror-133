...
You can also use <a href="https://github.com/encode/databases" class="external-link" target="_blank">`encode/databases`</a> with **Squall** to connect to databases using `async` and `await`.
...
!!! tip
    You could adopt ideas from the section about SQLAlchemy ORM ([SQL (Relational) Databases](../tutorial/sql-databases.md){.internal-link target=_blank}), like using utility functions to perform operations in the database, independent of your **Squall** code.
...
Here, this section would run directly, right before starting your **Squall** application.
...
* Create your `Squall` application.
