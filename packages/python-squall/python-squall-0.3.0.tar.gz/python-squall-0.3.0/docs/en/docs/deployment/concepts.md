...
When deploying a **Squall** application, or actually, any type of web API, there are several concepts that you probably care about, and using them you can find the **most appropriate** way to **deploy your application**.
...
In the next chapters, I'll give you more **concrete recipes** to deploy Squall applications.
...
In general, you will probably want the server program (e.g. Uvicorn) to be started automatically on server startup, and without needing any **human intervention**, to have a process always running with your API (e.g. Uvicorn running your Squall app).
...
When building web APIs with Squall, if there's an error in our code, Squall will normally contain it to the single request that triggered the error. 🛡
...
With a Squall application, using a server program like Uvicorn, running it once in **one process** can serve multiple clients concurrently.
...
!!! tip
    ...
    I'll tell you more about container images, Docker, Kubernetes, etc. in a future chapter: [Squall in Containers - Docker](./docker.md){.internal-link target=_blank}.
...
!!! tip
    I'll give you more concrete examples for doing this with containers in a future chapter: [Squall in Containers - Docker](./docker.md){.internal-link target=_blank}.
