...
from esg._handlers.http import handle_http
from esg.config import Config
from esg.protocols.http.protocol import Protocol

if TYPE_CHECKING:
    from esg.protocols.http.h11_impl import H11Protocol
    from esg.protocols.http.httptools_impl import HttpToolsProtocol
...
logger = logging.getLogger("esg.error")
...
class ServerState:
    ...
    __slots__ = ["total_requests", "connections", "tasks", "default_headers", "time"]
...
class Server:
    ...
    async def startup(self, sockets: list = None) -> None:
        ...
        protocol = partial(Protocol, config=config, server_state=self.server_state)
        ...
        if sockets is not None:
            ...
            for sock in sockets:
                ...
                server = await loop.create_server(
                    protocol, sock=sock, ssl=config.ssl, backlog=config.backlog
                )
    ...
    def _log_started_message(self, listeners: List[socket.SocketType]) -> None:
        ...
        if config.fd is not None:
            sock = listeners[0]
            logger.info(
                "ESG running on socket %s (Press CTRL+C to quit)",
                sock.getsockname(),
            )

        elif config.uds is not None:
            logger.info(
                "ESG running on unix socket %s (Press CTRL+C to quit)", config.uds
            )
            ...
            message = f"ESG running on {addr_format} (Press CTRL+C to quit)"
            color_message = (
                "ESG running on "
