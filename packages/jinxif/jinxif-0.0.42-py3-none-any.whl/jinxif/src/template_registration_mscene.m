...
for i = 1:length(m_file_ref)
    if contains(m_file_ref(i).name, m_round_ref)
        ...
        for j = 1:length(m_crop)
            ...
            % save file
            s_pathfilename = sprintf('%s%s%s/Registered-%s%s%s%s%s%s', m_dst_dir, m_slide, m_pxscene{j}, m_round_ref, m_marker_ref, m_slide, m_pxscene{j}, m_micchannel_ref, m_ext);
        ...
        for i = 1:length(m_file_nonref)
            if contains(m_file_nonref(i).name, m_round_nonref)
                ...
                for j = 1:length(m_crop)
                    ...
                    % save file
                    s_pathfilename = sprintf('%s%s%s/Registered-%s%s%s%s%s%s', m_dst_dir, m_slide, m_pxscene{j}, m_round_nonref, m_marker_nonref, m_slide, m_pxscene{j}, m_micchannel_nonref, m_ext);
                    sprintf('write file: %s', s_pathfilename)
