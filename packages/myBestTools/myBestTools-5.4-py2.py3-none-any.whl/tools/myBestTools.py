...
class Tool:
    ...
    def get_html(self, url, headers={}, proxy={}, cookies={}, data={}, params={}, timeout=5, debug=False, try_count=5):
    ...
    def post_html(self, url, headers={}, data={}, json={}, params={}, proxy={}, cookies={}, timeout=5, debug=False, try_count=5):

        for i in range(try_count):
            try:
                res = self.session.post(url, data=data, json=json, headers=headers, timeout=timeout, proxies=proxy, params=params, cookies=cookies, verify=False)
                return res
            except Exception as e:
                if debug:
                    logging.info(e)
            ...
            for index, data in enumerate(data_list):
                ...
                for i in data:
                    if isinstance(data[i], int) or isinstance(data[i], float) or isinstance(data[i], bytes):
                        ...
                        key.append(i)
                        values.append('"%s"' % escape_string(data[i]))
    ...
    def dup_insert(self, table: str, datas: dict or list) -> list:
        ...
        def analysis_dict(item, type="install"):
            if type == "install":
                key = item.keys()
                value = item.values()
                return ", ".join(key), ", ".join(["\"{}\"".format(escape_string(v) if isinstance(v, str) else v) for v in value])
            elif type == "update":
                str_list = ["{}=\"{}\"".format(k, escape_string(v) if isinstance(v, str) else v) for k, v in item.items()]
    ...
    def create_table(self, table_name, table_field: dict):
        table_field = ",".join(["`%s` varchar(255)" % i for i in table_field.keys()])
        sql = f"CREATE TABLE IF NOT EXISTS {table_name} ({table_field});"
        print("创建表成功：%s" % sql)
        self.con_execute(sql)
    ...
    def parse_paramsStr2dict(self, paramStr):
        paramStr = [i.split("=") for i in paramStr.split("&")]
        params_dict = {i[0]: i[1] for i in paramStr}
        return params_dict
...
class MongoUtils:
    ...
    def __init__(self, db_name, host="127.0.0.1", port=27017, username=None, password=None):
        self.client = pymongo.MongoClient(host=host, port=port)
        self.db = self.client[db_name]
        self.index_map = {}

    def insert(self, table_name, data_list):
        data_list = self.transform_list(data_list)
        for i in data_list:
            if "_id" not in data_list:
                i["_id"] = Tool.get_uuid()
        self.db[table_name].insert_many(data_list)

    def dup_insert(self, table_name, data_list):
        data_list = self.transform_list(data_list)
        update_list = []
        for i in data_list:
            update_filter = {key: i[key] for key in self.index_map[table_name]}
            update_list.append(pymongo.UpdateOne(update_filter, {"$set": i}, True))
        if update_list:
            self.db[table_name].bulk_write(update_list, ordered=False)

    def insert_only(self, table_name, data_list):
        """
        insert data but if exists ignore
        :param table_name:
        :param data_list:
        :return:
        """
        ...
        for i in data_list:
            update_filter = {key: i[key] for key in self.index_map[table_name]}
            update_list.append(pymongo.UpdateOne(update_filter, {"$setOnInsert": i}, True))
        if update_list:
            self.db[table_name].bulk_write(update_list, ordered=False)

    def create_index(self, table_name, index_keys, is_unique=False):
        index_keys = self.transform_list(index_keys)
        self.db[table_name].create_index([(key, 1) for key in index_keys], unique=is_unique)

    def create_unique_index(self):
        for table, indexs in self.index_map.items():
            indexs = [(i, pymongo.ASCENDING) for i in indexs]
            self.db[table].create_index(indexs, unique=True)
