...
class Clipper:
    def __init__(self, root):
        ...
        self.btp = ttk.Button(root, text="Read Construct", command=self.readec)
        self.btp.pack(pady=1, padx=2, expand=1, fill="both")
    ...
    def updatesel(self, event=None):
        if self.upt:
            ...
            self.upt = self.ls.curselection()
            if self.upt: self.sel.append(self.upt[0])
            ...
            if ask:
                ...
                class MyDialog(simpledialog.Dialog):
                    def body(self, master):
                        tk.Label(master, text="Pass: ").grid(
                            row=0, column=0, sticky=tk.E
                        )
                        self.e1 = tk.Entry(master, show="-")
                        self.e1.grid(row=0, column=1)
                        tk.Label(master, text="Var: ").grid(
    ...
    def readec(self):
        ...
        if fl and self.lock is None:
            ...
            if d.result:
                ori = os.getcwd()
                fld = self.fold()
                fln = d.result.rpartition('.')[0] + '.json'
                ...
                if pssd:
                    os.chdir(fld)
                    zippy(name = fln, pssd = pssd, st = False)
                    con = construct(jn)
