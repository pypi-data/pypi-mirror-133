...
class ListObjectsPaginator():
    ...
    def paginate(self, **kwargs):
        ...
        if self.bucket not in self.bucketlist:
            ...
            print('ListObjectPaginator.paginate[' + str(os.getpid()) + ']: kwargs=' + str(kwargs))
            self.kwargs = kwargs
            return self
    ...
    def __next__(self):
        if self.finished:
            print('ListObjectPaginator.__next__[' + str(os.getpid()) + ']: finished')
            raise StopIteration
        if (self.next_token):
            if self.type2:
        ...
        if self.type2:
            print('ListObjectPaginator.__next__[' + str(os.getpid())
                    + ']: Calling list_v2. kwargs=' + str(self.kwargs))
            rv = self.infin_boto_client.list_objects_v2(None, **self.kwargs)
            if 'NextContinuationToken' in rv:
                print('ListObjectPaginator.__next__[' + str(os.getpid())
                        + ']: returned from list_v2. continuation present. rv=' + str(rv))
                self.next_token = rv['NextContinuationToken']
                del rv['NextContinuationToken']
            else:
                print('ListObjectPaginator.__next__[' + str(os.getpid())
                        + ']: returned from list_v2. continuation absent. rv=' + str(rv))
                self.finished = True
            return rv
        else:
            print('ListObjectPaginator.__next__[' + str(os.getpid())
                    + ']: Calling list. kwargs=' + str(self.kwargs))
            rv = self.infin_boto_client.list_objects(None, **self.kwargs)
            if 'NextMarker' in rv:
                print('ListObjectPaginator.__next__[' + str(os.getpid())
                        + ']: returned from list. marker present. rv=' + str(rv))
                self.next_token = rv['NextMarker']
                del rv['NextMarker']
            else:
                print('ListObjectPaginator.__next__[' + str(os.getpid())
                        + ']: returned from list. marker absent. rv=' + str(rv))
                self.finished = True
            return rv
...
class InfinBotoResource():
    ...
    def __getattr__(self, attr):
        if attr == 'buckets':
            ...
            print('InfinBotoResource: forwarding ' + str(attr) + ' to default resource', flush=True)
...
class InfinBotoClient():
    ...
    def download_file(self, *args, **kwargs):
        if self.input_spec['type'] == 'mlflow-run-artifacts':
            ...
            request_file = args[1]
            if request_bucket in self.input_spec['bucketlist']:
                versionId = self.get_version_id(request_bucket, request_file)
                if (versionId != None):
                    if 'ExtraArgs' in kwargs:
                        kwargs['ExtraArgs']['VersionId'] = versionId
                    else:
                        kwargs['ExtraArgs'] = {'VersionId': versionId}
                    print('InfinBotoClient[' + str(os.getpid()) + ']: Downloading version '
                            +versionId +' of ' +request_file)
                    return self.defaultClient.download_file(*args, **kwargs)
                else:
                    print('WARNING: versionId not present in GETFILESTATUS. '
                            'Defaulting to latest snap of bucket')
                    return self.defaultClient.download_file(*args, **kwargs)
            else:
                return self.defaultClient.download_file(*args, **kwargs)
        else:
    ...
    def get_version_id(self, bucket, prefix):
        ...
        if 'versionId' in js:
            ...
            print('versionId not present in response=')
            return None

    def list_objects_at(self, type2, *args, **kwargs):
        request_bucket = kwargs['Bucket']
        request_prefix = kwargs.get('Prefix')
        print('InfinBotoClient.list_objects_at[' + str(os.getpid()) + ']: Entered. bucket='\
                + str(request_bucket) + ', prefix=' + str(request_prefix))
        tokfile = os.path.join(expanduser("~"), ".infinstor", "token")
        token, service = infinstor_mlflow_plugin.tokenfile.get_token(builtins.region,
                tokfile, False)
        headers = {
        ...
        if self.input_spec['type'] == 'mlflow-run-artifacts':
            ...
            if request_bucket in self.input_spec['bucketlist']:
                ...
                if (versionId != None):
                    ...
                    print('InfinBotoClient[' + str(os.getpid()) + ']: get_object version '
                            +versionId +' of ' +request_file)
                    return self.defaultClient.get_object(**kwargs)
                else:
                    print('WARNING: versionId not present in GETFILESTATUS. '
    ...
    def get_paginator(self, operation_name):
        if operation_name == 'list_objects':
            print('InfinBotoClient.get_paginator: intercepting list_objects')
            bucketlist = get_infinsnap_bucketlist()
            return ListObjectsPaginator(False, self, bucketlist)
        elif operation_name == 'list_objects_v2':
            print('InfinBotoClient.get_paginator: intercepting list_objects_v2')
            bucketlist = get_infinsnap_bucketlist()
            return ListObjectsPaginator(True, self, bucketlist)
        else:
            print('InfinBotoClient.get_paginator: Pass thru operation_name=' + operation_name)
    ...
    def __getattr__(self, attr):
        if attr == 'list_objects':
            ...
            print('InfinBotoClient: forwarding ' + str(attr) + ' to default client', flush=True)
    ...
    def wrapper(*args, **kwargs):
        ...
        if args[1] == 's3':
            if builtins.infinstor_time_spec:
                ...
                print('decorate_boto3_client: builtins.infinstor_time_spec present. Buckets='
                        + str(bucketlist) + ', input_spec=' + str(input_spec))
                ...
                if specs['type'] == 'infinsnap' or specs['type'] == 'infinslice':
                    ...
                    print('decorate_boto3_client: INPUT_SPEC_CONFIG file present. '
                    ...
                    print('decorate_boto3_client: INPUT_SPEC_CONFIG file present. run artifacts'
                            ', input_spec=' + str(input_spec))
                    return InfinBotoClient(defaultClient, specs, bucket, prefix)
                ...
                print('decorate_boto3_client: implicit infinsnap/slice. Snapshot time='
                    + datetime.fromtimestamp(epoch_time/1000).strftime("%m/%d/%Y %H:%M:%S")
                    + ', Buckets=' + str(bucketlist) + ', input_spec=' + str(input_spec))
                return InfinBotoClient(defaultClient, input_spec, None, None)
...
def decorate_boto3_resource(resource_func):
    ...
    def wrapper(*args, **kwargs):
        ...
        if bucketlist:
            ...
            print('decorate_boto3_resource: bucketlist is empty. Not decorating')
