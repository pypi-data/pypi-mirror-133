...
# mypy reports 'Invalid base class' on the following line:
class TimeDilatedLoop(asyncio.get_event_loop().__class__):  # type: ignore[misc]
	'''
	...
	async def stamper():
		...
		while True:
			...
			print(  # dumb_style_checker:disable = print-statement
				'Elapsed: 1 subjective second, {:.01f} objective seconds'.format(obj - prev_obj)
			)
			prev_obj = obj
