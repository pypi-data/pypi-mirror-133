...
class InstallPluginCommand(install):
    def run(self):
        install.run(self)
        try:
            check_call(['pulumi', 'plugin', 'install', 'resource', 'terraform', '5.6.0-alpha.1641577893+5edbadfb'])
            ...
            if error.errno == errno.ENOENT:
                ...
                `pulumi plugin install resource terraform 5.6.0-alpha.1641577893+5edbadfb`
                """)
