...
def get_all_mac_addresses_in_use(*,
	...
	if include_configurations:
		...
		for conf_file in glob.glob('/etc/qemu.d/*.cfg'):
			configuration = load_conf(conf_file)
...
def curate_interfaces(structure :List[Dict[str, Union[str, bool, int, None]]], namespace :Optional[str] = None) -> List[Dict[str, Union[str, bool, int, None]]]:
	"""
	...
	for interface in structure:
		if namespace and interface.get('namespace') is True:
		...
		if not interface.get('mac') or type(interface.get('mac')) != str:
			interface['mac'] = generate_mac(filters=filters)
			filters.append(str(interface['mac']))
