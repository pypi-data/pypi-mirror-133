...
def deploy(directory):
    ...
    """
    if not glob.glob("*.glide.json") and not glob.glob(directory + "/*.glide.json"):
    ...
    for file in glob.glob("*/*.glide.json"):
        with open(file, 'r') as f:
            ...
            if glideJson['project_name'] != f.name[len(directory)+1:].replace('.glide.json', ''):
...
def sites():
    ...
    """
    if not glob.glob("*.glide.json") and not glob.glob("*/*.glide.json"):
        err("ERROR: Glide file not found. Please run init command.")
