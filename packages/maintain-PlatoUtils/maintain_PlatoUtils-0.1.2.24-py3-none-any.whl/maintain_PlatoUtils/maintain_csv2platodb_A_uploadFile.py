...
import numpy as np
from maintain_PlatoUtils.maintain_PlatoUtils import existTag,wrapNebula2Df,existEdge,delEdge
...
def remakeRawSchema(rawSchemaJson,csvFolder="",gClient=None):
    ...
    for vertexI,_ in enumerate(rawSchemaJson["vertex"]):
        ...
        if vertexAttrProcess=="escape":
            ...
            for nodeIdItem in nodeIdList:
                if existTag(nodeType, nodeIdAttr, nodeIdItem,gClient)==True:
                    nodeItemGInfoDf=wrapNebula2Df(gClient.execute_query("LOOKUP ON {nodeType} WHERE {nodeType}.{nodeIdAttr}=='{nodeIdVal}' FETCH PROP ON {nodeType} $-.VertexID".format(nodeType=nodeType,
                    ...
                    for keyItem in nodeItemGInfoDict:
                        if "." in keyItem:
                            realcKeyItem=keyItem.split(".")[1]
                            if nodeItemGInfoDict[keyItem]!="null" and \
                                nodeItemGInfoDict[keyItem] is not None and\
                                (
                                    nodeItemCInfoDict[realcKeyItem] == "null" or\
                                    nodeItemCInfoDict[realcKeyItem] is None or\
                                    (type(nodeItemCInfoDict[realcKeyItem])==float and np.isnan(nodeItemCInfoDict[realcKeyItem]))
                                ):
                                nodeItemCInfoDict[realcKeyItem]=nodeItemGInfoDict[keyItem]
        ...
        if vertexDataProcess=="cover":
            ...
            for sysIdBatchI in range(0,len(sysIdList),1024):
                delSysGroupStr=",".join(sysIdList[sysIdBatchI:sysIdBatchI+1024])
                gClient.execute_query("DELETE VERTEX {}".format(delSysGroupStr))
