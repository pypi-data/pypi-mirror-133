...
def remakeRawSchema(rawSchemaJson,csvFolder="",gClient=None):
    ...
    for vertexI,_ in enumerate(rawSchemaJson["vertex"]):
        ...
        coverLabel=rawSchemaJson["vertex"][vertexI].get("cover_label","cover,escape")
        vertexAttrProcess=coverLabel.split(",")[0]
        vertexDataProcess=coverLabel.split(",")[1]

        # 节点属性控制
        if vertexAttrProcess=="escape":
            nodeType=rawSchemaJson["vertex"][vertexI]["node_type"]
            nodeIdAttr=rawSchemaJson["vertex"][vertexI]["id_col"]
            csvDfItem=csvDfItem.loc[existTag(nodeType, nodeIdAttr, csvDfItem[rawSchemaJson["vertex"][vertexI]["id_col"]],gClient)==False,:]
        elif vertexAttrProcess=="plus":
            nodeType=rawSchemaJson["vertex"][vertexI]["node_type"]
            nodeIdAttr=rawSchemaJson["vertex"][vertexI]["id_col"]
            nodeIdList=csvDfItem[rawSchemaJson["vertex"][vertexI]["id_col"]].values.flatten().tolist()
            csvDfItemList=[]
            for nodeIdItem in nodeIdList:
                if existTag(nodeType, nodeIdAttr, nodeIdItem,gClient)==True:
                    queryStr="LOOKUP ON {nodeType} WHERE {nodeType}.{nodeIdAttr}=='{nodeIdVal}'| FETCH PROP ON {nodeType} $-.VertexID".format(nodeType=nodeType,
                                                                                                                                                            nodeIdAttr=nodeIdAttr,
                                                                                                                                                            nodeIdVal=nodeIdItem)
                    print(queryStr)
                    nodeItemGInfoDf=wrapNebula2Df(gClient.execute_query(queryStr))
                    nodeItemGInfoDict=json.loads(nodeItemGInfoDf.to_json(orient="records"))[0]
                    nodeItemCInfoDf=csvDfItem.loc[csvDfItem[rawSchemaJson["vertex"][vertexI]["id_col"]]==nodeIdItem,:]
                    nodeItemCInfoDict=json.loads(nodeItemCInfoDf.to_json(orient="records"))[0]
                    for keyItem in nodeItemGInfoDict:
                        if "." in keyItem:
                            realGKeyItem=keyItem.split(".")[1]
                            if nodeItemGInfoDict[keyItem]!="null" and \
                                nodeItemGInfoDict[keyItem] is not None and\
                                (
                                    nodeItemCInfoDict[realGKeyItem] == "null" or\
                                    nodeItemCInfoDict[realGKeyItem] is None or\
                                    (
                                        type(nodeItemCInfoDict[realGKeyItem])==str and\
                                        len(nodeItemCInfoDict[realGKeyItem])==0
                                    ) or\
                                    (type(nodeItemCInfoDict[realGKeyItem])==float and np.isnan(nodeItemCInfoDict[realGKeyItem]))
                                ):
                                nodeItemCInfoDict[realGKeyItem]=nodeItemGInfoDict[keyItem]
                    csvDfItemList.append(nodeItemCInfoDict)
                else:
                    csvDfItemList.extend(json.loads(csvDfItem.loc[csvDfItem[rawSchemaJson["vertex"][vertexI]["id_col"]]==nodeIdItem,:].to_json(orient="records")))
    ...
    for edgeI,_ in enumerate(rawSchemaJson["edge"]):
        ...
        coverLabel=rawSchemaJson["edge"][edgeI].get("cover_label","cover,escape,escape")
        attrDataProcess,headEdgeDataProcess,tailEdgeDataProcess=coverLabel.split(",")

        # 边属性控制
        if attrDataProcess=="escape":
            stList=csvDfItem.loc[:,[srcIdAttr,tgtIdAttr]].values.tolist()
            newStList=[]
            csvDfItemList=[]
            for stPair in stList:
            ...
            for htItem in htList:
                ...
                htItemDf=wrapNebula2Df(gClient.execute_query("LOOKUP ON {headType} WHERE {headType}.{headIdAttrKey}=='{headIdAttrVal}'|\
        ...
        if headEdgeDataProcess=="cover":
            ...
            for batchI in range(0,len(oldSysId),1024):
