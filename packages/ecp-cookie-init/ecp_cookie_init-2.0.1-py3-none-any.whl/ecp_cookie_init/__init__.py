...
from ciecplib.tool.ecp_curl import (
    DEFAULT_COOKIE_FILE,
    main as ecp_curl,
)
...
def _inner(args):
    ...
    # store the cookies (that's what we came for in the first place)
    ciecplibargs.append("--store-session-cookies")

    # run the thing
    for idp in args.idp_hosts:
        curargs = list(ciecplibargs)
        if idp:
            curargs.extend(("--identity-provider", idp))
        if args.debug:
            print("Delegating call to ecp-curl...")
            print("$ ecp-curl {}".format(" ".join(curargs)))
        try:
            ecp_curl(curargs)
