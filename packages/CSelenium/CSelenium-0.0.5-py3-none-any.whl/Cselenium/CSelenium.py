...
class ChromeSelenium(CSeleniumABS):
    ...
    def __init__(self, url: str = None, is_interface: bool = True,
                 ...
                 info_tracking: bool = False,ignore_err:bool=True,wait_time: float = 1.0,
...
class CSeleniumThread(ChromeSelenium):
    ...
    '''
    def __init__(self, browser_names: list=["chrome"],*args,**kwargs):
        # 给父类驱动传-1,阻止打开浏览器
    ...
    def _getDriver(self,driver):
        ...
        if _driver == "ie" or _driver == "internet explorer":
            return webdriver.Ie
        if _driver == "safari":
    ...
    # 多线程运行,不稳定,测试中,Multithreaded running, unstable, testing
    def runThread(self):
        '''
        ...
        # ---多线程方案-放弃
        def _execFunc(self,e):
            # 设置驱动
            self.lock.acquire()  # 驱动加锁
            _driver_ = e[0]
            try:
                self.setDriver(_driver_())
            except:
                self.setDriver(_driver_(executable_path="/Users/lx/Downloads/geckodriver"))
            self.lock.release()
            # 逐个执行函数
            for _e in e[1:]:
                f_name = _e[0]
        ...
        temp_t = []
        for v in waiting_statement.values():
            # _execFunc(self,v)
            # 无法使用多线程,光标只有一个
            _t = threading.Thread(target=_execFunc,args=(self,v))
            # _t = Process(target=_execFunc,args=(self,v))
            temp_t.append(_t)
            # print("--->",v)
        # 启动线程
        for t in temp_t:
