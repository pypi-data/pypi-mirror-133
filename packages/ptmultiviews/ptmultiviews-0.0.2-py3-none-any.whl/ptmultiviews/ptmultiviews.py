...
class ptmultiviews:
    ...
    def run(self, args):
        ...
        if self.url_domain_file_test:
            if self._check_vulnerable(url_list[0]):
                ptmisclib.ptprint(ptmisclib.out_ifnot("Enumerated:", "INFO", self.use_json))
                try:
    ...
    def _enumerate_files(self, url):
        ...
        try:
            ...
            if self.file_test:
                ptmisclib.ptprint(ptmisclib.out_ifnot(f"Cannot connect to {url}, skipping", "ERROR", self.use_json))
                return
            else:
                ptmisclib.ptprint(ptmisclib.out_ifnot(f"Cannot connect to server - {url}", "ERROR", self.use_json))
                return
        if r.status_code == 406:
            ...
            for f in enumerated_files:
                ...
                ptmisclib.ptprint(ptmisclib.out_ifnot(path, "", self.use_json))
    ...
    def _check_vulnerable(self, url: str):
        ...
        try:
            ...
            ptmisclib.ptprint(ptmisclib.out_ifnot(f"Testing URL: {r.url}", "INFO", self.use_json))
            ptmisclib.ptprint(ptmisclib.out_ifnot(f"Status code: {r.status_code}", "INFO", self.use_json))
            ...
            if r.status_code in STATUS_CODES and r.headers.get("Vary") and "negotiate" in r.headers.get("Vary"):
                ptmisclib.ptprint(ptmisclib.out_ifnot(f"Multiviews: Enabled", "VULN", self.use_json))
                self.ptjsonlib.set_vulnerable(self.json_no, "True")
                return True
            if r.status_code == 200:
                ptmisclib.ptprint(ptmisclib.out_ifnot(f"Multiviews: Disabled", "NOTVULN", self.use_json))
                self.ptjsonlib.set_vulnerable(self.json_no, "False")
                return False
            if self.domain_file_test:
                self.ptjsonlib.set_vulnerable(self.json_no, "False")
                ptmisclib.ptprint(ptmisclib.out_ifnot(f"Multiviews: Disabled / Unknown status code, setting to false & going next url", "", self.use_json))
            else:
    ...
    def _process_parameters(self, url, domain, file_urls):
        ...
        if file_urls and not url and not domain:
            self.file_test = True
            ptmisclib.ptprint(ptmisclib.out_ifnot(f"Reading file: {file_urls}", "INFO", self.use_json))
            try:
                with open(file_urls, 'r') as fh:
        ...
        for line in file_handler:
            ...
            if self.file_test:
                if re.match("http[s]?$", l.scheme):
                    ...
                    if not is_error:
                        ptmisclib.ptprint(ptmisclib.out_ifnot(f"Found lines containing invalid URLs - they will be skipped.", "ERROR", self.use_json))
                        is_error = True
                    continue
