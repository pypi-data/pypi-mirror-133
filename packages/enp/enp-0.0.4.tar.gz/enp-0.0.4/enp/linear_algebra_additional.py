...
def arbitrary_partial_solution(a, b):
    ...
    if not does_system_of_linear_equations_have_solutions(a, b):
        raise ValueError
...
def find_normal_vector_using_equation(a, b, c, d):
    """
    Plane is given by the equation ax + by + cz + d = 0
    Find the normal vector to the plane.

    References
    ----------
    .. [1] `Normal (Wikipedia)
        <https://en.wikipedia.org/wiki/Normal_(geometry)>`_
    """
    print(f"Normal vector is n=({a}, {b}, {c})")
    return a, b, c
