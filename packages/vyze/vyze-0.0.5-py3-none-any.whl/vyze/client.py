...
class Client:
    ...
    def create_relation(self, origin_id: str, target_id: str, abstracts: List[str], name='', space=None):
        if not space:
            if not self.__space:
                raise RuntimeError('no universe or space selected')
            space = self.__space

        return self.__post(f'{self.__system_url}/relations', {
            'origin': origin_id,
            'target': target_id,
            'abstracts': self.resolve(abstracts),
            'space': space,
    ...
    def add_value(self, id: str, field: str, value):
        ...
        return self.create_relation(id, target['id'], [field_id])
    ...
    def __get_values_query(self, field_id: str, name: str, time=None, sampling=None) -> Tuple[str, str]:
        targets = self.get_targets(field_id)
        target_id = targets['ffffffffffffffffffffffffffffffff']
        data = self.__get_type(target_id)
        abbr = 'A' + field_id[:6]
