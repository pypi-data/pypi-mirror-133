    ...
    def load_claims(self):
        ...
        for label in self.history:
            for historic in self.history[label]:
                log.info("matching history: %s from %s to %s", label, historic.file_id, historic.label_id)
                if historic.label_id in self.claims:
                    ...
                    log.error('note: for nodes with multiple test cases, ensure that OC/OCL indexes are 2 wide')
                    log.error('warning: loose OCL file, unknown behavior, ignoring file')
                    log.error('file: %s', historic.file_id)
...
class KleeTestClaim:
    def __init__(self, claims_dir: ClaimsDirectory, filename: str):
        ...
        self.claim_json = read_json(self.claims_dir.location, filename)
