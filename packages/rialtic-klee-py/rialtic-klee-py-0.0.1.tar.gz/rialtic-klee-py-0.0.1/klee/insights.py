...
def load_insights() -> InsightDict:
    ...
    for function in (read_engine_result, read_local_files):
        try:
            insights += function()
        except:
...
def read_local_files(folder: str = '') -> InsightDict:
    insights, search_dirs = InsightDict(), ['test_claims', 'test_cases']
    # custom directory
    if folder:
    ...
    for directory in search_dirs:
        ...
        for option in ('insights_and_defense', 'insights', 'defense'):
        ...
        if os.path.exists(insight_json):
            for label, combined in files.read_json(insight_json):
