...
import pytest, re
from klee.consts import BINARY_FILE, log
from klee.cases import InsightEngineTestCase
from klee.files import KleeFile


with KleeFile(BINARY_FILE, 'rb') as file:
...
class TestEngineV1:
    ...
    def actual(self, label, response: InsightEngineResponse, expected: Insight) -> Insight:
        ...
        for item in response.insights:
            log.info('\n\ttype: %s, \n\tpolicy: %s \n\texit: %s \n\t\t%s', item.type, item.policy_name,
                " | ".join([f"{x.end_label}::{x.tree_name}" for x in item.trace]),
                "\n\t\t".join([f"{a} <- {q}" for t in item.trace for q, a in t.traversal]))
        ...
        for item in response.insights:
            if item.claim_line_sequence_num == line and item.trace[-1].end_label == label:
                insight = item
    ...
    def test_defense_message(self, actual, expected):
        fn = lambda x: x.defense.script.messages[0].message
        actual, expected = map(fn, (actual, expected))
