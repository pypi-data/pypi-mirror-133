...
class RuntimeCounter(object):
    ...
    def report(self, now):
        return "{}. {} / {}".format(next(self.number), self.interval(now), self.elapsed(now))
...
def main():
    ...
    parser.add_argument("command", nargs=argparse.REMAINDER, help="A command to execute")
    ...
    _main(args.command, patterns, args.recursive, args.path, args.initial_run, args.delay)
