...
def get_j_obj_to_py_obj_rules():
    ...
    rules = {
        ...
        'org.apache.flink.ml.api.misc.param.Params': lambda v: Params.fromJson(v.toJson()),
    }
    ...
    for py_cls in py_classes:
        rules[py_cls.j_cls_name] = py_cls
    return rules
...
def py_obj_to_j_obj(value):
    ...
    if isinstance(value, (JavaObjectWrapper,)):
        ...
        j_params = j_params_cls.fromJson(value.toJson())
        return j_params
    return value
...
def csv_content_to_dataframe(content, colnames, coltypes):
    ...
    force_dtypes = {
        colname: _G_ALINK_TYPE_TO_PTYPE.get(coltype)
        for colname, coltype in zip(colnames, coltypes)
        if coltype == "VARCHAR" or coltype == "STRING"
    }
    date_colnames = [
        colname
        for colname, coltype in zip(colnames, coltypes)
