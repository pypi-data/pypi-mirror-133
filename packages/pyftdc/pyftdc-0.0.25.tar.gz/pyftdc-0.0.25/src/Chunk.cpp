...
Chunk::Chunk (const uint8_t *data, size_t size, int64_t id=-1, bool logMetricNames=0) {
    ...
    this->id = id/1000; //TODO: Why divide?
...
Chunk::ConstructMetrics(const uint8_t* data ) {
    ...
    BOOST_LOG_TRIVIAL(debug) << "Metrics in chunk to reserve: " << metricsInChunk ;
...
Chunk::getJsonAtPosition(size_t position) {
    ...
    auto newBson = bson_copy(bsonStruct);
            ...
            if (bson_iter_find_descendant(&iter, metrics[i]->name.c_str(), &baz)) {
                ...
                switch (t) {
                    ...
                    case BSON_TYPE_BOOL:
                        ...
                        break;
                    case BSON_TYPE_UTF8:
                        //bson_iter_overwrite(&baz, metrics[i]->values[position]);
                        break;
                }
            }
        }
