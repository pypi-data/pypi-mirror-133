...
from certora_cli.certoraUtils import debug_print_, print_general_error_before_exit
...
from certora_cli.certoraUtils import print_warning, Mode, get_version
...
def main_with_args(args: List[str], is_library: bool = False) -> None:
    ...
    timings = {}
    try:
        collect_run_metadata(wd='.', raw_args=sys.argv, conf_dict=conf_dict, args=parsed_args, debug=DEBUG)\
            .dump(debug=DEBUG)
        ...
        if parsed_args.build_only:
            ...
            if parsed_args.local:
                ...
                run_cmd(check_cmd, compare_with_tool_output, RUN_IS_LIBRARY)
                ...
                '''
                Before running the local type checker, we see if the current package version is compatible with the
                latest. We check it before running the local type checker, because local type checking errors could be
                simply a result of syntax introduced in the newest version.
                '''
                ...
                if mode_has_spec_file(parsed_args.mode):
                    if parsed_args.disableLocalTypeChecking:
                        ...
                        run_local_spec_check(with_typechecking=True, is_library=RUN_IS_LIBRARY)
                ...
                if parsed_args.typecheck_only:
                    exit_if_not_library(0)
                else:
                    cv = CloudVerification(parsed_args, timings)
                    ...
                    result = cv.cli_verify_and_report(cl_args, parsed_args.send_only)
        ...
        if cv:  # delete the inputs zip file
            remove_file(cv.ZipFileName)
...
def type_tool_output_path(filename: str) -> str:
    ...
    if os.path.isfile(filename):
        print_warning(f"--toolOutPut {filename} file already exists")
    ...
    if not args.disable_auto_cache_key_gen and not os.environ.get("CERTORA_DISABLE_AUTO_CACHE") is not None:
        if args.mode == Mode.VERIFY or args.mode == Mode.ASSERT:
            if args.cache is None:
                ...
                debug_print(f"setting cache key to {args.cache}")
...
def check_packages_arguments(args: argparse.Namespace) -> None:
    ...
    if args.packages is not None and len(args.packages) > 0:
        ...
        if not os.path.exists(PACKAGE_FILE):
            ...
            print_warning(f"No read permissions for default package file {PACKAGE_FILE}")
        else:
            try:
...
def validate_certora_key(args: argparse.Namespace) -> None:
    ...
    if not key:
        key = PUBLIC_KEY
        print_warning("Couldn't find Certora Key. Proceeding with Certora Public Key.")
        print_warning("If you have a private key, press ctrl+c and set the environment variable CERTORAKEY")
        time.sleep(1)
...
def check_args_post_argparse(args: argparse.Namespace) -> None:
    ...
    if args.send_only:
        if args.local:
            print_warning("--send_only has no effect in local tool runs")

        if args.short_output:
            print_warning("When using --send_only, --short_output is automatically enabled; "
...
def __get_argparser() -> argparse.ArgumentParser:
    ...
    run_args.add_argument("--rule_sanity", action='store_true',
                          help="Check that all rules are not vacuous - there is an input that reaches their final "
                               "assertion (we expect the callback function only to fail this check)")
    ...
    info_args.add_argument("--debug", action='store_true', help="Use this flag to see debug prints")
...
def prepare_replay_mode(args: argparse.Namespace) -> None:
    ...
    if replay_conf:
        debug_print_("using conf from replay to update args", DEBUG)
        read_from_conf(replay_conf, args)
    elif replay_namespace:
        debug_print_("using args from replay file as args", DEBUG)
...
def dump_replay_files(replay_json_filename: str) -> Tuple[Optional[Dict[str, Any]], Optional[argparse.Namespace]]:
    ...
    with open(replay_json_filename, "r") as replay_json_file:
        ...
        try:
            ...
            debug_print_(f' read conf from replay json: {conf_json}', DEBUG)
            namespace = None
        except KeyError:
            ...
            debug_print_(f' parsed back raw_args from replay json: {namespace}', DEBUG)
            conf_json = None
