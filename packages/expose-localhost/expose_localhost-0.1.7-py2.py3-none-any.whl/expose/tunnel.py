import json
import logging
from os import environ, path, rename, system, remove, getcwd
...
class Tunnel:
    ...
    def _configure_vm(self, public_dns: str, public_ip: str):
        ...
        copy_files = [f"{getcwd()}{SEP}server.conf", f"{getcwd()}{SEP}nginx.conf"]
        if path.isdir(f"{HOME_DIR}{SEP}.ssh") and \
            ...
            secured = True
            copy_files.extend([f"{HOME_DIR}{SEP}.ssh{SEP}cert.pem", f"{HOME_DIR}{SEP}.ssh{SEP}key.pem",
                              f"{getcwd()}{SEP}options-ssl-nginx.conf"])
            rename(src="nginx-ssl.conf", dst="nginx.conf")
        elif path.isfile('cert.pem') and path.isfile('key.pem'):
            self.logger.info(f'Found certificate and key in {path}')
            secured = True
            copy_files.extend([f"{getcwd()}{SEP}cert.pem", f"{getcwd()}{SEP}key.pem",
                               f"{getcwd()}{SEP}options-ssl-nginx.conf"])
            rename(src="nginx-ssl.conf", dst="nginx.conf")
        elif generate_cert(common_name=endpoint, email_address=email):
            self.logger.info('Generated self-signed SSL certificate and private key.')
            secured = True
            CONFIGURATION_FILES.extend(['cert.pem', 'key.pem'])
            copy_files.extend([f"{getcwd()}{SEP}cert.pem", f"{getcwd()}{SEP}key.pem",
                               f"{getcwd()}{SEP}options-ssl-nginx.conf"])
            rename(src="nginx-ssl.conf", dst="nginx.conf")
        else:
            ...
            secured = False
            rename(src="nginx-non-ssl.conf", dst="nginx.conf")
        ...
        CONFIGURATION_FILES.extend(['nginx.conf'])
