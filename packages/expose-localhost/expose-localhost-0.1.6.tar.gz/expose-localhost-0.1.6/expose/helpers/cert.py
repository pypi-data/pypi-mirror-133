import json
import binascii
from os import getlogin, rename, stat, urandom
...
def generate_cert(common_name: str,
                  email_address: str = f"{getlogin()}@expose-localhost.com",
    ...
    Args:
        ...
        organization_name: Name of the organization. Defaults to ``Amazon``
        organization_unit_name: Name of the organization unit. Defaults to ``AWS``
    ...
    if not (serial := cert.get_serial_number()):
        serial = _generate_serial_hash()
