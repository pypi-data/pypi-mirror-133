...
CONFIGURATION_LOCATION = f'https://raw.githubusercontent.com/thevickypedia/expose/main/configuration/'
...
class Tunnel:
    ...
    def _configure_vm(self, public_dns: str, public_ip: str):
        ...
        for conf_file in CONFIGURATION_FILES:
            ...
            response = requests.get(url=f'{CONFIGURATION_LOCATION}{conf_file.lstrip(CURRENT_DIR)}')
            if not response.ok:
                raise ConnectionError(f'Failed to download the config file: '
                                      f'{CONFIGURATION_LOCATION}{conf_file.lstrip(CURRENT_DIR)}')
