// kaldifst/csrc/util/kaldi-holder-inl.h
...
#ifndef KALDIFST_CSRC_UTIL_KALDI_HOLDER_INL_H_
#define KALDIFST_CSRC_UTIL_KALDI_HOLDER_INL_H_
 ...
 public:
  ...
  static bool Write(std::ostream &os, bool binary, const T &t) {
    ...
    try {
      ...
      KALDIFST_WARN << "Exception caught writing Table object. " << e.what();
  ...
  bool Read(std::istream &is) {
    ...
    if (!InitKaldiInputStream(is, &is_binary)) {
      KALDIFST_WARN << "Reading Table object, failed reading binary header\n";
      return false;
    }
    try {
      ...
      KALDIFST_WARN << "Exception caught reading Table object. " << e.what();
      delete t_;
      t_ = NULL;
      return false;
    }
  ...
  T &Value() {
    // code error if !t_.
    if (!t_) KALDIFST_ERR << "KaldiObjectHolder::Value() called wrongly.";
    return *t_;
  }
...
template<class BasicType> class BasicHolder {
 public:
  ...
  bool Read(std::istream &is) {
    bool is_binary;
    if (!InitKaldiInputStream(is, &is_binary)) {
      KALDIFST_WARN << "Reading Table object [integer type], failed reading binary"
          " header\n";
      return false;
    }
    try {
      ...
      if (!is_binary) {  // This is to catch errors, the class would work
        ...
        if (is.peek() == '\n') {
          KALDIFST_WARN << "Found newline but expected basic type.";
      ...
      if (!is_binary) {  // This is to catch errors, the class would work
        ...
        if (is.peek() != '\n') {
          KALDIFST_WARN << "BasicHolder::Read, expected newline, got "
 ...
 public:
  ...
  static bool Write(std::ostream &os, bool binary, const T &t) {
    ...
    try {
      if (binary) {  // need to write the size, in binary mode.
        KALDIFST_ASSERT(static_cast<size_t>(static_cast<int32_t>(t.size())) ==
                     t.size());
      ...
      KALDIFST_WARN << "Exception caught writing Table object (BasicVector). "
                 << e.what();
      return false;  // Write failure.
    }
  }
  ...
  bool Read(std::istream &is) {
    ...
    if (!is_binary) {
      ...
      if (is.fail()) {
        KALDIFST_WARN << "BasicVectorHolder::Read, error reading line " <<
            (is.eof() ? "[eof]" : "");
      ...
      try {
        ...
        KALDIFST_WARN << "BasicVectorHolder::Read, could not interpret line: "
                   << "'" << line << "'" << "\n" << e.what();
        return false;
      }
    } else {  // binary mode.
      ...
      try {
        ...
        KALDIFST_WARN << "BasicVectorHolder::Read, read error or unexpected data"
...
template<class BasicType> class BasicVectorVectorHolder {
 public:
    ...
    try {
      if (binary) {  // need to write the size, in binary mode.
        ...
        for (typename std::vector<std::vector<BasicType> >::const_iterator
                 iter = t.begin();
             iter != t.end(); ++iter) {
          KALDIFST_ASSERT(static_cast<size_t>(static_cast<int32_t>(iter->size()))
                       == iter->size());
  ...
  bool Read(std::istream &is) {
    ...
    bool is_binary;
    if (!InitKaldiInputStream(is, &is_binary)) {
      KALDIFST_WARN << "Failed reading binary header\n";
      return false;
    }
    if (!is_binary) {
      // In text mode, we terminate with newline.
      try {  // catching errors from ReadBasicType..
        ...
        while (1) {
          int i = is.peek();
          if (i == -1) {
            KALDIFST_WARN << "Unexpected EOF";
            return false;
          } else if (static_cast<char>(i) == '\n') {
            if (!v.empty()) {
              KALDIFST_WARN << "No semicolon before newline (wrong format)";
              return false;
            } else {
              is.get();
              return true;
        ...
        KALDIFST_WARN << "BasicVectorVectorHolder::Read, read error. " << e.what();
        return false;
      }
    } else {  // binary mode.
      size_t filepos = is.tellg();
      try {
        ...
        KALDIFST_WARN << "Read error or unexpected data at archive entry beginning"
...
template<class BasicType> class BasicPairVectorHolder {
 public:
  ...
  static bool Write(std::ostream &os, bool binary, const T &t) {
    ...
    try {
      if (binary) {  // need to write the size, in binary mode.
        KALDIFST_ASSERT(static_cast<size_t>(static_cast<int32>(t.size())) ==
                     t.size());
        // Or this Write routine cannot handle such a large vector.
  ...
  bool Read(std::istream &is) {
    ...
    if (!is_binary) {
      ...
      try {  // catching errors from ReadBasicType..
        ...
        while (1) {
          ...
          if (i == -1) {
            ...
            if (t_.empty() && v.empty()) {
              ...
              KALDIFST_WARN << "Unexpected newline, reading vector<pair<?> >; got "
            ...
            if (v.size() != 2) {
              KALDIFST_WARN << "Wrong input format, reading vector<pair<?> >; got "
                         << v.size() << " elements, expected 2.";
              return false;
            }
            t_.push_back(std::make_pair(v[0], v[1]));
        ...
        KALDIFST_WARN << "BasicPairVectorHolder::Read, read error. " << e.what();
        return false;
      }
    } else {  // binary mode.
      size_t filepos = is.tellg();
 ...
 public:
  ...
  static bool Write(std::ostream &os, bool, const T &t) {  // ignore binary-mode
    for (std::vector<std::string>::const_iterator iter = t.begin();
         iter != t.end();
         ++iter) {
      KALDIFST_ASSERT(IsToken(*iter));  // make sure it's whitespace-free,
  ...
  bool Read(std::istream &is) {
    ...
    getline(is, line);  // this will discard the \n, if present.
    if (is.fail()) {
      KALDIFST_WARN << "BasicVectorHolder::Read, error reading line " << (is.eof()
                                                                       ? "[eof]" : "");
      return false;  // probably eof.  fail in any case.
    }
...
class HtkMatrixHolder {
 public:
  ...
  static bool Write(std::ostream &os, bool binary, const T &t) {
    if (!binary)
      KALDIFST_ERR << "Non-binary HTK-format write not supported.";
    bool ans = WriteHtk(os, t.first, t.second);
    if (!ans)
      KALDIFST_WARN << "Error detected writing HTK-format matrix.";
    return ans;
  }
  ...
  bool Read(std::istream &is) {
    ...
    if (!ans) {
      KALDIFST_WARN << "Error detected reading HTK-format matrix.";
      return false;
    }
    return ans;
  }
...
template<int kFeatDim> class SphinxMatrixHolder {
 public:
  ...
  static bool Write(std::ostream &os, bool binary, const T &m) {
    if (!binary) {
      KALDIFST_WARN << "SphinxMatrixHolder can't write Sphinx features in text ";
      return false;
    }
    ...
    for (MatrixIndexT i = 0; i < m.NumRows(); i++) {
      std::vector<float> tmp(m.NumCols());
      for (MatrixIndexT j = 0; j < m.NumCols(); j++) {
        tmp[j] = static_cast<float>(m(i, j));
        if (MachineIsLittleEndian())
          KALDI_SWAP4(tmp[j]);
      }
  ...
  bool Read(std::istream &is) {
    ...
    if (MachineIsLittleEndian())
      KALDI_SWAP4(nmfcc);
    KALDIFST_LOG << "#feats: " << nmfcc;
    int32 nfvec = nmfcc / kFeatDim;
    if ((nmfcc % kFeatDim) != 0) {
      KALDIFST_WARN << "Sphinx feature count is inconsistent with vector length ";
      return false;
    }
    ...
    for (MatrixIndexT i = 0; i < feats_.NumRows(); i++) {
      if (sizeof(BaseFloat) == sizeof(float)) {
        is.read(reinterpret_cast<char*> (feats_.RowData(i)),
                kFeatDim * sizeof(float));
        if (!is.good()) {
          KALDIFST_WARN << "Unexpected error/EOF while reading Sphinx features ";
          return false;
        }
        if (MachineIsLittleEndian()) {
 ...
 private:
  KALDIFST_DISALLOW_COPY_AND_ASSIGN(SphinxMatrixHolder);
  T feats_;
};
...
#endif  // KALDIFST_CSRC_UTIL_KALDI_HOLDER_INL_H_
