// kaldifst/csrc/util/kaldi-io.cc
...
#include "kaldifst/csrc/util/kaldi-pipebuf.h"
#include "kaldifst/csrc/util/kaldi-table.h"
#include "kaldifst/csrc/matrix/kaldi-matrix.h"
...
#include "kaldifst/csrc/util/parse-options.h"
...
class FileOutputImpl: public OutputImplBase {
 public:
  virtual bool Open(const std::string &filename, bool binary) {
    if (os_.is_open()) KALDIFST_ERR << "FileOutputImpl::Open(), "
  ...
  virtual ~FileOutputImpl() {
    if (os_.is_open()) {
      os_.close();
      if (os_.fail())
        KALDIFST_ERR << "Error closing output file " << filename_;
    }
  }
 private:
...
class StandardOutputImpl: public OutputImplBase {
 public:
  ...
  virtual bool Open(const std::string &filename, bool binary) {
    if (is_open_) KALDIFST_ERR << "StandardOutputImpl::Open(), "
                     "open called on already open file.";
#ifdef _MSC_VER
  ...
  virtual ~StandardOutputImpl() {
    if (is_open_) {
      std::cout << std::flush;
      if (std::cout.fail())
        KALDIFST_ERR << "Error writing to standard output";
    }
  }
 private:
  bool is_open_;
...
class PipeOutputImpl: public OutputImplBase {
 public:
  ...
  virtual bool Open(const std::string &wxfilename, bool binary) {
    ...
    KALDIFST_ASSERT(wxfilename.length() != 0 && wxfilename[0] == '|');  // should
    // start with '|'
  ...
  virtual std::ostream &Stream() {
    if (os_ == NULL) KALDIFST_ERR << "PipeOutputImpl::Stream(),"
  ...
  virtual bool Close() {
    if (os_ == NULL) KALDIFST_ERR << "PipeOutputImpl::Close(), file is not open.";
    bool ok = true;
    os_->flush();
  ...
  virtual ~PipeOutputImpl() {
    if (os_) {
      if (!Close())
        KALDIFST_ERR << "Error writing to pipe " << PrintableWxfilename(filename_);
    }
  }
 private:
  std::string filename_;
...
class FileInputImpl: public InputImplBase {
 public:
  virtual bool Open(const std::string &filename, bool binary) {
    if (is_.is_open()) KALDIFST_ERR << "FileInputImpl::Open(), "
                                << "open called on already open file.";
...
class StandardInputImpl: public InputImplBase {
 public:
  ...
  virtual bool Open(const std::string &filename, bool binary) {
    if (is_open_) KALDIFST_ERR << "StandardInputImpl::Open(), "
                     "open called on already open file.";
    is_open_ = true;
#ifdef _MSC_VER
  ...
  virtual int32_t Close() {
    if (!is_open_) KALDIFST_ERR << "StandardInputImpl::Close(), file is not open.";
    is_open_ = false;
    return 0;
  }
...
class OffsetFileInputImpl: public InputImplBase {
 ...
 public:
  ...
  static void SplitFilename(const std::string &rxfilename,
    ...
    KALDIFST_ASSERT(pos != std::string::npos);  // would indicate error in calling
    ...
    *filename = std::string(rxfilename, 0, pos);
    ...
    if (!ans)
      KALDIFST_ERR << "Cannot get offset from filename " << rxfilename
...
template <> void ReadKaldiObject(const std::string &filename,
  ...
  if (!filename.empty() && filename[filename.size() - 1] == ']') {
    ...
    if (!ExtractRangeSpecifier(filename, &rxfilename, &range)) {
      KALDIFST_ERR << "Could not make sense of possible range specifier in filename "
    ...
    if (!ExtractObjectRange(temp, range, m)) {
      KALDIFST_ERR << "Error extracting range of object: " << filename;
    }
  } else {
