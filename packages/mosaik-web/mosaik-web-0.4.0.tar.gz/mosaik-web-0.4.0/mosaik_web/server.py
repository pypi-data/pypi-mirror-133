...
class Server(object):
    def __init__(self, env, server_sock, activate_ssl=False):
        self.env = env
        ...
        if self.activate_ssl:
            mosaik_js = os.path.abspath(os.path.join(self.basedir, 'media/mosaik.js'))
            mosaik_js_ssl = os.path.abspath(os.path.join(self.basedir, 'media/mosaik_ssl.js'))

            if not os.path.isfile(mosaik_js_ssl):
                with open(mosaik_js, 'r') as reader:
                    lines = reader.readlines()
                with open(mosaik_js_ssl, 'w') as writer:
                    for line in lines:
                        writer.write(line.replace('ws://', 'wss://'))
    ...
    def serve_static(self, uri):
        if self.activate_ssl and uri == '/media/mosaik.js':
