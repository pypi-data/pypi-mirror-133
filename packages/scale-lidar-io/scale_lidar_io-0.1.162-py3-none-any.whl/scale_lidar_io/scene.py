...
class LidarScene:
    ...
    def to_dict(self, base_url: str = None) -> dict:
        ...
        return dict(
            frames=[
                "%s/frame-%s.json" % (base_url, frame.id) for frame in self.frames
            ]
        )
    ...
    def scale_file_upload(
        ...
        :type verbose: Set to false to not show the progress bar
        ...
        frames_attachments_urls = []
        with futures.ThreadPoolExecutor(max_workers=UPLOAD_POOL_SIZE) as executor:
            if verbose is False:
                future_list = [
                    executor.submit(LidarFrame.scale_file_upload, frame, project_name)
                    for frame in self.frames
                ]
            else:
                future_list = [
        ...
        for attachment in futures.as_completed(future_list):
            frames_attachments_urls.append(attachment.result())

        self.scale_file_attachments = frames_attachments_urls

        print(f"Finishes uploading scene to Scale file, uploaded {len(frames_attachments_urls)} frames")
