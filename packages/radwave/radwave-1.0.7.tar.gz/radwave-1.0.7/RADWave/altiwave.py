...
class waveAnalysis(object):
    """
    **RadWave** is a package to perform wave analysis using AODN *Radar
    Altimeter Data*. Radar altimeters are mounted on satellites and measure a
    footprint of the ocean directly under the satellite (approx. 5-7 km wide).
    When the water surface is calm and flat, the reflection of the radar pulse
    back to the altimeter is almost instantaneous. In contrast, when waves are
    present the pulse is first reflected at the crest of the wave, then
    progressively reflected as the pulse reaches the wave trough.

    Method:
        Calibrated altimeter dataset provide **wave height** and **wind speed**
        through a series of equations described in [RibalYoung2019]

    .. [RibalYoung2019]   Ribal, A. & Young, I. R. -
        33 Years of Globally Calibrated Wave Height and Wind Speed Data Based
        on Altimeter Observations. Sci Data 6, 77,
        `DOI: 10.1038/s41597-019-0083-9`_, 2019.

    .. _`DOI: 10.1038/s41597-019-0083-9`: https://www.nature.com/articles/s41597-019-0083-9

    Data:
        Altimeter data are read from netcdf files that are provided via a list
        of URLs. Here we propose a series of examples using the Australian
        Ocean Data Network (AODN) (`https://portal.aodn.org.auIMOS`_) but other
        web portals could be easely added.
    ...
    Note:
        This remote sensing technique does not provide individual wave
        statistics, but rather returns the **average value over satellite
    ...
    def __init__(
        ...
        if cycloneCSV is not None:
            ...
            for col in column_list:
                if col not in self.cyclone.columns:
                    raise ValueError(
                        "Both columns lat, lon and datetime are \
                                     required in the cyclone track file"
                    )
        ...
        if altimeterURL is not None:
            ...
            for k in range(self.satNb):
                satFile = self._extractURLsatellite(
                    fileURL=altimeterURL, satName=satNames[k]
                )
                if len(satFile) > 0:
            ...
            dhours = delta_time.total_seconds() / 3600.0
            if dhours <= 0.0:
                raise ValueError(
                    "Error the defined start time 'stime' is set \
                                 after the end time 'etime'."
                )

    def _extractURLsatellite(self, fileURL, satName):
        """
        Function to convert the list of URL’s generated by the IMOS portal to a
        list of OPeNDAP data URL’s for specific satellites.
        From downloaded dataset from the AODN portal user choose the options
        *List of URLs* to obtain NetCDF files. IMOS NetCDF files are accessible
        via an OPeNDAP catalog at : `IMOScatalog`_

        .. _`IMOScatalog`: http://thredds.aodn.org.au/thredds/catalog/IMOS/catalog.html

        The advantage of the THREDDS catalog is that the data files do not have
        to be downloaded to your local machine first – the OPeNDAP data URL
        can be parsed into Python.

        Note:
            The list of URL’s generated by the IMOS portal can be converted to
            a list of OPeNDAP data URL’s by replacing string
            *http://data.aodn.org.au/IMOS/opendap* with
    ...
    def processAltimeterData(
        ...
        The function can take some times to execute depending on the number of
        files to load and the size of the dataset to query.

        Todo:
            Here we use default NetCDF variables keys based on the IMOS AODN
            dataset: 'LATITUDE'/'LONGITUDE'/'WSPD_CAL'...
            In the future, we could provide additional capabilities to extract
            additional keys by reading each file variables
            [vars = data_nc.variables.keys()]

        Note:
            This function relies mostly on Pandas (library) and writes the
            processed dataset to file that can be later used to access more
            efficiently altimeter information.

        Warning:
            Because the data is accessed via the THREDDS catalog, there can be
        ...
        for u in range(len(self.allURL)):
            ...
            if altimeter_pick == self.nameSat[u] or altimeter_pick == "all":
                for k in range(len(picked_url)):
                    ...
                    if k == 0:
                        print(
                            "   +  name {:<11s} / number of tracks \
                              {:<4d}".format(
                                self.nameSat[u], len(picked_url)
                            )
                        )
                    ...
                    latbound = np.where(
                        np.logical_and(lats >= self.latmin, lats <= self.latmax)
                    )[0]
                    lonbound = np.where(
                        np.logical_and(lons >= self.lonmin, lons <= self.lonmax)
                    )[0]
                    ...
                    timebound = np.where(
                        np.logical_and(
                            timing >= self.start_date, timing <= self.end_date
                        )
                    )[0]
        ...
        if p > 0:
            ...
            combineframe.to_csv(
                str(self.saveCSV),
                columns=["lat", "lon", "wh", "time", "ws"],
                sep=" ",
                index=False,
                header=1,
            )
            altiData = pd.read_csv(
                str(self.saveCSV),
                sep=r"\s+",
                engine="c",
                header=0,
    ...
    def readAltimeterData(self, saveCSV=None):
        """
        In case where the *processAltimeterData* function has already been
        executed, one can load directly the processed data from the created CSV
        file.
    ...
    def plotCycloneTracks(
        self,
        title="Cyclone data tracks",
        markersize=100,
        zoom=4,
        extent=None,
        fsize=(12, 10),
        ...
        """
        This function **plots** and **saves** in a figure a specific cyclone
        track loaded during the initialisation phase of the waveAnalysis class.

        Example of cyclone track for Australia can be found on the Australian
        Bureau of Meteorology (BOM_).
        ...
        Warning:
            The cyclone tracks contains the following variables *lon*,
            *lat* & *datetime*.
        """
    ...
    def visualiseData(
        self,
        title="Altimeter data coordinates",
        extent=None,
        addcity=None,
        markersize=100,
        zoom=4,
        ...
        """
        This function **plots** and **saves** in a figure the geographical
        coordinates of processed altimeter data. In case where a cyclone
        track has also been loaded during the initialisation phase of the
        waveAnalysis class, the cyclone path is also added to the figure.
    ...
    def _waveAge(self, H, U, grav=9.80665):
        """
        This function computes the wave age from the following altimeter data:
        *wave height* and *surface wind speed*.
    ...
    def wavePeriod(self, H, U, grav=9.80665):
        """
        This function computes the wave period from the following altimeter
        data: *wave height* and *surface wind speed*.

        Method:
            Altimeters do not directly measure wave period, an essential
            component to adequately characterise wave climate.
            To calculate wave period from altimeter backscatter we use the
            algorithm proposed by Remya et al. (2010) based on
            wind speed (U) and significant wave height (H). This method has a
            high accuracy of estimating wave period (T) in both wind
            and swell-generated seas (root mean square error = 0.76 s).

            Remya et al., 2010: Altimeter-derived ocean wave period using
            genetic algorithm - IEEE Geoscience and Remote Sensing Letters 8(2),
            354–358.
    ...
    def meanEnergy(self, H, grav=9.80665, seadensity=1025.0):
        """
        In a sea state, the average(mean) energy density per unit area of
        gravity waves on the water surface is proportional
        ...
        Method:
            ...
            World Scientific, ISBN 978-981-02-3256-6.

            Holthuijsen L.H., 2007: Waves in oceanic and coastal waters -
            Cambridge: Cambridge University Press, ISBN 978-0-521-86028-4.
    ...
    def waveGroupVelocity(self, T, grav=9.80665):
        """
        As the waves propagate, their energy is transported and the energy
        transport velocity is the group velocity.
    ...
    def waveEnergyFlux(self, H, T, grav=9.80665, seadensity=1025.0):
        """
        The rate at which energy is carried by waves is determined using total
        wave energy and wave energy speed (kW/m)
        ...
        E = self.meanEnergy(H, grav=9.80665, seadensity=1025.0)
    ...
    def generateTimeSeries(self, days=30):
        ...
        Time series compute the following **instantaneous** and **monthly**
        variables:
        ...
        Args:
            days (int): moving average window for time series calculation [default: '30']

        Note:
            The class **waveAnalysis()** saves a pandas dataframe called
            **timeseries** that stores the wave time series
            for further analysis.

        Returns:
            timeseries (dataframe): pandas dataframe containing time series of wave characteristics
        """
        ...
        self.timeseries = pd.DataFrame(
            data={
                "date": sort_time,
                "wh": self.wh,
                "wh_rolling": self.wh_rolling,
                "period": self.T,
                "period_rolling": self.period_rolling,
                "power": self.power1,
                "power_rolling": self.power_rolling,
                "energy": self.we,
                "energy_rolling": self.energy_rolling,
                "speed": self.speed,
                "speed_rolling": self.speed_rolling,
                "lat": self.lat,
                "lon": self.lon,
            }
        )
    ...
    def plotTimeSeries(self, time="all", series="H", fsize=(12, 5), fsave=None):
        """
        This function **plots** and **saves** in a figure a time series for a
        specific parameter from the processed altimeter data.
        ...
        if time == "all":
            ...
            tmpdf = self.timeseries.copy()
            tmpdf = tmpdf[
                (tmpdf["date"].dt.year > time[0]) & (tmpdf["date"].dt.year < time[1])
            ]
        ...
        if series == "H":
            ...
            ax1.plot(
                ...
                tmpdf.wh_rolling,
                color="blue",
                label=str(self.days) + "-Day Average $H_s$",
            )

            ax1.legend(
                labels=["wave height $H_s$", str(self.days) + "-Day Average $H_s$"],
                loc="upper left",
            )
            ...
            print(
                "95th percentile wave height: {:0.3f} m".format(
                    np.percentile(tmpdf.wh, 95)
                )
            )
            ...
            ax1.plot(
                ...
                tmpdf.period,
                color="lightgrey",
                label="wave period T$_\mathit{z}$",
            )
            ax1.plot(
                tmpdf.date,
                tmpdf.period_rolling,
                color="blue",
                label=str(self.days) + "-Day Average $H_s$",
            )
            ...
            ax1.legend(
                labels=[
                    "wave period T$_\mathit{z}$",
                    str(self.days) + "-Day Average T$_\mathit{z}$",
                ],
                loc="upper left",
            )
            ...
            print(
                "95th percentile wave period: {:0.3f} s".format(
                    np.percentile(tmpdf.period, 95)
                )
            )
            ...
            ax1.plot(
                ...
                tmpdf.power_rolling,
                color="blue",
                label=str(self.days) + "-Day Average $\mathit{P}$",
            )

            ax1.legend(
                labels=[
                    "wave power $\mathit{P}$",
                    str(self.days) + "-Day Average $\mathit{P}$",
                ],
                loc="upper left",
            )
            ...
            print(
                "95th percentile wave power: {:0.3f} kW/m".format(
                    np.percentile(tmpdf.power, 95)
                )
            )
            ...
            ax1.plot(
                ...
                tmpdf.speed,
                color="lightgrey",
                label="wave celerity $\mathit{C}$",
            )
            ax1.plot(
                tmpdf.date,
                tmpdf.speed_rolling,
                color="blue",
                label=str(self.days) + "-Day Average $\mathit{C}$",
            )

            ax1.legend(
                labels=[
                    "wave celerity $\mathit{C}$",
                    str(self.days) + "-Day Average $\mathit{C}$",
                ],
                loc="upper left",
            )
            ...
            print(
                "95th percentile wave celerity: {:0.3f} m/s".format(
                    np.percentile(tmpdf.speed, 95)
                )
            )
            ...
            ax1.plot(
                ...
                tmpdf.energy,
                color="lightgrey",
                label="wave energy $\mathit{E}$",
            )
            ax1.plot(
                tmpdf.date,
                tmpdf.energy_rolling,
                color="blue",
                label=str(self.days) + "-Day Average $\mathit{E}$",
            )

            ax1.legend(
                labels=[
                    "wave energy $\mathit{E}$",
                    str(self.days) + "-Day Average $\mathit{E}$",
                ],
                loc="upper left",
            )
            ...
            print(
                "95th percentile wave energy: {:0.3f} J/m2".format(
                    np.percentile(tmpdf.energy, 95)
                )
            )

        else:
            raise ValueError(
                "Not recognised series... choices are H, P, T, \
                             Cg, E"
            )
    ...
    def close2Track(self, radius=2.0, dtmax=6):
        """
        From all cyclone tracks, this function finds the closest processed
        altimeter geographical locations that have been recorded in the
        database.
        In addition to their coordinates, the altimeter dataset has to be
        recorded during a user defined time lapse close enough to the cyclone
        path time at each position.

        The recorded dataframe **cyclone_data** contains the
        following variables:
        ...
        if self.cyclone is None:
            raise ValueError(
                "There is no cyclone path loaded in the class, \
                             this needs to be done during the class \
                             initialisation..."
            )
        ...
        for k in range(len(pts)):
            ...
            if ngbhs > 0:
                for p in range(ngbhs):
                    delta_time = self.timeseries.date.iloc[
                        pts[k][p]
                    ] - self.cyclone.datetime.iloc[k].replace(tzinfo=None)
                    dhours = delta_time.total_seconds() / 3600.0
                    if abs(dhours) < dtmax:
                        ...
                        d = geopy.distance.geodesic(
                            (cXY[k, 1], cXY[k, 0]),
                            (
                                self.timeseries["lat"].iloc[pts[k][p]],
                                self.timeseries["lon"].iloc[pts[k][p]],
                            ),
                        ).km
    ...
    def plotCycloneAltiPoint(
        ...
        """
        This function **plots** a series of figures of the geographical
        coordinates for processed altimeter data close to each cyclone path
        position.
        ...
        if self.cyclone_data is None:
            raise ValueError(
                "There is no altimeter data defined based on \
                             cyclone path, you need to run the \
        ...
        for p in range(unill.shape[0]):
            ...
            if showinfo:
                ...
                print("")
                print(
                    "Considered cyclone path point ("
                    + str(cyclons)
                    + ","
                    + str(cyclats)
                    + ") at "
                    + str(unill.date.iloc[p])
                )
                print("")
            ...
            ax.set_title(
                "Altimeter data close to cyclone path point ("
                + str(cyclons)
                + ","
                + str(cyclats)
                + ") at "
            ...
            d = geopy.distance.geodesic(
                (cycdata.clat[0], cycdata.clon[0]),
                (cycdata.clat[0] + 2.0, cycdata.clon[0]),
            ).km
            d = round(d, 3) * 1.5
                ...
                for k in range(cycdata.shape[0]):
                    if cycdata.clon[k] == cyclons:
                        if cycdata.clat[k] == cyclats:
                            print(
                                "Altimeter point ("
                                + str(round(cycdata.lon[k], 1))
                                + ","
                                + str(round(cycdata.lat[k], 1))
                                + ") records dt: "
                                + str(round(cycdata.hours[k], 2))
                                + "h"
                            )
                            print("    +    Power ", round(cycdata.power[k], 2), "kW/m")
                            print(
                                "    +   Energy ", round(cycdata.energy[k], 2), "J/m2"
                            )
    ...
    def computeSeasonalCharacteristics(
        ...
        dataframe. User has the option to plot the computed
        wave paraneter characteristics as a heatmap, a box plot and a standard
        deviation graph.

        For the wave height series, a Seasonal Mann-Kendall test is also
        performed to determine monotonic trends in computed dataset using the
        package from Hussain & Mahmud (2019).

        Hussain & Mahmud, 2019: pyMannKendall: a python package for non
        parametric Mann Kendall family of trend tests - JOSS, 4(39), 1556.

        Note:
            This function uses the seaborn and matplotlib libraries to plot
            the *heatmap* and the seasonal *boxplot*.
        ...
        if lonlat is not None:
            ...
            if lonlat[0] > lonlat[1]:
                raise ValueError(
                    "Error wrong definition of min and max \
                                 longitude in lonlat"
                )
            ...
            tdf = self.timeseries.drop(
                self.timeseries[self.timeseries.lon < lonlat[0]].index
            )
        ...
        if plot:
            ...
            sns.heatmap(
                dfseason, annot=True, fmt=".2f", cmap="YlGnBu", linewidths=1, cbar=False
            )
            if series == "wh":
            ...
            if series == "wh":
                ax.set_title(
                    "Standard deviation in significant wave height \
                             for chosen time interval",
                    fontsize=12,
                )
            if series == "period":
                ax.set_title(
                    "Standard deviation in wave period for chosen \
                             time interval",
                    fontsize=12,
                )
            if series == "power":
                ax.set_title(
                    "Standard deviation in wave power for chosen \
                             time interval",
                    fontsize=12,
                )
            if series == "energy":
                ax.set_title(
                    "Standard deviation in wave energy for chosen \
                             time interval",
                    fontsize=12,
                )
            if series == "speed":
                ax.set_title(
                    "Standard deviation in wave speed for chosen \
                             time interval",
                    fontsize=12,
                )
            ...
            if series == "wh":
                ...
                print(" ")
                print(
                    "Change in yearly wave height trend accounting \
                      for seasonality:"
                )
                print("    +           trend: ", seasonal_trend.trend)
                print(
                    "    +    slope (cm/y): ",
                    str(round(seasonal_trend.slope * 100.0, 2)),
                )
