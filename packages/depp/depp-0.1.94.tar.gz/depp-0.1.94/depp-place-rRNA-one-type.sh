...
awk -v size=1000000 -v pre=${upptmpdir}/seq -v pad="${#N}" '
...
function dist_place(){
  ...
  run_apples.py -d ${i}/depp.csv -t ${accessory_dir}/wol.nwk -o ${tmpdir}/${data_type}_${jplace_suffix}.jplace -f 0 -b 5 -T $apples_core > /dev/null 2>&1
}

echo "calculating distance matrix..."
pushd ${tmpdir}/split_seq/ > /dev/null 2>&1
depp_p=`python -c "print(max($c//4,1))"`
#ls -1 seq* | xargs -n1 -P$depp_p -I% depp_distance.py query_seq_file="%" backbone_seq_file=${accessory_dir}/${data_type}_a.fasta outdir=../dist/"%" model_path=${accessory_dir}/${data_type}.ckpt replicate_seq=True recon_model_path=${accessory_dir}/${data_type}.recon.ckpt > /dev/null 2>&1
ls -1 seq* | xargs -n1 -P$depp_p -I% dist_place "%"
popd > /dev/null 2>&1

## do placement
#echo "placing queries..."
#cnt=0
#for i in ${tmpdir}/dist/*;
#do
#  run_apples.py -d ${i}/depp.csv -t ${accessory_dir}/wol.nwk -o ${tmpdir}/${data_type}_${cnt}.jplace -f 0 -b 5 > /dev/null 2>&1
#  rm ${i}/depp.csv
#  cnt=$((cnt+1))
#done
