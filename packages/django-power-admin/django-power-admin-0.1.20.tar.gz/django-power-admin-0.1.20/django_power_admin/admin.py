...
def get_power_admin_class():
    ...
    class MyAdmin(PowerAdmin):
        pass
    """
    admin_class = getattr(settings, "DJANGO_POWER_ADMIN_EXTENDED_CLASS", "django_power_admin.admin.PowerAdmin")
...
class PowerAdmin(admin.ModelAdmin):
    ...
    def get_extra_view_names(self):
        ...
        extra_view_names += self.auto_discover_extra_view_names()
        return extra_view_names

    def get_extra_changelist_parameters(self, request):
    ...
    def get_admin_view_url(self, view_name, obj=None):
        ...
        if isinstance(obj, (str, int)):
            ...
            args = [obj.pk]
        view_name_full = self.get_admin_view_name_full(view_name)
        view_url = reverse(f"admin:{view_name_full}", args=args)
        return view_url
    ...
    def get_changelist_view_extra_context(self, request, extra_context):
        self.get_changelist_toolbar_buttons(request, extra_context)
        ...
        for key in self.get_extra_view_names():
            ...
            if view and callable(view):
                ...
                urlpatterns.append(self.make_urlpattern({
                    "name": self.get_admin_view_name_full(view_name),
                    "view": self.admin_view_wrap(view),
                    "url": url,
                }))
        return urlpatterns

    def make_xlsx_reponse(self, workbook, filename=None):
        app_label = self.model._meta.app_label
        model_name = self.model._meta.model_name
        default_filename = f"{app_label}.{model_name}.xlsx"
    ...
    def admin_view_response_handler(self, view):
        def wrapper(*args, **kwargs):
            ...
            if isinstance(result, HttpResponse):
                ...
                return self.make_xlsx_reponse(result)
            elif result is None:
                request = get_request()
    ...
    def auto_discover_extra_view_names(self):
        if hasattr(self, "_auto_discover_extra_view_names"):
            return getattr(self, "_auto_discover_extra_view_names")
        names = []
            ...
            if (not name.startswith("_")) and (not name in _EXTRA_VIEW_AUTO_DISCOVER_INGORE_PROPS):
                ...
                if not view is None:
                    if self.is_extra_view(view):
                        names.append(name)
        setattr(self, "_auto_discover_extra_view_names", names)
        return names
    ...
    def changelist_object_toolbar(self, obj):
        return self.make_changelist_object_toolbar(obj, "changelist_object_toolbar_buttons")
    changelist_object_toolbar.short_description = _("Operations")

    def make_changelist_object_toolbar(self, obj, toolbar_name):
        buttons = []
    ...
    def add_child_button(self, obj):
        app_label = self.model._meta.app_label
        model_name = self.model._meta.model_name
        url = reverse(f"admin:{app_label}_{model_name}_add") + "?{parent_field_name}_id={pk}".format(parent_field_name=self.parent_field_name, pk=obj.pk)
        return url
    ...
    def sort_arrows(self, obj):
        return self.make_changelist_object_toolbar("sortable_admin_sort_arrows", obj)
    ...
    move_up.short_description = _("Move Up")
    move_up.classes = "sortable_admin_move_up_arrow"
    move_up.icon = "fa fa-arrow-up"
    ...
    move_down.short_description = _("Move Down")
    move_down.classes = "sortable_admin_move_down_arrow"
    move_down.icon = "fa fa-arrow-down"
    ...
    def get_changelist_toolbar_buttons(self, request, extra_context):
        ...
        power_admin_changelist_toolbar_buttons = self.make_changelist_toolbar_buttons(changelist_toolbar_buttons)
    ...
    def make_changelist_toolbar_buttons(self, changelist_toolbar_button_names):
        buttons = []
        for name in changelist_toolbar_button_names:
            ...
            if button_func:
                ...
                if is_group:
                    button = self.make_changelist_toolbar_buttongroup(name)
                else:
                    button = self.make_changelist_toolbar_button(name)
                buttons.append(button)
        return buttons
    
    def make_changelist_toolbar_button(self, button_func_name):
        return self.ChangelistToolbarButton(self, button_func_name)

    def make_changelist_toolbar_buttongroup(self, button_group_func_name):
    ...
    def simple_export_all(self, request):
        result_list = self.model.objects.all()
        return self.do_simple_export(request, result_list)
    simple_export_all.is_extra_view = True
    simple_export_all.name = "simple_export_all"
    simple_export_all.short_description = _("Export All")
    simple_export_all.icon = "fa fa-download"

    def simple_export_filtered(self, request):
        result_list = self.get_simple_export_items(request)
        return self.do_simple_export(request, result_list)
    simple_export_filtered.is_extra_view = True
    simple_export_filtered.add_referer_parameters = True
    simple_export_filtered.name = "simple_export_filtered"
    simple_export_filtered.short_description = _("Export Filtered")
    simple_export_filtered.icon = "fa fa-download"


    def get_simple_export_settings(self):
    ...
    def do_simple_export(self, request, result_list):
        settings = self.get_simple_export_settings()
        # create workbook
        ...
        for sheet_setting in sheet_settings:
            sheet_index += 1
            # create a sheet
            name = sheet_setting.get("name", f"Sheet{sheet_index}")
            try:
                sheet = workbook[name]
            except KeyError:
            ...
            for row_data in data:
                ...
                row += 1
        return workbook
    def get_simple_export_items(self, request):
    ...
    # simple share model
    # ---------------------------------------------------------------------------------
    def get_simple_share_model_queryset(self, request, queryset):
    ...
    def get_queryset(self, request):
        ...
        if getattr(self.model, "_is_simple_share_model", False) :
            queryset = self.get_simple_share_model_queryset(request, queryset)
        return queryset
    ...
    def get_changelist(self, request, **kwargs):
        ...
        class IgnoreExtraParametersChangeList(ChangeListBase):
            def __init__(self, request, *args, **kwargs):
                ...
                for name in modeladmin.get_extra_changelist_parameters(request):
