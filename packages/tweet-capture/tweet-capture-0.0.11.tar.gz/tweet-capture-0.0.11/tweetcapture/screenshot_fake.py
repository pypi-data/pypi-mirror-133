    ...
    def process(self, color, base, driver):
        ...
        if self.verified is not None:
            try:
                if self.verified is False: 
                    xpath_vrf = driver.find_element_by_xpath(VERIFIED_XPATH)
        ...
        if self.name is not None and len(self.name) > 0:
            try:
                ...
                driver.execute_script(f"""arguments[0].innerHTML = "{self.name}";""", driver.find_element_by_xpath(NAME_XPATH))
            except:
                pass
        if self.username is not None and len(self.username) > 0:
            try:
                ...
                driver.execute_script(f"""arguments[0].innerHTML = "{self.username}";""", driver.find_element_by_xpath(USERNAME_XPATH))
            except:
                pass
        try:
            if self.content is not None and len(self.content) > 0:
                content = driver.find_element_by_xpath(base + "/article/div/div/div/div[3]") or None
        ...
        if self.image is not None and len(self.image) > 0:
            ...
            try:
                ...
                """, driver.find_element_by_xpath(IMG_XPATH), driver.find_element_by_xpath(IMG2_XPATH))
            except:
                pass