...
2021-12-06, boolean properties restrict_attribute, restrict_threshold of PopulationMixtureBase
...
PRIOR_PSEUDO_RESPONDENT = 0.2  # 0.1  # 0.001  # 0.2  # 0.1 -> higher inter-individual variance
# = prior total pseudo-count re ONE real respondent
# for hyper-prior of all GMM components learned_weight
...
# -> favor very small precision <=> large inter-individual variance
# -> mode of inter-individual variance = PRIOR_PREC_B /(PRIOR_PREC_A + 1) approx= PRIOR_PREC_B
...
class PopulationMixtureBase:
    ...
    def initialize(cls, n_comp, emf, effects, rv_class,
        ...
        logger.debug('mode{1 / comp_prior.prec}= '
                     + np.array_str(comp_prior.prec.mode_inv(),
                                    precision=5))
        # logger.debug('exp(-E{log comp_prior.prec}= '
        #              + np.array_str(np.exp(- comp_prior.prec.mean_log),
        #                             precision=6))
        comp = [_make_prior(n_parameters, n_beta,
                            attribute_slices, rv_class)
        ...
        # All comp are initialized equal, for now,
        # will be separated later, before VI learning.
    ...
    def beta_0_size(self):
        ...
        """
        return np.prod([len(self.emf.scenarios[sc_i]) for sc_i in self.effects[0]],
                       dtype=int)
...
def response_thresholds(log_w):
    ...
    n_overflow = np.sum(log_w > np.log(np.finfo(float).max))
    if n_overflow > 0:
        logger.warning(f'response_thresholds: {n_overflow} log_w samples too large. '
                       + 'Maybe too few responses?')
        log_w = np.minimum(log_w, np.log(np.finfo(float).max) - 1.)
        # just to avoid numerical overflow in exp(log_w), should not be needed !
...
def tau_inv(tau):
    """Inverse of response_thresholds(log_w)
    :param tau: 1D or 2D array tau, with all elements in (-inf, +inf), EXCEPT extremes at inf
...
def _make_prior(n_parameters, n_beta,
    ...
    return GaussianRV.initialize(loc=np.zeros(n_parameters),
...
def _check_effects(emf, effects):
    ...
    for sc in emf.scenarios.keys():
        if len(emf.scenarios[sc]) > 1 and sc not in effect_keys:
...
def _theta_map(emf, effects):  # , restrict_attribute=False):
    ...
    def theta_one(effect):
        ...
        beta_size = np.prod(beta_shape, dtype=int)
        ...
        # t = t[1:]  # *** version 0.5.1
        # *** fixed theta == 0 for first category in ALL effect terms *** NO good ***
        if effect != effects[0]:  # **** version 0.6 ???
            t = t[1:]
            # fixed effect beta[0] for all additional effect terms, EXCEPT FIRST
            # -> ONE overall theta intercept parameter across all effect
    ...
    # ---------------------------------------------------------------
    # Need to make model identifiable:
    # *** Version 0.5.1: NO free intercept term
    # *** Attribute location == 0 for first scenario
    th = np.concatenate([theta_one(e_tuple)
                         for e_tuple in effects],
                        axis=0)
    # *** Version 0.6???: include intercept parameter, first element for each attribute:
    # th_0 = np.ones((1, *emf.scenario_shape))
    # th = np.concatenate((th_0, th), axis=0)
