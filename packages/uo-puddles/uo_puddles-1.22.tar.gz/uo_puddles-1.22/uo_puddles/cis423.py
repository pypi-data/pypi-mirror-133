  ...
  def transform(self, X):
    ...
    if self.action=='drop':
      if actual_list != self.column_list:
        print(f'{self.__class__.__name__}.transform warning: columns to drop not in X: {set(self.column_list) - set(xcols)}')
      X_ = X_.drop(columns=actual_list)
    else:
      if actual_list != self.column_list:
        print(f'{self.__class__.__name__}.transform warning: columns to keep not in X: {set(self.column_list) - set(xcols)}')
      X_ = X_[actual_list]
    return X_
