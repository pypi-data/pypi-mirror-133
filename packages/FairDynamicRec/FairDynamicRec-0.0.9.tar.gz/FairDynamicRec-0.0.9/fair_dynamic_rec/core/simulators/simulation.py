...
RankerModel = {'random': RandomPolicy, 'naive_greedy': NaiveGreedy, 'naive_epsilon_greedy': NaiveEpsilonGreedy,
               'thompson_sampling': ThompsonSampling, 'kl_upper_confidence_bound': KLUCB, 'linear_submodular_bandit': LSB,
               'linear_upper_confidence_bound': LinUCB, 'hybrid-lsb-linucb': HybridLSBLinUCB}
...
class OnlineSimulator():
    ...
    def run(self, config, dataObj):
        print(str(datetime.now().strftime('%Y-%m-%d %H:%M:%S')) + ": Simulation started with " + str(config.list_size) + " rounds")
        overall_optimal_reward = np.zeros(config.rounds)
        self.rankers = self.set_rankers(config, dataObj)
        ...
        for i in tqdm(range(config.rounds)):
            ...
            for j in range(len(self.rankers)):
                ...
                rankings = self.rankers[j]["ranker"].get_ranking(user_ids)
                # Compute rewards
                clicks, rewards = self.compute_rewards(config, user_ids, rankings)
                # Update policy based on rewards
                self.rankers[j]["ranker"].update(user_ids, rankings, clicks)
    ...
    #     cumulative_regrets = {policies_name[j]: list(np.cumsum(overall_optimal_reward - overall_rewards[j])) for j in
    #                           range(n_policies)}
    #     with open(args.output_path, 'w') as fp:
    #         json.dump(cumulative_regrets, fp)


    def set_rankers(self, config, dataObj):
        rankers = []
        for params in config.rankers:
            if len(params["multiple_val_params"]) > 0:
                for param in params["multiple_val_params"]:
                    rankers.append(
                        {'ranker': RankerModel[params["single_val_params"]["name"]["value"]](config, dataObj, param),
                         'config': dict(list(params["single_val_params"].items()) + list(param.items()))}
                    )
    ...
    def get_optimal_ranking(self, config, dataObj):
        ...
        if config.optimal_ranker == 'naive_relevance':
            ...
            for i in range(n_users):
                # optimal_scores[i] = expit(self.user_features[i].dot(batch_item_features[i].T))
            ...
            n_users = self.user_features.shape[0]
            optimal_rankings, optimal_scores = np.zeros((n_users, config.list_size), dtype=int), np.zeros((n_users, config.list_size, self.item_features.shape[1]))
            ...
            n_users = self.user_features.shape[0]
            optimal_rankings, optimal_scores = np.zeros((n_users, config.list_size), dtype=int), np.zeros((n_users, config.list_size), dataObj.feature_data['test_item_topical_features'].shape[1] + dataObj.feature_data['test_item_latent_features'].shape[1])
            for i in range(n_users):
                ...
                self.optimal_scores[:, :dataObj.feature_data['test_item_topical_features'].shape[1]] = gain_in_topic_coverage_t
    ...
    def compute_rewards(self, config, batch_users, rankings):
