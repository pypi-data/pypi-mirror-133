...
from fluidtopics.markdown.metadata import FT_ORIGIN_ID, FT_LANG, get_md_metas, MISSING_TITLE
        ...
        if file.is_dir():
            ...
            # if no README => create ft:originID from dir title (ToC entry only)
                ...
                if not implicit_meta:
                    raise MD2FTError(
                        f"No README.md in directory {file} while in explicit mode, add one with an 'ft:title' meta."
                    )
                else:
            ...
            metas, _, heading_title = get_md_metas(file, implicit_meta, str(file.relative_to(relto)) == 'README.md',
                                                   dev_mode=dev_mode)
...
def _create_zip_file(output: str, pinput: Path, image_dir: str, ftmap_file: TemporaryFile, collected_files: List[Path],
                     dev_mode: bool):
...
def do_collect(
    ...
    if implicit_meta:
        ...
        implicit_metas_values = {
            ...
            "media_dir": "images",
            SemanticMetadata.TITLE: f"{pinput.name}",
            FT_ORIGIN_ID: input,
    ...
    with TemporaryFile() as ftmap_file:
        ...
        ftmap_file.seek(0)
        _create_zip_file(output, pinput, image_dir, ftmap_file, collected_files, dev_mode=dev_mode)
...
def do_publish(
    ...
    if not dry_run:
        ...
        ft_session.get("/api/admin/khub/sources", json_result=False)
    ...
    if not dry_run:
        answer = ft_session.post(f"/api/admin/khub/sources/{source_id}/upload", files=zipfile)
        logger.info(f"{zipfile['file'][0]} file published on {ft_session.portal_base_url}.")
        pprint(answer)
    else:
        logger.info(
            f"DRY RUN: {zipfile['file'][0]} file NOT published."
        )
