...
class Hyperparams(hyperparams.Hyperparams):
    ...
    min = hyperparams.Union[typing.Union[datetime, float, None]](
        configuration=collections.OrderedDict(
            datetime=hyperparams.Hyperparameter[datetime](utils.DEFAULT_DATETIME),
            negative_infinity=hyperparams.Constant(None),
            unix_timestamp=hyperparams.Hyperparameter[float](utils.DEFAULT_DATETIME.timestamp())
        ),
    ...
    max = hyperparams.Union[typing.Union[datetime, float, None]](
        configuration=collections.OrderedDict(
            datetime=hyperparams.Hyperparameter[datetime](utils.DEFAULT_DATETIME),
...
class DatetimeRangeFilterPrimitive(transformer.TransformerPrimitiveBase[Inputs, Outputs, Hyperparams]):
    ...
    metadata = metadata_base.PrimitiveMetadata(
        {
            ...
            'python_path': 'd3m.primitives.data_transformation.datetime_range_filter.Common',
            'source': {
    ...
    def produce(self, *, inputs: Inputs, timeout: float = None, iterations: int = None) -> base.CallResult[Outputs]:
        ...
        if self.hyperparams['min'] is None:
            min = datetime.min
        elif type(self.hyperparams['min']) is float:
            min = datetime.fromtimestamp(self.hyperparams['min'])
        else:
        ...
        if self.hyperparams['max'] is None:
            ...
            max = datetime.fromtimestamp(self.hyperparams['max'])
        else:
