...
def test_version_mismatch(request):
    ...
    if (lv is None
    or (settings.SITE.never_build_site_cache
    and settings.SITE.kernel.lino_version == lv)
    or float(lv) == settings.SITE.kernel.code_mtime):
        return {}
    # print("20201217", lv, settings.SITE.kernel.code_mtime)
    cache.clear()
    if settings.SITE.kernel.default_ui.verbose_name == 'React':
        return dict(version_mismatch=True)
