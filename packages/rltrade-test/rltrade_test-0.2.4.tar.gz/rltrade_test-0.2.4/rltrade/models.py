...
from rltrade.environments import (SmartDayTradeEnv3, SmartDayTradeEnv4, SmartPortfolioEnv, SmartPortfolioWithShortEnv,
...
from rltrade.data import FeatureEngineer,IBKRDownloader,DayTradeFeatureEngineer,OandaDownloader
...
class SmartDayTradeAgent4:
    ...
    def make_prediction(self):
        environment = SmartDayTradeEnv4(df=self.test,**self.env_kwargs)
        test_env, test_obs = environment.get_sb_env()
        """make a prediction"""
        account_memory = []
        actions_memory = []
    ...
    def train_model(self):
        ...
        self.env_kwargs['exchanges'] = self.exchanges
        self.env_kwargs['epochs'] = self.epochs
        environment = SmartDayTradeEnv4(df=self.train,**self.env_kwargs)
        model = self.get_model(environment)
        model = model.learn(total_timesteps= self.n_steps * self.epochs ,tb_log_name=self.tb_log_name)
        self.filter_ticker_list = environment.ticker_list
        self.filter_sec_types = environment.sec_types
    ...
    async def get_day_trade_actions(self,time_now,duration,trade_period):
        ...
        temp = self.fe.create_data(temp)
        temp = temp.tail(2*len(self.ticker_list))
        environment = SmartDayTradeEnv4(df=temp,**self.env_kwargs)
