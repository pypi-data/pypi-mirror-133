...
from rltrade.models import SmartDayTradeAgent4
...
trades_per_stock = [100] * len(ticker_list) # max number per day is 24h/time_frame
...
async def train_model():
    ...
    try:
        ...
        df = pd.read_csv('testdata/5mdf.csv')
        df['date'] = pd.to_datetime(df['date'])

        agent = SmartDayTradeAgent4("ppo",
                            df=df,
                            account=account,
                            time_frame=time_frame,
                            ticker_list=ticker_list,
