...
class Material(object):
    ...
    def fispact2mcnp(self, ntrans_avail_nucs, record_drop=False,
                     record_file=None, total_dropped_atoms=0.0):
        """
        Convert the mcnp material to fispact material
        ...
        total_dropped_atoms : float
            The total dropped atoms for each conversion.
        iter_step : int
            ...
            if nuc_m in ntrans_avail_nucs:
                ...
                if record_file and record_drop:
                    with open(record_file, 'a') as fo:
                        fo.write(
                            f"{nuc},{utils.format_single_output(self.fispact_material_atoms_kilogram[i])}\n")
        # record the total dropped atoms
        if record_drop:
            total_dropped_atoms += drop_atoms
...
def decompose_fispact_nuc(nuc):
    ...
    m: int
        Metastable state. Only 0 or 1 is supported.
    """
    z, a, m = 0, 0, 0
    ELE_TABLE = utils.get_ele_table()
...
def get_material_index(materials, mid):
    """get_material_index, return material index of the materials according to the mid"""
    midx = -1
    for i in range(len(materials)):
        if materials[i].id == mid:
            midx = i
