...
class Jss:
            ...
            if isinstance(values, dict):
                ...
                for i in values:
                    if isinstance(i, dict):
                        encode_data.extend(encode_data(i))
                    else:
                        encode_data.append(str(i).replace("(", "").replace(")", "").replace("@", ""))
            else:
                encode_data.append(str(values).replace("(", "").replace(")", "").replace("@", ""))
        return encode_data    

    def save_data(self, api_name, post_data, resp, bucket_name="pfinder-data-bucket"):
        file_name = api_name + '/' + '/'.join(self._encode_post_data(post_data))
    ...
    def get_data(self, api_name, post_data, bucket_name="pfinder-data-bucket"):
        file_name = api_name + '/' + '/'.join(self._encode_post_data(post_data))
        resp = self.bucket(bucket_name).object(file_name).get().content
        data = json.loads(resp)
        return data

    def has_data(self, api_name, post_data, bucket_name="pfinder-data-bucket"):
        file_name = api_name + '/' + '/'.join(self._encode_post_data(post_data))
