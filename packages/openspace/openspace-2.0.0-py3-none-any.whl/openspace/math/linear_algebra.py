...
class Matrix(np.ndarray):
    ...
    def __new__(cls, elements):
        ...
        self = np.asarray(elements, float).view(cls)
...
class Vector(np.ndarray):
    ...
    def __new__(cls, elements):
        ...
        if type(elements) == list:
            self = np.asarray(
                elements, 
                float
            ).reshape(len(elements), 1).view(cls)
        else:
            self = np.asarray(elements, float).view(cls)
    ...
    def rotate_about_axis(self, axis, ang):
        norm_axis = axis.normalized()
        x = norm_axis.get_element(0)
        y = norm_axis.get_element(1)
        z = norm_axis.get_element(2)

        m = Matrix(
            [
                [
                    cos(ang)+x**2*(1-cos(ang)), 
                    x*y*(1-cos(ang))-z*sin(ang), 
                    x*z*(1-cos(ang))+y*sin(ang)
                ],
                [
                    y*x*(1-cos(ang))+z*sin(ang),
                    cos(ang)+y**2*(1-cos(ang)),
                    y*z*(1-cos(ang))-x*sin(ang)
                ],
                [
                    z*x*(1-cos(ang))-y*sin(ang),
                    z*y*(1-cos(ang))+x*sin(ang),
                    cos(ang)+z**2*(1-cos(ang))
                ]
            ]
        )
