...
def createGraph(listOfActors: list, listOfLocations: list, \
    listOfProducts: list) -> DiGraph:
    '''
    ...
    :param listOfLocations: a list of locations involved (e.g. Landkreise in Germany)
    :param listOfProducts: a list of products being transported (e.g. vegetables, eggs, ...)
    ...
    if howManyActors == 0:
        raise Exception('incorrect parameter: actors should be a list, but it is: %20s' % (actors) )
    if howManyProducts == 0:
        raise Exception('incorrect parameter: products should be a list, but it is: %20s' % (products) )
    if howManyLocations == 0:
        raise Exception('incorrect parameter: locations should be a list, but it is: %20s' % (locations) )

    
    DG = nx.DiGraph()

    # create names for node attributes (currently hardcoded, since this never changes (as of this moment))
    actor = "actor"
    location = "location"
    product = "product"
    ...
    '''
    nodes = zip(range(len(listOfActors)*len(listOfLocations)*len(listOfProducts)),\
...
def combineActorBrand(actor: str, brand: list) -> list:
    '''
...
def getListOfNodeIDs(SCgraph: Graph, actors=None, products=None, locations=None) -> list:
    '''
    purpose: You provide an existing supply chain network and list of actors, products and 
    locations, this function returns a list of node ids with these attributes.
    If an attribute is missing, this function assumes the user wants all nodes of 
    the missing attribute.    
    If an empty list is given, this function assumes the user searches for nodes that 
    dont have this particular attribute. This should result in an Exception if zero 
    nodes are found with not one of the properties.
    ...
    createGraph(listOfActors: list, listOfLocations: list, listOfProducts: list)
    ...
    '''
    if actors==None:
        actors=list(set(val for val in nx.get_node_attributes(SCgraph,'actor').values()))
    if products==None:
        products=list(set(val for val in nx.get_node_attributes(SCgraph,'product').values()))
    if locations==None:
        locations=list(set(val for val in nx.get_node_attributes(SCgraph,'location').values()))
    ...
    listOfNodes = [n for n in myDict if myDict[n]['actor'] in actors \
    ...
    if len(listOfNodes) == 0:
        testloNA = bool([n for n in myDict if myDict[n]['actor'] in actors])
        testloNP = bool([n for n in myDict if myDict[n]['product'] in products])
        testloNL = bool([n for n in myDict if myDict[n]['location'] in locations])
        
        if not testloNA:
            raise Exception('no nodes found with these conditions:  actors = %20s, length = %2d !)'\
                % (actors,howManyActors))
        if not testloNP:
            raise Exception('no nodes found with these conditions:  products = %20s, length = %2d !)'\
                % (products,howManyProducts))
        if not testloNL:
            raise Exception('no nodes found with these conditions:  locations = %20s, length = %2d !)'\
                % (locations,howManyLocations))

    return(listOfNodes)

def addAttr2ExistingNodes(SCGraph: Graph, listOfNodeIDs: list, \
    nameOfAttr: str, listOfAttr: list) -> bool:
    '''
    purpose: add a custom attribute to a specified list of node IDs to your supply chain 
    nodes with the correct IDs
    
    :param SCgraph: the graph created with 
    createGraph(listOfActors: list, listOfLocations: list, listOfProducts: list)
    :param listOfNodeIDs:  should be a list of the nodes you want to add your attribute to 
    (if set to 'None', this function assumes, you want to give all existing nodes this 
    attribute)
    :param nameOfAttr: should be a recognizable string (e.g. 'outgoing', 'incoming', ...)
    :param listOfAttr:should be the list of values given to the chosen attributes name 
    (length of this list should be equal to listOfNodeIDs)

    :return: Boolean: True if all nodeIDs with corresponding attribute values are added to 
    the graph
    ...
    didItWork = go.addAttr2ExistingNodes(myNW, listOfNodeIDs=producerIDs, \\
        nameOfAttr='ags', listOfAttr=listOfAGS)
    ...
    if len(listOfNodeIDs) == len(listOfAttr):
        ...
        raise Exception('length of list of node ids is not equal to length of list of attributes')
...
def getNodeIDswAttr(SCGraph: Graph, nameOfAttr: str, listOfNodeIDs=None):
    '''
    ...
    if nameOfAttr not in allAttrSoFar:
        raise Exception('Attribute given is not added to any node in the graph, please check!')
...
def convertLoT2NTs(myListOfTuples: list):
    '''
    TODO 
...
def proxyModel(inputNr: float, proxyData: list, proxyNr: float) -> list:
    '''
    ...
    of chicken in each county(list) and proxyNr is the sum of all chicken in all
    counties. The returned list is then the modelled production of eggs in each 
    county.
    ...
    inputNr/ModelData = proxyNr/proxyData -> ModelData = proxyData/proxyNr*inputNr

    :param inputNr: float. (usage see explanation above)
    :param proxyData: list or List of tuples(if you want to connect your node IDs 
    with the respective attribute). (usage see explanation above)
    :param proxyNr: float. (usage see explanation above)
    

    :return: list of modelled data or list of tuples of (NodeID, modelled data), 
    depending on proxyData.
    ...
    regionalProduction = go.proxyModel(inputNr=nationalProduction, \\
        proxyData=proxyregProduction, proxyNr=proxynatProduction)
    ...
    regionalProduction = go.proxyModel(inputNr=nationalProduction, \\
        proxyData=proxyregProdWithNodeIDs, proxyNr=proxynatProduction)
    print(regionalProduction)
    ...
    try:
        ...
        pd = proxyData
        print('Warning: proxyModel expected a list of tuples, but does its job anyway. \
...
def getExistingAttrs(SCGraph: Graph, gtype = 'n'):
    '''
    ...
    if gtype == 'nodes' or gtype == 'n':
        listOfAttributes = list(set(chain(*[list(x.keys()) \
            for x in dict(SCGraph.nodes(data=True)).values()])))
    elif gtype == 'edges' or gtype == 'e':
        listOfAttributes = list(set(chain(*[list(x.keys()) \
            for x in [w for _,_,w in list(SCGraph.edges(data=True))]])))
    else:
...
def getAllCombinations(list1: list, list2: list, order='1st'):
    '''
    ...
    >> ['producer', 'producer', 'producer', 'consumer', 'consumer', \\
        'consumer', 'warehouse', 'warehouse', 'warehouse', 'store', 'store', 'store']
    ...
    if order == '1st':
        ...
        raise Exception('please set the parameter \'order\' to either \'1st\' or \'2nd\'')

    return(allCombinations1, allCombinations2)

def getEdgeID(SCGraph: Graph, outgoingNodes: list, incomingNodes: list):
    '''
    ...
    NOTE this function doesnt check if any of the edges already contain information
 
    input:
    :param SCGraph: the graph, needed for confirmation that the nodes to be connected
    do indeed exist
    ...
    myNW = go.createGraph(listOfActors=act,listOfLocations=loc,listOfProducts=prod)
    producerIDs = go.getListOfNodeIDs(myNW, actors=['producer'])
    consumerOfSchnapsIDs = go.getListOfNodeIDs(myNW, products=['schnaps'],\\
        actors=['producer'])
    allcomb1,allcomb2 = go.getAllCombinations(producerIDs,consumerOfSchnapsIDs)

    myEdgeIDs = go.getEdgeID(myNW, outgoingNodes=allcomb1, incomingNodes=allcomb2)



    ===============================================================================================
    '''
    completeListOfAllNodesinGraph = [n for n in dict(SCGraph.nodes(data=True)).keys()]

    allThere = all([all(r in completeListOfAllNodesinGraph for r in outgoingNodes),\
        all(r in completeListOfAllNodesinGraph for r in incomingNodes)])

    if allThere:
        listOfEdgeIDs = convertTup2LoT(outgoingNodes,incomingNodes)
    else:
        outHere = all(r in completeListOfAllNodesinGraph for r in outgoingNodes)
        inHere = all(r in completeListOfAllNodesinGraph for r in incomingNodes)
        if not outHere:
            raise Exception('found some nodes that are not existing! (check outgoing nodes)')
        if not inHere:
            raise Exception('found some nodes that are not existing! (check incoming nodes)')
    return(listOfEdgeIDs)

def addAttr2Edges(SCGraph: Graph, listOfEdgeIDs: list, \
    listOfContent: list, attr = 'weight') -> bool:
    '''
    purpose: add an attribute to a given list of edge IDs
    NOTE that if an edge ID is not recognized, a node with the address given will be 
    created with no attributes!

    :param SCGraph: This is the graph in question.
    :param listOfEdgeIDs: a list of 2-tuples gotten from getEdgeID
    :param listOfContent: a list of numbers(e.g. double or int) in the same order as the 
    list of edges
    :param attr(optional): default='weight'. Another str may be given (e.g. distance).
    ...
    consumerOfSchnapsIDs = go.getListOfNodeIDs(myNW, products=['schnaps'],\\
        actors=['producer'])
    myEdgeIDs = go.getEdgeID(myNW, outgoingNodes=producerIDs, \\
        incomingNodes=consumerOfSchnapsIDs)

    listOfShipping = [10,1,2000]
    listOfDistances = [50.2,10.3,111.2]

    didItWork1 = go.addAttr2Edges(myNW, listOfEdgeIDs=myEdgeIDs, \\
        listOfContent=listOfShipping)
    didItWork2 = go.addAttr2Edges(myNW, listOfEdgeIDs=myEdgeIDs, \\
        listOfContent=listOfDistances, attr='distance')
    ...
    if len(listOfEdgeIDs) != len(listOfContent):
        raise Exception('Error: length of input lists are unequal('+\
            str(len(listOfEdgeIDs))+' vs. '+str(len(listOfContent))+')')
...
def getEdgesAttr(SCGraph: Graph, attr = 'weight', listOfEdgeIDs = None):
    '''
    ...
    print(go.getEdgesAttr(myNW)) # all active edges with attribute 'weight'
    print(go.getEdgesAttr(myNW, attr = 'distance', \\
        listOfEdgeIDs = [myEdgeIDs[1]])) # just one edge
...
def getEdgeIDswAttr(SCGraph: Graph, attr = 'weight', listOfEdgeIDs = None):
    '''
    purpose: get me the edge attributes content from the edges i provide,
    ...
    nodeOut, nodeIn, weight = convertLoTToNTs(getEdgeIDswAttr(SCGraph,attr,listofEdgeIDs))
    ...
    consumerOfSchnapsIDs = go.getListOfNodeIDs(myNW, products=['schnaps']\\
        ,actors=['producer'])
    myEdgeIDs = go.getEdgeID(myNW, outgoingNodes=producerIDs, \v
        incomingNodes=consumerOfSchnapsIDs)
    ...
    print(go.getEdgeIDswAttr(myNW, attr = 'distance', listOfEdgeIDs = myEdgeIDs))

    nodeOut, nodeIn, weight = go.convertLoT2NTs(go.getEdgeIDswAttr(myNW,\\
        'distance',[myEdgeIDs[1]]))
...
def minOptFlowForBeta(SCGraph: Graph, listOfSenderIDs: list, \
    listOfReceiverIDs: list, beta: float, edgeattrDistName: str, distanceModel: str):
    '''
    ...
    this calculates the optimum flow by minimizing sum_{i,j}(flow_{i,j}*dist_{i,j})
    ...
    :return: variable beta for optimimum flow. needed for minimum transport distance.
    ...
    ===============================================================================================
    '''
    return(float(optimize.minimize(optFlow, beta, args=(SCGraph, listOfSenderIDs, listOfReceiverIDs, edgeattrDistName, distanceModel)).x))

def optFlow(beta: float, SCGraph: Graph, listOfSenderIDs: list, \
    listOfReceiverIDs: list, edgeattrDistName: str, distanceModel: str):
    '''
    purpose: calculate the flow between the senders and the receivers of goods given 
    the free parameter beta, the distances between all locations of participants
    and the distance model
    
    :param beta: free parameter (float) determining the average amount of flow 
    :param SCGraph: the graph with information about outgoing goods from senders and 
    incoming goods for receivers
    :param listOfSenderIDs: list of node IDs in G (should have 'output' attribute)
    :param listOfReceiverIDs: list of node IDs in G (should have 'input' attribute)
    :param edgeattrDistName: str name of the edge attribute for the geographical 
    ...
    ===============================================================================================
    '''
    sendID,sendOut = convertLoT2NTs(getNodeIDswAttr(SCGraph=SCGraph, nameOfAttr='output', listOfNodeIDs=listOfSenderIDs))
    recID,recIn = convertLoT2NTs(getNodeIDswAttr(SCGraph=SCGraph, nameOfAttr='input', listOfNodeIDs=listOfReceiverIDs))

    if sendID == []:
        raise Exception('please enter attribute OUTPUT with corresponding content to the network')
    if recID == []:
        raise Exception('please enter attribute INPUT with corresponding content to the network')

   
    distanceMatrix = getDistMatrix(SCGraph, listOfSenderIDs, listOfReceiverIDs, edgeattrDistName)
...
def getDistMatrix(SCGraph: Graph, listOfSenderIDs: list, \
    listOfReceiverIDs: list, edgeattrDistName: str):
    '''   
    purpose: If all edges between listOfSenderIDs and listOfReceiverIDs contain the 
    attribute edgeattrDistName, this function returns a numpy matrix of all distances 
    between all combinations of participating nodes.
    ...
    :param listOfReceiverIDs: list of node IDs in G (should have 'input' attribute)
    :param edgeattrDistName: str name of the edge attribute for the geographical 
    distance between nodes listOfSenderIDs, order of rows should be identical to 
    listOfReceiverIDs,
    NOTE hymanModel expects this to be 'distance' !
    
    :return: a numpy matrix of all combinations of distances between participating nodes
    ...
    didItWork3 = go.addAttr2Edges(myNW,myEdges4Graph, myDF.values.flatten(),\\
        attr='distance')

    print(go.getDistMatrix(myNW, listOfSenderIDs=senderIDs, \\
        listOfReceiverIDs=receiverIDs, edgeattrDistName='distance'))
    ...
    myLoD = getEdgesAttr(SCGraph,attr=edgeattrDistName,listOfEdgeIDs=myEdges)
    distanceMatrix = np.reshape(myLoD,(len(listOfSenderIDs),len(listOfReceiverIDs)))
    return(distanceMatrix)

def propFlow(SCGraph: Graph, listOfSenderIDs: list, listOfReceiverIDs: list):
    '''
    purpose: calculate the flow between the senders and the receivers of goods given 
    a proportional flow of goods.
    ...
    flow = np.outer(np.array(sendOut),np.array(recIn))/np.sum(np.array(sendOut))
    return(flow)

def getWeightedDist(flow,dist) -> float:
    '''
...
def calcPotTransportDistances(SCGraph: Graph, listOfSenderIDs: list, \
    listOfReceiverIDs: list, nrOfValues = 5) -> list:
    '''
    purpose: calculates nrOfValues possible transport distances, given a range of possible values.
    The minimum value of this range is the optimum flow (see optFlow for details).
    The maximum value of this range is the proportional flow (see propFlow for details).
    Both values are calculated internally and depend on the flow between participants of 
    supply and demand regarding a specific product.
    NOTE that the node attributes 'output' for the supplier nodes and 'input' for the 
    receiver nodes of goods, as well as the edge attributes 'distance' and 'weight' 
    between ALL the participating nodes MUST NOT be missing!
    
    :param SCGraph: the graph with information about outgoing goods from senders and 
    incoming goods for receivers
    :param listOfSenderIDs: list of node IDs in G (should have 'output' attribute)
    :param listOfReceiverIDs: list of node IDs in G (should have 'input' attribute)
    :param nrOfValues(optional): int value. Please insert the number of possible transport distances
    (default is 5).
    
    :return: a list of possible transport distances. needed for hymanModel to determine beta
    (details see hymanModel)
    ...
    myTDs = go.calcPotTransportDistances(myNW, listOfSenderIDs=senderIDs, listOfReceiverIDs=receiverIDs)



    ===============================================================================================
    '''
    #should not depend on beta, but optimization needs a starting value, thus hardcoded here
    betaHere = 0.1
    edgeattrDistName = 'distance'
    distanceModel = 'exp'

    distanceMatrix = getDistMatrix(SCGraph, listOfSenderIDs, listOfReceiverIDs, edgeattrDistName)
    betaOpt = minOptFlowForBeta(SCGraph, listOfSenderIDs, listOfReceiverIDs, betaHere, edgeattrDistName, distanceModel)
    flowopt = furnessModel(SCGraph, listOfSenderIDs, listOfReceiverIDs, betaOpt, edgeattrDistName, distanceModel)
    ...
    return(list(np.round(np.linspace(minDist,maxDist,nrOfValues+2)[1:nrOfValues+1])))

def hymanModel(SCGraph: Graph, listOfSenderIDs: list, listOfReceiverIDs: list, \
    transportDistance: float, tolerance = 0.1):
    '''
    purpose: Gravity Model. calculate the flow of goods, based on a transport distance 
    (got from calcPotTransportDistances), by estimating unkown beta.
    Hardcoded here is the exponential distance model, input for furnessModel 
    (see furnessModel for details).
    If the user wants to create their own distance model, the user may start with 
    furness, probably wants to create their own version of hyman, as well.
    
    1. initial guess of beta0 = 1./transportDistance
    2. calculate flow based on beta_0: flow_0=furnessModel(beta_0)
    3. calculate currentTransportDistance_0 = getWeightedDist(flow_0,getDistMatrix)
    4. start iterative process
        4a. calculate beta_i(beta_i-1, currentTransportDistance_0)
        4b. calculate flow_i=furnessModel(beta_i)
        4c. calculate currentTransportDistance_i = getWeightedDist(flow_i,getDistMatrix)
    ...
    :param listOfSenderIDs: list of node IDs in G (should have 'output' attribute)
    :param listOfReceiverIDs: list of node IDs in G (should have 'input' attribute)
    :param transportDistance: weighted average transport distance. Calculated via 
    calcPotTransportDistances
    ...
    myTDs = go.calcPotTransportDistances(myNW, listOfSenderIDs=senderIDs, \\
        listOfReceiverIDs=receiverIDs)

    # Run the gravity model with given transport distance and return the flow
    print(go.hymanModel(myNW, listOfSenderIDs=senderIDs, \\
        listOfReceiverIDs=receiverIDs, transportDistance=myTDs[0], tolerance = 0.01))
    ...
    flow0 = furnessModel(SCGraph, listOfSenderIDs, listOfReceiverIDs, beta0, edgesAttr, distModel)

    distanceMatrix = getDistMatrix(SCGraph, listOfSenderIDs, listOfReceiverIDs, edgesAttr)
    ...
    while (abs(transportDistance - distList[countLoops]) > tolerance):
        if countLoops == 0:
            ...
            betaNext = np.abs(((transportDistance-distList[countLoops-1])*betaList[countLoops]-\
                (transportDistance-distList[countLoops])*betaList[countLoops-1])/(distList[countLoops]-distList[countLoops-1]))
            betaList.append(betaNext)
        betaThisTime = betaList[countLoops+1]

        flow = furnessModel(SCGraph, listOfSenderIDs, listOfReceiverIDs, betaThisTime, edgesAttr, distModel)
        ...
        # break if in local minimum and check if any dist was closer to the given transport distance
        if countLoops > 20:
            if abs(distList[countLoops]-distList[countLoops-5])<0.001:
                betaBest = betaList[distList.index(min(distList))]
                flow = furnessModel(SCGraph, listOfSenderIDs, listOfReceiverIDs, betaBest, edgesAttr, distModel)
                break
        ...
        print("On the %2d. iteration: tolerance is down to %3.4f" % (countLoops, abs(transportDistance - distList[countLoops])))    

        if np.isnan(transportDistance):
            raise Exception('something went wrong, given transport distance returned nan!')
        if np.isnan(distList[countLoops]):
            raise Exception('something went wrong, current transport distance returned nan!')
        

    # check if by running into a local minimum, a transport distance was found that was better than the last one
    ...
    if tolThisTime > tolBest[tolBest.index(min(tolBest))]:
        ...
        flow = furnessModel(SCGraph, listOfSenderIDs, listOfReceiverIDs, betaBest, edgesAttr, distModel)
    print("On the last iteration (%2d.): tolerance is down to %3.4f" % (tolBest.index(min(tolBest)), tolBest[tolBest.index(min(tolBest))]))
    return(flow)

def furnessModel(SCGraph: Graph, listOfSenderIDs: list, listOfReceiverIDs: list, \
    beta: float, edgeattrDistName = 'distance', distanceModel = 'exp'):
    '''
    purpose: calculate the flow between the senders and the receivers of goods given 
    the free parameter beta, the distances between all locations of participants
    and the distance model.
    ...
    :param listOfReceiverIDs: list of node IDs in G (should have 'input' attribute)
    :param beta: free parameter (float) determining the average amount of flow 
    :param edgeattrDistName(optional): str name of the edge attribute for the geographical 
    distance between nodes listOfSenderIDs, order of rows should be identical 
    to listOfReceiverIDs. Default is 'distance'
    :param distanceModel(optional): 'exp'(default) for exponential, 'lin' for linear
    ...
    # determining beta
    myBeta = 0.1
    flow = furnessModel(myNW, senderIDs, receiverIDs, myBeta, 'distance', 'exp'):
...
def calibrationOLD(flowTensor: np.ndarray, calibrationMatrix: np.array, \
    nrIter=100_000, wait=1000):
    '''
    ...
    transport distances for each product are known (and may be calculated by the 
    ...
    >> import numpy as np
    >> np.ndarray((rowsOfFlow,colsOfFlow,nrOfProducts,nrOfTransportationDistances))
    with 
    ...
    If for all products and all available transport distance(m) a flow_product_m is 
    calculated via hymanModel, then this function chooses randomly an ensemble 
    of all flow_product_m, then tries to minimize the difference to FTIP by
    switching out flow_product_<m> following a metropolis MC-algorithm scheme.
    ...
    Note that, the algorithm ends the minimization routine if for wait iterations 
    nothing has changed.
    ...
    :return listOfBestEnsembleConfiguration: a list of ids that correspond to the 
    transport distances m. For each product a specific m was chosen, so that the 
    difference between the input flows and the calibration measure in minimized.
    ...
    if outputFlow!=outputCal or inputFlow!=inputCal:
        raise Exception('Error: Expected dimensions of the flow matrices and the calibration measure to be identical.\
            Please consider using the function \'convertMatrix\' from within this library.')
    ...
    for p in range(products):
        oneFlow = flowTensor[0:outputFlow,0:inputFlow,p,initialGuess[p]] # choose 1 member from ensemble
    ...
    for run in range(0,nrIter):
        ...
        if not np.mod(run,wait):
            if run>nrIter/10:
                if np.abs(oldTolerance - relativeTolerance)/relativeTolerance < 0.001:
                    print('nothing changed for quite a while, abort')
                    break
            oldTolerance = relativeTolerance
            print('Tolerance is now: '+str(relativeTolerance)+', iteration: '+str(run))
        if relativeTolerance < np.finfo(float).eps/outputCal/inputCal:
            newGuess = initialGuess
            break
        else:
            whichP = np.mod(run,products) # changing the ensemble product by product
            oldFlow = flowTensor[0:outputFlow,0:inputFlow,whichP,oldGuess[whichP]]

            # choose different member 
            newGuess = oldGuess
            newGuess[whichP] = np.random.choice([i for i in range(0,members) if not i==newGuess[0]])

            # replace old flow with new flow
            newFlow = flowTensor[0:outputFlow,0:inputFlow,whichP,newGuess[whichP]]
            newSumFlow = sumFlow - oldFlow + newFlow

            # calculate functional to minimize again
            functionalNEW = np.abs(newSumFlow-calibrationMatrix).sum(1).sum(0)
            ...
            if functionalNEW <= functionalOLD:
                ...
                myProb = np.exp(-(functionalNEW-functionalOLD)) # a number between 0 and 1, getting smaller, the bigger f2 is comparaed to f1
...
def calibration(flowTensor: np.ndarray, calibrationMatrix: np.array):
    '''
    ...
    Manual for input: create a 4D-numpy array for the input parameter flowTensor
    ...
    for run,p in enumerate(prod):
        ...
        # creating edgeIDs for adding 'distance' to graph
        allcombIDs1, allcombIDs2 = go.getAllCombinations(senderIDs, receiverIDs, \\
            order='1st')
        myEdges4Graph = go.getEdgeID(myNW, allcombIDs1, allcombIDs2)
        didItWork3 = go.addAttr2Edges(myNW, myEdges4Graph, myDF.values.flatten(), \\
            attr='distance')
        ...
        # get the transport distances
        myTDs.append(go.calcPotTransportDistances(myNW, listOfSenderIDs=senderIDs, \\
            listOfReceiverIDs=receiverIDs))
        ...
        for it,td in enumerate(myTDs[run]):
            allFlows[0:len(senderIDs),0:len(receiverIDs),run,it] = \\
                go.hymanModel(myNW, listOfSenderIDs=senderIDs, \\
                    listOfReceiverIDs=receiverIDs, transportDistance=td, tolerance = 0.01)

    #creating calibration measure that halfway might make sense
    allFoodTransported = sum(sum(sendingProd,[]))
    dummy = np.random.rand(8)*allFoodTransported/80+allFoodTransported/\\
        (len(sum(sendingProd,[]))+1)
    myCalMat = np.append(dummy,allFoodTransported-np.sum(dummy)).reshape((len(senderIDs),\\
        len(receiverIDs)))
    ...
    # initalising the sum of all flows to compare to calibration matrix with the lowest transport distance
    sumFlow = np.add.reduce(flowTensor[0:outputFlow,0:inputFlow,0:products,0],axis=2)
    run=-1
    for p in range(products):
        for td in range(members):
            ...
            if functionalNEW < functionalOLD:
                currentConf[p] = td
    print('exiting with relative difference to calibration matrix: '+str(functionalNEW/np.sum(calibrationMatrix)))
    return currentConf




def convertMatrix(haveSenderAttrs: list, haveReceiverAttrs: list, \
    inputMatrix: np.array, wantSenderAttrs: list, wantReceiverAttrs: list):
    '''
    purpose: converts the input matrix to a contracted version or a widened version
    of itself
    ...
    If p<m and q<n, the contracted rows and columns are added to each other, respectively.
    Application for calibrationProcess.

    If p>m and q>n, the new rows and columns are copied their contents, respectively.
    Application for quick creation of a distance matrix with different actors, sending 
    or receiving.
    
    input:
    :param haveSenderAttrs: the list of senders connected to the flow matrix
    :param haveReceiverAttrs: the list of receivers connected to the flow matrix
    ...
    _,attrS = go.convertLoT2NTs(go.getNodeIDswAttr(myNW,nameOfAttr='location',\\
        listOfNodeIDs=senderIDs))
    _,attrR = go.convertLoT2NTs(go.getNodeIDswAttr(myNW,nameOfAttr='location',\\
        listOfNodeIDs=receiverIDs))
    

    # EXAMPLE 1: expansion
    myTrafo = go.convertMatrix(loc, loc, myDF.to_numpy(),attrS,attrR)
    

    # creating edgeIDs for adding 'distance' to graph
    allcombIDs1,allcombIDs2 = go.getAllCombinations(senderIDs,receiverIDs,order='1st')
    myEdges4Graph = go.getEdgeID(myNW,allcombIDs1,allcombIDs2)
    didItWork3 = go.addAttr2Edges(myNW,myEdges4Graph,myTrafo.flatten().tolist(),\\
        attr='distance')

    # creating list of node IDs of participants
    senderIDs = go.getListOfNodeIDs(myNW, actors=['producer'],products=['milk'])
    receiverIDs = go.getListOfNodeIDs(myNW, actors=['consumer','warehouse'],\\
        products=['milk'])
    ...
    # Run the gravity model with given transport distance and return the flow
    flow = go.hymanModel(myNW, listOfSenderIDs=senderIDs, \\
        listOfReceiverIDs=receiverIDs, transportDistance=myTDs[0], tolerance = 0.01)


    # EXAMPLE 2 (contraction)
    locFlowOnly = convertMatrix(haveSenderAttrs=senderIDs, haveReceiverAttrs=receiverIDs, \\
    inputMatrix=flow, wantSenderAttrs=loc, wantReceiverAttrs=loc)
    ...
    if p==m and q==n:
        raise Exception('Error, dimensions identical. Please have a look at the documentation')
...
def draw(s):
    ...
    node_labels = dict((n, d['actor']) for n, d in s.nodes(data=True))
...
def edge_sizes(s): 
    return [EDGE_SIZE
        [
        c_(list(s.edges[edge[0], edge[1]].keys()))[-1]
        ] for edge in s.edges()
    ] # /!\ multiple links => one size
...
def edge_colors(s): 
    ...
    ] # /!\ multiple links => one color

def c_(list_edges): 
    return [a for a in list_edges if a in list(EDGE_COLOR.keys())]
    """