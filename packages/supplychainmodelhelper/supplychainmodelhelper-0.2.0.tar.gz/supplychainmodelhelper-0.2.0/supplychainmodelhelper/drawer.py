...
version 0:
    boundary conditions:
        stages: order of actors are drawn as stages
            ...
            -> producer are on stage 1, consumer are on stage 2 of this 2-stages-graph
...
class Drawer:
    ...
    def __init__(self, SCgraph):
        ...
        found_nodes = [node_type for node_type
        ...
        if len(found_nodes) < len(needed_nodes):
            raise Exception("found these node types: "+
                            str(found_nodes)+
                            ", but need these: "+
                            str(needed_nodes)
                            )
        if needed_edges not in go.getExistingAttrs(self.SCgraph, gtype='edges'):
            raise Exception("Please add edge attribute: " +
                            needed_edges +
                            "to the graph")
    ...
    def draw_it(self, product, actor_type_names=None):
        ...
        :param actor_names: (optional) list of actor types as defined in
        ...
        if actor_type_names:
            if isinstance(actor_type_names, list):
                if len(actor_type_names) < 2:
                    raise Exception("number of elements in 'actor_type_names' "
                                    "should be at least 2!")
            else:
                raise Exception("the parameter 'actor_type_names' "
                                "should be a list")
        ...
        legend_info = self.legend_node_size(math.sqrt(math.pi*node_size_min),
    ...
    def node_position(self, node_attr='actor', filtered_nodes=None):
        ...
        node_ids_with_actors = go.getNodeIDswAttr(SCGraph=self.SCgraph,
        ...
        stagewise_actors = [list(group) for _, group
                            in itertools.groupby(node_ids_with_actors,
        ...
        for layer in range(nr_of_layers):
            ...
            if not found_all_edges_to_be_drawn:
                raise Exception("ERROR! at least 1 of the edges between "
        ...
        for st_id, stage in enumerate(stagewise_actors):
            ...
            name_of_stage = ''.join(list(map(str,set([j for (i,j) in stage]))))
            my_stage_coord = nr_of_stages-st_id
            pos.update((nid, (i, my_stage_coord))
                       for i, (nid, _) in enumerate(stage))
    ...
    def node_size(self, node_attr=['output', 'input']):
        ...
        if isinstance(node_attr, list):
            do_they_exist = [attr for attr in node_attr
                             if attr in
                             go.getExistingAttrs(self.SCgraph, gtype='nodes')]
            if not do_they_exist:
                raise Exception("no attribute by the names: "+
                                str(node_attr)+
                                " exist on the graph!")
    ...
    def edge_color(self, weight='weight', color_map=plt.cm.plasma):
        ...
        if not weight in go.getExistingAttrs(self.SCgraph, gtype='edges'):
            raise Exception("given edge attribute: "+
                            str(weight)+
