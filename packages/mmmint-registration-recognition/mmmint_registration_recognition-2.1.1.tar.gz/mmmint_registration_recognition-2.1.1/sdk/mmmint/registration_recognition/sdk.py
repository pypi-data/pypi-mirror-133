...
class Client():
    ...
    def get_fahrzeugschein_xml(self):
        url = '{}/fahrzeugschein/{}/xml?access_token={}'.format(
                            self.base_url, 
                            self.session, 
                            self.api_key)
        r = requests.get(url)
    ...
    def get_fahrzeugschein_xml_sas(self, attachment=None):
        url = '{}/fahrzeugschein/{}/xml/sastoken?access_token={}'.format(
                            self.base_url, 
                            self.session, 
                            self.api_key)
        ...
        r = requests.post(url, data=attachment)
        if r.status_code == 200:
            response_dict = json.loads(r.text)
