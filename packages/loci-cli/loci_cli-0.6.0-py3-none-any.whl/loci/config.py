...
@click.option("-k", "--key",
              prompt="User API Key",
              help="Loci Server User API Key",
              required=True,
              type=str,
              default=lambda: lcu.get_loci_config_value("api_key"))
def config(server, key):
        ...
        if r.ok:
            ...
            if r.ok:
                ...
                if r.ok:
                    ...
                    lcu.print_error("The call process to obtain an API key has failed."
                                    " You will need to do this manually.", fatal=False)
                ...
                lcu.print_error("The login process to obtain an API key has failed. You will need to do this manually.",
                                fatal=False)
                url = urllib.parse.urljoin(server, "/docs#/api_keys/create_my_apikey_api_users_me_apikey_post")
                lcu.print_error("See the docs at [bold]%s[/bold]." % url)
...
def test():
    ...
    if r is not None:
        lcu.print_success("Loci configuration is correct, you are logged in as [bold]%s[/bold]." % r["full_name"])
