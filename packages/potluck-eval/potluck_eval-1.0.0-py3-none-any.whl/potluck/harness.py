...
def create_module_import_payload(
    ...
    def payload(context):
        ...
        # Attempt to release any input capturing set up by optimism calls
        # in the file we just ran:
        if OPTIMISTIC:
...
def create_run_function_payload(
    ...
    case). The "args" and "kwargs" slots will also get copies of the
    arguments, not the original values, and these will be separate copies
    from those given to the function, so they'll retain the values used
    as input even after the function is finished. However, "used_args"
    and "used_kwargs" slots will be added if `copy_args` is set to true
    that hold the actual arguments sent to the function so that any
    changes made by the function can be measured if necessary.
...
def create_execute_code_block_payload(block_name, src, nodes=None):
    ...
    def payload(context):
        ...
        if len(start) > 0:
            code = compile(ast.Module(start), block_name, 'exec')
            exec(code, env)
...
def with_fake_input(payload, inputs, extra_policy="error"):
    ...
    def fake_input_payload(context):
        ...
        # If we're dealing with optimism, it's mechanism for providing
        # input (replacing stdin) won't work with our mock_input
        # function! So we need to patch that up, and let
        # optimism-specified input override mock input until
        # restoreInput is called.
        if OPTIMISTIC:
            original_provideInput = optimism.provideInput

            def provideInputNoMock(*args, **kwargs):
                """
                Disables mock input and calls original provideInput.
                """
                builtins.input = original_input
                original_provideInput(*args, **kwargs)

            original_restoreInput = optimism.restoreInput

            def restoreInputToMock(*args, **kwargs):
                builtins.input = mock_input
                original_restoreInput(*args, **kwargs)

            optimism.provideInput = provideInputNoMock
            optimism.restoreInput = restoreInputToMock
        ...
        try:
            ...
            # Reset optimism input mechanisms
            if OPTIMISTIC:
                optimism.provideInput = original_provideInput
                optimism.restoreInput = original_restoreInput
...
def report_argument_modifications(target, *args, **kwargs):
    ...
    report on whether or not each mutable argument to the function was
    modified or not after the function is finished. It only checks
    arguments which are are lists or dictionaries at the top level, so
    its definition of modifiable is rather narrow.
