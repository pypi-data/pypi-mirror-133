...
class HasGoal:
    ...
    def compare_strings_gently(self):
        """
        Overrides the comparator (see `compare_using`) with
        ...
        """
        self.ensure_goal_constructor_arg("checker")
        self.goal_args["checker"] = compare.very_fuzzy_string_compare
        return self
    ...
    def compare_strings_firmly(self):
        ...
        which works like strings_are_equal_modulo_whitespace but it
...
class FunctionCall(Check):
    ...
    def count_by_names(self):
        ...
        Name as its func expression, it will not be counted at all. Note
        that this only really makes sense in conjunction with a bindable
        slot as the function name (e.g., '_'), or with multiple possible
        function names.
...
def RequireExpectations(
    ...
    """
    Registers a miscellaneous goal requiring that the submitted code
    creates a certain number of expectations (using the `optimism` module)
    for each of certain target functions. `required_per_function` must be
    a dictionary mapping functions that need testing to the minimum
    number of tests required. A custom category and description may be
    provided; the default category is core. The underlying goal created
    is a `potluck.rubrics.DefinesEnoughTests`.
    """
    args = {
        "reqs": required_per_function,
        "tags": { "category": category },
    }
    if description is not None:
        args["description"] = description
...
def ObserveExpectationResults(
    category="extra",
    description=None
):
    """
    Registers a miscellaneous goal requiring that all expectations
    established by the submitted code (using the `optimism` module) must
    pass (in their original context). The goal is a
    `potluck.rubrics.TestsPassedInSubmission`. A category (other than the
    ...
    register_goal(
        rubrics.TestsPassedInSubmission(
            file_utils.deduce_task_id(),
            **args
        )
    )
...
def ValidateExpectations(
    relevant_functions,
    category="feedback_only",
    description=None
):
    """
    TODO: This should not be used until TestsPassForSolution can be
    fixed to work properly when provideInput was used before a test
    case!

    Registers a miscellaneous goal requiring that each expectation (from
    the `optimism` module) in the submitted code which applies to an
    expression involving one of a list of relevant functions should also
    pass when run using the solution versions of those functions. The
    relevant functions are simply a list of function names; methods may
    be named using a '.', but ANY function that shares that name will get
    replaced, whether it's a method or not, so be careful (TODO: fix
    that). A custom category and/or description may be provided; the
    default category is "extra".

    This creates a `potluck.rubrics.TestsPassForSolution` goal.

    Note the by default, the category is 'feedback_only,' meaning that
    this goal will NOT be counted as part of the grade.
    """
    args = {
        "targets": relevant_functions,
        "tags": { "category": category },
    }
    if description is not None:
        args["description"] = description
...
def rubric():
    ...
    # Directly-registered goals
    all_goals.extend(GOALS.get(sname, []))

    # Goals via providers
    for provider in GOAL_PROVIDERS.get(sname, []):
        try:
            all_goals.append(provider.provide_goal())
        except Exception:
    ...
    for cat in checks:
        ...
        for goal_type in cat_registry:
            ...
            for check_obj in list_of_checks:
                ...
                all_goals.extend(cat_gt_map.values())
