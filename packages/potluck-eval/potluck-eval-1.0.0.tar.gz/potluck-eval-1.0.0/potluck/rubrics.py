...
class Rubric:
    ...
    performance based on the evaluation of each individual goal.
    """
    def __init__(self, goals, performance_metric, spec_file=None):
        """
    ...
    def all_contexts(self):
        """
        Crawls our goals and their subgoals to find all relevant
        `potluck.contexts.Context` objects that might possibly be used by
        tests in this rubric. Returns a list in breadth-first traversal
        order of this rubric's goals, their contexts, and those contexts'
        dependencies.
        """
        # Map of object IDs
        idmap = {}
    ...
    def create_contexts_list(self, base_context=None):
        """
        Returns a list of context summary dictionaries describing all of
        the contexts used in this rubric. It has the same format as
    ...
    def create_blank_report(self, task_info):
        ...
        report = {
            ...
            "table": [],
            "contexts": self.create_contexts_list()
        }
        ...
        metric_result = self.metric(self.goals, blank=True)
    ...
    def evaluate(self, task_info, username, submission_target):
        ...
        If student code cannot be loaded due to a syntax error or parsing
        fails for some other reason, the report will mention that in as much
        detail as it can, and the normal rubric item will be skipped.

        Note: This function completely resets all goals and clears the
        caches of any associated `potluck.contexts.Context` objects
        before it starts evaluating goals.
        ...
        metric_result = self.metric(self.goals)
        ...
        # same values sued during testing:
        report["contexts"] = self.create_contexts_list(base_context)
    ...
    if core_score == len(core):
        ...
        if core_score >= half:
            return (
                "partially complete",
                (
                    f"<p>You accomplished {core_score} (at least {half})"
                    f" of the {len(core)} core goals.</p>"
                )
            )
        else:
            return (
                "incomplete",
                (
                    f"<p>You accomplished only {core_score} (less than"
                    f" {half}) of the {len(core)} core goals.</p>"
                )
            )
...
def core_extras_categorized_metric(goals, blank=False):
    ...
    same as for the `codder.buoy.foundational_core_extras_metric`, and
    the goal types are not relevant to the evaluation result.
    """
            ...
            if ngoals == 0:
                ...
                if len(extra_subrow["subtable"]) == 0:
                    ...
                    if row["status"] == "accomplished":
                        ...
                        if ngoals > 1:
                            row["explanation"] = (
                                "Did not accomplished at least half of"
                                " the {} extra {} goals."
                            ).format(ngoals, gtype)
                        else:
                            row["explanation"] = (
    ...
    def __init__(
        ...
        **kwargs
        ...
        - subslip: A number of subgoals which are allowed to be violated
            ...
            as 1/2 a point. By default this number will be set to half of
            the number of subgoals, rounded down.
        ...
        if self.subslip is None:
            self.subslip = len(self.subrules) // 2
...
class DoesntWasteBoxes(LintCheck):
    ...
    hints that a value will not be used.
    ...
    other scopes.
    """
    def __init__(self, taskid, exclude=None, tolerance=2, **kwargs):
        """
        A task ID is required. A list of strings specifying names of
    ...
    def check(self, context):
        ...
        # are a part of.
        self.gather_loads_and_stores(node, scopes, exclude=self.exclude)
...
#-----------------------------------------#
# Goal subtypes for checking expectations #
#-----------------------------------------#

class ExpectationsTest(Goal):
    """
    Runs a function against the auto-context for "expectations". Inherit
    and override the `check` method with a function that accepts a
    context and returns a goal evaluation result to define your test.
    ...
    def check(self, context):
        ...
        """
        raise NotImplementedError(
            "ExpectationsTest is an abstract class that can't be used"
            " directly."
        )
    ...
    def __init__(
        ...
        **kwargs
        ...
        The categorizer "expectations:" will be prepended to the given
        identifier.
        ...
        if len(depends) == 1:
            ...
            test_context = contexts.Context(
                description=(
                    "Expectations defined by your code",
                    (
                        "The " + phrasing.comma_list(uses_slots)
                      + " of your code."
                    )
    ...
    def evaluate_in_context(self, context=None):
        ...
        try:
            ...
            if self.result is None:
                raise ValueError(
                    f"Expectations test for {self.__class__.__name__}"
                    f" returned None!"
                )
        except Exception:
            self.result = {
            ...
            self.set_explanation(
                ...
                status="crash",
                default=html_tools.html_traceback(
                    title="Error while checking your expectations.",
                    linkable=context_utils.linkmap(context)
                )
            )
            return self.result
...
class DefinesEnoughTests(ExpectationsTest):
    """
    An expectations checker which ensures that for certain listed
    functions, a certain number of expectations are established (using
    the `optimism` module) that make use of each (directly by name).

    Note that this ONLY captures function calls using bare names, so you
    can't require tests for an attribute of another module or for a
    method of a class, for example.
    """
    def __init__(self, taskid, reqs, **kwargs):
        """
        A task ID is required. The other required argument is a
        dictionary mapping function names to integers specifying how many
        tests are required.

        Other arguments get passed through to `ExpectationsTest` and
        potentially thence to `Goal`.
        ...
        if "description" not in kwargs:
            kwargs["description"] = (
                "Defines required expectations",
                (
                    "Your code must use the <code>optimism</code> module"
                    " to establish a certain number of test cases which"
                    " use the following functions directly in the test"
                    " case expression. (Each test case must be followed"
                    " by at least one expectation):"
                ) + html_tools.build_list(
                    f"<code>{fname}</code>: {reqs[fname]} test cases"
                    for fname in reqs
                )
            )

        if "uses_slots" not in kwargs:
            kwargs["uses_slots"] = ["expectations"]
    ...
    def check(self, context):
        ...
        given context.
        """
        expectations = context_utils.extract(context, "expectations")
        cases = {
            id(exp["case"]): exp["case"]
            for exp in expectations
        }
        by_fn = {}
        for case_id in cases:
            case = cases[case_id]
            node = case["context"]["expr"]
            for child in ast.walk(node):
                if (
                    ...
                    and child.func.id in self.reqs
                    ):
                        fname = child.func.id
                    elif (
                        isinstance(child.func, ast.Attribute)
                    and child.func.attr in self.reqs
                    ):
                        fname = child.func.attr
                    else:
                        # This child is not a matching function call
                        continue

                    # Build out our by_fn mapping
                    by_fn.setdefault(fname, set()).add(case_id)

        any_tests = False
        deficient = False
        reports = []
        for req_name in self.reqs:
            tests = [cases[ident] for ident in by_fn.get(req_name, set())]
            if len(tests) > 0:
                any_tests = True
            if len(tests) < self.reqs[req_name]:
                deficient = True
                symbol = '✗'
            else:
                symbol = '✓'
            ...
            reports.append(
                f"{symbol} <code>{req_name}</code>: {len(tests)} /"
                f" {self.reqs[req_name]}"
            )
        ...
        if not any_tests:
            return {
                ...
                "explanation": (
                    ...
                    " cases for required functions."
                )
            }
            # TODO: require expectations to be established by a test_*
            # function instead?
        elif deficient:
            return {
                "status": "partial",
                "explanation": (
                    ...
                    " were required for all functions:\n"
            ...
            return {
                ...
                "explanation": (
                    ...
                    " function it was required to test."
                )
            }


def list_expectations(expectations):
    """
    Creates an HTML list out of expectation objects.
    """
    items = []
    for exp in expectations:
        short_tag = exp['tag'].split('/')[-1]
        message = html_tools.escape(exp.get('message', '???'))
...
class TestsPassedInSubmission(ExpectationsTest):
    """
    An expectations checker which ensures that each expectation
    established by the submitted module was met when it was established.
    """
    def __init__(self, taskid, **kwargs):
        ...
        The identifier will be "met_in_submission".
        """
        ...
        try:
            ...
            raise NotImplementedError(
                "TestsPassedInSubmission cannot be used because the"
                " 'optimism' module cannot be imported."
            )
        ...
        if "description" not in kwargs:
            kwargs["description"] = (
                (
                    "All defined expectations were met"
                ),
                (
                    "Every expectation that your code establishes"
                    " using the <code>optimism</code> module must be met,"
                    " and it must establish at least one expectation."
                )
            )
        ...
        super().__init__(taskid, "met_in_submission", **kwargs)

    def check(self, context):
        """
        Looks for any failed expectations in the given context.
        """
        expectations = context_utils.extract(context, "expectations")
        any_failed = False
        any_passed = False
        ...
        if any_failed:
            fail_list = list_expectations(failed)
            if any_passed:
                return {
                    "status": "partial",
                    "explanation": (
                        "Some of your code's expectations were not met:\n"
                    ) + fail_list
                }
            else:
                return {
                    "status": "failed",
                    "explanation": (
                        "None of your code's expectations were met:\n"
                    ) + fail_list
                }
        else:
            if any_passed:
                return {
                    "status": "accomplished",
                    "explanation": (
                        "All of your code's expectations were met."
                    )
                }
            else:
                return {
                    "status": "failed",
                    "explanation": (
                        "Your code did not define any expectations."
                    )
                }


class TestsPassForSolution(ExpectationsTest):
    """
    An expectations checker which ensures that each expectation involving
    one of certain target functions established by the submitted module
    passes if run using the target functions from the solution module.
    Note that this is... inexact in some ways. This test passes if it
    can't find any relevant expectations.

    TODO: This test fails for any tests using provideInput! Because we
    cannot know what inputs were left in stdin at the start of the test!
    Don't use this until we can fix that!
    """
    def __init__(self, taskid, targets, **kwargs):
        """
        A task ID and a list of function names to target are required.
        Additional arguments are passed through to `ExpectationsTest`.

        The identifier will be "met_in_solution".
        """
        ...
        try:
            ...
            raise NotImplementedError(
                "TestsPassForSolution cannot be used because the"
                " 'optimism' module cannot be imported."
            )
        ...
        if "description" not in kwargs:
            kwargs["description"] = (
                (
                    "Core expectations are correct"
                ),
                (
                    "Every expectation that your code establishes"
                    " involving any of the following functions must"
                    " work when run again using the solution code:\n"
                ) + html_tools.build_list(
                    f"<code>{fname}</code>"
        ...
        if "uses_slots" not in kwargs:
            kwargs["uses_slots"] = ["expectations", "ref_module"]

        super().__init__(taskid, "met_in_solution", **kwargs)

    def relevant_expectations(self, expectations):
        """
        Filters a list of expectations down to just expectations that
        include calls to one or more of our target functions.
        """
        result = []
        for exp in expectations:
            case = exp["case"]
            ...
            for child in ast.walk(node):
                ...
                and (
                        (
                            isinstance(child.func, ast.Name)
                        and child.func.id in self.targets
                        )
                     or (
                        isinstance(child.func, ast.Attribute)
                    and child.func.attr in self.targets
                        )
                    )
                ):
                    result.append(exp)
                    break # no need to check more nodes in this tree
        return result

    def check(self, context):
        """
        Rechecks all established expectations involving one of the target
        functions within the context of the solution code, and expects
        them all to pass.
        """
        expectations = self.relevant_expectations(
            context_utils.extract(context, "expectations")
        )
        soln_module = context_utils.extract(context, "ref_module")

        # TODO: Smart method remapping!
        #method_names = [t.split('.')[-1] for t in self.targets if '.' in t]
        soln_vals = {
            target.split('.')[-1]: getattr(soln_module, target)
        ...
        for exp in expectations:
            ...
            crashed = False
            try:
                new_exp = optimism.recheck_expectation(
                    exp,
                    optimism.deepish_copy(soln_vals)
                )
            except Exception:
                crashed = True
                # Intentionally to uncaptured stdout instead of stderr:
                print(f"Warning: recheck of {exp['tag']} crashed.")
                tb = traceback.format_exc()
                print(tb)
                print(tb, file=sys.stderr) # capture it too

            finally:
                sys.stderr = old_stderr

            if crashed or not new_exp.get("met", False):
                failed.append(new_exp)
                any_failed = True
            else:
                any_passed = True
        ...
        if any_failed:
            ...
            if any_passed:
                return {
                    "status": "partial",
                    "explanation": (
                        "Some of your code's relevant expectations were"
                        " not met when applied to the solution code"
                ...
                return {
                    "status": "failed",
                    "explanation": (
                        "None of your code's relevant expectations were"
                        " met when applied to the solution code (they"
                        " may be incorrect):\n"
                    ) + fail_list
                }
        else:
            if any_passed:
                return {
                    "status": "accomplished",
                    "explanation": (
                        "All of your code's relevant expectations were"
                        " met when applied to the solution code."
                    )
                }
            else:
                return {
                    "status": "accomplished",
                    "explanation": (
                        "Your code did not define any relevant"
                        " expectations."
                    )
                }
