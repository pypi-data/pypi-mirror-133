...
Usage:
  ...
  opereto diagnostics <from_iso_date> <to_iso_date> [--file=OUTPUT_FILE]
  opereto token
  opereto (-h | --help)
  opereto --version
...
Options:
    ...
    diagnostics          : Generates a diagnostics file. Requires a few additional python packages
...
from pyopereto.diagnostics import run_diagnostics
...
def diagnostics(arguments):
    ...
    startdate = arguments['<from_iso_date>']
    enddate = arguments['<to_iso_date>']
    file = arguments['--file'] or None
    run_diagnostics(startdate, enddate, file)
