...
def data_download(target_number, outliers, sigma):
    """                
    ...
    TICID,Camera,CCD,Tmag,RA,Dec = np.loadtxt("Targets/sector_1.csv", delimiter=',', unpack=True)
    
    target = int(TICID[target_number])
    search_result = lk.search_lightcurve(f'TIC {target}') # serching target with the lightkurve
    ...
    if outliers == True:
        ...
        max_outliers_index = np.array(np.argwhere(flux > highest_allowed).flatten(), dtype = 'int')
        min_outliers_index = np.array(np.argwhere(flux < lowest_allowed).flatten(), dtype = 'int')
...
def angular_frequency(time, flux, flux_err, min_period, max_period):
    ...
    best_period, best_period_err = period_search(time, flux, flux_err, min_period, max_period)
    ...
    for i in range(0,l):
        ...
        for a in range(0,lf):
            for b in range(0,lf):
                ...
                ff = func(time[i])
                sig2 = sig2 + (ff[a]*ff[b])*A_inv[a,b] # Con esto el sum A_{alpha,beta}*f_alpha(t)*f_beta(t)
        ...
        y_err[0,i] = np.sqrt(sig2*sigstar)  # Con esto se tiene la raiz cuadrada de la formula 4 con grd 0

    y_err = y_err.flatten() 
        
    return y, y_err

def evaluation_ini(func, time, flux, flux_err):    #Pa no calcular el error y optimizar chi^2
...
def amplitude_func(flux, flux_err, time, mins , omega, T_0):
    ...
    amplitude_err = fitted_err[np.argwhere(fitted == max(fitted))]**2 + fitted_err[np.argwhere(fitted == min(fitted))]**2
    amplitude_err = np.sqrt(amplitude_err)    
        
    return amplitude, amplitude_err

def light_curve_fit(target_number, outliers, sigma, min_period, max_period):
    """                
    ...
    time, flux, flux_err = data_download(target_number, outliers, sigma)
    T_0 = 0
    period, period_err = period_search(time, flux, flux_err, min_period, max_period)
    omega, omega_err = angular_frequency(time, flux, flux_err, min_period, max_period)
    minChi, mins = best_fit(time, flux, flux_err, omega, T_0)
    func = harm_fourier_series(mins, omega, T_0)
    fitted, fitted_err = evaluation(func, time, flux, flux_err)
    amplitude, amplitude_err = amplitude_func(flux, flux_err, time, mins , omega, T_0)
    
    return time, flux, flux_err, fitted, minChi, mins, period, period_err, amplitude, amplitude_err

def phase_plot_fit(target_number, outliers, sigma, min_period, max_period):
    """                
    ...
    amplitude, amplitude_err = amplitude_func(flux, flux_err, phase, mins , omega, T_0)
   
    return time, phase, flux, flux_err, fitted, minChi, mins, period, period_err, amplitude, amplitude_err

def phase_plot(target_number, outliers, sigma, min_period, max_period):
    """                
    ...
    """
    TICID,Camera,CCD,Tmag,RA,Dec = np.loadtxt("Targets/sector_1.csv", delimiter=',', unpack=True)
    time, phaseA, flux, flux_err, fitted, minChi, mins, period, period_err, amplitude, amplitude_err = phase_plot_fit(target_number, outliers, sigma, min_period, max_period)
    
    phase = np.sort(np.concatenate((phaseA, phaseA + 1)))
    residuals = flux- fitted
    fig, phase_plot = plt.subplots(2, sharex=True, sharey=False, gridspec_kw={'height_ratios': [3, 1]})
...
def amplitude_test(min, max, amp):
    """                
    ...
    for i in range(min, max):
        ...
        time, flux, flux_err, fitted, minChi, mins, period, period_err, amplitude, amplitude_err = light_curve_fit(i, True, 4, 0.1, 27.)

        if amplitude >= amp:
            variable = int(TICID[i])
            candidates.append(i)
    ...
    return candidates, chis, degrees, periods, period_errors, amplitudes, amplitude_errors

def csv_amplitude_test(file, min, max, amp):
    """                
                       amplitude_test DOCUMENTATION
    ---------------------------------------------------------------------
    Detects variable stars with amplitude higher than threshold.
    ...
    ----------------------------------------------------------------------
    """
    candres, chisres, degres, perres, pererrres, ampres, amperrres = amplitude_test(
        min, max, amp)
    with open(file, 'w', newline='') as f:
        ...
        for i in range(len(candres)):
            writer.writerow([str(candres[i]), str(chisres[i]), str(degres[i]), str(
                perres[i]), str(pererrres[i]), str(ampres[i]), str(amperrres[i])])
    return candres, chisres, degres, perres, pererrres, ampres, amperrres