...
class CVXOPTSolver(Solver):
    ...
    def _affine_expression_to_G_and_h(self, expression):
        ...
        # Construct G.
        I, J, V = [], [], []
        for variable, coefficients in expression._linear_coefs.items():
            cvxoptVarOffset = self._cvxoptVarOffset[variable]

            if not isinstance(coefficients, cvxopt.spmatrix):
                coefficients = cvxopt.sparse(coefficients)

            I.extend(coefficients.I)
            J.extend([cvxoptVarOffset + j for j in coefficients.J])
            V.extend(coefficients.V)
        G = cvxopt.spmatrix(V, I, J, (length, self._numVars), tc="d")

        # Construct h.
        # TODO: Check if making a copy is necessary.
        h = cvxopt.matrix(expression._constant_coef, tc="d")
    ...
    def _solve(self):
        ...
        # Allow solving GPs that are not strictly feasible and work around
        # constraint matrix rank requirements.
        ...
        with self._header(subsolverText), self._stopwatch():
            if self.is_smcp:
                ...
                if self.ext.options.cvxopt_kktsolver is not None:
                    kwargs["kktsolver"] = self.ext.options.cvxopt_kktsolver
