...
class MOSEKSolver(Solver):
    ...
    def _index_function(self, picosVar, localIndex):
        """Support :meth:`_affinexp_pic2msk`."""
        if picosVar in self._mosekBarVarIndex:
            assert picosVar not in self._mosekVarOffset
            return None
        else:
            return self._mosekVarOffset[picosVar] + localIndex
    ...
    def _affinexp_pic2msk(self, picosExpression):
        ...
        for localIndex, (J, V, c) in enumerate(picosExpression.sparse_rows(
                varOffsetMap=None, indexFunction=self._index_function)):
            # Remove indices and coefficients corresponding to bar vars.
            JV = tuple(zip(*(jv for jv in zip(J, V) if jv[0] is not None)))
