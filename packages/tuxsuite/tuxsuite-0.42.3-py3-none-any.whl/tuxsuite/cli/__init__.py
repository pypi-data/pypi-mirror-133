...
def format_plan_result(build, tests):
    ...
    builds = (
        build.uid
        + " "
        + click.style(f"{icon} {message}", fg=cli_color, bold=True)
        + " "
        + build.target_arch
        + "@"
        + build.toolchain
...
def build(
    ...
    **build_params,
    ...
    if patch_series:
        build_params["patch_series"] = click.format_filename(patch_series)
    ...
    **build_params,
    ...
    if no_wait:
        ...
        if build_status_list:
            json_out.write(json_lib.dumps(build_status_list, sort_keys=True, indent=4))
        return

    build_set_result = wait_for_object(build_set)

    if json_out:
        json_out.write(json_lib.dumps(build_set.status_list, sort_keys=True, indent=4))
