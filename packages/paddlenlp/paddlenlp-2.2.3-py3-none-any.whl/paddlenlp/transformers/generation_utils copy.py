...
class GenerationMixin(object):
    ...
    @staticmethod
    def get_logits_processor(min_length=None, eos_token_id=None, num_beams=1, num_beam_groups=1, diversity_rate=0.0):
    ...
    def generate(self,
        ...
        logits_processors = self.get_logits_processor(min_length=min_length,eos_token_id=eos_token_id,num_beams=num_beams, num_beam_groups=num_beam_groups, diversity_rate=diversity_rate)
        ...
        if decode_strategy == 'greedy_search':
            ...
            if num_beam_groups > 1:
                ...
                return self.group_beam_search(input_ids, diverse_beam_scorer, logits_processors,
    ...
    def group_beam_search(self, input_ids, beam_scorer, logits_processors, max_length,
                    diversity_rate, pad_token_id, eos_token_id, **model_kwargs):
        ...
        while cur_len < max_length:
            ...
            for beam_group_idx in range(num_beam_groups):
                ...
                print(next_scores)
                next_scores = next_scores + beam_scores[batch_group_indices].unsqueeze(-1).expand_as(
                    next_scores
                )
                ...
                group_input_ids = paddle.concat([paddle.index_select(group_input_ids, index=beam_idx), beam_next_tokens.unsqueeze(-1)], axis=-1)
...
class HammingDiversityLogitsProcessor(LogitsProcessor):
    ...
    def __call__(self, input_ids, scores, current_tokens, beam_group_idx):
        ...
        for batch_idx in range(batch_size):
            print("current_tokens",current_tokens,current_tokens.shape)
            print("scores",scores,scores.shape)
            # predicted tokens of last time step of previous groups
            previous_group_tokens = current_tokens[
                batch_idx * self._num_beams : batch_idx * self._num_beams + group_start_idx
            ]
            print("previous_group_tokens",previous_group_tokens,previous_group_tokens.shape)
            token_frequency = paddle.histogram(previous_group_tokens, bins=vocab_size, min=0, max=vocab_size+1)
            print("token_frequency",token_frequency,token_frequency.shape)
