...
class HoldingAnalysor:
    def __init__(self, fund_name, start_date, end_date, threshold=1):
        self.fund_name = fund_name
    ...
    def get_construct_result(self, is_tl_show=False):
        ...
        data, layout = self.plotly_area(100 * asset_allo_series.T, '资产配置走势')
        plot_render({"data": data, "layout": layout}, width=1200, height=600)

        data, layout = self.plotly_area(100 * industry_allo_df.T, '行业配置走势')
        plot_render({"data": data, "layout": layout}, width=1200, height=600)
        # fig = go.Figure(data=data, layout=layout)
        # plot_ly(fig, filename="D:\\主观股多估值表基地\\图表所在\\B-行业配置走势.html", auto_open=False)

        data, layout = self.plotly_area(100 * sector_allo_df.T, '板块配置走势')
        plot_render({"data": data, "layout": layout}, width=1200, height=600)
        ...
        data, layout = self.plotly_area(100 * bm_dis.T, '宽基成分配置走势')
        plot_render({"data": data, "layout": layout}, width=1200, height=600)

        data, layout = self.plotly_area(100 * mkt_dis.T, '持股市值分布走势')
        plot_render({"data": data, "layout": layout}, width=1200, height=600)
...
if __name__ == '__main__':
    ...
    HoldingAnalysor(name, start_date='20200520', end_date='20211201').get_construct_result()