...
class IndustryCr:
    ...
    def calculate(self):
        ...
        industry_factor['ind_spread'] = industry_return.apply(
            lambda x: sm.OLS(x.sort_values().values, sm.add_constant(np.arange(industry_return.shape[1]))).fit().params[1], axis=1)

        # tmp = industry_attr.abs().divide(industry_attr.abs().sum(axis=1), axis=0)
        # industry_factor['ind_cr_3'] = tmp.apply(lambda x: x.nlargest(3).sum() / x.sum(), axis=1)
        # industry_factor['ind_cr_5'] = tmp.apply(lambda x: x.nlargest(5).sum() / x.sum(), axis=1)
        # industry_factor['ind_cr_10'] = tmp.apply(lambda x: x.nlargest(10).sum() / x.sum(), axis=1)
