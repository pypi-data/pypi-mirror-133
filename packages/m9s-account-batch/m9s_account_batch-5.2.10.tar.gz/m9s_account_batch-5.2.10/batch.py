...
class BatchLine(ModelSQL, ModelView):
    ...
    tax = fields.Many2One('account.tax', 'Tax',
        domain=[
            ('parent', '=', None),
            ['OR',
                ('start_date', If(Eval('date'), '<=', '!='), Eval('date')),
                ('start_date', '=', None),
                ],
            ['OR',
                ('end_date', If(Eval('date'), '>=', '!='), Eval('date')),
                ('end_date', '=', None),
                ],
            ],
        states={'readonly':
        ...
        depends=_LINE_DEPENDS + ['invoice', 'date'])
    ...
    def post(cls, lines):
        ...
        _update_tax_cash_basis = getattr(
            Invoice, '_update_tax_cash_basis', None)
        ...
        for line in [l for l in lines if l.invoice
            ...
            # Compatibility with account_tax_cash
            if _update_tax_cash_basis:
    ...
    def create(cls, vlist):
        ...
        to_write = []
        for line in lines:
            sequence = Sequence.get('account.batch.line.create')
            ...
            to_write.extend([
                    [line], {'move': move.id, 'sequence': sequence},
                    ])
        if to_write:
        ...
        if self.tax:
            ...
            for tax in taxes:
                ...
                if not self.tax.reverse_charge:
                    base_amount = tax.reverse_compute(amount, [tax])
