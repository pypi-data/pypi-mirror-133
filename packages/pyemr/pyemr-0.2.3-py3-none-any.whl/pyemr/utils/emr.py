...
from .config import (
    _pyproject_toml_exists, cprint, get_cluster_name, get_env_name, load_config,color_text
)
    ...
    try:
        for state in pbar:
            if state != last_state:
                ...
                pbar.set_description(color_text(f"{description}:step_id={step_id},state={state}", color))
        ...
        pbar.set_description(color_text(f"{description}:step_id={step_id},state=KeyboardInterrupt", color))
...
def list_steps(cluster_name, n, step_name_pattern, state_patterm, print_info=False) -> pd.DataFrame:
    """List steps
