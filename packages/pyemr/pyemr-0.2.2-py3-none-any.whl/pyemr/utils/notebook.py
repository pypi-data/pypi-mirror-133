...
def install_mock_python_kernal():
    """
...
def run_notebook_in_poetry():
    ...
    if not os.path.exists("pyproject.toml"):
        ...
        print(
            f"poetry run python -m pyemr.cli_lite notebook --env=sys --additional_site_package_paths={site_packages}",
        )
        pipe_cmd(
            f"poetry run python -m pyemr.cli_lite notebook --env=sys --additional_site_package_paths={site_packages}",
        )
...
def launch_mock_notebook_docker():
    ...
    docker_build_run(f"{SH_DIR}/run_notebook.sh")
