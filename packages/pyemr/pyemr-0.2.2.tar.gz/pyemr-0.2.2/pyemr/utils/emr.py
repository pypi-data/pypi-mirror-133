...
from .config import (
    _pyproject_toml_exists, cprint, get_cluster_name, get_env_name, load_config,
)
...
def wait_else_cancel(cluster_id, step_id, description="Waiting"):
    ...
    pbar = tqdm(
        ...
        desc=f"{description}:step_id={step_id},state=SUBMITTING",
    )
    ...
    try:
        for state in pbar:
            if state != last_state:
                pbar.set_description(f"{description}:step_id={step_id},state={state}")
...
def get_cluster_id(cluster_name: str = "") -> str:
    ...
    Args:
      ...
      cluster_name: str:  (Default value = '')
      cluster_name: str:  (Default value = "")
      cluster_name: str:  (Default value = "")
      cluster_name: str:  (Default value = "")
      cluster_name: str:  (Default value = "")
...
def list_steps(cluster_name, n, step_name_pattern, state_patterm) -> pd.DataFrame:
    """List steps
