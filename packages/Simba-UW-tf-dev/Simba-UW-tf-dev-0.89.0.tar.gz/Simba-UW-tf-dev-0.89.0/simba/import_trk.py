...
def import_trk(inifile, dataFolder, idlist, interpolation_method):
    ...
    bpNamesCSVPath = os.path.join(project_path, 'logs', 'measures', 'pose_configs', 'bp_names', 'project_bp_names.csv')
    poseEstimationSetting = config.get('create ensemble settings', 'pose_estimation_body_parts')
    print('Converting TRK into dataframes...')
    ...
    for filename in filesFound:
        ...
        try:
            ...
            with h5py.File(filename, 'r') as trk_dict:
                t_first = list(trk_dict['pTrk'])
                t_second = np.array(t_first)
