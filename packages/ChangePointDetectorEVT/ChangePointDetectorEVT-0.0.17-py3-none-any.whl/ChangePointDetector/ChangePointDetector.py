...
class ModuleResults:
    def __init__(self,Trend,TrendSmoothed,TrendVariance,ChangePoints,Prediction,PredictionVariance,ExtendedDates):
            self.Trend=Trend
            self.TrendSmoothed=TrendSmoothed
...
#         NormalisedData=RemoveOutliers(OriginalData)/np.linalg.norm(RemoveOutliers(OriginalData))
    ...
    def InitialiseFilter(self):
        ...
        MeasureVar=.01   #Doesn't seem to do anything??
        kf_model=KalmanModel(NormalisedData,transition=self.StateArrays.transition,design=self.StateArrays.design)
        
        start_params = [np.var(NormalisedData)**3, MeasureVar] #State convariance and observation covariance. These settings will underfit the curve
        kf_model_fit = kf_model.fit(start_params,maxiter = 20,method = 'bfgs', hessian= 'true')
        Growth=kf_model_fit.predict(1,len(self.data))
        ...
        while Success ==False and i>0:  #Increae state covariance until fitting improves over underfit base case
            start_params = [np.var(NormalisedData)**(i), MeasureVar]
            kf_model_fit = kf_model.fit(start_params,maxiter = 20,method = 'bfgs', hessian= 'true')
            Growth=kf_model_fit.predict(1,len(self.data))
            Ratio=np.mean(abs(NormalisedData-Growth))/Base
            if Ratio<.3 or Ratio==OldRatio:  
                Success =True
            i-=0.05
            OldRatio=Ratio
...
#       gs_orig=self.TimeseriesModel.KalmanModelFit.filtered_state[-1,:]
        gs_orig=self.TimeseriesModel.KalmanModelFit.smoothed_state[-1,:]
        gs_orig= np.insert(gs_orig[:-1], 0, 0, axis=0)
        ...
        for i in range(self.SeasonalityPeriods+1):
            EVMT.append(0)
        ChangePoints=[]
        ChangePoint=self.SeasonalityPeriods+2
#         ChangePoint=0
        PrLmOld=0
        for t in range(self.SeasonalityPeriods+3,len(MDout)+1):
            EV=0
            ...
            if   EV>0.97 and t>self.SeasonalityPeriods*2+2 and (t-(ChangePoint+1))>2:  
                EV=1
                ChangePoint=t
                PrLmOld=0
                EV_Old=1
        ...
        if  not sum([i > len(self.ts.data)-4 for i in ChangePoints]):
        ...
        TrendSmoothed=[self.TimeseriesModel.KalmanModelFit.smoothed_state[-1,x] \
               for x in range(self.TimeseriesModel.KalmanModelFit.filtered_state.shape[1])]
        TrendSmoothed=[x*np.linalg.norm(self.ts.data) for x in TrendSmoothed]
        ...
        ChangePoints=[1 if x in ChangePoints else 0 for x in range(1,len(gs_orig)+1)]
        
        Results= ModuleResults(Trend=Trend,TrendSmoothed=TrendSmoothed,TrendVariance=TrendVariance,ChangePoints=ChangePoints,Prediction=Prediction,\
