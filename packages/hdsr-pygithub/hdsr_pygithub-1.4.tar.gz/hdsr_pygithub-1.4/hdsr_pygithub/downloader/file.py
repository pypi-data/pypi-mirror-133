...
class GithubFileDownloader(GithubDownloaderBase):
    ...
    def get_download_url(self) -> str:
        files = self.downloadable_content
        assert len(files) == 1, f"there should be 1 file, as GithubFileDownloader has only 1 target_file {files}"
    ...
    def downloadable_content(self) -> List[ContentFile]:
        ...
        self._content_file = [self._content_file]
        return self._content_file
    ...
    def __check_target_file_is_recent_enough(self) -> None:
        ...
        commit date."""
        assert len(self.downloadable_content) == 1
        downloadable_content = self.downloadable_content[0]
        last_modified_date = self._get_file_last_modified_date(file_content=downloadable_content)
        ...
        if period_file_not_updated > self.allowed_period_no_updates:
            msg = (
                f"Github file (name={downloadable_content.name}) is too old. "
                f"\n File is {period_file_not_updated.days} days not updated, while allowed is "
                f"{self.allowed_period_no_updates.days} days: {downloadable_content.html_url}"
            )
            raise exceptions.GithubFileTooOldError(msg=msg)
        logger.info(
            f"github file ({downloadable_content.name}) is recent enough: not updated for "
