...
def test_file_too_old():
    ...
    with pytest.raises(exceptions.GithubFileTooOldError):
        GithubFileDownloader(repo_name=repo_name, target_file=target_file, allowed_period_no_updates=short_timedelta)
...
def test_file_works():
    ...
    content_file = downloader.downloadable_content[0]
    assert isinstance(content_file, ContentFile)
    assert content_file.name == target_file.name
...
def test_download_url():
    ...
    url = downloader.downloadable_content[0].download_url
