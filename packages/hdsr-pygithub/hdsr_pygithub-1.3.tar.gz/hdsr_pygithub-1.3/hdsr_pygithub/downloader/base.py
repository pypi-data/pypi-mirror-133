...
class GithubDownloaderBase:
    ...
    def _get_branch_absolute_paths(self, start_dir: Path) -> Tuple[List[Path], List[ContentFile]]:
        ...
        contents = self.repo_instance.get_contents(path=start_dir.as_posix(), ref=self.branch_name)
        while contents:
            content = contents.pop(0)
            if content.type == "dir":
                # add subdir content files
                dir_paths.append(Path(content.path))
    ...
    def download_files(self, use_tmp_dir: bool = False, download_directory: Path = None) -> Path:
        ...
        for content_file in self.downloadable_content:
