...
def test_dir_does_not_exist():
    ...
    with pytest.raises(exceptions.GithubDirNotFoundError):
        GithubDirDownloader(repo_name=repo_name, target_dir=target_dir)


def test_dir_works():
    repo_name = "startenddate"
    target_dir = Path("data/output/")
    downloader = GithubDirDownloader(repo_name=repo_name, target_dir=target_dir)
...
def test_target_dir():
    repo_name = "startenddate"
    _timedelta = timedelta(weeks=52 * 10)

    # find all files starting at root_dir
    dir1 = Path("data/input/")
    downloader1 = GithubDirDownloader(repo_name=repo_name, target_dir=dir1, allowed_period_no_updates=_timedelta)
    assert len(downloader1.downloadable_content) == 1

    dir2 = Path("data/output/results/")
    downloader2 = GithubDirDownloader(repo_name=repo_name, target_dir=dir2, allowed_period_no_updates=_timedelta)
    assert len(downloader2.downloadable_content) == 4
...
def test_dir_too_old():
    ...
    with pytest.raises(exceptions.GithubFileTooOldError):
        GithubDirDownloader(repo_name=repo_name, target_dir=target_dir, allowed_period_no_updates=short_timedelta)
...
def test_filter_extension_good():
    ...
    # only
    downloader = GithubDirDownloader(repo_name=repo_name, target_dir=target_dir, only_these_extensions=extensions)
    ...
    # exclude
    downloader = GithubDirDownloader(repo_name=repo_name, target_dir=target_dir, exclude_these_extensions=extensions)
    file_extensions_found = [Path(x.path).suffix for x in downloader.downloadable_content]
...
def test_filter_extension_wrong():
    ...
    with pytest.raises(AssertionError):
        ...
        GithubDirDownloader(repo_name=repo_name, target_dir=target_dir, only_these_extensions=wrong_extensions)

    with pytest.raises(AssertionError):
        wrong_extension = ".csv"
        GithubDirDownloader(repo_name=repo_name, target_dir=target_dir, only_these_extensions=wrong_extension)  # noqa


def test_filter_files_good():
    repo_name = "startenddate"
    target_dir = Path("data/output/results/")
    files = [Path("caw_oppervlaktewater_hymos_short.csv"), Path("caw_oppervlaktewater_long.csv")]

    # only
    downloader = GithubDirDownloader(repo_name=repo_name, target_dir=target_dir, only_these_files=files)
    assert len(downloader.downloadable_content) == 2
    assert sorted([Path(x.name) for x in downloader.downloadable_content]) == files

    # exclude
    downloader = GithubDirDownloader(repo_name=repo_name, target_dir=target_dir, exclude_these_files=files)
    assert len(downloader.downloadable_content) == 2
...
def test_download_wrong():
    ...
    with pytest.raises(TypeError):
        GithubDirDownloader(repo_name=repo_name, target_file=Path("doesnotmatter"))  # noqa

    # wrong key argument (should be target_file)
    with pytest.raises(TypeError):
        GithubFileDownloader(repo_name=repo_name, target_dir=Path("doesnotmatter"))  # noqa
...
def test_wis_config():
    github_downloader = GithubDirDownloader(
        ...
        branch_name="202002-prd",
        repo_organisation="hdsr-mid",
    )
    assert len(github_downloader.downloadable_content) > 400
