...
class AuthorizationMixin:
    ...
    def pat(username: str = None, password: str = None) -> Dict:
        ...
        response = httpx.post(config.uma2.token_endpoint, data=payload, headers=headers, verify=False)
    ...
    def ticket(self, resources: List = [], access_token: str = None) -> Dict:
        ...
        response = httpx.post(
            config.uma2.permission_endpoint, json=payload, headers=headers, verify=False
        )
        response.raise_for_status()
    ...
    def introspect(rpt: str) -> Dict:
        ...
        response = httpx.post(
            config.uma2.introspection_endpoint, data=payload, headers=headers, verify=False
        )
        response.raise_for_status()
