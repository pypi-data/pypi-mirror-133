...
class Base(object):
    ...
    def loadConfig(self, filePath, root=False):
        ...
        for subConfig in importedConfigsBefore:
            baseConfig = self.overwriteConfig(baseConfig, subConfig)

        self.logDebug("parse user config: %s" % filePath)
        if not isFullConfig:
            baseConfig = pdict({"config": baseConfig})

        importedConfigs = self.importConfigs("importAfter", baseConfig, filePath)
        for subConfig in importedConfigs:
            baseConfig = self.overwriteConfig(baseConfig, subConfig)
