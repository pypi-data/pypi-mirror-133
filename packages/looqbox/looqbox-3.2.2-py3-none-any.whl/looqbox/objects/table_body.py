...
class TableBody(LooqObject):
    ...
    def _collapse_table(self, data: list) -> list:
        ...
        while row > 0:
            if self._is_single_children(row):
                ...
                #used to "retake previous level"
                data = self._insert_children_previous_row(data, row)
                row -= 1
            else:
                row -= 1
        #remove duplicated rows from table root level
    ...
    def _generete_row_key(self, row: int) -> str:
        #Generete unique key for root rows
        import string
        import random
    ...
    def _stack_tree_rows(self, data: list, stack_row: int) -> dict:
        #stack rows with the same hirarchy level
        tree_rows = dict()
        ...
        while self.row_hierarchy[stack_row] == self.row_hierarchy[stack_row + 1]:
            ...
            stack_row -= 1
        stack_data.reverse()
        tree_rows["row"] = stack_row #last row with the same hirarchy
        tree_rows["data"] = stack_data #staced rows with the same hierachy level
        return tree_rows

    def _remove_collapsed_rows_from_root(self, data: list) -> list:
        for row in range(len(data)-1, -1, -1):
            if self.row_hierarchy[row] > 1:
                data.remove(data[row])
        return data

    def _previous_row_same_hierarchy(self, counter: int, row: int) -> int:
        #retrive the previous row with the same hierarchy within the same root
        next_row_hierarchy = 0
        while counter > 1:
            if self.row_hierarchy[counter - 1] < self.row_hierarchy[counter] and self.row_hierarchy[row] != \
                    self.row_hierarchy[counter]:# and self.row_hierarchy[counter] != 1:
                next_row_hierarchy = counter - 1
            elif self.row_hierarchy[counter] == 1:
    ...
    def _insert_single_children(self, data: list, row: int) -> list:
        if data[row - 1].get("tree_data") is None:
            ...
            data[row - 1]["tree_data"] = data[row - 1]["tree_data"]
        ...
        if data[row].get("tree_data") is None:
            ...
            data[row - 1]["tree_data"][-1]["key"] = self._generete_row_key(row)
        data[row - 1]["key"] = self._generete_row_key(row)
        return data
    ...
    def _insert_stacked_children(self, data: list, row: int) -> (int, list):
        ...
        row = stacked_data["row"]  # update row with the next level

        data[row]["key"] = self._generete_row_key(row)
        # Check if there is any row with the same hierarchy within the same root e.g. (1,2,3,2,2)
        counter = row+1#len(data) - 1
        same_hierarchy = self._previous_row_same_hierarchy(counter, row)

        if same_hierarchy == row:
            data[row]["tree_data"] = stacked_data["data"]
        else:
            data[same_hierarchy]["tree_data"] = stacked_data["data"]
        return row, data
    ...
    def _insert_children_previous_row(self, data: list, row: list) -> list:
        ...
        next_row_hierarchy = self._previous_row_same_hierarchy(counter, row)
        if data[row].get("tree_data") is not None:
            children_line["tree_data"] = data[row]["tree_data"]
        if data[next_row_hierarchy - 1].get("tree_data") is None:
            data[next_row_hierarchy - 1]["tree_data"] = [children_line]
        else:
            data[next_row_hierarchy - 1]["tree_data"].insert(0, children_line)
        return data
