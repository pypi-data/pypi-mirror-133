...
def _get_feature_definition_paths(feature_definition_paths: List[str]) -> List[str]:
    ...
    for path in feature_definition_paths:
        paths.extend(glob(path, recursive=True))
    return paths
...
def package(args: Namespace) -> None:
    ...
    generator.generate_package(
        package_name, features, out_dir, lock_controller=lock_controller, auth_features=auth_features
    )
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    package_parser.add_argument("-n", "--package-name", help="Package name")
