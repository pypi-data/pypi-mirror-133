...
class NearestPoint:
    ...
    def get_nearest(self, lat, lon):
        ...
        index = closest[1][0]
        return {
            f"{self.headers[0]}": self.points_dataframe.get(self.headers[0])[index],
            f"{self.headers[1]}": self.points_dataframe.get(self.headers[1])[index],
            f"{self.headers[2]}": self.points_dataframe.get(self.headers[2])[index],
            f"{self.distance_keyword}": closest[0][0],
        }
