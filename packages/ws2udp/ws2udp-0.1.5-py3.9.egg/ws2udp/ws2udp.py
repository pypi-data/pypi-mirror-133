...
async def run(udp_addr, websocket_addr, websocket_port):
    ...
    def send_broadcast(addr, *args):
        for client in clients:
            client.send_ws(json.dumps(args))
    dispatcher.map("/trig", send_broadcast)
    
    print(udp_addr)
    server = AsyncIOOSCUDPServer(udp_addr, dispatcher, asyncio.get_event_loop())
...
async def rrun(udp_addr, websocket_addr, websocket_port):
    dispatcher = Dispatcher()
    dispatcher.map("/trig", filter_handler)
...
def go():
    asyncio.run(run(("127.0.0.1", 57142), "0.0.0.0", 8765))
