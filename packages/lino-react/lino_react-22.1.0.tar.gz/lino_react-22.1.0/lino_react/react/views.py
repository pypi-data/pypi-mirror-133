            ...
            if display_mode == constants.DISPLAY_MODE_STORY:
                ...
                def serialize(ar, row):
                    ...
                    if display_mode == constants.DISPLAY_MODE_TABLE:
                        row_classes = None
                        if ar.actor.get_row_classes is not None and not isinstance(row, PhantomRow):
                            classes = ar.actor.get_row_classes(row, ar)
                            if isinstance(classes, GeneratorType):
                                row_classes = " ".join([klass for klass in classes])
                            else:
                                row_classes = classes
                        rowData = rh.store.row2list(ar, row)
                        rowData.append(row_classes)
                        return rowData
    ...
    def get(self, request):
        ...
        def getit():
            ...
            if (not settings.SITE.build_js_cache_on_startup
                ...
                settings.SITE.plugins.react.renderer.build_js_cache(False)
                separate_site_cache_root = False
            else:
                separate_site_cache_root = True
            user_settings = dict(
            ...
            if separate_site_cache_root:
                user_settings['separate_site_cache_root'] = True
