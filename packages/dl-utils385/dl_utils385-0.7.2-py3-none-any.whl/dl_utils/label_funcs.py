...
def translate_labellings(trans_from_labels,trans_to_labels,subsample_size='none',preserve_sizes=False):
    ...
    trans_dict, leftovers = get_trans_dict(trans_from_labels,trans_to_labels,subsample_size,preserve_sizes)
    return np.array([trans_dict[l] for l in trans_from_labels])

def get_trans_dict(trans_from_labels,trans_to_labels,subsample_size='none',preserve_sizes=False):
    ...
    if num_from_labs <= num_to_labs:
        ...
        trans_dict,leftovers = get_fanin_trans_dict(trans_from_labels,trans_to_labels,subsample_size,preserve_sizes)
...
def get_fanout_trans_dict(trans_from_labels,trans_to_labels,subsample_size):
    unique_trans_from_labels = [l for l in unique_labels(trans_from_labels) if l != -1]
    unique_trans_to_labels = [l for l in unique_labels(trans_to_labels) if l != -1]
    if subsample_size == 'none':
        cost_matrix = np.array([[label_assignment_cost(trans_from_labels,trans_to_labels,l1,l2) for l2 in unique_trans_to_labels] for l1 in unique_trans_from_labels])
    else:
        num_trys = 0
        while True:
            num_trys += 1
            ...
            if unique_labels(trans_from_labels_subsample) == set(unique_trans_from_labels) and unique_labels(trans_to_labels_subsample) == set(unique_trans_to_labels): break
        cost_matrix = np.array([[label_assignment_cost(trans_from_labels_subsample,trans_to_labels_subsample,l1,l2) for l2 in unique_trans_from_labels] for l1 in unique_trans_from_labels])
...
def get_fanin_trans_dict(trans_from_labels,trans_to_labels,subsample_size,preserve_sizes):
    unique_trans_from_labels = [l for l in unique_labels(trans_from_labels) if l != -1]
    unique_trans_to_labels = [l for l in unique_labels(trans_to_labels) if l != -1]
    if subsample_size == 'none':
        cost_matrix = np.array([[label_assignment_cost(trans_to_labels,trans_from_labels,l1,l2) for l2 in unique_trans_from_labels] for l1 in unique_trans_to_labels])
    else:
        while True: # Keep trying random indices unitl you reach one that contains all labels
            sample_indices = np.random.choice(range(trans_from_labels.shape[0]),subsample_size,replace=False)
            trans_from_labels_subsample = trans_from_labels[sample_indices]
            trans_to_labels_subsample = trans_to_labels[sample_indices]
            if [l for l in unique_labels(trans_from_labels_subsample) if l != -1] == unique_trans_from_labels and [l for l in unique_labels(trans_to_labels_subsample) if l != -1] == unique_trans_to_labels: break
        sample_indices = np.random.choice(range(trans_from_labels.shape[0]),subsample_size,replace=False)
        trans_from_labels_subsample = trans_from_labels[sample_indices]
        trans_to_labels_subsample = trans_to_labels[sample_indices]
        cost_matrix = np.array([[label_assignment_cost(trans_to_labels_subsample,trans_from_labels_subsample,l1,l2) for l2 in unique_trans_from_labels] for l1 in unique_trans_to_labels])
    row_ind, col_ind = linear_sum_assignment(cost_matrix)
    ...
    while True:
        ...
        cost_matrix2 = np.array([[label_assignment_cost(trans_from_labels,trans_to_labels,l1,l2) for l2 in unique_trans_to_labels] for l1 in unique_untranslated])
