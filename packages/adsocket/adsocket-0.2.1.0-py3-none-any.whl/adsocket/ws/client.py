...
class Client:
    ...
    async def message(self, msg: Message) -> None:
        try:
            ...
            _logger.debug(f"Client {self} seems to be dangling")
    ...
    async def disconnect(self):
        ...
        :rtype:
        """
        if self._ws and not self.ws.closed:
...
class ClientPool:
    ...
    def __init__(self, app):
        ...
        self._loop.create_task(self._report())
    ...
    def _client_disconnected(self, client: Client):
        asyncio.create_task(self._app['channels'].client_disconnected(client))
    ...
    async def append(self, client: Client):
        ...
        if self._app['settings'].DISCONNECT_UNAUTHENTICATED:
            asyncio.create_task(self._schedule_for_disconnect(client))
