...
class Parser(object):
    def __init__(self, grammar_filename):
        ...
        if exists(grammar_filename):
            ...
            try:
                ...
                raise GrammarNotFound("specified grammar could not be " "found")
        ...
        self._trigger_startswith = [
            (k, v["trigger_startswith"])
            for k, v in self.grammar.items()
            if "trigger_startswith" in v
        ]
        self._trigger_endswith = [
            (k, v["trigger_endswith"])
            for k, v in self.grammar.items()
            if "trigger_endswith" in v
        ]
        self._trigger_contains = [
            (k, v["trigger_contains"])
            for k, v in self.grammar.items()
            if "trigger_contains" in v
        ]
        self._trigger_re = [
            (k, re.compile(v["trigger_re"]))
            for k, v in self.grammar.items()
            if "trigger_re" in v
        ]
    ...
    def parse(self, filehandle):
        ...
        for line in filehandle:
            ...
            sw_matches = [
                (k, t)
                for k, t in self._trigger_startswith
                if any([l.startswith(t) for l in line_pp])
            ]
            ew_matches = [
                (k, t)
                for k, t in self._trigger_endswith
                if any([l.endswith(t) for l in line_pp])
            ]
            co_matches = [
                (k, t)
                for k, t in self._trigger_contains
                if any([l.find(t) >= 0 for l in line_pp])
            ]
            re_matches = [
                (k, r)
                for k, r in self._trigger_re
                if any([len(r.findall(l)) > 0 for l in line_pp])
            ]

            section_match = {
                k for k, _ in (sw_matches + ew_matches + co_matches + re_matches)
            }
            ...
            if len(section_match) == 1:
                ...
                if len(sw_matches) > 0:
                    _, t = sw_matches[0]
                    line_unmatched = [l[len(t) :] for l in line_pp if l.startswith(t)][
                        0
                    ]
                elif len(ew_matches) > 0:
                    _, t = ew_matches[0]
                    line_unmatched = [
                        l[: -(len(t) + 1)] for l in line_pp if l.endswith(t)
                    ][0]
                    ...
                    _, r = re_matches[0]
                    line_unmatched = [
                        r.sub("", l) for l in line_pp if len(r.findall(l)) > 0
                    ][0]
            ...
            if active_section.get(
                "use_unmatched", self._section_type not in DEFAULT_NOT_USE_UNMATCHED
            ):
            ...
            if section_type == "table":
                ...
                if section_name not in output:
                    ...
                    if type(colmap) == list:
                        ...
                        if has_header:
                            table_header = [
                                mn or rn for rn, (mn, _) in zip(row, colmap)
                            ]
