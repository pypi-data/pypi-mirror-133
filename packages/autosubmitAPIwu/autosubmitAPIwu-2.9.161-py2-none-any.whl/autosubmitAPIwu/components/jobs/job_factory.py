#!/usr/bin/env python
import collections
import autosubmitAPIwu.common.utils as util
...
class Job:
  ...
  def __init__(self):
    ...
    self._submit = None # type: str
    self._start = None # type: str
    self._finish = None # type: str
  ...
  @property
  def submit_ts(self):
    return util.tostamp(self.submit)
  
  @property
  def start_ts(self):
    return util.tostamp(self.start)
  
  @property
  def finish_ts(self):
    return util.tostamp(self.finish)
  ...
  def update_from_jobrow(self, jobrow):
    ...
    if jobrow:
      ...
      self._submit = jobrow.submit
      self._start = jobrow.start
      self._finish = jobrow.finish
  ...
  def from_old_job_data(cls, job_data):    
    ...
    job.queue_time = job_data.queuing_time()
    job.run_time = job_data.running_time()
