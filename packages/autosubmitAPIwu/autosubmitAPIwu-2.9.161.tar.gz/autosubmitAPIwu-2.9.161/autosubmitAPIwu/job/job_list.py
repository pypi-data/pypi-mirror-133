...
from autosubmitAPIwu.job.job_utils import SubJobManager, job_times_to_text, datechunk_to_year, calculate_SYPD_perjob, calculate_ASYPD_perjob
...
class JobList:
    ...
    def get_tree_structured_from_previous_run(expid, BasicConfig, run_id, chunk_unit=None, chunk_size=1):
        ...
        for job in job_list:
            ...
            nodes.append({'id': job.job_name,
                          ...
                          'date': ini_date,
                          'date_plus': end_date,
                          'SYPD': calculate_SYPD_perjob(chunk_unit, chunk_size, job.chunk, job.running_time() if job else 0),
                          'ASYPD': calculate_ASYPD_perjob(chunk_unit, chunk_size, job.chunk, job.running_time() + job.queuing_time(package_to_jobs_for_normalization.get(job.rowtype, None)) if job else 0, average_post_time),
    ...
    def get_tree_structured(self, BasicConfig, chunk_unit=None, chunk_size=1):
        ...
        for job in allJobs:
            ...
            nodes.append({'id': job.name,
                          ...
                          'date': ini_date,
                          'date_plus': end_date,
                          'SYPD': calculate_SYPD_perjob(chunk_unit, chunk_size, job.chunk, job_info.run_time if job_info else 0),
                          'ASYPD': calculate_ASYPD_perjob(chunk_unit, chunk_size, job.chunk, job_info.run_time + job_info.queue_time if job_info else 0, average_post_time),
    ...
    @staticmethod
    def get_job_times_collection(BasicConfig, allJobs, expid, job_to_package=None, package_to_jobs=None, timeseconds=True):
        """
        ...
        job_data, warning_messages = JobDataStructure(
            expid).get_total_job_data(allJobs, job_times)
        # Result variables
        ...
        for sub in Manager.get_subjoblist():
            current_job_info = job_name_to_job_info.get(
                sub.name, None)  # if sub.name in job_name_to_job_info.keys(
            # ) else None
            if current_job_info:
                ...
                job_running_to_runtext[sub.name] = job_times_to_text(
                    sub.queue, sub.run, sub.status)
