...
class JobListLoader(object):
  ...
  def __init__(self, expid):
    ...
    self.joblist_helper = JobListHelper(expid, self.pkl_organizer.get_simple_jobs(self.configuration_facade.tmp_path))        
    self._jobs = [] # type: List[Job]
    self._structure_adjacency = {} # type: Dict[str, List[str]]
    self._job_dictionary = {} # type: Dict[str, Job]

  def load_jobs(self):    
    """ Reads Pkl -> Makes StandardJobs -> Loads Adjacency -> Loads Packages -> Loads Q+R times -> Finds Logs -> Updates StandardJobs """
  ...
  def _update_job_logs(self):
    ...
    for job in self._jobs:
      if job.status in [Status.COMPLETED, Status.FAILED]:
          job.out_path_local = new_outs.get(job.name, 'NA')
          job.err_path_local = new_errs.get(job.name, 'NA')
