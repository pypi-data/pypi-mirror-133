...
class EmailOperator:
    ...
    async def send(cls, email: Email):
        template_loader = FileSystemLoader(searchpath=ConfigHub.get('email.templates'))
        template_env = Environment(loader=template_loader, enable_async=True)
        template = template_env.get_template(email.template)
        rendered_template = await template.render_async(email.message_data)
    ...
    async def create(cls, **params):
        email_model_path = ConfigHub.get('email.model')
        if not email_model_path:
            email_model = Email
        else:
            email_model = load_class(email_model_path)
