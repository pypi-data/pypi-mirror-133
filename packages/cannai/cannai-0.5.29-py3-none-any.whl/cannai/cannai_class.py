...
class Cmodel(Cannaibase):
    ...
    def avr_two_models(self, e_score,e_score_key=None, kaisu=50, addition_len=5, save_higher=False):
        if e_score_key == None:
        ...
        for round_ in range(15):
            for ii in range(kaisu):
                kari_weight = []
                kari_weightb = np.array([0.0 for qwe in range(sml_len)])
                sum_ka_wi = 0.0
                ...
                for ism in range(totyu_len):
                    ...
                    sss_k = model_list_additional_kari_weight2[ism] * kari_weight[ism]
                ...
                if len_mod_lak >= totyu_len:
                    ...
                    model_list_additional_kari_weight2 = np.delete(model_list_additional_kari_weight2, len_mod_lak, 0)
        ...
        print(type(model_list_additional_kari[ism][1]))
        print(model_list_additional_kari[ism][1])
        model_list_additional_kari = model_list_additional_kari[:addition_len]
        sml_len = len(model_list_additional_kari)
        smln = []
        for ism in range(sml_len):
            smln.append(model_list_additional_kari[ism][0])
            model_list_additional_kari[ism][1] = self.output_to_df(model_list_additional_kari[ism][1])
        self.model_name_list_additional = smln
        self.model_list_additional = copy.deepcopy(model_list_additional_kari)
