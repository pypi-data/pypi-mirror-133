...
class Cmodel(Cannaibase):
    ...
    def avr_two_models(self, e_score,e_score_key=None, kaisu=100, addition_len=5, save_higher=False):
        ...
        for ii in range(kaisu):
            ...
            model_list_additional_kari_weight.insert(jyuni, np.array(kari_weight))
        ...
        for round_ in range(5):
            for ii in range(kaisu):
                kari_weight = []
                kari_weightb = np.array([0.0 for qwe in range(len_mod_lak)])
                sum_ka_wi = 0.0
                ...
                for ism in range(totyu_len):
                    kari_weight[ism] /= ka_wi_av
                    print("model_list_additional_kari_weight2")
                    print(model_list_additional_kari_weight2)
                    print(kari_weight[ism])
                    kari_weightb +=  model_list_additional_kari_weight2 * kari_weight[ism]

                mod_val = all_mod_data[0] , kari_weightb[0]

                for ism in range(1, sml_len):
                    mod_val += all_mod_data[ism] * kari_weightb[ism]
                dat = self.set_date()
                if e_score_key == None:
                    e_out_l = self.get_line(mod_val, 0)
                else:
