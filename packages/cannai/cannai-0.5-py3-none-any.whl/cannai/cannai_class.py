...
#get model and model_name
    ...
    def get_data(self, key):
        key2,addtional_model = self.modelname_to_num(key)
        if addtional_model:
...
class Cmodel(Cannaibase):
    ...
    def avr_two_models(self, e_score,e_score_key=None, kaisu=100, addition_len=5, save_higher=False):
        if e_score_key == None:
            e_ans_l = self.answer.iloc[:,0]
        else:
            e_ans_l = self.answer.loc[:, e_score_key]
        ...
        totyu_len = addition_len * 4
        for ii in range(kaisu):
            kari_weight = []
            sum_ka_wi = 0.0
            for ism in range(sml_len):
            ...
            if e_score_key == None:
                e_out_l = mod_val.iloc[:, 0]
            else:
                e_out_l = mod_val.loc[:, e_score_key]
            comv_score = self.Cal_s.cal_score_single_sub(e_score,e_out_l, e_ans_l)
            len_mod_lak = len(model_list_additional_kari)
            new_mod_group = ["combine0_" + str(ii), mod_val, dat, "No_model", None, None, None]
            jyuni = 0
            if len_mod_lak > 0:
            ...
            if len_mod_lak >= totyu_len:
                model_list_additional_kari.pop(len_mod_lak)
                model_list_additional_kari_score.pop(len_mod_lak)
                model_list_additional_kari_weight.pop(len_mod_lak)
                model_list_additional_kari_group.pop(len_mod_lak)
        ...
        for round_ in range(3):
            for ii in range(kaisu):
                kari_weight = []
                kari_weightb = [0.0 for qwe in range(len_mod_lak)]
                sum_ka_wi = 0.0
                for ism in range(totyu_len):
                    kka_wi = random.uniform(-0.3, 1.5)
                    kari_weight.append(kka_wi)
                    sum_ka_wi += kka_wi
                ...
                for ism in range(totyu_len):
                    kari_weight[ism] /= ka_wi_av
                    kari_weightb += kari_weight[ism] * model_list_additional_kari_weight2
                mod_val = model_list_additional_kari[0] * kari_weightb[0]
                kari_weight0 = model_list_additional_kari_weight2[0] * kari_weightb[0]
                for ism in range(1, sml_len):
                    mod_val += model_list_additional_kari_weight2[ism] * kari_weightb[ism]
                    kari_weight0 += model_list_additional_kari_weight2[ism] * kari_weightb[ism]
                dat = self.set_date()
                if e_score_key == None:
                    e_out_l = mod_val.iloc[:, 0]
                else:
                    e_out_l = mod_val.loc[:, e_score_key]
                comv_score = self.Cal_s.cal_score_single_sub(e_score, e_out_l, e_ans_l)
                new_mod_group = ["combine_" + str(ii + round_ * kaisu), mod_val, dat, "No_model", None, None, None]
                len_mod_lak = len(model_list_additional_kari2)

                jyuni = 0
                if len_mod_lak > 0:
                    for iij in range(len_mod_lak):
                        if ((model_list_additional_kari_score2[iij] - comv_score) * jyousan) > 0:
                            break
                        else:
                            jyuni += 1
                ...
                model_list_additional_kari_weight2.insert(jyuni, kari_weightb)
                model_list_additional_kari_group2.insert(jyuni, new_mod_group)

                if len_mod_lak >= totyu_len:
                    model_list_additional_kari2.pop(len_mod_lak)
                    model_list_additional_kari_score2.pop(len_mod_lak)
                    model_list_additional_kari_weight2.pop(len_mod_lak)
                    model_list_additional_kari_group2.pop(len_mod_lak)
        ...
        smln = []
        for ism in range(1, sml_len):   smln.append(model_list_additional_kari[ism][0])
        self.model_list_additional = smln
        self.model_name_list_additional = copy.deepcopy(model_list_additional_kari)
