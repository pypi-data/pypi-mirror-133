...
class Cal_trained_score:
    ...
    def combine_inout(self,key,addtional_model): #if key is -1, throw true value
    ...
    def get_inout(self, key, target_line,addtional_model):
        out_df = self.parent.get_data(key)[1]
    ...
    def get_roc(self,key,target_line,addtional_model):
        out_l, ans_l = self.get_inout(key, target_line,addtional_model)
        return self.get_roc_base(out_l, ans_l)

    def get_roc_base(self,out_l, ans_l):
        return sk_met.roc_curve(ans_l,out_l)

    def cal_score_single(self, key, target_line, e_score0,addtional_model):
        ...
        self.cal_score_single_sub(e_score,out_l,ans_l)

    def cal_score_single_sub(self, e_score, out_l, ans_l):
    ...
    def cal_score_multiple(self, key_list, target_line, e_score,addtional_model):
        self.ans_checked = False
        score_list = []
        for key in key_list: score_list.append(self.cal_score_single(key, target_line, e_score,addtional_model))
        return score_list

    def cal_score_multiple_2d(self, key_list, target_line, e_score_list,addtional_model):
        # score[e_score][model]
        self.ans_checked = False
        score_list = []
        for e_score in e_score_list:
            score_list0 = []
            for key in key_list: score_list0.append(self.cal_score_single(key, target_line, e_score,addtional_model))
            score_list.append(score_list0)
        return score_list

    def cal_classtering(self, key, target_line,addtional_model):
        out_l0, ans_l = self.get_inout(key, target_line,addtional_model)
        out_l = out_l0.copy()
