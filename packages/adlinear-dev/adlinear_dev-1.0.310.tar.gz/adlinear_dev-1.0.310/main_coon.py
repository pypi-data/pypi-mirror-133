...
if __name__ == "__main__":
    ...
    parser.add_argument("-m", "--ncmin", type=float, default=2, help="Minimum number of components")
    parser.add_argument("-M", "--ncmax", type=float, default=45, help="Maximum number of components")
    ...
    for itrial in range(nruns):
        ...
        if itrial % writefreq == 0:
            print(f"Saving iteration {itrial}")
            assert not df_mini_scree_plots.empty
            output.to_csv(df_mini_scree_plots)
