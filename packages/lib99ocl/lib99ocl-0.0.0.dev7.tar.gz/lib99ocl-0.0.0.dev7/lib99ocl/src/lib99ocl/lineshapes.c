...
WITHIN_KERNEL ftype gaussian_integral(
...
WITHIN_KERNEL ftype power_law_integral(
const ftype xL, const ftype xU, const ftype alpha, const ftype n )
{
  bool useLog = false;
  if(fabs(n - 1.0) < 1.0e-14)  useLog = true;
...
WITHIN_KERNEL ftype double_crystal_ball(
const ftype x, const ftype mu, const ftype sigma, const ftype aL, 
...
const ftype aL, const ftype nL, const ftype aR, const ftype nR )
{
  ...
  ans += power_law_integral(max(-tU, fabs(aL)), max(-tL, fabs(aL)), aL, nL);
  ans += power_law_integral(max( tL, fabs(aR)), max( tU, fabs(aR)), aR, nR);
  // ans += gaussian_integral(max( tL,-fabs(a1)), min( tU, fabs(a2)), 0., 1.);
  ans += gaussian_integral(max( tL,-fabs(aL)), min( tU, fabs(aR)), 0., 1.);
...
WITHIN_KERNEL ftype amoroso(
const ftype x, const ftype a, const ftype theta, const ftype alpha, 
const ftype beta )
{
...
WITHIN_KERNEL ftype amoroso_integral(
const ftype x, const ftype a, const ftype theta, const ftype alpha, 
const ftype beta )
{
  const ftype d = (x - a) / theta;
...
WITHIN_KERNEL ftype johnson_su(
const ftype x, const ftype mu, const ftype sigma, const ftype gamma,
const ftype delta )
{
