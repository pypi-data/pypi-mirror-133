...
def update_class_grade():
    today = datetime.date.today().isoformat()
    grade_graduate = models.Grade.objects.get(number=choices.GRADE_GRADUATE)
    grade_pending = models.Grade.objects.get(number=choices.GRADE_PENDING)
    models.Class.objects.filter(entrance_session__end_date__lt=today).update(
        stage=choices.STAGE_GRADUATED,
        grade=grade_graduate
    )
    models.Class.objects.filter(entrance_session__begin_date__gt=today).update(
        stage=choices.STAGE_PENDING,
        grade=grade_pending
    )
    models.Class.objects.filter(
        entrance_session__begin_date__lte=today,
        entrance_session__end_date__gt=today).update(
        stage=choices.STAGE_STUDYING
    )
...
def get_similar_session_values_by_name(name):
    year = grade_name_to_number(name)
    d = {}
    s = models.Session.objects.filter(name__endswith=name[4:]).first()
    if s:
        year2 = grade_name_to_number(s.name)
        name2 = s.name.replace(str(year2), str(year))
        if name2 == name:
            d['number'] = s.number.replace(str(year2), str(year))
            d['begin_date'] = add_year(s.begin_date, year - year2)
            d['end_date'] = add_year(s.end_date, year - year2)
    return d
...
def init_student(user, profile):
    ...
    session_name = profile.get('届别')
    if session_name:
        session, created = models.Session.objects.get_or_create(name=session_name)
    else:
        session_number = grade_name_to_number(grade_name)
        session, created = models.Session.objects.get_or_create(number=session_number)

    college_name = profile.get('院系')
    if college_name:
        college, created = models.College.objects.get_or_create(name=college_name)
    else:
        college = None
    ...
    with transaction.atomic():
        ...
        if hasattr(old_user, 'as_school_student'):
            ...
            if old_student.is_formal:
                ...
                old_student.classes = []
                old_student.delete()
                to_add_event.send_robust(sender=old_user._meta.model, instance=old_user, name='unbind')
        student.save()
