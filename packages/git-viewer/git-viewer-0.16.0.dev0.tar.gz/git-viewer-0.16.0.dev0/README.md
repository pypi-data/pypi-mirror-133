...
See `gitl --help` for the supported command line arguments.
...
[gitd](diff.py) is a wrapper around `git diff` using gitl's details view.


You can use it for example to:

- show unstaged changes `gitd`
- show staged changes `gitd --cached`
- compare two branches `gitd master origin/otherbranch`
...
[gits](show.py) is a wrapper around `git show` using gitl's details view.


You can use it for example in your vimrc to open the commit under the cursor, see [vim integration](#vim-integration).
