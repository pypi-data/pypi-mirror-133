...
For more information see `git log --help` and `git diff --help`.
"""
...
class LogView(SelectableListbox):
	...
	def _focus_by_key(self, key):
		if key == settings.LOG_FOCUS_FIRST_LINE:
			...
			self.show_error(_("INTERNAL ERROR: focus function %r unknown") % key)
			return False
...
class App():
	...
	def export_config(self, fn, *, settings=True, keybindings=True, overwrite=False, default_path=False):
		...
		with open(fn, 'wt') as f:
			...
			if settings:
				for ln in self.iter_setting_lines():
					write(ln)
				write("")
	...
	def iter_setting_lines(self):
		for key in settings.keys():
			yield self.format_export_setting_line(key)

	def format_export_setting_line(self, key):
	...
	def open_config(self, fn, *, ignore_existing=True, default_path=False):
		...
		def do():
			model.open_file_in_editor(fn, line_number=None)
...
class OverlayPressedKeys(urwid.Overlay):
	...
	class BgContainer(urwid.delegate_to_widget_mixin('_original_widget'), urwid.WidgetDecoration):
		...
		def render(self, size, focus=False):
			return self.original_widget.render(size, self.master.is_focused)
...
def run():
	...
	config_file = get_arg_with_one_argument(args, "--config")
	cmdpipe = get_arg_with_one_argument(args, "--command-pipe")
	...
	a = App(show_unreachable=unreachable, log_args=log_args, unreachable_args=unreachable_args, diff_args=diff_args, config_file=config_file, command_pipe=cmdpipe, open_always=open_always)
