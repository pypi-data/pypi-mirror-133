...
class tJobInst(mettle.io.ISerializable, mettle.db.IPrimaryKey):
    ...
    def __repr__(self) -> str:
        return '<JobInst [id:%r, parent_id:%r, job_id:%r, batchinst_id:%r, process_date:%s, priority:%r, status:%.32s, extra_args:%.32s, group_job:%.32s, group_batch:%.32s, stamp_by:%.32s, stamp_tm:%s]>' % (
            self.id, self.parent_id, self.job_id, self.batchinst_id, self.process_date, self.priority, self.status, self.extra_args, self.group_job, self.group_batch, self.stamp_by, self.stamp_tm)
    ...
    class List(list, mettle.io.ISerializable):
        ...
        def __repr__(self) -> str:
            return '<tJobInst.List[%d]>' % (len(self))
