...
class iBatchByName(mettle.io.ISerializable):
    ...
    def _copy_from(self, rec: "iBatchByName"):
        """
    ...
    def _get_davs(self, dvc=None):
        iBatchByName._cache_davs(dvc)

    def __repr__(self) -> str:
        return '<Batch [name:%.32s]>' % (
            self.name)
    ...
    class List(list, mettle.io.ISerializable):
        ...
        def __repr__(self) -> str:
            return '<iBatchByName.List[%d]>' % (len(self))

        def _get_davs(self, dvc=None):
            return iBatchByName._cache_davs(dvc)
