# This file was generated by mettle.genes.braze.GenPy3 [ver 2.1] on Tue Aug 24 15:21:31 2021
#
...
class MonitorAsyncServerMarshaler(IServerMarshaler, MonitorAsyncServerInterface):
    ...
    async def _serve_async(self, server: Server, remote_sig: "str|int", in_stream: IStream, impl_data: dict = None):
        ...
        try:
            ...
            if remote_sig == 'batchCreate':
                out_stream = await self.batch_create(in_stream)
            elif remote_sig == 'batchRead':
                out_stream = await self.batch_read(in_stream)
            elif remote_sig == 'batchUpdate':
                out_stream = await self.batch_update(in_stream)
            elif remote_sig == 'batchDelete':
                out_stream = await self.batch_delete(in_stream)
            elif remote_sig == 'batchList':
                out_stream = await self.batch_list(in_stream)
            elif remote_sig == 'batchLastestInstList':
                out_stream = await self.batch_lastest_inst_list(in_stream)
            elif remote_sig == 'batchinstRead':
                out_stream = await self.batchinst_read(in_stream)
            elif remote_sig == 'batchinstList':
                out_stream = await self.batchinst_list(in_stream)
            elif remote_sig == 'batchitemCreate':
                out_stream = await self.batchitem_create(in_stream)
            elif remote_sig == 'batchitemRead':
                out_stream = await self.batchitem_read(in_stream)
            elif remote_sig == 'batchitemUpdate':
                out_stream = await self.batchitem_update(in_stream)
            elif remote_sig == 'batchitemShift':
                out_stream = await self.batchitem_shift(in_stream)
            elif remote_sig == 'batchitemDelete':
                out_stream = await self.batchitem_delete(in_stream)
            elif remote_sig == 'batchitemList':
                out_stream = await self.batchitem_list(in_stream)
            elif remote_sig == 'jobCreate':
                out_stream = await self.job_create(in_stream)
            elif remote_sig == 'jobRead':
                out_stream = await self.job_read(in_stream)
            elif remote_sig == 'jobUpdate':
                out_stream = await self.job_update(in_stream)
            elif remote_sig == 'jobDelete':
                out_stream = await self.job_delete(in_stream)
            elif remote_sig == 'jobList':
                out_stream = await self.job_list(in_stream)
            elif remote_sig == 'jobSchedule':
                out_stream = await self.job_schedule(in_stream)
            elif remote_sig == 'jobinstRead':
                out_stream = await self.jobinst_read(in_stream)
            elif remote_sig == 'jobinstRerun':
                out_stream = await self.jobinst_rerun(in_stream)
            elif remote_sig == 'jobinstForceOk':
                out_stream = await self.jobinst_force_ok(in_stream)
            elif remote_sig == 'jobinstStop':
                out_stream = await self.jobinst_stop(in_stream)
            elif remote_sig == 'jobinstList':
                out_stream = await self.jobinst_list(in_stream)
            elif remote_sig == 'jobinstMetrics':
                out_stream = await self.jobinst_metrics(in_stream)
            elif remote_sig == 'mondateCreate':
                out_stream = await self.mondate_create(in_stream)
            elif remote_sig == 'mondateRead':
                out_stream = await self.mondate_read(in_stream)
            elif remote_sig == 'mondateUpdate':
                out_stream = await self.mondate_update(in_stream)
            elif remote_sig == 'mondateDelete':
                out_stream = await self.mondate_delete(in_stream)
            elif remote_sig == 'mondateList':
                out_stream = await self.mondate_list(in_stream)
            elif remote_sig == 'mondateIncrement':
                out_stream = await self.mondate_increment(in_stream)
            elif remote_sig == 'mondateSetValue':
                out_stream = await self.mondate_set_value(in_stream)
            elif remote_sig == 'purgeHistory':
                out_stream = await self.purge_history(in_stream)
            elif remote_sig == 'serverDateTime':
                out_stream = await self.server_date_time(in_stream)
            else:
    ...
    async def batch_create(self, _i):
        ...
        tok = await self._server.auth_async("batchCreate", "monitor.batch.create", {"rec":rec})
        self._simpl._set_rpc_token_data(tok)

        _x = await self._simpl.batch_create(rec)
    ...
    async def batch_read(self, _i):
        ...
        tok = await self._server.auth_async("batchRead", "monitor.batch.read", {"batchId":batch_id, "batchName":batch_name})
        self._simpl._set_rpc_token_data(tok)
        ...
        _x = await self._simpl.batch_read(batch_id,
                                          batch_name)
    ...
    async def batch_update(self, _i):
        ...
        tok = await self._server.auth_async("batchUpdate", "monitor.batch.update", {"rec":rec})
        self._simpl._set_rpc_token_data(tok)

        _x = await self._simpl.batch_update(rec)
    ...
    async def batch_delete(self, _i):
        ...
        tok = await self._server.auth_async("batchDelete", "monitor.batch.delete", {"batchId":batch_id})
        self._simpl._set_rpc_token_data(tok)

        await self._simpl.batch_delete(batch_id)

        self._server.logger().debug("[batch_delete - Done]")

        return _o

    async def batch_list(self, _i):
        ...
        tok = await self._server.auth_async("batchList", "monitor.batch.read", {"lazyLoad":lazy_load, "query":query})
        self._simpl._set_rpc_token_data(tok)

        _x = await self._simpl.batch_list(lazy_load,
                                          query)
    ...
    async def batch_lastest_inst_list(self, _i):
        ...
        tok = await self._server.auth_async("batchLastestInstList", "monitor.batch.read", {"lazyLoad":lazy_load, "query":query})
        self._simpl._set_rpc_token_data(tok)

        _x = await self._simpl.batch_lastest_inst_list(lazy_load,
                                                       query)
    ...
    async def batchinst_read(self, _i):
        ...
        tok = await self._server.auth_async("batchinstRead", "monitor.batch.read", {"batchinstId":batchinst_id})
        self._simpl._set_rpc_token_data(tok)

        _x = await self._simpl.batchinst_read(batchinst_id)
    ...
    async def batchinst_list(self, _i):
        ...
        tok = await self._server.auth_async("batchinstList", "monitor.batch.read", {"lazyLoad":lazy_load, "query":query})
        self._simpl._set_rpc_token_data(tok)

        _x = await self._simpl.batchinst_list(lazy_load,
                                              query)
    ...
    async def batchitem_create(self, _i):
        ...
        tok = await self._server.auth_async("batchitemCreate", "monitor.batch.create", {"rec":rec})
        self._simpl._set_rpc_token_data(tok)

        _x = await self._simpl.batchitem_create(rec)
    ...
    async def batchitem_read(self, _i):
        ...
        tok = await self._server.auth_async("batchitemRead", "monitor.batch.read", {"batchitemId":batchitem_id, "batchId":batch_id})
        self._simpl._set_rpc_token_data(tok)

        _x = await self._simpl.batchitem_read(batchitem_id,
                                              batch_id)
    ...
    async def batchitem_update(self, _i):
        ...
        tok = await self._server.auth_async("batchitemUpdate", "monitor.batch.update", {"rec":rec})
        self._simpl._set_rpc_token_data(tok)

        _x = await self._simpl.batchitem_update(rec)
    ...
    async def batchitem_shift(self, _i):
        ...
        tok = await self._server.auth_async("batchitemShift", "monitor.batch.update", {"batchitemId":batchitem_id, "batchId":batch_id, "shiftIdx":shift_idx})
        self._simpl._set_rpc_token_data(tok)

        _x = await self._simpl.batchitem_shift(batchitem_id,
                                               batch_id,
                                               shift_idx)
    ...
    async def batchitem_delete(self, _i):
        ...
        tok = await self._server.auth_async("batchitemDelete", "monitor.batch.delete", {"batchitemId":batchitem_id, "batchId":batch_id})
        self._simpl._set_rpc_token_data(tok)

        await self._simpl.batchitem_delete(batchitem_id,
                                           batch_id)
    ...
    async def batchitem_list(self, _i):
        ...
        tok = await self._server.auth_async("batchitemList", "monitor.batch.read", {"batchId":batch_id})
        self._simpl._set_rpc_token_data(tok)

        _x = await self._simpl.batchitem_list(batch_id)
    ...
    async def job_create(self, _i):
        ...
        tok = await self._server.auth_async("jobCreate", "monitor.job.create", {"rec":rec})
        self._simpl._set_rpc_token_data(tok)

        _x = await self._simpl.job_create(rec)
    ...
    async def job_read(self, _i):
        ...
        tok = await self._server.auth_async("jobRead", "monitor.job.read", {"jobId":job_id, "jobName":job_name})
        self._simpl._set_rpc_token_data(tok)
        ...
        _x = await self._simpl.job_read(job_id,
                                        job_name)
    ...
    async def job_update(self, _i):
        ...
        tok = await self._server.auth_async("jobUpdate", "monitor.job.update", {"rec":rec})
        self._simpl._set_rpc_token_data(tok)

        _x = await self._simpl.job_update(rec)
    ...
    async def job_delete(self, _i):
        ...
        tok = await self._server.auth_async("jobDelete", "monitor.job.delete", {"jobId":job_id})
        self._simpl._set_rpc_token_data(tok)

        await self._simpl.job_delete(job_id)

        self._server.logger().debug("[job_delete - Done]")

        return _o

    async def job_list(self, _i):
        ...
        tok = await self._server.auth_async("jobList", "monitor.job.read", {"lazyLoad":lazy_load, "wcJobName":wc_job_name, "wcGroup":wc_group})
        self._simpl._set_rpc_token_data(tok)
        ...
        _x = await self._simpl.job_list(lazy_load,
                                        wc_job_name,
                                        wc_group)
    ...
    async def job_schedule(self, _i):
        ...
        tok = await self._server.auth_async("jobSchedule", "monitor.job.run", {"jobId":job_id, "jobName":job_name, "runDate":run_date, "extraArgs":extra_args, "priority":priority, "parentId":parent_id})
        self._simpl._set_rpc_token_data(tok)
        ...
        _x = await self._simpl.job_schedule(job_id,
                                            job_name,
                                            run_date,
                                            extra_args,
                                            priority,
    ...
    async def jobinst_read(self, _i):
        ...
        tok = await self._server.auth_async("jobinstRead", "monitor.jobinst.read", {"jobinstId":jobinst_id})
        self._simpl._set_rpc_token_data(tok)

        _x = await self._simpl.jobinst_read(jobinst_id)
    ...
    async def jobinst_rerun(self, _i):
        ...
        tok = await self._server.auth_async("jobinstRerun", "monitor.jobinst.update", {"jobinstId":jobinst_id, "forceRerun":force_rerun})
        self._simpl._set_rpc_token_data(tok)

        _x = await self._simpl.jobinst_rerun(jobinst_id,
                                             force_rerun)
    ...
    async def jobinst_force_ok(self, _i):
        ...
        tok = await self._server.auth_async("jobinstForceOk", "monitor.jobinst.update", {"jobinstId":jobinst_id, "reason":reason})
        self._simpl._set_rpc_token_data(tok)
        ...
        _x = await self._simpl.jobinst_force_ok(jobinst_id,
                                                reason)
    ...
    async def jobinst_stop(self, _i):
        ...
        tok = await self._server.auth_async("jobinstStop", "monitor.jobinst.update", {"jobinstId":jobinst_id, "reason":reason})
        self._simpl._set_rpc_token_data(tok)
        ...
        await self._simpl.jobinst_stop(jobinst_id,
                                       reason)
    ...
    async def jobinst_list(self, _i):
        ...
        tok = await self._server.auth_async("jobinstList", "monitor.jobinst.read", {"lazyLoad":lazy_load, "query":query})
        self._simpl._set_rpc_token_data(tok)

        _x = await self._simpl.jobinst_list(lazy_load,
                                            query)
    ...
    async def jobinst_metrics(self, _i):
        ...
        tok = await self._server.auth_async("jobinstMetrics", "monitor.jobinst.read", {"lazyLoad":lazy_load, "jobinstId":jobinst_id})
        self._simpl._set_rpc_token_data(tok)

        _x = await self._simpl.jobinst_metrics(lazy_load,
                                               jobinst_id)
    ...
    async def mondate_create(self, _i):
        ...
        tok = await self._server.auth_async("mondateCreate", "monitor.mondate.create", {"rec":rec})
        self._simpl._set_rpc_token_data(tok)

        _x = await self._simpl.mondate_create(rec)
    ...
    async def mondate_read(self, _i):
        ...
        tok = await self._server.auth_async("mondateRead", "monitor.mondate.read", {"mondateId":mondate_id})
        self._simpl._set_rpc_token_data(tok)
        ...
        _x = await self._simpl.mondate_read(mondate_id)
    ...
    async def mondate_update(self, _i):
        ...
        tok = await self._server.auth_async("mondateUpdate", "monitor.mondate.update", {"rec":rec})
        self._simpl._set_rpc_token_data(tok)

        _x = await self._simpl.mondate_update(rec)
    ...
    async def mondate_delete(self, _i):
        ...
        tok = await self._server.auth_async("mondateDelete", "monitor.mondate.delete", {"mondateId":mondate_id})
        self._simpl._set_rpc_token_data(tok)
        ...
        await self._simpl.mondate_delete(mondate_id)

        self._server.logger().debug("[mondate_delete - Done]")

        return _o

    async def mondate_list(self, _i):
        ...
        tok = await self._server.auth_async("mondateList", "monitor.mondate.read", {"lazyLoad":lazy_load, "wcMondateId":wc_mondate_id})
        self._simpl._set_rpc_token_data(tok)
        ...
        _x = await self._simpl.mondate_list(lazy_load,
                                            wc_mondate_id)
    ...
    async def mondate_increment(self, _i):
        ...
        tok = await self._server.auth_async("mondateIncrement", "monitor.mondate.update", {"mondateId":mondate_id, "daysValue":days_value})
        self._simpl._set_rpc_token_data(tok)
        ...
        _x = await self._simpl.mondate_increment(mondate_id,
                                                 days_value)
    ...
    async def mondate_set_value(self, _i):
        ...
        tok = await self._server.auth_async("mondateSetValue", "monitor.mondate.update", {"mondateId":mondate_id, "value":value})
        self._simpl._set_rpc_token_data(tok)
        ...
        _x = await self._simpl.mondate_set_value(mondate_id,
                                                 value)
    ...
    async def purge_history(self, _i):
        ...
        tok = await self._server.auth_async("purgeHistory", "monitor.purge", {"fromDate":from_date})
        self._simpl._set_rpc_token_data(tok)

        await self._simpl.purge_history(from_date)

        self._server.logger().debug("[purge_history - Done]")

        return _o

    async def server_date_time(self, _i):
        ...
        tok = await self._server.auth_async("serverDateTime", "__LOGGED_IN__", {})
        self._simpl._set_rpc_token_data(tok)

        _x = await self._simpl.server_date_time()
