...
class iBatchInstSearch(mettle.io.ISerializable):
    ...
    def _copy_from(self, rec: "iBatchInstSearch"):
        """
    ...
    def _get_davs(self, dvc=None):
        iBatchInstSearch._cache_davs(dvc)

    def __repr__(self) -> str:
        return '<BatchInst [date_from:%s, date_to:%s, criteria:%.32s, limoff:%.32s]>' % (
            self.date_from, self.date_to, self.criteria, self.limoff)
    ...
    class List(list, mettle.io.ISerializable):
        ...
        def __repr__(self) -> str:
            return '<iBatchInstSearch.List[%d]>' % (len(self))

        def _get_davs(self, dvc=None):
            return iBatchInstSearch._cache_davs(dvc)
