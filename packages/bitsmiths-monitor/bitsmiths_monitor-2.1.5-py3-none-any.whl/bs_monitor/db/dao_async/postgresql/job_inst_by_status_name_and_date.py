...
from bs_monitor.db.tables.ijob_inst_by_status_name_and_date import iJobInstByStatusNameAndDate
from bs_monitor.db.tables.job_inst import tJobInst

class dJobInstByStatusNameAndDate:
    ...
    def __init__(self, dbcon: mettle.db.IAConnect):
        ...
        self._dbcon   = dbcon
        self._dbstmnt = None
        self.irec     = iJobInstByStatusNameAndDate()
        self.orec     = tJobInst()
    ...
    async def exec_deft(self,
                        ...
                        name: str,
                        status: str) -> "dJobInstByStatusNameAndDate":
        """
    ...
    async def exec(self, irec: iJobInstByStatusNameAndDate = None) -> "dJobInstByStatusNameAndDate":
        """
        ...
        self._dbstmnt = await self._dbcon.statement("JobInstByStatusNameAndDate", self._dbcon.STMNT_TYPE_READ)
...
where
  ...
  i.job_id        = j.id    and
  i.status        = :status and
  i.process_date <= :processDate
order by
  i.process_date""")

        self._dbstmnt.bind_in("processDate", self.irec.process_date, datetime.datetime)
    ...
    async def fetch_all(self, outList: tJobInst.List, clearList: bool = True) -> int:
        """
