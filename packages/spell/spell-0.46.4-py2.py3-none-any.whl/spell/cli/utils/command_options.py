...
def dependency_params(include_docker=True, include_conda_file=True, resource_type="run"):
...
def description_param(resource_type="run"):
    def wrapper(f):
        ...
        @click.option(
            ...
            "--description",
            default=None,
            help="Description of the {}. If unspecified defaults to the current commit message".format(resource_type),
        )
        @wraps(f)
