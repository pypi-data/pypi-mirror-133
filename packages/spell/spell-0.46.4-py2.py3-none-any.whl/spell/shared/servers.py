...
# TODO(Justin): In process of deprecation. pip_strings and pip_env_strings should be
# renamed pip and pip_env after 0.47.0
def create_environment(python_env_deps, pip_packages, requirements_file, apt_packages, envvars):
    python_deps = merge_dependencies(python_env_deps, None, requirements_file, pip_packages).to_payload()
    return Environment(
        pip_env_strings=python_deps["pip_env"],
        requirements_file=python_deps["requirements_file"],
        pip_strings=python_deps["pip"],
        conda_file=None,
