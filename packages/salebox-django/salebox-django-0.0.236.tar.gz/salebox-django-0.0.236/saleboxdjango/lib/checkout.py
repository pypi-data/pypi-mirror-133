...
class SaleboxCheckout:
    ...
    def get_checkout_nav(self, curr_page_name=None):
        nav = {"order": [], "lookup": {}, "previous": None, "next": None}

        for pos, s in enumerate(self.sequence["order"]):
            accessible = (
                curr_page_name is not None and self.sequence["lookup"][s]["accessible"]
            )
        ...
        for pos, n in enumerate(nav["order"]):
            try:
                nav["order"][pos]["previous_accessible"] = nav["order"][pos - 1][
                    "accessible"
                ]
            except:
                nav["order"][pos]["previous_accessible"] = False
            try:
                nav["order"][pos]["next_accessible"] = nav["order"][pos + 1][
                    "accessible"
                ]
            except:
    ...
    def get_next_page(self, page_name):
        ...
        if next < len(self.sequence["order"]):
            return self.sequence["lookup"][self.sequence["order"][next]]["path"]
        else:
            return None
    ...
    def save_to_store(
        self, user, gateway_code="default", sent_to_gateway=True, visible_id=None
    ):
    ...
    def set_invoice_address(self, required, address_or_id, meta, request):
        if address_or_id is None:
            ...
            if isinstance(address_or_id, int):
                self.data["invoice_address"]["address"] = model_to_dict(
                    UserAddress.objects.get(id=address_or_id)
                )
                ...
                self.data["invoice_address"]["address"] = {
                    ...
                    "country_state": model_to_dict(address_or_id.country_state)
                    if address_or_id.country_state
                    else None,
                    "country": model_to_dict(address_or_id.country)
                    if address_or_id.country
                    else None,
                    "id": None,
    ...
    def set_message(self, key, value, request):
        self.data['messages'][key] = value
        self._write_session(request)

    def set_payment_method(self, id, meta, request):
        self.data["payment_method"] = {"id": int(id), "meta": meta}
        self._write_session(request)

    def set_shipping_address(self, required, address_or_id, meta, request):
        if isinstance(address_or_id, int):
            self.data["shipping_address"]["address"] = model_to_dict(
                UserAddress.objects.get(id=address_or_id)
            )
    ...
    def _update_sequence(self):
        for i, o in enumerate(self.sequence["order"]):
            if o in self.data["completed"]:
                ...
                try:
                    self.sequence["lookup"][self.sequence["order"][i + 1]][
                        "accessible"
                    ] = True
                except:
                    pass
