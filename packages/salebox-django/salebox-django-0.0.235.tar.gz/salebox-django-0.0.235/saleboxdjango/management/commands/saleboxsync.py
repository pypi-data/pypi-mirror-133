...
class Command(BaseCommand):
    def handle(self, *args, **options):
        ...
        if user_equals_member != "MANUAL":
            ...
            if user_equals_member == "ALWAYS":
                user_ids = (
                    get_user_model()
                    .objects.filter(salebox_member_id__isnull=True)
            ...
            if user_equals_member == "PURCHASE":
                user_ids = (
                    CheckoutStore.objects.filter(status__lte=30)
                ...
                if len(user_ids) > 0:
                    user_ids = (
                        get_user_model()
                        .objects.filter(id__in=list(set(user_ids)))
                ...
                for i, value in enumerate(response["sync"]):
                    if value["code"] == "attribute":
                        self.pull_model_attribute(
                            value["data"], value["lu"], sync_from_dict
                        )
    ...
    def pull_images(self):
        ...
        imgs = (
            ProductVariantImage.objects.exclude(img__isnull=True)
    ...
    #             post["time_offset"] = (
    #                 60 * 30
    #             )  # everything that changed in the last 30 minutes
    #         else:
    #             # attempt sync 'lowstock'
    #             variant_ids = (
    #                 ProductVariant.objects.filter(active_flag=True)
    ...
    #                 else:
    #                     self.send_admin_email(
    #                         "API response recieved but inventory missing (request type: %s)"
    #                         % post["request"]
    #                     )
    #             else:
    #                 self.send_admin_email(
    #                     "API response recieved but status not OK (request type: %s)"
    #                     % post["request"]
    #                 )
    #         else:
    #             self.send_admin_email(
    #                 "API response status code: %s (request type: %s)"
    #                 % (r.status_code, post["request"])
    #             )
    #     except:
    #         self.send_admin_email(
    #             "Unknown error connecting to Salebox POSv2 API (request type: %s)"
    #             % post["request"]
    #         )
    #
    #     # apply results
    ...
    def pull_model_attribute(self, data, api_lu, sync_from_dict):
        try:
            ...
            self.pull_set_sync_from_dict(
                "attribute", len(data) < 100, sync_from_dict, api_lu
            )
    ...
    def pull_model_attribute_item(self, data, api_lu, sync_from_dict):
        try:
            ...
            # update sync_from
            self.pull_set_sync_from_dict(
                "attribute_item", len(data) < 100, sync_from_dict, api_lu
            )
    ...
    def pull_model_country(self, data, api_lu, sync_from_dict):
        try:
            ...
            # update sync_from
            self.pull_set_sync_from_dict(
                "country", len(data) < 100, sync_from_dict, api_lu
            )
    ...
    def pull_model_country_state(self, data, api_lu, sync_from_dict):
        try:
            ...
            # update sync_from
            self.pull_set_sync_from_dict(
                "country_state", len(data) < 100, sync_from_dict, api_lu
            )
    ...
    def pull_model_event(self, data, api_lu, sync_from_dict):
        try:
            ...
            # update sync_from
            self.pull_set_sync_from_dict(
                "event", len(data) < 100, sync_from_dict, api_lu
            )
    ...
    def pull_model_member(self, data, api_lu, sync_from_dict):
        try:
            ...
            # update sync_from
            self.pull_set_sync_from_dict(
                "member", len(data) < 100, sync_from_dict, api_lu
            )
    ...
    def pull_model_member_group(self, data, api_lu, sync_from_dict):
        try:
            ...
            # update sync_from
            self.pull_set_sync_from_dict(
                "member_group", len(data) < 100, sync_from_dict, api_lu
            )
    ...
    def pull_model_product(self, data, api_lu, sync_from_dict):
        ...
        try:
            ...
            # update sync_from
            self.pull_set_sync_from_dict(
                "product", len(data) < 100, sync_from_dict, api_lu
            )
    ...
    def pull_model_product_category(self, data, api_lu, sync_from_dict):
        try:
            ...
            # update sync_from
            self.pull_set_sync_from_dict(
                "product_category", True, sync_from_dict, api_lu
            )
    ...
    def pull_model_product_variant(self, data, api_lu, sync_from_dict):
        ...
        try:
            ...
            # update sync_from
            self.pull_set_sync_from_dict(
                "product_variant", len(data) < 100, sync_from_dict, api_lu
            )
        ...
        try:
            for d in data:
                ...
                o, created = ProductVariantImage.objects.get_or_create(
                    id=d["id"], variant=ProductVariant.objects.get(id=d["variant"])
                )
            ...
            # update sync_from
            self.pull_set_sync_from_dict(
                "product_variant_image", len(data) < 100, sync_from_dict, api_lu
            )
        ...
        try:
            if "translations" in data:
                for language_code in data["translations"]:
                    self.pull_model_translation_recurse(
                        language_code, data["translations"][language_code]
                    )

            # update sync_from
            self.pull_set_sync_from_dict("i18n", True, sync_from_dict, api_lu)
        ...
        for d in data:
            if isinstance(data[d], str):
                t, created = Translation.objects.get_or_create(
                    language_code=language_code, key="%s%s" % (prefix, d)
                )
                t.value = data[d]
                t.save()
            else:
    ...
    def pull_transaction_history(self):
        members = Member.objects.filter(
            salebox_transactionhistory_request_dt__isnull=False
    ...
    #     if (
    #         settings.SALEBOX["ANALYTICS"]["SEND"]
    ...
    def push_member(self, user):
        ...
        post["salebox_member_sync"] = json.dumps(
            user.salebox_member_sync, ensure_ascii=False
        )
    ...
    def push_members(self):
        if hasattr(get_user_model(), "salebox_member_sync"):
            users = get_user_model().objects.filter(salebox_member_sync__isnull=False)
            for user in users:
        ...
        for cs in css:
            ...
            if user_equals_member != "MANUAL" and cs.user is not None:
                ...
                member = Member.objects.filter(
                    salebox_member_id=user.salebox_member_id
                ).first()
                if member is None:
    ...
    def transaction_invoice(self, store):
        ...
        return {
            ...
            if isinstance(data["address"]["country"], dict)
            else data["address"]["country"],
            "country_state_id": data["address"]["country_state"]["id"]
            if isinstance(data["address"]["country_state"], dict)
            else data["address"]["country_state"],
    ...
    def transaction_payment(self, store):
        total_price = (
            store.data["basket"]["sale_price"] + store.data["shipping_method"]["price"]
        )
    ...
    def transaction_shipping(self, store):
        ...
        return {
            ...
            if isinstance(address["address"]["country"], dict)
            else address["address"]["country"],
            "country_state_id": address["address"]["country_state"]["id"]
            if isinstance(address["address"]["country_state"], dict)
            else address["address"]["country_state"],
