...
class Response(object):
    ...
    def __init__(self, response=None, status_code=200, file_object=None, redirect=None, ignore_and_drop=False, gzip=False):
        ...
        self.add_cookie = self.cookies.add_cookie
        self.headers['server'] = "MkNxGn Mako - Python3;"
        self.headers['status'] = status_code
        self.access_control_allow_origin = None
        self.content_type = None
        self.file_object:File_Object = file_object
        self.redirect:Redirect = redirect
        self.ignore_and_drop:bool = ignore_and_drop
        self.gzip:bool = gzip
    ...
    def format(self):
        ...
        for key in self.cookies.cookies:
            formatted.append(self.cookies.cookies[key].__prep__())
...
class HTTP_Request(BaseHTTPRequestHandler):
    def __init__(self, server, request_text=b"", user_socket=None, addr=None):
        ...
        self.path_args = urlsplit(self.path)
        self.query = parse_qs(self.path_args.query, keep_blank_values=True)
        for item in self.query:
            if type(self.query[item]) == type([]):
                if len(self.query[item]) == 1:
                    self.query[item] = self.query[item][0]
        self.args = self.query
        self.path = unquote(self.path_args.path)
        self.files = []
        self.form_data = {}
        self.json = None
        self.data = None
...
class Resolver(object):
    ...
    def __init__(self, urls:Union[str, list]=None, methods=["GET"], function=None, ignore_case:bool=False):
        self.urls:Union[str, list] = urls
    ...
    def check_match(self, request:HTTP_Request) -> Union[list, str, bool]:
...
class HTTP_Server(object):
    ...
    def __request_message__(self, method, addr, path, resp_code):
        if self.print_on_request:
            ...
            if self.adaptive_prints:
                if len(method) + 1 > self.rql:
                    self.rql = len(method) + 1
                pmethod = method.ljust(self.rql)
            else:
                pmethod = method
            print("[", datetime.datetime.now().strftime(self.time_format), "] -", self.print_on_request_format.format(method=pmethod, addr=addr, path=path, resp_code=resp_code))   
        ...
        while True:
            ...
            if self.__run_on_directory__ == False:
                ...
                if isinstance(resp, Response):
                    ...
                    if type(resp[0]) == type("") and type(resp[1]) == type(1):
                        resp = Response(resp[0], resp[1])
                    else:
                        raise ValueError("Tuple responses must be set as (resp, status code) ")
