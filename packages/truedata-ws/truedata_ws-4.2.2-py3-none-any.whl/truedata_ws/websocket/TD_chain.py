...
class OptionChain:
    ...
    def init_dataframe(self):
        df = pd.DataFrame(columns=['symbols' , 'strike' , 'type' , 'ltp' , 'ltt' ,'volume' , 'oi' , 'prev_oi' , 'oi_change' ,
    ...
    def update_chain(self ,td_obj , symb_ids ):
        ...
        for symb_id in symb_ids:
            ...
            try:
                ...
                oi_change_perc = 0
            try:
                self.chain_dataframe.loc[td_obj.touchline_data[symb_id].symbol , ['ltp' , 'ltt'  , 'volume' ,  'oi' , 'prev_oi', 'oi_change' ,
                                                                                    'oi_change_perc','bid' , 'bid_qty' , 'ask' , 'ask_qty' ]] = [
                    td_obj.touchline_data[symb_id].ltp, td_obj.touchline_data[symb_id].timestamp , td_obj.touchline_data[symb_id].ttq ,
                    td_obj.touchline_data[symb_id].oi, td_obj.touchline_data[symb_id].prev_oi , td_obj.touchline_data[symb_id].oi - td_obj.touchline_data[symb_id].prev_oi ,
                    oi_change_perc , td_obj.touchline_data[symb_id].best_bid_price, td_obj.touchline_data[symb_id].best_bid_qty,
                    td_obj.touchline_data[symb_id].best_ask_price, td_obj.touchline_data[symb_id].best_ask_qty]
            except TypeError:
                self.chain_dataframe.loc[td_obj.touchline_data[symb_id].symbol , ['ltp' , 'ltt'  , 'volume' ,  'oi' , 'prev_oi', 'oi_change' ,
                                                                                    'oi_change_perc','bid' , 'bid_qty' , 'ask' , 'ask_qty' ]] =[ np.NaN for i in range(0,11)]
        ...
        while self.chain_status:
            for symb_id in symb_ids:
                if not td_obj.live_data[symb_id] == prev_tick_obj[symb_id]:
                    ...
                    self.chain_dataframe.loc[td_obj.live_data[symb_id].symbol , ['ltt' , 'volume' ,  'oi' ,'prev_oi' , 'oi_change' ,
                                                                                'oi_change_perc' ,'bid' , 'bid_qty' , 'ask' , 'ask_qty' ]] = [
                        td_obj.live_data[symb_id].timestamp ,td_obj.live_data[symb_id].ttq , td_obj.live_data[symb_id].oi,
                        td_obj.live_data[symb_id].prev_day_oi , td_obj.live_data[symb_id].oi_change ,
                        round(td_obj.live_data[symb_id].to_dict()['oi_change_perc'] ,2 ) ,bid, bid_qnty, ask, ask_qnty]
