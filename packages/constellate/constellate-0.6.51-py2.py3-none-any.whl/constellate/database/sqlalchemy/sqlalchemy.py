...
from typing import Dict, Tuple, List, Iterator, AsyncGenerator, Optional, Any, Union
...
from constellate.database.migration.yoyo.table import YoyoVersionR
...
class SQLAlchemy:
    ...
    async def _create_engine(self, instance: SQLAlchemyDBConfig, options: Dict = {}) -> _EngineConfig:
    ...
    async def migrate(self, options: Dict = {}):
        ...
        - key: migration_priority: int [0, max_size]. Default: 0. Migration priority amongst all engines.
               Engines with identical priority will be migrated concurrently
        """
        ...
        async def _run(i: SQLAlchemyDBConfig = None, o: Dict = None, concurrent: bool = None):
            log_prefix = f"Migrating {'concurrently' if concurrent else ''}: {i.identifier}"
            try:
                try:
                    i.logger.info(f"{log_prefix}: started")
                    await self._migrate(instance=i, options=o)
                    i.logger.info(f"{log_prefix}: completed")
                except BaseException as e:
                    raise MigrationException() from e
            except BaseException as e:
                i.logger.error(f"{log_prefix}: failed", exc_info=1)
                raise e

        for priority, migrations in migration_orders.items():
            concurrent = len(migrations) > 0
            await asyncio.gather(*[_run(i=i, o=o, concurrent=concurrent) for i, o in migrations])
    ...
    async def database_compatible(self, options: Dict = {}) -> bool:
        ...
        Option per engine:
        - database_user_version_class_column: SQLALchemy class + column. To query the current database version from
        - database_user_version_min: int. Min database version to consider the app compatible with the current database
        ...
        async def has_compatibility(
            instance=None,
            class_column=None,
            database_user_version_min=sys.maxsize,
            execution_options={},
        ):
            async with self.session_scope(config=instance) as session:
                stmt = select(class_column).limit(1)
                database_user_version = (
                    await session.execute(stmt, excution_options=execution_options)
                ).scalar()
                return database_user_version_min <= database_user_version

        compatibles = [
            has_compatibility(
                instance=instance,
                class_column=i_options.get("database_user_version_class_column"),
                database_user_version_min=i_options.get("database_user_version_min", -1),
