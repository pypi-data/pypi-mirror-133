...
class SQLAlchemy:
    ...
    async def migrate(self, options: Dict = {}):
        ...
        async def __migrate(instance: SQLAlchemyDBConfig = None, options: Dict = None, concurrent: bool = None):
        ...
        for priority, migrations in migration_orders.items():
            ...
            await asyncio.gather(*[__migrate(instance=i, options=o, concurrent=concurrent) for i, o in migrations])
    ...
    async def database_compatible(self, options: Dict = {}) -> bool:
        ...
        async def has_compatibility(
            ...
            @execute_scalar
            async def _read_database_user_version(**kwargs):
                return select(class_column).limit(1)

            async with self.session_scope(config=instance) as session:
                database_user_version = await _read_database_user_version(session=session)
