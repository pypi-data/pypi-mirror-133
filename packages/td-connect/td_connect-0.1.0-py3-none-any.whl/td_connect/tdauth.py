...
class TDAuth:
    ...
    def __init__(self, auth_file: str, **storage_options):
        """
        Args:
    ...
    @classmethod
    def from_gcs(cls, bucket: str, file: str, project: str, token: Union[Dict[str, Any], str]):
        """Initialize from a configuration file stored in Google Cloud Storage.

        Args:
            bucket (str): Name of bucket the file is in.
        ...
        """
        return cls(f'gs://{bucket}/{file}', gcs={'project': project, 'token': token})
    
    @classmethod
    def from_gcs_env(cls, bucket_var: str = 'TD_AUTH_BUCKET', file_var: str = 'TD_AUTH_FILE', project_var: str = 'TD_AUTH_PROJECT', token_var: str = 'TD_AUTH_TOKEN'):
        return cls.from_gcs(bucket=os.environ[bucket_var], file=os.environ[file_var], project=os.environ[project_var], token=os.environ[token_var])
    ...
    async def get_stream_websocket(self):
        ...
        connection = await websockets.client.connect(endpoint)
        if connection.close:
            raise ConnectionError(
                f'Could not connect to websocket endpoint: {endpoint}.')
        # login.
        date = dateutil.parser.parse(
            self._stream_info['tokenTimestamp'], ignoretz=True)
        # timestamp in ms.
        timestamp = (date - datetime.utcfromtimestamp(0)
        ...
        while True:
            ...
            if response['content']['code'] == 3:
                raise ValueError(
                    f"STREAM LOGIN ERROR: {response[0]['content']['msg']}")
    ...
    def _get_user_principles(self):
        resp = requests.get(url="https://api.tdameritrade.com/v1/userprincipals",
                            params={
    ...
    def _set_access_token_from_refresh_token(self):
        ...
        if resp.status_code == 200:
            logger.debug(
                f"Successfully got new access token from refresh token.")
            data = resp.json()
    ...
    def _set_access_and_refresh_tokens(self):
        resp = requests.post(r"https://api.tdameritrade.com/v1/oauth2/token",
                             headers={
        ...
        if resp.status_code == 200:
            ...
            url = input(f"""
                ...
                Sign in, wait to be redirected to a new page, then paste the URL of the redirect page here: """).strip()
