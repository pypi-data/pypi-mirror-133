...
from optilog.autocfg import config_str_to_dict
...
def main(args):
    ...
    cfg_input = {}
    if args.cfg_json:
        with open(args.cfg_json) as fh:
            cfg_input = json.load(fh)
    
    cfg_input = {
        reverse_index_lookup[k]: v
        for k,v in cfg_input.items()
    }
    ...
    res = entrypoint(**kwargs)

    print("c Return value:", res, flush=True)
