...
class BaseHandler(RequestHandler):
    ...
    def write_error(self, status_code, **kwargs):
        ...
        if status_code == 404:
            ...
            logging.error("参数:" + (json.dumps(self.params, ensure_ascii=False) if hasattr(self, "params") else str(self.request.arguments or {})))
    ...
    def write_success(self, data=None):
        ...
        if is_slow_request:
            logging.warning("%d %s %.2fms", self.get_status(), self._request_summary(), request_time)
...
class BaseSchema(object):
    ...
    @classmethod
    def distinct(cls, spec, distinct, shard_key_value=None, **kwargs):
        """
        根据spec条件，查询并返回键为distinct的列表，且不重复
        # 返回所有uid的列表，且不重复
        cls.distinct({"a": 2}, "uid")
        [2, 4]
        ...
        if not kwargs:  # 利用cursor游标
            result = cls.find(spec, is_cursor=True, shard_key_value=shard_key_value).distinct(distinct)
        else:
    ...
    def find_list_by_page(cls, spec, page, count=10, sort={"create_at": -1}, fields=False, shard_key_value=None):
        ...
        data_list = [cls._parse_data(cls.__parse_oid(d)) for d in list(cls.__meta_cursor(cursor, page, count, sort)) if d]
    ...
    def find_list_by_last_id(cls, spec, count=10, sort={"create_at": -1}, fields=False, last_id_field=False, shard_key_value=None):
        ...
        """
        cursor = cls.find(spec, fields=fields, is_cursor=True, shard_key_value=shard_key_value)
        data_list = [cls._parse_data(cls.__parse_oid(d)) for d in list(cls.__meta_cursor(cursor, count=count, sort=sort)) if d]
    ...
    def get_batch_data(cls, *vals, **kwargs):
        ...
        cursor = cls.find({key: {"$in": vals}}, fields=fields, is_cursor=True, shard_key_value=shard_key_value)
        ...
        return {k: cls._parse_data(v) for k, v in data_mp.items()}
