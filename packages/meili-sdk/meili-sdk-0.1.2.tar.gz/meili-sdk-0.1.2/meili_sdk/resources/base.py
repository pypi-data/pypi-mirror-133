...
class BaseResource:
    ...
    def make_request(
        ...
        return self.handle_response(response, expected_class)

    def handle_response(
        self, response, expected_class=None
    ) -> t.Tuple[
        int, t.Optional[t.Union[dict, object]], t.Optional[bytes], t.Optional[int]
    ]:
        ...
        if response.status_code >= 300:
            raise exceptions.APIError(
                f"Retrieved status code {response.status_code} from remote API"
            )
        ...
        try:
            ...
            raise exceptions.APIError(f"Cannot decode response: {exc}")

        next_page = None
        if self.is_response_paginated(data):
