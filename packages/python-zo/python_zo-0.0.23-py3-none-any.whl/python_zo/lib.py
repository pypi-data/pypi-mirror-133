...
def mix_all(args, mappings, tags):
		...
		for v in x['data']:
			...
			for f in v['fields']:
				...
				if y['kind'] == 'gql' and not f[y['kind']]:
					f[y['kind']] = f_map.get(f.get('nameExact')) or f_map.get(f.get('tagGql'))
...
def render_template(env, schemas, args):
	...
	if args.cmd == 'scaffold':
		...
		if args.graphql.count("/") != 1:
			return eprint('--graphql pattern should be like "Query/MySettings"')
					...
					if m['key'] == gql_type:
						for f in m['fields']:
							if f['key'] == gql_model:
								api_field = f
		if not db_model:
			return eprint('model not specified or not found')
		if not api_field:
			return eprint(gql_model, 'not found')

		if args.verb.count('/') != 1:
			return eprint('verb must be in pattern Get/Getter')
		verb, verb_er = args.verb.split('/', maxsplit=2)

		skel_path = os.path.join(args.template_path, 'skel')
		skels = walk(skel_path)
		if not skels:
			return eprint('skel folder not found in template')
		...
		for skel in skels:
			...
			patch = not args.force and os.path.exists(out_path) and any(x for x in args.patch if x in skel_rel_path)
			template = env.from_string(content)
			output = template.render(my_vars, patch=patch)
			if not os.path.exists(out_path) or (args.force and not patch):
				eprint('[DONE] file written', out_path)
				...
				func_names = GO_FUNC_RE.findall(output)
				if GO_FUNC_RE.search(existing):  # and args.force
					existing = GO_FUNC_RE.sub(lambda x: '' if x.group(1) in func_names else x.group(0), existing)
				existing = existing.strip() + '\n\n' + output.strip()
				with open(out_path, 'w') as dst:
					dst.write(existing)
				eprint('[PATCHED] existing file patched', out_path)
			else:
				eprint('[SKIPPED] output file exists', os.path.relpath(rel_path))
				# print(output)
		...
		for f, tbls in targets.items():
			...
			if type_tpls:
				...
				schema_map = {s['kind']: {x['key']: x for x in s['data']} for s in schemas}
				for x in tbls:
					out = template.render(table(Table(x, schema=schema_map)))
					if out.strip():
			...
			if not type_contents:
				eprint(f'[{os.path.basename(f)}] no template found\n')
				continue
...
def parse_args():
	...
	parser_s.add_argument('--patch', nargs='+', default="", help="patch files")
...
def main():
	...
	tags = {x[1]: x[0] for x in os.path.exists(tags_path) and csv.reader(open(tags_path).read().strip().splitlines()[1:]) or []
