...
class Xovsp(Recombination):
    """
    Xovsp - class : 一个用于调用内核中的函数xovsp(单点交叉)的类，
                    该类的各成员属性与内核中的对应函数的同名参数含义一致，
                    可利用help(xovsp)查看各参数的详细含义及用法。
    ...
    def do(self, OldChrom):  # 执行内核函数
        return xovsp(OldChrom, self.XOVR, self.Half_N, self.GeneID, self.Parallel)
