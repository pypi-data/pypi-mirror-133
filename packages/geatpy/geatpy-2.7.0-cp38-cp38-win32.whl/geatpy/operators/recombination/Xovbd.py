...
class Xovbd(Recombination):
    """
    Xovbd - class : 一个用于调用内核中的函数xovbd(二项式分布交叉)的类，
                    该类的各成员属性与内核中的对应函数的同名参数含义一致，
                    可利用help(xovbd)查看各参数的详细含义及用法。
    ...
    def __init__(self, XOVR=0.7, Half_N=False, GeneID=None, Parallel=False):
        ...
        self.GeneID = GeneID  # 基因ID，是一个Numpy ndarray一维数组，若设置了该参数，则该函数会对具有相同基因ID的染色体片段进行整体交叉
        self.Parallel = Parallel  # 表示是否采用并行计算，缺省或为None时默认为False

    def do(self, OldChrom):  # 执行内核函数
        return xovbd(OldChrom, self.XOVR, self.Half_N, self.GeneID, self.Parallel)
