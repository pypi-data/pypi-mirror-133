...
class moea_psy_RVEA_RES_templet(ea.MoeaAlgorithm):
    """
moea_psy_RVEA_RES_templet : class - 带参考点再生策略的多染色体多目标进化优化RVEA算法类(RVEA With the Reference Vector Regeneration Strategy)
    
描述:
    采用带参考点再生策略的RVEA进行多目标优化，即参考文献[1]中的RVEA*算法。
    该算法与RVEA算法类似，不过可以更好地解决具有复杂帕累托前沿面的多目标优化问题。
    该算法类是内置算法类moea_RVEA_RES_templet的多染色体版本。
