...
class soea_psy_GGAP_SGA_templet(ea.SoeaAlgorithm):
    """
soea_psy_GGAP_SGA_templet : class - Polysomy Generational Gap Simple GA Algorithm(带代沟的简单遗传算法类)

算法类说明:
    该算法类是内置算法类soea_GGAP_SGA_templet的多染色体版本，
    因此里面的种群对象为支持混合编码的多染色体种群类PsyPopulation类的对象。

算法描述:
    本算法类实现的是带代沟的简单遗传算法，
    它在SGA算法类的基础上增加“代沟”，用于控制使用多少个子代替换父代来形成新一代种群，算法流程如下：
    1) 根据编码规则初始化N个个体的种群。
    2) 若满足停止条件则停止，否则继续执行。
