...
approval_task_name: Any = re.compile(
    "([A-Za-z0-9\\@]+(_|\\-))+approval$")
...
def service_validate(data: Dict[str, Any], general_config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def is_valid_command(validate_command: Dict[str, Any]) -> bool:
        ...
        for name, parameters in validate_command.items():
            ...
            for param_name in config_parameters:
                if not param_name in validate_command[name] and "default" not in config_parameters[param_name]:
                    raise SchemaError(
    ...
    serviceSchema = Schema({
        ...
        "layers": And([  # layers definding an order of job execution
            {
                str:  # layer name
                Or(
                    # Approval task converted to a circleci workflow approval job
                    {
                        ...
                        {
                            Optional("requires", default=[]): [And(str, is_requires_id)]
                        }
                    },
                    {  # Task converted to a Circleci job; can be referenced via "ServiceName:TaskName"
                        str:
                        {
                            ...
                            "commands": [
                                Optional(
                                    And(str, Use(convert_string_command), is_valid_command)),
                                And({
                                    ...
                                    str:  # command name
                                    {
                                        ...
                                        Optional(str): Or(str, float, bool)
                                    }
                                }, is_valid_command)
                            ],
                            Optional("resource_class", default="large"): And(str, Or("small", "medium", "medium+", "large", "xlarge", "2xlarge", "2xlarge+")),
                            Optional("executor", default="default"): And(str, is_an_executor),
        ...
        Optional("commands"): {
            str: {  # all commands must have a parameters section
                "parameters":
                {
                    ...
                    Optional(str):
                    {
                        "type": Or("string", "boolean", "integer", "enum", "executor", "steps"),
                        Optional("default"): Or(str, bool, float)
                    }
                },
