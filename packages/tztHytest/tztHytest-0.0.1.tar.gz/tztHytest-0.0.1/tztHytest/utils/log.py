...
_L='Traceback:\n'
_K='suite'
_J='case_teardown_fail'
_I='suite_teardown_fail'
_H='case_setup_fail'
_G='suite_setup_fail'
_F='class'
_E='case_pass'
...
from tztHytest.common import GSTORE,IMAGE2BASE64
...
class Stats:
	def test_start(self,_title='Test Report'):self.result={_O:Collector.case_number,_P:0,_E:0,_C:0,_D:0,_G:0,_H:0,_I:0,_J:0,'case_pass_list':[],'case_fail_list':[],'case_abort_list':[]};self.start_time=time.time()
	def test_end(self,runner):
		A='---ret---';self.end_time=time.time();self.test_duration=self.end_time-self.start_time
		if self.result[_C]or self.result[_D]or self.result[_G]or self.result[_H]or self.result[_I]or self.result[_J]:GSTORE[A]=1
		else:GSTORE[A]=0
...
class ConsoleLogger:
	...
	def setup_fail(self,name,utype,e,stacktrace):utype='套件'if utype==_K else _X;print(f"\n{utype} 初始化失败 | {name} | {e}",style=_B)
	def teardown_fail(self,name,utype,e,stacktrace):utype='套件'if utype==_K else _X;print(f"\n{utype} 清除失败 | {name} | {e}",style=_B)
	def debug(self,msg):
...
class TextLogger:
	...
	def case_result(self,case):
		if case.execRet==_R:logger.info('  PASS ')
			...
			stacktrace=_L+case.stacktrace.split(_M,3)[3]
	...
	def setup_fail(self,name,utype,e,stacktrace):stacktrace=_L+stacktrace.split(_M,3)[3];logger.info(f"{utype} setup fail | {e} \n{stacktrace}")
	def teardown_fail(self,name,utype,e,stacktrace):stacktrace=_L+stacktrace.split(_M,3)[3];logger.info(f"{utype} teardown fail | {e} \n{stacktrace}")
...
class HtmlLogger:
	...
	def test_start(self,_title=''):
		...
		self.doc=document(title=f"测试报告");self.doc.head.add(meta(charset='UTF-8'),style(raw(_css_style)),script(raw(_js),type='text/javascript'));self.main=self.doc.body.add(div(_class='main_section'));self.main.add(h1(f"测试报告 - tztHytest v{version}",style='font-family: auto'));self.main.add(h3(f"统计结果"));resultDiv=self.main.add(div(_class='result'));self.result_table,self.result_barchart=resultDiv.add(table(_class='result_table'),div(_class='result_barchart'));_,self.logDiv=self.main.add(div(h3('执行日志',style='display:inline'),style='margin-top:2em'),div(_class='exec_log'));self.ev=div(div('∧',_class=A,onclick='previous_error()',title='上一个错误'),div('∨',_class=A,onclick='next_error()',title='下一个错误'),_class='error_jumper');
		# ,div('教程',_class=A,onclick='window.open("http://www.byhy.net/tut/auto/tztHytest/01", "_blank"); ')
		self.main.add(div(div('页首',_class=A,onclick='document.querySelector("body").scrollIntoView()'),div('精简',_class=A,id='display_mode',onclick='toggle_folder_all_cases()'),self.ev,id='float_menu'));self.curEle=self.main;self.curSuiteEle=_A;self.curCaseEle=_A;self.curCaseLableEle=_A;self.curSetupEle=_A;self.curTeardownEle=_A;self.suitepath2element={}
	def test_end(self,runner):
		B='%Y%m%d %H:%M:%S';A='color:red';execStartTime=time.strftime(B,time.localtime(stats.start_time));execEndTime=time.strftime(B,time.localtime(stats.end_time));ret=stats.result;errorNum=0;trs=[];trs.append(tr(td('开始时间'),td(f"{execStartTime}")));trs.append(tr(td('结束时间'),td(f"{execEndTime}")));trs.append(tr(td('耗时'),td(f"{stats.test_duration:.3f} 秒")));trs.append(tr(td('预备执行用例数量'),td(f"{ret[_O]}")));trs.append(tr(td('实际执用例行数量'),td(f"{ret[_P]}")));trs.append(tr(td('通过'),td(f"{ret[_E]}")));case_count_toberun=ret[_O];num=ret[_C];style=''if num==0 else A;trs.append(tr(td('失败'),td(f"{num}",style=style)));errorNum+=num;num=ret[_D];style=''if num==0 else A;trs.append(tr(td('异常'),td(f"{num}",style=style)));errorNum+=num;blocked_num=case_count_toberun-ret[_E]-ret[_C]-ret[_D];style=''if blocked_num==0 else A;trs.append(tr(td('阻塞'),td(f"{blocked_num}",style=style)));num=ret[_G];style=''if num==0 else A;trs.append(tr(td('套件初始化失败'),td(f"{num}",style=style)));errorNum+=num;num=ret[_I];style=''if num==0 else A;trs.append(tr(td('套件清除失败'),td(f"{num}",style=style)));errorNum+=num;num=ret[_H];style=''if num==0 else A;trs.append(tr(td('用例初始化失败'),td(f"{num}",style=style)));errorNum+=num;num=ret[_J];style=''if num==0 else A;trs.append(tr(td('用例清除失败'),td(f"{num}",style=style)));errorNum+=num;self.ev['display']='none'if errorNum==0 else'block';self.result_table.add(tbody(*trs))
	...
	def case_result(self,case):
		if case.execRet==_R:self.curCaseEle[_F]+=' pass';self.curCaseLableEle+=' PASS'
		else:
			stacktrace=_L+case.stacktrace.split(_M,3)[3]
			if case.execRet==_S:self.curCaseEle[_F]+=_c;self.curCaseLableEle+=' FAIL';self.curEle+=div(f"{case.error} \n{stacktrace}",_class=_V)
			elif case.execRet==_T:self.curCaseEle[_F]+=' abort';self.curCaseLableEle+=' ABORT';self.curEle+=div(f"{case.error} \n{stacktrace}",_class=_V)
	def setup(self,name,utype):
		_class=f"{utype}_setup setup"
		if utype==_K:stHeaderEle=div(span('套件初始化',_class=_N),span(name),span(datetime.now().strftime(_U),_class=_a),_class=_b);stBodyEle=self.curEle=div(_class=_Z);self.curSetupEle=div(stHeaderEle,stBodyEle,_class=_class,id=f"{_class} {name}");self.curSuiteEle.add(self.curSetupEle)
		else:self.curSetupEle=self.curEle=div(span('用例初始化',_class=_N),_class=_class,id=f"{_class} {name}");self.curCaseBodyEle.add(self.curSetupEle);self.curEle[_F]+=_f
	def teardown(self,name,utype):
		_class=f"{utype}_teardown teardown"
		if utype==_K:stHeaderEle=div(span('套件清除',_class=_N),span(name),span(datetime.now().strftime(_U),_class=_a),_class=_b);stBodyEle=self.curEle=div(_class=_Z);self.curTeardownEle=div(stHeaderEle,stBodyEle,_class=_class,id=f"{_class} {name}");self.curSuiteEle.add(self.curTeardownEle)
		else:self.curTeardownEle=self.curEle=div(span('用例清除',_class=_N),_class=_class,id=f"{_class} {name}");self.curCaseBodyEle.add(self.curTeardownEle);self.curEle[_F]+=_f
	def setup_fail(self,name,utype,e,stacktrace):self.curSetupEle[_F]+=_c;stacktrace=_L+stacktrace.split(_M,3)[3];self.curEle+=div(f"{utype} setup fail | {e} \n{stacktrace}",_class=_V)
	def teardown_fail(self,name,utype,e,stacktrace):self.curTeardownEle[_F]+=_c;stacktrace=_L+stacktrace.split(_M,3)[3];self.curEle+=div(f"{utype} teardown fail | {e} \n{stacktrace}",_class=_V)
	def info(self,msg):
	...
	def log_img(self,imgPath,width=_A):
		if self.curEle is _A:return
		base64_path = IMAGE2BASE64(imgPath) # html src也能填base64编码图片
		self.curEle+=div(img(src=base64_path,width='aa'if width is _A else width,_class='screenshot'))
