...
class WinHOW:
    ...
    class draw2D:
                                            ...
                                            def drawarc(col=(),pos=[],start_angle=0,stop_angle=0,rad=1,sh=1,st='-',surf=0):
                                                    ugl1 = start_angle/56.5
                                                    if stop_angle >= 360: stop_angle=stop_angle-720
                                                    for l in range(int(stop_angle*3.5)):
                                                        if st=='-': ugl1+=0.005
                                                        elif st=='+': ugl1-=0.005
                                                        for i in range(0,rad,2): 
                                                            xl=pos[0]+i*math.sin(ugl1);yl=pos[1]+i*math.cos(ugl1)
                                                            if i == rad - sh:
                                                                xpos = xl;ypos = yl
                                                        pygame.draw.line(surf,
                                                                        col,
