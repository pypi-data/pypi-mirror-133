...
def run(  # noqa: C901
    ...
    if l2_data.FOLDER_TYPE == ProcessType.MEX:
        ...
        i_integral = 0 if type(i_Surface) == int else i_integral[0]
        fsup = DATA.FSUP
        error = DATA.ERROR
    else:
        # fsky = DATA.FSKY
    ...
    while stay_while:
        ...
        if radiocc.gui is not None:
            ...
            radiocc.gui.label_data.set_label(
                f"{DATA_ID_i_profile} BANDE {Bande} {item} IS NOW INTERACTIVE."
            )
            radiocc.gui.interactive()
            radiocc.gui.label_data.set_label(
                f"PROCESSING {DATA_ID_i_profile} BANDE {Bande} {item}..."
            )
            NEW_THRESHOLD = radiocc.gui.altitude
            NEW_TYPE = radiocc.gui.label_switch.get_label()
        else:
    ...
    while stay_while:
        if radiocc.gui is not None:
            ...
            B_Cor_Type = radiocc.gui.label_switch.get_label()
            if radiocc.gui.go_next:
                break
        else:
            if B_Cor_Type == "Linear":
                print("\t p0 = ", p0)
        ...
        Doppler_debias2, Doppler_biasfit2 = R16_Biasfit_Correction.Off_Cor2(
