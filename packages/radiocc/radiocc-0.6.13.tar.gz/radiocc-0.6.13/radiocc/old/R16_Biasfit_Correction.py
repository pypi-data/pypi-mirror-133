...
def Off_Cor2(Type, Doppler, Diff_doppler, ET, delET, N_data, distance):
    ...
    if Type == 'Linear':
        ...
        if radiocc.gui is not None:
            p0 = float(radiocc.gui.entry_intercept.get_text())
            p1 = float(radiocc.gui.entry_slope.get_text())
        else:
        ...
        if radiocc.gui is not None:
            p0 = float(radiocc.gui.entry_intercept.get_text())
            p1 = float(radiocc.gui.entry_slope.get_text())
            p2 = float(radiocc.gui.entry_quadratic.get_text())
        else:
            p0 = float(input("Enter the new p[0], i.e; p[0] +dp[0] = " ))
            p1 = float(input("Enter the new p[1], i.e; p[1] +dp[1] = " ))
    ...
    return Doppler_debias2, Doppler_biasfit2
