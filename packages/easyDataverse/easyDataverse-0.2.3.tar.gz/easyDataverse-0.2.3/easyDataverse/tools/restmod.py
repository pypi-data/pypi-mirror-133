...
def generate_rest_api_code(metadatablock_loc: str, lib_name: str, lib_path: str) -> None:
    ...
    import_template = Template(
        pkg_resources.read_text(
            jinja_templates, "rest_imports_template.jinja2"
        )
    )
    create_template = Template(
        pkg_resources.read_text(
            jinja_templates, "rest_create_template.jinja2"
        )
    )
    ...
    for path in glob.glob(os.path.join(metadatablock_loc, "*.py")):
        ...
        # Get the class name
        class_name = get_class_name(module_name, path)

        endpoints.append(create_template.render(
