...
def main():
    ...
    computations = Window(3)  # <- manage computation history
    ...
    # Initialize REPL globals with nil value (as in Clojure REPL)

    # Initialize NanamiLang Loader mechanism ...
    loader.Loader.initialize(module.Module, loader.LocalIOLoader)

    # Here, we finally can start the main loop, hooray!
    while True:
        try:
            # First, let the user to enter something they want
            src = input('REPL> ')
            ...
            for dt in m.evaluate().results():
                if isinstance(dt, nanamilang.datatypes.NException):
                    ...
                    if not args.include_traceback:
                        print(dt.format(source=src))
                    else:
                        print('\n\b', dt.format(source=src,
