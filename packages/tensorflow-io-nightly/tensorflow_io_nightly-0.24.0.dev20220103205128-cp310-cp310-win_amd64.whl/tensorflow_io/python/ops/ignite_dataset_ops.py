    ...
    def __init__(self, host, port, certfile=None, keyfile=None, password=None):
        ...
        if certfile is not None:
            ...
            if keyfile is not None:
                raise ValueError(
                    "SSL is disabled, keyfile must not be specified "
                    "(to enable SSL specify certfile)"
                )
            if password is not None:
                raise ValueError(
                    "SSL is disabled, password must not be specified "
                    "(to enable SSL specify certfile)"
                )
...
class IgniteClient(TcpClient):
    ...
    def get_cache_type(self, cache_name):
        ...
        if row_count == 0:
            raise RuntimeError(
                "Scan Query returned empty result, so it's "
    ...
    def _get_type(self, type_id):
        ...
        if status != 0:
            ...
            if err_msg is None:
                raise RuntimeError(
                    "Get Binary Type Error [status=%d, message='%s']"
                    % (status, err_msg)
                )
...
class IgniteDataset(data.Dataset):
    ...
    def __init__(
        ...
        self.host = tf.convert_to_tensor(host, dtype=dtypes.string, name="host")
        self.port = tf.convert_to_tensor(port, dtype=dtypes.int32, name="port")
        self.local = tf.convert_to_tensor(local, dtype=dtypes.bool, name="local")
        self.part = tf.convert_to_tensor(part, dtype=dtypes.int32, name="part")
        ...
        self.permutation = tf.convert_to_tensor(
            self.cache_type.to_permutation(), dtype=dtypes.int32, name="permutation"
        )
        super().__init__()
