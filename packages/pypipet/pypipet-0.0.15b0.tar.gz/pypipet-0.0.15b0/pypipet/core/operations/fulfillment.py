...
def get_unfulfilled_order_inprocesss(table_objs, session, status='processing', params:dict={}):
    ...
    logger.debug(f'invalid params {parmas}')

def get_fulfilled_orders_inprocess(table_objs, session, status='processing', params:dict={}):
    params.update({'status': status})
    fulfilled = get_fulfilled_orders(table_objs, session, status=status,
                params=params)
...
def add_fulfillment_bulk(table_objs, session, data_list:[dict]):
    for data in data_list:
        add_fulfillment(table_objs, session, data)

def add_fulfillment(table_objs, session, data:dict):
...
def get_fulfillment(table_obj, session, params):
    ...
    logger.debug('fulfillment not found')
    return {}


def update_fulfillment_to_front_shop(table_objs, session, 
             shop_connector, params: dict, tracking_info:dict={}):
    ...
    logger.debug('invalid identifier info for order')
