...
def _edit_fulfilment(session, table_classes, fulfillment_info, 
    ...
    order_info = None 
    if action == 1:
        order_info =  get_shipping_info(table_classes, session,
                                     {'destination_order_id': order_id,
    ...
    if order_info is None:
            click.secho('order not found: {}'.format(order_id),
                        fg='yellow')
            return 

    if action == 1 and order_info.get('tracking_id') is not None:
        click.secho('tracking exist: {}'.format(order_info['tracking_id']),
                    fg='yellow')
        return
