...
class CustomPeriod:
    ...
    def appendperiod(self, df, refcolumn = "period_code"):

        if refcolumn not in df.columns: 

            print(f"{refcolumn} not found in dataframe. Cant append period")
            
        # add checking for df[refcolumn] datatype to check if compatible
        
        return self.perioddf.merge(df, left_on = "period_code", right_on = refcolumn)
