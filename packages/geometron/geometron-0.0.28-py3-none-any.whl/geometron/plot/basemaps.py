...
def get_image_cluster(left, top, right, bottom, zoom, xyz_server, tile_size=256, cache_dir='./.tiles_cache',
                      verbose=False):
    """ Gets an cluster of tiles from an xyz server, assembles it and crops it to the given extent
    ...
    tuple
        (image, extent)
    """
    xyz_server = xyz_server.split('https://', maxsplit=1)[0]
    ...
    for x_tile, y_tile in product(range(x0_tile, x1_tile), range(y0_tile, y1_tile)):
        if not exists(join(cache_dir, f'{xyz_server}_{zoom}_{x_tile}_{y_tile}.png')):
            if verbose:
                print(f'Getting tile {xyz_server}/{zoom}/{x_tile}/{y_tile}.png ...')
            with requests.get(f'https://{xyz_server}/{zoom}/{x_tile}/{y_tile}.png') as resp:
                tile_img = Image.open(BytesIO(resp.content))
                if cache_dir is not None:
                    makedirs(cache_dir, exist_ok=True)
                    tile_img.save(join(cache_dir, f'{xyz_server}_{zoom}_{x_tile}_{y_tile}.png'), format='png')
        else:
            if verbose:
                print(f'Retrieving {xyz_server}/{zoom}/{x_tile}/{y_tile}.png from cache {abspath(cache_dir)}...')
            tile_img = Image.open(join(cache_dir, f'{xyz_server}_{zoom}_{x_tile}_{y_tile}.png'))
...
def plot_basemap(extent, ax=None, **kwargs):
    ...
    max_zoom: int, default: 17
        maximum zoom level
    xyz_server: str, default: 'opentopomap.org'
        url of the xyz server
    ...
    """
    left, right, bottom, top = extent
    xyz_server = kwargs.pop('xyz_server', 'opentopomap.org')
    ...
    img, extent, ratio = get_image_cluster(left, top, right, bottom, zoom, xyz_server,
