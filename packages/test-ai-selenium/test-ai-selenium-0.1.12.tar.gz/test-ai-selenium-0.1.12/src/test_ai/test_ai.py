...
class TestAiDriver():
    def __init__(self, driver, api_key, test_case_name=None, debug=False, use_classifier_during_creation=True, train=False, server_url=None):
        self.debug = debug
        self.train = train
        self.driver = driver
    ...
    def _classify(self, element_name):
        ...
        if self.test_case_creation_mode:
            ...
            if element_box:
                ...
                element = testai_elem(real_elem.parent, real_elem, element_box, self.driver,
                                      self.multiplier)
            ...
            if resp_data['success'] and 'box' in resp_data:
                ...
                real_elem = self._match_bounding_box_to_selenium_element(resp_data['box'], multiplier=self.multiplier)
                element = testai_elem(real_elem.parent, real_elem, resp_data['box'], self.driver,
                                      self.multiplier)
            ...
            try:
                ...
                if response.get('success', False):
                    ...
                    real_elem = self._match_bounding_box_to_selenium_element(pred_elem, multiplier=self.multiplier)
                    element = testai_elem(real_elem.parent, real_elem, pred_elem, self.driver,
                                          self.multiplier)
                else:
                    msg = 'Classification failed for element_name: %s - Please visit %s to classify' % (element_name, self.url + '/label/' + element_name)
            except Exception:
    ...
    def _upload_screenshot_if_necessary(self, element_name):
        ...
        try:
            ...
            if response['success'] == True:
                ...
                data = {'api_key': self.api_key, 'screenshot_uuid': key, 'screenshot': screenshotBase64, 'label': element_name}
