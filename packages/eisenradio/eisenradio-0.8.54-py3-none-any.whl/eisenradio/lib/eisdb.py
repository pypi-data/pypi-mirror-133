...
schema_dir = script_path
app_db = ''
try:
    app_db = environ['DATABASE']
except KeyError:
    app_db = current_app.config["DATABASE"]
...
def get_db_smaller():
    conn = sqlite3.connect(app_db, isolation_level=None)
    conn.execute("VACUUM")
    conn.close()
...
def install_new_db(db_path):
    global schema_dir
    global app_db
    schema_db = path.join(schema_dir, 'database.db')  # name schema is default
    ...
    if testing == 'True':
        if this_db.is_file():
            remove(this_db)
            # raise ValueError(f'-42-DEV  {current_app.config}')
        empty_db = environ["EMPTY_DB"]
        print(f' -empty_db->{environ["EMPTY_DB"]}<- environ install_new_db()')

        if empty_db == 'True':
            empty_db_from_schema()
        if empty_db == 'False':
            make_db_from_schema()
        shutil.move(schema_db, db_path)
