...
try:
    ...
    pass
from eisenradio.config import script_path as conf_path

this_script = path.dirname(__file__)
...
def write_config(conf, empty_db=None):
    ...
    remove_config()
    with open(path.join(this_script, '.env'), 'w') as writer:
        for line in loader:
            writer.write(line + '\n')
        writer.flush()
...
def load_config_os():
    global this_script
    dotenv_loader = load_dotenv(path.join(this_script, '.env'))


def remove_config():
    global this_script
    try:
        remove(path.join(this_script, '.env'))
    except OSError:
        pass
