import os
from flask import Blueprint, render_template, request, url_for, flash, redirect, make_response, jsonify
import eisenradio.lib.ghetto_recorder as ghetto
from eisenradio.lib.eisdb import get_db_connection, status_read_status_set, render_picture, get_download_dir, get_post
from eisenradio.eisenutil.eisutil import make_folder
from eisenradio.eisenutil.measure import measure_meta
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            conn.execute('INSERT INTO posts (title, content, download_path, pic_data, pic_content_type, pic_comment) '
                         'VALUES (?, ?, ?, ?, ?, ?)',
                         (title, content, request_path, image, content_type, text))
...
@eisenutil_bp.route('/console_info', methods=['GET'])
def console_info():
    request_time, request_suffix, request_icy_genre, request_icy_name, request_icy_view_id, icy_br = measure_meta()
    return jsonify({'request_time': str(request_time),
                    'request_suffix': request_suffix,
                    'request_icy_genre': request_icy_genre,
                    'request_icy_name': request_icy_name,
                    'request_icy_view_id': request_icy_view_id,
                    'request_icy_br': icy_br})
