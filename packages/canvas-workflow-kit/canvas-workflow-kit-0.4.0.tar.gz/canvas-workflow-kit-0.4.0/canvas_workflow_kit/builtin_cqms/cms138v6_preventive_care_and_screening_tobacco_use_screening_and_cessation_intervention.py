...
class ClinicalQualityMeasure138v6(ClinicalQualityMeasure):
    ...
    def tobacco_cessation_intervention_medication(self) -> arrow:
        if not self.tobacco_use_screening_user:
            return None
        # @ide-format:off
        medication = (self.patient
                          .medications
                          .find(TobaccoUseCessationPharmacotherapy)
                          .intersects(self.timeframe, still_active=self.patient.active_only))  # yapf: disable
        # @ide-format:on
        if medication:
    ...
    def in_initial_population(self) -> bool:
        ...
        result = (self.patient.age_at(self.timeframe.end) >= self.MINIMUM_AGE and
                  (self.context == CONTEXT_GUIDANCE or
