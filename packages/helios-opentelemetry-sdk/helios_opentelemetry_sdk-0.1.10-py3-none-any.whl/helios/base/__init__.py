...
class HeliosBase(ABC):
    ...
    def is_running_in_databricks(self) -> bool:
        if os.environ.get(self.DATABRICKS_ENV_VAR):
    ...
    def extract_context_from_databricks(self):
        try:
            ...
            new_context = extract({"traceparent": traceparent})
            attach(new_context)
    ...
    @staticmethod
    def set_test_triggered_baggage() -> Context:
        test_triggered_trace = HeliosTags.TEST_TRIGGERED_TRACE
        return set_baggage(test_triggered_trace, test_triggered_trace)
