...
class dataListBase(list):
    ...
    def fit(self, model, freepar={}, fixpar={}, mapNames={}, method='leastsq', xslice=slice(None), condition=None,
        ...
        - Resulting errors (only for 'lastsq') are 1-sigma errors as estimated from the covariance matrix diagonal.
        ...
        **Basic examples with synthetic data.**
        ...
        - Fit for **vector valued results**
          ...
          Vectors (multidimensional results or vector Y values) like a field vector are often fitted by minimizing
        ...
        * We minimise the unbiased estimate :math:`\chi^2 = \frac{1}{n-p} \sum_i \frac{[X_i-f(X_i,a)]^2}{\sigma_i^2}`
          ...
          dependent on parameters :math:`a_i`. Differences are weigthed by measurement errors :math:`\sigma_i^2`.
        ...
        * Resulting parameter errors are 1-sigma errors as determined from the covariance matrix (see [1]_).
          ...
          - We have a well-behaved likelihood function which is asymptotically Gaussian near its maximum
            (equal to minimum in :math:`\chi^2`).
        ...
        * If errors exist (:math:`\sigma_i` = *.eY*) and are not zero, the error weighted :math:`\chi^2` is minimized.
    ...
    def estimateError(self, output=True):
        """
        Estimate error using a refined fit (``method='leastsq'``).
