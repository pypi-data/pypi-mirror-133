...
from tecton_spark.offline_store import OfflineStoreParams
...
class FeatureDefinition(Fco):
    ...
    def _delete_keys(
        self,
        keys: Union[pysparkDF, pd.DataFrame, DataFrame],
        online: bool = True,
        offline: bool = True,
    ) -> None:
        spark = TectonContext.get_instance()._spark
        ...
        if fd.is_temporal_aggregate:
            time_column = TEMPORAL_ANCHOR_COLUMN_NAME
        else:
            time_column = fd.time_key
        offline_store_params = OfflineStoreParams(
            s3_path=fd.fv_materialization.materialized_data_location.path,
        ...
        if isinstance(keys, pd.DataFrame):
            keys = spark.createDataFrame(keys)
        elif isinstance(keys, DataFrame):
            keys = keys.to_spark()
        store_writer = get_offline_store_writer(offline_store_params, fd.offline_store_config, 1, spark)
        store_writer.delete_keys(keys)
        self._delete_keys_online(keys)

    def _delete_keys_online(self, keys: pysparkDF):
        if all([not x.online_enabled for x in self._proto.materialization_state_transitions]):
            print("Not creating online store deletion task since online materialization was never enabled")
            return
        ...
        join_key_df = keys.withColumn("join_keys_array", array(*self.join_keys)).select(serialize("join_keys_array"))

        # coalesce(1) causes it to write to 1 file, but the jvm code
        # is actually robust to multiple files here
        join_key_df.coalesce(1).write.csv(s3_path)

        online_deletion_request = DeleteEntitiesRequest()
        online_deletion_request.fco_locator.id.CopyFrom(self._id_proto)
        online_deletion_request.fco_locator.workspace = self.workspace
        online_deletion_request.join_key_list_path = s3_path
        metadata_service.instance().DeleteEntities(online_deletion_request)
