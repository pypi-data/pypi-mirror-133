...
class _PandasPipelineBuilder:
    ...
    def _transformation_node_to_df_with_mock_input(
        ...
        output_df = snowflake_input_df.select(
            *snowflake_input_df.columns,
            call_udf(ondemand_udf, array_construct(*input_columns)).name(self._output_name),
        )
        return output_df
    ...
    def _generate_on_demand_udf(
        ...
        sql_str = ONDEMAND_UDF_TEMPLATE.render(
            ...
            output_type=SPARK_TO_NATIVE_TYPES[self._output_type],
            snowflake_output_type=SPARK_TO_SNOWFLAKE_TYPES[self._output_type],
            output_column=self._output_name,
