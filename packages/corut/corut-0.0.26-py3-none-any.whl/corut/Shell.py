...
def run_cmd(command, password=None, encoding='utf-8', print_output_status=True, **kwargs):
...
class _SysReader:
    ...
    def data_append(self, txt):
        self.__lock.acquire()
        self.data.append(add_date_time_to_line(txt))
        self.__lock.release()
...
class SysReader:
    ...
    def start(self):
        ...
        self.__thread = ThreadWithReturnValue(
            target=self.__sys_reader_function, name='SYS READER', daemon=True
        )
        self.__thread.start()
    ...
    def data_clean(self):
        self.__lock.acquire()
        self.data.clear()
        self.data.append(add_date_time_to_line(''))
        self.__lock.release()
