...
class TextIndexer(BaseIndexer):
    ...
    def build_vocab(self, x_data, y_data=None):
        if not self.token2id:
            token2id = self._initial_vocab_dic
            max_len = 0
            ...
            for token, token_count in token2count.items():
                if token not in token2id and token_count >= self.min_count:
                    token2id[token] = len(token2id)
            self.token2id = token2id
            self.id2token = dict([(v, k) for k, v in self.token2id.items()])

            top_k_vocab = [k for (k, v) in list(self.token2id.items())[:10]]
            logger.debug(f"--- Build vocab dict finished, Total: {len(self.token2id)} ---")
    ...
    def transform(self, samples):
        ...
        for seq in samples:
            ...
            if self.token_unk in self.token2id:
                unk_index = self.token2id[self.token_unk]
                numerized_samples.append([self.token2id.get(token, unk_index) for token in seq])
            else:
                numerized_samples.append([self.token2id[token] for token in seq])
    ...
    def inverse_transform(self, labels, *, lengths=None, threshold=0.5, **kwargs):
        ...
        for index, seq in enumerate(labels):
            ...
            for idx in seq:
                labels_.append(self.id2token[idx])
    ...
    def doc2bow(self, doc, allow_update=False, return_missing=False):
        ...
        if allow_update or return_missing:
            missing = sorted(x for x in iteritems(counter) if x[0] not in self.token2id)
            if allow_update:
                for w, _ in missing:
                    # new id = number of ids made so far;
                    # NOTE this assumes there are no gaps in the id sequence!
                    self.token2id[w] = len(self.token2id)
        result = {self.token2id[w]: freq for w, freq in iteritems(counter) if w in self.token2id}
