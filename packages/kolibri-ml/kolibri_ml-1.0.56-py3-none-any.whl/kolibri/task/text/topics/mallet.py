...
This module allows both LDA model_type estimation from a training corpus and inference of topic distribution on new,
...
The wrapped model_type can NOT be updated with new documents for online training -- use
...
class LdaMallet(utils.SaveLoad, basemodel.BaseTopicModel):
    ...
    def __init__(self, mallet_path, corpus=None, num_topics=100, alpha=50, id2word=None, workers=4, prefix=None,
        ...
        optimize_interval : int, optional
            Optimize configs every `optimize_interval` iterations
    ...
    def __getitem__(self, bow, iterations=100):
        ...
        return self.get_document_topics(bow, iterations)

    def get_document_topics(self, bow, iterations=100):
    ...
    def load_word_topics(self):
        """Load words data topics matrix from :meth:`gensim.models.wrappers.ldamallet.LdaMallet.fstate` file.
        Returns
        -------
        numpy.ndarray
            Matrix words data topics.
        """
    ...
    def get_topics(self):
        """Get topics data words matrix.
        Returns
        -------
        numpy.ndarray
            Topics data words matrix, shape `num_topics` x `vocabulary_size`.
        """
        topics = self.word_topics
        return topics / topics.sum(axis=1)[:, None]


    def get_top_topics_words(self, topn=10):
        return [self.get_top_topic_words(topic, topn=topn) for topic in range(self.num_topics)]
    ...
    def get_top_topic_words(self, topicid, topn=10, num_words=None):
        """Get `num_words` most probable words for the given `topicid`.
        Parameters
        ----------
        topicid : int
        ...
        topic = topic / topic.sum()  # normalize to probability dist
        bestn = matutils.argsort(topic, topn, reverse=True)
        best_tokens = [self.id2word[idx]  for idx in bestn]
        return best_tokens
...
def malletmodel2ldamodel(mallet_model, gamma_threshold=0.001, iterations=50):
    ...
    This works by copying the training model_type weights (alpha, beta...) from a trained mallet model_type into the gensim model_type.
    Parameters
    ----------
    mallet_model : :class:`~gensim.models.wrappers.ldamallet.LdaMallet`
        Trained Mallet model_type
