# -*- coding: utf8 -*-
# python >= 3.5
# python3 en supprimant import typing et math.inf
...
"""
        ...
        self.__fonction_transfert = acelectricity.Ratio(fw=lambda w: A/(1+2*coeff_amortissement*1j*w/w0-(w/w0)**2))
...
"""
        ...
        if 0 < m < 1:
            ...
            m0 = math.sqrt(m*m-1)
            # réponse indicielle unitaire
            return A*(1-(0.5/m0)*(math.exp(-w0*(m-m0)*t)/(m-m0)-math.exp(-w0*(m+m0)*t)/(m+m0)))
...
1550.27045...
"""
        return self.abaque_frequence_resonance_harmonique(self.__coeff_amortissement, self.__w0)
...
"""
        return self.abaque_facteur_resonance_harmonique(self.__coeff_amortissement)
...
2427.97905105...
"""
        return self.abaque_frequence_coupure_harmonique(self.__coeff_amortissement, self.__w0)
...
"""
    ...
    @staticmethod
    def abaque_coeff_amortissement_harmonique(facteur_resonance_db: float) -> float:
...
"""
        ...
        if facteur_resonance_db > 1000:
            # TODO :
            # développement limité
            return 0.0

        # résolution à partir de la fonction inverse
        # !calcul aberrant quand facteur_resonance_db > 1000 :
        # on obtient 0.707 au lieu de 0!
        # def facteur_resonance(coeff_amortissement):
        #    m = coeff_amortissement
        #    f = 1/(2*m*math.sqrt(1-m*m))
        #    return 20*math.log10(f)
        # return _rechercheSolution(1e-300, 2**-0.5, facteur_resonance_db, 1e-9,
        # facteur_resonance)
    ...
    @staticmethod
    def abaque_facteur_resonance_harmonique(coeff_amortissement: float) -> tuple:
        """ En régime harmonique, retourne le facteur de résonance \
sans unité et en dB (tuple)
Retourne 1 et 0 dB si le coefficient d'amortissement est >= 1/√2 \
(pas de résonance)
...
"""
        ...
        if m < 0:
            raise ValueError("Le coefficient d'amortissement doit être positif")
        if m == 0:
            return math.inf, math.inf  # python >= 3.5
    ...
    @staticmethod
    def abaque_pulsation_propre_resonance_harmonique(coeff_amortissement: float, frequence_resonance: float) -> float:
...
"""
    ...
    @staticmethod
    def abaque_frequence_resonance_harmonique(coeff_amortissement: float, w0: float) -> float:
        """ En régime harmonique, retourne la fréquence de résonance \
(en hertz)
Retourne 0 si le coefficient d'amortissement est >= 1/√2 \
(pas de résonance)
...
entre 0 et 100 %")
        ...
        if m == 0:
            ...
            def u(t):
                # réponse indicielle unitaire
                return 1-(0.5/m0)*(math.exp(-w0*(m-m0)*t)/(m-m0)-math.exp(-w0*(m+m0)*t)/(m+m0))
            # recherche solution
            # solution tr 5 % # 4.74/w0 pour m=1
            # solution tr 1 % # 6.64/w0 pour m=1
...
"""
        if 0 <= depassement <= 100:
            if depassement == 0:
                ...
                # résolution à partir de la fonction inverse
                # return _rechercheSolution(0, 1, depassement, 1e-9,
                #                           Ordre2.abaque_depassement)

                # solution exacte !
                return -math.log(depassement/100)/math.sqrt(math.pi**2+math.log(depassement/100)**2)
