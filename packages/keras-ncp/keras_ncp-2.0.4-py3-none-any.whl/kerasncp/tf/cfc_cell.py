...
class CfcCell(tf.keras.layers.AbstractRNNCell):
    ...
    def build(self, input_shape):
        ...
        if self.mode == "pure":
            self.ff1 = tf.keras.layers.Dense(self.state_size, name="ff1")
            if self.sparsity_mask is not None:
                self.ff1.build((None, self.sparsity_mask.shape[0]))
            ...
            self.ff1 = tf.keras.layers.Dense(
                self.state_size, self._activation, name="ff1"
            )
            self.ff2 = tf.keras.layers.Dense(
                self.state_size, self._activation, name="ff2"
            )
            if self.sparsity_mask is not None:
                self.ff1.build((None, self.sparsity_mask.shape[0]))
    ...
    def call(self, inputs, states, **kwargs):
        ...
        if self.sparsity_mask is not None:
            ff1_kernel = self.ff1.kernel * self.sparsity_mask
            ff1_bias = self.ff1.bias
            ff1 = tf.matmul(x, ff1_kernel) + ff1_bias
        else:
            ff1 = self.ff1(x)
        if self.mode == "pure":
            # Solution
            ...
            # Cfc
            if self.sparsity_mask is not None:
                ff2_kernel = self.ff2.kernel * self.sparsity_mask
                ff2_bias = self.ff2.bias
                ff2 = tf.matmul(x, ff2_kernel) + ff2_bias
            else:
                ff2 = self.ff2(x)
            t_a = self.time_a(x)
            t_b = self.time_b(x)
