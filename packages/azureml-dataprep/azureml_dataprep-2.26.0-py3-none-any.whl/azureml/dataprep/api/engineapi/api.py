...
def _try_rslex_upload(destination, source_path, base_dir, overwrite, is_volume=False):
    ...
    try:
        ...
        if overwrite != True:
            import warnings
            warnings.warn('Rslex upload is experimental and only supports overwrite mode for now.\nFalling back to clex.\nPlease set overwrite=True to use rslex based upload.')
            logger.warn('Unable to use rslex upload because overwrite flag is not set. Using clex upload.')
            return False
        ...
        with tracer.start_as_current_span('_try_rslex_upload', trace.get_current_span()):
            ...
            copier = Copier(
                destination_info,
                base_dir,
                PyIfDestinationExists.MERGE_WITH_OVERWRITE if overwrite else PyIfDestinationExists.FAIL)
