...
class CustomDatasetV2(Dataset):
    ...
    def get_oversample_patch(self, images, seg):
        ...
        max_coords = np.max(non_zero_coords, axis = 1) + 1
        min_coords = np.min(non_zero_coords, axis = 1)

        oversample_high = np.minimum(max_coords, shape_to_index - self.patch_size//2)
        oversample_high = np.maximum(oversample_high, self.patch_size // 2)
        oversample_low = np.minimum(min_coords, shape_to_index - self.patch_size//2)
        oversample_low = np.maximum(oversample_low, self.patch_size // 2)
        oversample_high[oversample_high == oversample_low] += 1

        print("\n oversample high : ", oversample_high, "oversample low : ", oversample_low, 'image shape : ', images.shape, 'patch size : ', self.patch_size)
        oversample_center = np.random.randint(low = oversample_low, high = oversample_high + 1)

        oversample_patch_lower_bound = np.maximum(oversample_center - (self.patch_size/2) -1 , [0, 0, 0])
        oversample_patch_upper_bound = oversample_patch_lower_bound + self.patch_size

        print("upper bound :", oversample_patch_upper_bound, "\nlower bound :",oversample_patch_lower_bound)
        ...
        print('oversmaple shape : ', oversample_patch_images.shape)
    ...
    def get_normal_patch(self, images, seg):
        ...
        shape_to_index = np.array(images[0].shape) - 1 
        low = self.patch_size/2
        high = np.maximum(low, shape_to_index - (self.patch_size/2) + 1 )
        high[high == low] += 1
        print("\n normal high : ", high, "normal low : ", low, 'image shape : ', images.shape)

        normal_center = np.random.randint(low = low, high = high)
        normal_patch_upper_bound = normal_center + (self.patch_size/2)
        normal_patch_lower_bound = normal_center - (self.patch_size/2)
        print("upper bound :", normal_patch_upper_bound, "\nlower bound :",normal_patch_lower_bound)
        ...
        print("normal shape : ", normal_patch_images.shape)
    ...
    def pad_image_seg(self, images, seg): #patch size보다 전체 이미지 크기가 작을 경우 padding을 진행
        ...
        images = F.pad(images.float(), pad_shape.tolist()[::-1], "constant", self.pad_val)
        seg = F.pad(seg.float(), pad_shape.tolist()[::-1], "constant", 0)
        print('\npadding image shape :', images.shape, 'seg shape : ', seg.shape, '\nbefore shape :', image_shape)
        return images, seg
    ...
    def __getitem__(self, idx):
        ...
        if np.random.uniform() > 0.5 or torch.unique(y).shape[0] == 1:
            result_images, result_seg = self.get_normal_patch(x, y)
        else:
            result_images, result_seg = self.get_oversample_patch(x, y)
