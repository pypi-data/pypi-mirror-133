...
class Clipping_Normalizer(object):
...
class ResamplingV2(object):
    def __init__(self, anisotropy_axis_index = [], target_spacing = [1.0, 1.0, 1.0], random = False, debug_shape = False,
                 device = None, spacing_order = [-3, -2, -1], do_check = False, check_threshold = 120):
        self.device = device
        self.anisotropy_axis_index = self.new_anisotropy_axis(anisotropy_axis_index)
        self.target_spacing = np.array(target_spacing)
        self.random = random
    ...
    def correct_dims(self, shape):
        shape = np.array(shape)
        if np.any(shape) < self.check_threshold:
            shape[np.where(shape < self.check_threshold)] = np.max(shape) // 2
        return tuple(shape)
    ...
    def __call__(self, img, info_data, seg = None):
        ...
        if self.do_check:
            new_shape = self.correct_dims(new_shape)
        ...
        if self.debug_shape:
            print("img shape : ", img.shape, "seg shape : ", seg.shape)
            img_shape = np.array(img.shape[-3:])
            seg_shape = np.array(seg.shape[-3:])
            
            if np.any(img_shape != seg_shape):
                print("Waring, Resampling was wrong")
