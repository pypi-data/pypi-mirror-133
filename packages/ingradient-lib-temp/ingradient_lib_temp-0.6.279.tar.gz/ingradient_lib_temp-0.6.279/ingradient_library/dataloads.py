...
class CustomDatasetV2(Dataset):
    ...
    def get_oversample_patch(self, images, seg):
        ...
        print('\n max : ', max_coords, '\n min : ', min_coords)
        oversample_high = np.clip(max_coords, self.patch_size // 2 + 1, shape_to_index - self.patch_size/2)
        oversample_low = np.clip(min_coords, self.patch_size // 2 + 1,  shape_to_index - self.patch_size/2)
