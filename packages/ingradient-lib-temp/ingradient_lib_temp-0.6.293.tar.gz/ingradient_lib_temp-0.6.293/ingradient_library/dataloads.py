...
class CustomDatasetV2(Dataset):
    def __init__(self, images_dir, label_dir = None, normalizer = None, mode = 'train', dim = '3d', correct_direction = False, resample = None, do_random_permute = False,
        ...
        if self.nifti:
            ...
            self.labels_list = sorted(os.listdir(label_dir))
    ...
    def __getitem__(self, idx):
        ...
        plt.imshow(seg[img.shape[0]//2])
        plt.show()
        plt.imshow(seg[:,img.shape[1]//2])
        plt.show()
        plt.imshow(seg[:,:,img.shape[2]//2])
        plt.show()
