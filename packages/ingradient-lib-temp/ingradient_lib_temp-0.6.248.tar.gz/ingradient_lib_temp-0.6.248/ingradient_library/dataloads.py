...
class CustomDatasetV2(CustomDataset):
    def __init__(self, images_dir, label_dir = None, device = None, normalizer = None, mode = 'train', dim = '3d', correct_direction = False, resample = None,
    ...
    def pad_image_seg(self, images, seg): #patch size보다 전체 이미지 크기가 작을 경우 padding을 진행
        ...
        if np.any(image_shape < self.patch_size):
            ...
            images = F.pad(images, even_pad, "constant", self.pad_val)
            seg = F.pad(seg, even_pad, "constant", self.pad_val)
