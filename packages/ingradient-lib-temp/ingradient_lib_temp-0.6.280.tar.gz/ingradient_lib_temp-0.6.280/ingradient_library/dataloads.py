...
class CustomDatasetV2(Dataset):
    ...
    def get_oversample_patch(self, images, seg):
        ...
        oversample_high = np.maximum(max_coords, self.patch_size // 2)
        oversample_high = np.minimum(oversample_high, shape_to_index - self.patch_size//2)
        oversample_low = np.maximum(min_coords, self.patch_size // 2)
        oversample_low = np.minimum(oversample_low, shape_to_index - self.patch_size//2)
        ...
        oversample_patch_upper_bound = oversample_center + (self.patch_size/2)
        oversample_patch_lower_bound = oversample_center - (self.patch_size/2)
