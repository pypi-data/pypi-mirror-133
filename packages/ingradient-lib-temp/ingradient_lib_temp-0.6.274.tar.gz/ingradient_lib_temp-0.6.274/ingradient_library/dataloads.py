...
class CustomDatasetV2(Dataset):
    ...
    def get_oversample_patch(self, images, seg):
        ...
        oversample_high = np.clip(np.max(non_zero_coords, axis = 1) + 1, self.patch_size/2, shape_to_index - self.patch_size/2)
        oversample_low = np.clip(np.min(non_zero_coords, axis = 1), self.patch_size/2, shape_to_index - self.patch_size/2)
