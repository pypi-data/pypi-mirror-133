...
# API Documents
from cnside.parsers import PIPInstallParser, NPMInstallParser, CLIPackageNameParam, PackageParamType
...
class AnalyzeRequestDoc(RequestDocument):
    ...
    def __init__(self, package_manager: Text, packages: List[UnifiedPackageID] = None, install_manifest: bool = None,
                 ...
                 project: Text = "default", analyzer_version: int = None):
        ...
        if self.install_manifest:
            assert all([self.manifest, self.lockfile]), "manifest param is required when install_manifest param is set"
...
class CLIParsedCommand:
    ...
    def _extract_yarn_packages(self) -> List[CLIPackageNameParam]:
        # TODO: add parser
        rv = [CLIPackageNameParam(data=metadata.packages.ManifestNames.YARN, typ=PackageParamType.MANIFEST)]
        return rv
