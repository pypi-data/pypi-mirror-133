...
def main():
    ...
    arg_parser.add_argument('--init', action='store_true',
                            help='Generate default configuration and export to ~/.config/powerline-shell/config.json')
    ...
    if args.init:
        CONFIG_FILE = os.path.expanduser('~/.config/powerline-shell/config.json')
        if os.path.exists(CONFIG_FILE):
            print("{0} already exists, aborting".format(CONFIG_FILE))
            sys.exit(1)
        else:
            ...
            print("{0} written".format(CONFIG_FILE))
            print("Add the following line to your .bashrc or .bash_profile:\n {}".format(INIT_BASH))
            sys.exit(0)
