...
class SC(CODE):
        ...
        if not error_probability is None:
            if BITWISE:
                ...
                if self.ML_DECODING_QUBITS_LIMIT<self.n:
                    if OUTPUT_LOG:
                        print("Warning: The num. of qubit exceed ML_DECODING_QUBITS_LIMIT.")
                    self.blockwise_error_probability = False
                else:
                    self.blockwise_error_probability = bitwise_to_blockwise_error_probability(error_probability) # By approximate the bitwise probability.
            else:
                self.blockwise_error_probability = error_probability
                self.bitwise_error_probability = blockwise_to_bitwise_error_probability(error_probability)
        else:
            if OUTPUT_LOG:
    ...
    def in_S(self,b):
        return sum(gaussjordan(np.append(self.H, b).reshape(self.nk+1,X_OR_Z * self.n))[self.nk])==0
