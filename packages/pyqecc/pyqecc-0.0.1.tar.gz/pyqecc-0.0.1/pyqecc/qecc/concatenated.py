...
class ParaCode(SC):
    ...
    def get_T(self,beta):
        ...
        for c in reversed(self.code_instances):
            ...
            T[nind - c.n+self.n:nind+self.n] = T_child[c.n:X_OR_Z*c.n]#Zを代入．
    ...
    def ML_decode(self,syndrome,**param):
        ...
        for c in self.code_instances:
            c.set_error_probability(self.bitwise_error_probability[nind:nind+c.n],iid=False)
            ...
            logical_error_probability[kind:kind+c.k][:] = blockwise_to_bitwise_error_probability(c_logical_error_probability)
        ...
        if self.BITWISE:
            ...
            self.decoder_output["LOGICAL_ERROR_PROBABILITY"] = bitwise_to_blockwise_error_probability(logical_error_probability)
    @property
...
class ConcCode(SC):
    def __init__(self,code_instances,P=None,BITWISE = True,mode="BP"): #,interleaver={}
        ...
        self.H_depth = {}
        super().__init__(self.code_instances[self.code_depth-1].n,self.code_instances[0].k,H=self.calc_grobal_H(),P=P,BITWISE = BITWISE,mode = mode)
