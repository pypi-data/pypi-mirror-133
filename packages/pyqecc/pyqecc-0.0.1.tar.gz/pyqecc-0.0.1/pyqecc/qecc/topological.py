...
def gen_torus(d1,d2):
    ...
    for i in range(d1):
        for j in range(d2):
            x = R*np.cos(2*np.pi*t[i])+r*np.cos(2*np.pi*p[j])*np.cos(2*np.pi*t[i])
            y = R*np.sin(2*np.pi*t[i])+r*np.cos(2*np.pi*p[j])*np.sin(2*np.pi*t[i])
    ...
    return np.r_[[np.arange(len(V))],np.array(V).T].T,np.r_[[np.arange(len(E))],np.array(E).T].T,np.array(F)
