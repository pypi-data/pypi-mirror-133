...
import string
from typing import Sequence, Any, Callable, MutableSet, Tuple, Set
...
from nv.utils.parsers.parser import (build_parsing_map, build_parser_from_map, as_element_parser,
                                     ...
                                     InternalParser)
from nv.utils.collections import MissingKey as _MissingKey, SafeDictWrapper, SafeDictWrapperWithTracker


__ALL__ = ['StringFormatter', 'safe_format', 'format_string', 'format_string_with_tracker', 'definitive_format', ]
...
class StringFormatter:
    ...
    def __init__(self,
        ...
        self.format = build_parser_from_map(parsing_map,
                                            ...
                                            wrapper=partial(StringFormatter.CleanUp.cleanup, behaviour=clean_up),
...
def format_string(obj: Any,
                  mapping: Mapping[str, str | None],
                  missing_key: StringFormatter.MissingKey.Behaviour = StringFormatter.MissingKey.Behaviour.RETURN_PLACEHOLDER,
                  missing_keys_set: MutableSet[str] | None = None,
                  clean_up: StringFormatter.CleanUp.Behaviour = StringFormatter.CleanUp.Behaviour.EMPTY,
                  format_keys: bool = False,
                  ):
...
def format_string_with_track(obj: Any,
                             mapping: Mapping[str, str | None],
                             missing_key: StringFormatter.MissingKey.Behaviour = StringFormatter.MissingKey.Behaviour.RETURN_PLACEHOLDER,
                             clean_up: StringFormatter.CleanUp.Behaviour = StringFormatter.CleanUp.Behaviour.EMPTY,
                             format_keys: bool = False,
    ...
    return formatter.format(obj), formatter.missing_keys


def definitive_format(obj: Any,
                      mapping: Mapping[str, str | None],
                      clean_up: StringFormatter.CleanUp.Behaviour = StringFormatter.CleanUp.Behaviour.EMPTY,
                      format_keys: bool = False,
                      ) -> str:
    ...
    formatter = StringFormatter(mapping,
                                clean_up=clean_up,
                                format_keys=format_keys,
                                missing_key=StringFormatter.MissingKey.Behaviour.RAISE,
                                track=False,
                                )
    return formatter.format(obj)
