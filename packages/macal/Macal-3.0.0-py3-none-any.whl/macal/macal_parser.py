...
from .macal_lextokentypes import (EOF, REF, FSTR, LPAREN, RPAREN, IDENTIFIER, KEYWORD, LINDEX, 
                                 RINDEX, COMMA, SEMICOLON, LBRACKET, RBRACKET, OP_NOTEQUAL, 
                                 OP_EQUAL, OP_GT, OP_GTEQUAL, OP_LT, OP_LTEQUAL, OP_SUB, OP_ADD, 
                                 OP_DIV, OP_MUL, OP_NOT, OP_DEFINE, OP_ASSIGN, NEW_ARRAY_INDEX,
                                 COMMENT, OP_AND, OP_OR)
...
class MacalParser:
    ...
    def _parse_instruction(self):
        ...
        if peek.token_type == IDENTIFIER:
            ...
            result = self._parse_instruction() # but we still need to return a valid value.
        if result is None:
    ...
    def parse(self, tokens):
        ...
        root = ast_Block(None)
        while not self._eof():
            root.add_instruction(self._parse_instruction())
        return root
