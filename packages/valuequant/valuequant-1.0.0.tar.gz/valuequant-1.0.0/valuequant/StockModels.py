...
class company_model:
    ...
    def __getincdata(self):
        res=getcompanydata(stockcode=self._mparams['stockcode'],reportdate=self._mparams['reportdate'],dimension=self._mparams['dimension'],adjdate=self._mparams['adjdate'])
        if type(res)==dict:
            data = pd.Series(res)
...
class strict_latest_status_model(company_model):
    ...
    def __init__(self,stockcode,dimension,reportdate,minstep,significance,seasonal=False,anneal=None,conservative=False,adjdate=None):
        mparams=locals()
        del mparams['self']
        del mparams['__class__']
        super().__init__(mparams)

    def resetparams(self,stockcode,dimension,reportdate,minstep,significance,seasonal=False,anneal=None,conservative=False,adjdate=None):
        mparams=locals()
        del mparams['self']
        del mparams['__class__']
...
class long_range_status_model(company_model):
    ...
    def __init__(self,stockcode,dimension,reportdate,significance,seasonal=False,anneal=None,samplerange=None,conservative=False,adjdate=None):
        mparams=locals()
        del mparams['self']
        del mparams['__class__']
        super().__init__(mparams)

    def resetparams(self,stockcode,dimension,reportdate,significance,seasonal=False,anneal=None,samplerange=None,conservative=False,adjdate=None):
        mparams=locals()
        del mparams['self']
        del mparams['__class__']
        super().resetparams(mparams)
...
class strict_latest_long_range_model(company_model):
    ...
    def __init__(self,stockcode,dimension,reportdate,minstep,significance,inertia,seasonal=False,anneal=None,samplerange=None,adjdate=None):
        mparams=locals()
        del mparams['self']
        del mparams['__class__']
        super().__init__(mparams)

    def resetparams(self, stockcode,dimension,reportdate,minstep,significance,inertia,seasonal=False,anneal=None,samplerange=None,adjdate=None):
        mparams = locals()
        del mparams['self']
        del mparams['__class__']
        super().resetparams(mparams)
...
class series_shape_model(company_model):
    ...
    def __init__(self,stockcode,dimension,reportdate,significance,seasonal=False,conservative=False,adjdate=None):
        mparams=locals()
        del mparams['self']
        del mparams['__class__']
        super().__init__(mparams)

    def resetparams(self,stockcode,dimension,reportdate,significance,seasonal=False,conservative=False,adjdate=None):
        mparams = locals()
        del mparams['self']
        del mparams['__class__']
        super().resetparams(mparams)
...
class ma_strict_latest_status_model(company_model):
    ...
    def __init__(self,stockcode,dimension,reportdate,mawindow,minstep,seasonal=False,anneal=None,conservative=False,adjdate=None):
        mparams=locals()
        del mparams['self']
        del mparams['__class__']
        super().__init__(mparams)

    def resetparams(self,stockcode,dimension,reportdate,mawindow,minstep,seasonal=False,anneal=None,conservative=False,adjdate=None):
        mparams = locals()
        del mparams['self']
        del mparams['__class__']
        super().resetparams(mparams)
