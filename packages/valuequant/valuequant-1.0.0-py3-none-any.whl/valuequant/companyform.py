...
def formatmodelparams(mfunc,mparams):
    ...
    return {'stockcode':stockcode,'dimension':dimension,'reportdate':reportdate,'adjdate':adjdate,'mcate':modelcate[mfunc],'mfunc':mfunc,'mparams':mparams}
...
def formatmodelsetparams(mfunc,mparams):
    ...
    adjdate = mparams.pop('adjdate')
    mfunc=mfunc[:-3]
    return {'stockcode':stockcode,'structure':structure,'reportdate':reportdate,'adjdate':adjdate,'mcate':modelcate[mfunc],'mfunc':mfunc,'mparams':mparams}
...
def formatmodelxparams(model,mparams,mfunc):
    ...
    reportdate=mparams.pop('reportdate')
    adjdate = mparams.pop('adjdate')
    return {'model':model,'stockcode':stockcode,'reportdate':reportdate,'adjdate':adjdate,'mcate':modelcate[mfunc]}
...
def getcompanydata(stockcode,reportdate,dimension,adjdate=None):
    fparams=locals()
    try:
