...
def test_linear_dimline_all_arrow_style(drawing, tmpdir):
    ...
    filename = str(
        tmpdir.join("all_dimline_styles_{}.dxf".format(drawing.dxfversion))
    )
    try:
...
def test_random_multi_point_linear_dimension(tmpdir):
    ...
    msp.add_multi_point_linear_dim(
        base=(0, length + 2), points=points, dimstyle="WITHTFILL"
    )
    msp.add_multi_point_linear_dim(
        base=(-2, 0), points=points, angle=90, dimstyle="WITHTFILL"
    )
    msp.add_multi_point_linear_dim(
        base=(10, -10), points=points, angle=45, dimstyle="WITHTXT"
    )
...
def test_draw_all_arrows(drawing, tmpdir):
    ...
    filename = str(
        tmpdir.join("draw_all_arrows_{}.dxf".format(drawing.dxfversion))
    )
    try:
        drawing.saveas(filename)
    except ezdxf.DXFError as e:
