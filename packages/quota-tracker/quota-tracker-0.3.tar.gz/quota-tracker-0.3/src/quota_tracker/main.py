...
class QuotaTracker(object):
    ...
    def is_exceeded_quota(self):
        ...
        current_quota = self._load_current_quota()
    ...
    def _validate_quota(self):
        if self.get_current_quota() < self.options.warning_rate * float(self.max_quota):
            self.logger.info("QuotaStatus: OK")
        elif self.get_current_quota() < self.max_quota:
