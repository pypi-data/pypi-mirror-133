...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=oEHwr(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not oEHwv
   return result
  except oEHwj:
   props=self.props
   ...
   if parts[0]in candidates and oEHwm(parts)>1:
 ...
 def fetch_details(cls,cluster_name):
  ...
  return(match or[oEHwv])[0]
 @oEHwl
 def get_deploy_templates():
  return{"create":{"function":"create_cache_cluster","parameters":lambda_convert_types(lambda_rename_attributes({"ClusterName":"CacheClusterId","VpcSecurityGroupIds":"SecurityGroupIds"}),{"NumCacheNodes":oEHwD})},"delete":{"function":"delete_cache_cluster","parameters":{"CacheClusterId":"ClusterName"}}}
