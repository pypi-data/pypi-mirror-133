...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or Vmihs
 @Vmiht
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return VmihA(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @VmihQ
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if Vmihk(origins,Vmihv)else origins.get("Items")
  origins_to_create=Vmihd([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if Vmihk(aliases,Vmihv)else aliases.get("Items")
  aliases_to_create=Vmihd(aliases)
  for distr in distrs:
   origins=Vmihd([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=Vmihd(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",Vmihx)
   config["Enabled"]=Vmihr(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if Vmihk(custom_errors,VmihW)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=Vmihr(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":Vmihx}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=Vmihr(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=Vmihs):
    parent=config if parent is Vmihs else parent
    items=parent.get(key)or[]
    if Vmihk(items,VmihW)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":Vmihu(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if Vmihk(cache_behavs,VmihW)else cache_behavs)
