...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[eSVnK])[0]
 def get_physical_resource_id(self,attribute=eSVnK,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if eSVnu(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return eSVnh(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @eSVnH
 def add_defaults(resource,stack_name:eSVns):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return eSVnh(SESReceiptRule,self).get_cfn_attribute(attribute)
 @eSVnI
