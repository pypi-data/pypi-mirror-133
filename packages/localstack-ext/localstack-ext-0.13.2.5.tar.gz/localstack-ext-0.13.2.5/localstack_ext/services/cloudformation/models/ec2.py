...
class EC2ElasticIP(GenericBaseModel):
 ...
 def cloudformation_type():
  return "AWS::EC2::EIP"
 def get_physical_resource_id(self,attribute=mXfxz,**kwargs):
...
class SecurityGroupInOrEgress(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  for perm in perms:
   if mXfxq(perm["IpProtocol"])!=mXfxq(rp["IpProtocol"]):
    continue
 ...
 @mXfxt
 def get_vpc_and_name_for_security_group(cls,group_id:mXfxq)->Tuple[mXfxq,mXfxq]:
  client=aws_stack.connect_to_service("ec2")
  groups=client.describe_security_groups(GroupIds=[group_id])["SecurityGroups"]
  vpc_id=groups and groups[0].get("VpcId")or mXfxz
  group_name=groups and groups[0]["GroupName"]or mXfxz
 ...
 def get_deploy_templates(cls):
  def create_params(params,**kwargs):
   ...
   source_group_id=result.pop("SourceSecurityGroupId",mXfxz)
   vpc_id=mXfxz
   ...
   dst_group_id=result.pop("DestinationSecurityGroupId",mXfxz)
   ...
   if result.get("IpProtocol"):
    result["IpProtocol"]=mXfxq(result.get("IpProtocol"))
   description=result.pop("Description",mXfxz)
   cidr_ipv6=result.pop("CidrIpv6",mXfxz)
