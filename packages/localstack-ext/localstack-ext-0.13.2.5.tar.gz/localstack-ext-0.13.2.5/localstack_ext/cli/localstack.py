...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return XqWoP if e else XqWoK
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->XqWov:
...
@click.option("--revert",is_flag=XqWoP,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:XqWox):
...
def _cpvcs_initialized(pod_name:XqWoh)->XqWox:
...
@click.option("-n","--name",help="Name of the cloud pod",required=XqWoP)
def cmd_cpvcs_init(name:XqWoh):
...
@click.option("-n","--name",help="Name of the cloud pod",required=XqWoP)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=XqWoP,default=XqWoK)
def cmd_cpvcs_delete(name:XqWoh,remote:XqWox):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=XqWoP)
def cmd_cpvcs_register_remote(name:XqWoh):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=XqWoP)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=XqWoP)
def cmd_cpvcs_rename(name:XqWoh,new_name:XqWoh):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=XqWoP)
def cmd_cpvcs_commit(message:XqWoh,name:XqWoh):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=XqWoP,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("--three-way",is_flag=XqWoP,default=XqWoK,help="")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=XqWoP)
def cmd_cpvcs_push(squash:XqWox,message:XqWoh,name:XqWoh,three_way:XqWox):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=XqWoP)
@click.option("-v","--version",type=XqWoY)
@click.option("-m","--message",required=XqWoK)
def cmd_cpvcs_push_overwrite(version:XqWoY,message:XqWoh,name:XqWoh):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=XqWoY,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=XqWoP,default=XqWoK,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=XqWoP)
def cmd_cpvcs_inject(version:XqWoY,reset:XqWox,name:XqWoh):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--inject/--no-inject",default=XqWoP,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=XqWoP,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=XqWoP)
@click.option("--lazy/--eager",default=XqWoP,help="Will only fetch references to existing versions, i.e. version state is only downloaded when required")
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:XqWoh,inject:XqWox,reset:XqWox,lazy:XqWox):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=XqWoP,default=XqWoK)
def cmd_cpvcs_list_pods(remote:XqWox):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=XqWoP)
def cmd_cpvcs_versions(name:XqWoh):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=XqWoP,type=XqWoY)
@click.option("-n","--name",help="Name of the cloud pod",required=XqWoP)
def cmd_cpvcs_version_info(version:XqWoY,name:XqWoh):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=XqWoY,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=XqWoP)
def cmd_cpvcs_version_metamodel(version:XqWoY,name:XqWoh):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=XqWoP,type=XqWoY,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=XqWoP,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=XqWoP,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=XqWoK,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=XqWoP)
def cmd_cpvcs_set_version(version:XqWoY,inject:XqWox,reset:XqWox,commit_before:XqWox,name:XqWoh):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=XqWoP)
def cmd_cpvcs_commits(version:XqWoY,name:XqWoh):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=XqWoP)
@click.option("--commit","-c",required=XqWoP)
@click.option("-n","--name",help="Name of the cloud pod",required=XqWoP)
def cmd_cpvcs_commit_diff(version:XqWoY,commit:XqWoY,name:XqWoh):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(XqWov)
  status.stop()
...
def print_table(columns:Dict[XqWoh,XqWoh],rows:List[Dict[XqWoh,Any]],formatters:Dict[XqWoh,Callable[[Any],XqWoh]]=XqWov):
 from rich.table import Table
 if formatters is XqWov:
  formatters=XqWoB()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  XqWob(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->XqWov:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->XqWov:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->XqWov:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->XqWov:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
