...
def install_package(packages:Union[List[MdxJT],MdxJT],api_name:MdxJT):
 if not(is_debian()and in_docker()):
  ...
  return
 if MdxJi(packages,MdxJT):
  packages=packages.split()
 LOG.info("Downloading dependencies for %s API. This may take a while."%api_name)
 with INSTALL_LOCK:
  run(["apt-get","update"],shell=MdxJY)
  run(["apt-get","install","-y","--no-install-recommends"]+packages,shell=MdxJY)
def check_or_install(command:MdxJT,packages:Union[List[MdxJT],MdxJT],api:MdxJT):
...
def setup_ssl_cert():
 ...
 try:
  ...
  try:
   ...
   raise
def download_github_artifact(url:MdxJT,target_file:MdxJT,timeout:int=MdxJn):
 def do_download(url,print_error=MdxJY):
  try:
