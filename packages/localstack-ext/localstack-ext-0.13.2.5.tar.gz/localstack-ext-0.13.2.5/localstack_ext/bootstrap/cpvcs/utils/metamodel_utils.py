...
CAPTURE_METAMODEL_SEPARATELY=VfgPA
...
class MetamodelDelta(SubtypesInstanceManager):
 def create_delta_for_state_files(self,sf1:StateFileRef,sf2:StateFileRef):
  ...
  if not sf1:
   backend2=load_persisted_object(config_context.get_obj_file_path(sf2.hash_ref))
   clazz=VfgPX(backend2)
   backend1=_infer_backend_init(clazz,sf2)
  else:
   backend1=load_persisted_object(config_context.get_obj_file_path(sf1.hash_ref))
   clazz=VfgPX(backend1)
   backend2=_infer_backend_init(clazz,sf1)
  diff=self.create_delta_json(backend1,backend2)
  return diff
 def create_delta_log(self,state_from:Set[StateFileRef],state_to:Set[StateFileRef])->VfgPy:
  ...
  def _create_sf_lookup(state_files:Set[StateFileRef])->Dict[VfgPy,StateFileRef]:
  ...
  for state_file_from in state_files_from_regular:
   ...
   if state_file_from.any_congruence(state_files_to_regular):
    ...
    diff_json=self.create_delta_for_state_files(state_file_from,VfgPi)
  ...
  for state_files_to in state_files_to_lookup.values():
   ...
   diff_json=self.create_delta_for_state_files(VfgPi,state_files_to)
...
class CommitMetamodelUtils:
 @VfgPY
 def create_metadata_archive(cls,version:Version,delete_reference:bool=VfgPA,overwrite:bool=VfgPA,metamodels_file:VfgPy=VfgPi):
 ...
 def get_metamodel_delta(cls,prev_metamodel:Dict,this_metamodel:Dict)->Dict:
  ...
  return result
 @VfgPY
 def create_delta_metamodel_file(cls,version:VfgPG,revision:VfgPG,store_to_zip=VfgPA)->Tuple[Dict,Dict]:
 ...
 @VfgPY
 def reconstruct_metamodel(cls,version:VfgPG,revision:VfgPG=VfgPi)->Dict:
  result={}
  for rev in VfgPl(1,revision+1):
  ...
  return result
 @VfgPY
 def get_version_metamodel(cls,version:VfgPG,revision:VfgPG=VfgPi)->Dict:
 ...
 @VfgPY
 def get_commit_diff(cls,version_no:VfgPG,commit_no:VfgPG)->Optional[Dict]:
  ...
  return result
def _infer_backend_init(clazz:Type,sf:StateFileRef)->VfgPK:
 if VfgPm(clazz,VfgPd):
  return{}
 constructor=VfgPT(clazz,"__init__",VfgPi)
...
def _create_metamodel_helper(obj,width=25,visited:set=VfgPi):
 if obj is VfgPi:
  return obj
