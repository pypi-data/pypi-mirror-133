...
class CPVCSConfigContext:
 ...
 def get_version_state_archive_path(self,version,with_format:BpIEe=BpIEJ)->BpIEc:
 ...
 def update_ver_log(self,author:BpIEc,ver_no:BpIEt,rev_id:BpIEc,rev_no:BpIEt):
  with BpIEo(self.get_ver_log_path(),"a")as fp:
   fp.write(f"{VER_LOG_STRUCTURE.format(author=author, ver_no=ver_no, rev_rid_no=f'{rev_id}_{rev_no}')}\n")
 def create_version_symlink(self,name:BpIEc,key:BpIEc=BpIEG)->BpIEc:
  return self._create_symlink(name,key,self.get_ver_refs_path())
 def create_revision_symlink(self,name:BpIEc,key:BpIEc=BpIEG)->BpIEc:
  return self._create_symlink(name,key,self.get_rev_refs_path())
 def is_initialized(self)->BpIEe:
  return self.pod_root_dir and os.path.isdir(self.pod_root_dir)
 def _create_symlink(self,name:BpIEc,key:BpIEc,path:BpIEc)->BpIEc:
...
def zip_directories(zip_dest:BpIEc,directories:List[BpIEc])->BpIEc:
 ...
 return zip_dest
def add_file_to_archive(archive:BpIEc,entry_name:BpIEc,content:Union[BpIEc,BpIEj]):
 with zipfile.ZipFile(archive,"a")as zip_file:
  zip_file.writestr(entry_name,content)
def read_file_from_archive(archive_path:BpIEc,file_name:BpIEc)->BpIEc:
 try:
