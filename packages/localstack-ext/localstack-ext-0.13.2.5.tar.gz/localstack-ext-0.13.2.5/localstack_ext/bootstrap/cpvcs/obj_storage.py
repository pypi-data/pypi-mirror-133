...
class StateFileLocator(abc.ABC):
 location="_none_"
 active_instance=RUrvs
 @abc.abstractmethod
 def get_state_file_location_by_key(self,key:RUrvE)->RUrvE:
  pass
 @RUrvh
 def get(cls,requested_file_locator:RUrvE):
...
class ObjectStorageProvider(abc.ABC):
 ...
 active_instance=RUrvs
 @RUrvh
 def get(cls,state_file_locator:RUrvE,requested_storage:RUrvE,serializers:Dict[RUrvE,CPVCSSerializer]):
  ...
  return cls.active_instance
 def __init__(self,state_file_locator:StateFileLocator,serializers:Dict[RUrvE,CPVCSSerializer]):
 ...
 def get_state_file_location_by_key(self,key:RUrvE)->RUrvE:
  pass
 @abc.abstractmethod
 def get_version_by_key(self,key:RUrvE)->Version:
  pass
 @abc.abstractmethod
 def get_revision_by_key(self,key:RUrvE)->Revision:
  pass
 @abc.abstractmethod
 def get_revision_or_version_by_key(self,key:RUrvE)->Union[Revision,Version]:
  pass
 @abc.abstractmethod
 def get_delta_file_by_key(self,key:RUrvE)->RUrvE:
  pass
 @abc.abstractmethod
 def upsert_objects(self,*args:CPVCSObj)->List[RUrvE]:
  pass
 @abc.abstractmethod
 def update_revision_key(self,old_key:RUrvE,new_key:RUrvE,referenced_by_version:RUrvE=RUrvs):
  pass
 @abc.abstractmethod
 def version_exists(self,key:RUrvE)->RUrvN:
  pass
 @abc.abstractmethod
 def merge_remote_into_local_version(self,remote_location:RUrvE,key:RUrvE):
  pass
 @abc.abstractmethod
 def _update_key(self,old_key:RUrvE,new_key:RUrvE,base_path:RUrvE)->RUrvN:
  pass
 @abc.abstractmethod
 def _serialize(self,*args:CPVCSObj)->List[RUrvE]:
  pass
 @abc.abstractmethod
 def _deserialize(self,key_serializer:Dict[RUrvE,RUrvE],remote_location:RUrvE=RUrvs)->List[CPVCSObj]:
  pass
class ObjectStorageLocal(ObjectStorageProvider):
 ...
 def _serialize(self,*args:CPVCSObj)->List[RUrvE]:
  ...
  for cpvcs_obj in args:
   if RUrvC(cpvcs_obj,Version):
    hash_refs.append(self.serializers["version"].store_obj(cpvcs_obj))
   elif RUrvC(cpvcs_obj,Revision):
    hash_refs.append(self.serializers["revision"].store_obj(cpvcs_obj))
  return hash_refs
 def _deserialize(self,key_serializer:Dict[RUrvE,RUrvE],remote_location:RUrvE=RUrvs)->List[Union[Revision,Version]]:
