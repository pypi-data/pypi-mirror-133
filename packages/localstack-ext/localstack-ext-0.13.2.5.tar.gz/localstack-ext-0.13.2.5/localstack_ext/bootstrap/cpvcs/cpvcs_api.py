...
ROOT_DIR_LOOKUP={SHAJY(Serialization.KINESIS):KINESIS_DIR,SHAJY(Serialization.DDB):DYNAMODB_DIR,SHAJY(Serialization.MAIN):API_STATES_DIR}
def init(pod_name="My-Pod"):
 ...
 r0_hash=random_hash()
 v0_hash=random_hash()
 r0=Revision(hash_ref=r0_hash,parent_ptr=NIL_PTR,creator=config_context.get_context_user(),rid=short_uid(),revision_number=0,state_files=SHAJu())
 v0=Version(hash_ref=v0_hash,parent_ptr=NIL_PTR,creator=config_context.get_context_user(),comment="Init version",active_revision_ptr=r0_hash,outgoing_revision_ptrs={r0_hash},incoming_revision_ptr=SHAJi,state_files=SHAJu(),version_number=0)
 ...
 with SHAJw(config_context.get_head_path(),"w")as fp:
  fp.write(ver_symlink)
 with SHAJw(config_context.get_max_ver_path(),"w")as fp:
  fp.write(ver_symlink)
 with SHAJw(config_context.get_known_ver_path(),"w")as fp:
  fp.write(ver_symlink)
...
def init_remote(version_space_archive:SHAJY,meta_archives:Dict[SHAJY,SHAJY],state_archives:Dict[SHAJY,SHAJY],remote_info:Dict[SHAJY,SHAJY],pod_name:SHAJY):
...
def merge_from_remote(version_space_archive:SHAJY,meta_archives:Dict[SHAJY,SHAJY],state_archives:Dict[SHAJY,SHAJY]):
 merge_version_space(version_space_archive)
 extract_meta_and_state_archives(meta_archives=meta_archives,state_archives=state_archives)
def is_remotely_managed()->SHAJE:
 return config_context.is_remotly_managed()
def set_pod_context(pod_name:SHAJY):
 config_context.set_pod_context(pod_name)
def rename_pod(new_pod_name:SHAJY)->SHAJE:
...
def list_locally_available_pods(show_remote_or_local:SHAJE=SHAJX)->Set[SHAJY]:
 ...
 for available_pod in available_pods:
  ...
  result.add(pod_name)
 return result
def create_state_file_from_fs(path:SHAJY,file_name:SHAJY,service:SHAJY,region:SHAJY,root:SHAJY,serialization:Serialization)->SHAJY:
 ...
 return key
def _create_state_file_from_in_memory_blob(blob)->SHAJY:
 ...
 return key
def _get_state_file_path(key:SHAJY)->SHAJY:
...
def _add_state_file_to_expansion_point(state_file:StateFileRef):
 ...
 updated_state_files=SHAJu(SHAJM(lambda sf:not sf.congruent(state_file),revision.state_files))
 updated_state_files.add(state_file)
 revision.state_files=updated_state_files
 object_storage.upsert_objects(revision)
def list_state_files(key:SHAJY)->Optional[SHAJY]:
...
def get_version_info(version_no:SHAJC)->Dict[SHAJY,SHAJY]:
 ...
 return result
def commit(message:SHAJY=SHAJi)->Revision:
 ...
 curr_expansion_point_parent_state_files=SHAJu()
 if curr_expansion_point.parent_ptr!=NIL_PTR:
  referenced_by_version=SHAJi
 ...
 new_expansion_point=Revision(hash_ref=random_hash(),state_files=SHAJu(),parent_ptr=curr_expansion_point_hash,creator=curr_expansion_point.creator,rid=short_uid(),revision_number=curr_expansion_point.revision_number+1)
...
def create_version_space_archive()->SHAJY:
...
def get_max_version_no()->SHAJC:
 with SHAJw(config_context.get_max_ver_path())as fp:
  return SHAJC(os.path.basename(fp.readline()))
...
def push_overwrite(version:SHAJC,comment:SHAJY)->SHAJE:
 ...
 CommitMetamodelUtils.create_metadata_archive(version_node,overwrite=SHAJX,metamodels_file=metamodels_file)
 version_node.comment=comment
 object_storage.upsert_objects(version_node)
 return SHAJX
def push(comment:SHAJY=SHAJi,three_way:SHAJE=SHAJQ)->Version:
 expansion_point,head_version=_get_expansion_point_with_head()
 max_version=_get_max_version()
 new_active_revision=Revision(hash_ref=random_hash(),state_files=SHAJu(),parent_ptr=NIL_PTR,creator=expansion_point.creator,rid=short_uid(),revision_number=0)
 ...
 if expansion_point.parent_ptr!=NIL_PTR:
  ...
  delta_log_ptr=create_delta_log(expansion_point.state_files,SHAJu())
 ...
 return new_version
def _get_dst_path_for_state_file(version_state_dir:SHAJY,state_file:StateFileRef):
 if state_file.serialization in[SHAJY(Serialization.KINESIS),SHAJY(Serialization.DDB)]:
 ...
 mkdir(dst_path)
 return dst_path
def _create_state_directory(version_number:SHAJC,state_file_refs:Set[StateFileRef],delete_files=SHAJQ,archive=SHAJX):
...
def set_active_version(version_no:SHAJC,commit_before=SHAJQ)->SHAJE:
 ...
 return SHAJQ
def _set_active_version(key:SHAJY):
 ...
 if current_head.hash_ref!=key and object_storage.version_exists(key):
  ...
  if requested_version.active_revision_ptr==NIL_PTR:
   new_path_root=Revision(hash_ref=random_hash(),state_files=SHAJu(),parent_ptr=NIL_PTR,creator=config_context.get_context_user(),rid=short_uid(),revision_number=0)
...
def get_version_by_number(version_no:SHAJC)->Version:
 versions=load_version_references()
 version_ref=SHAJB((version[1]for version in versions if version[0]==version_no),SHAJi)
 if not version_ref:
  LOG.warning(f"Could not find version number {version_no}")
  return
 return object_storage.get_version_by_key(version_ref)
def load_version_references()->List[Tuple[SHAJC,SHAJY]]:
 result={}
 with SHAJw(config_context.get_known_ver_path(),"r")as vp:
  symlinks=vp.readlines()
  for symlink in symlinks:
   symlink=config_context.get_pod_absolute_path(symlink.rstrip())
   with SHAJw(symlink,"r")as sp:
    result[SHAJC(os.path.basename(symlink))]=sp.readline()
 return SHAJy(result.items(),key=lambda x:x[0],reverse=SHAJX)
def list_versions()->List[SHAJY]:
 ...
 return result
def list_version_commits(version_no:SHAJC)->List[SHAJY]:
 if version_no==-1:
 ...
 return result
def _update_head(new_head_ver_no,new_head_key)->SHAJY:
 with SHAJw(config_context.get_head_path(),"w")as fp:
  ver_symlink=config_context.create_version_symlink(VER_SYMLINK.format(ver_no=new_head_ver_no),new_head_key)
  fp.write(ver_symlink)
  return ver_symlink
def _update_max_ver(new_max_ver_no,new_max_ver_key)->SHAJY:
 with SHAJw(config_context.get_max_ver_path(),"w")as fp:
...
def _add_known_ver(new_ver_no,new_ver_key)->SHAJY:
 with SHAJw(config_context.get_known_ver_path(),"a")as fp:
...
def merge_expansion_point_with_max(three_way=SHAJQ):
 ...
 for serialization_mechanism,new_version_state_files_path in new_version_tmp_dirs.items():
  try:
   merge_manager=CPVCSMergeManager.get(serialization_mechanism,raise_if_missing=SHAJX)
...
def create_delta_log(state_from:Set[StateFileRef],state_to:Set[StateFileRef],diff_method:MetamodelDeltaMethod=MetamodelDeltaMethod.SIMPLE)->SHAJY:
 try:
