...
SENT_EMAILS=[]
EMAIL_BLACKLIST=xTbOn()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",xTbOf(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=xTbON,from_name=xTbON,smtp_host=xTbON,smtp_user=xTbON,smtp_pass=xTbON,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if xTbOH(recipients,xTbOD)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=xTbON):
 ...
 result["From"]=formataddr((xTbOf(Header(from_name,"utf-8")),from_email))
 return result
