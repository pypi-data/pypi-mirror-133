...
def api_key_configured()->MOcEi:
 return(MOcEb if os.environ.get("LOCALSTACK_API_KEY")and os.environ.get("LOCALSTACK_API_KEY").strip()else MOcEr)
...
def configure_pro_container(container:LocalstackContainer):
 try:
  ...
  if MOcEV([is_api_enabled(api)for api in EXTERNAL_PORT_APIS]):
