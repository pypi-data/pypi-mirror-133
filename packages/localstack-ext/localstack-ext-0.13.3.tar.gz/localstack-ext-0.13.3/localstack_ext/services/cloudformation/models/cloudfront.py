...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or ofdGL
 @ofdGs
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return ofdGk(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @ofdGn
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if ofdGh(origins,ofdGH)else origins.get("Items")
  origins_to_create=ofdGm([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if ofdGh(aliases,ofdGH)else aliases.get("Items")
  aliases_to_create=ofdGm(aliases)
  for distr in distrs:
   origins=ofdGm([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=ofdGm(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",ofdGU)
   config["Enabled"]=ofdGX(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if ofdGh(custom_errors,ofdGT)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=ofdGX(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":ofdGU}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=ofdGX(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=ofdGL):
    parent=config if parent is ofdGL else parent
    items=parent.get(key)or[]
    if ofdGh(items,ofdGT)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":ofdGw(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if ofdGh(cache_behavs,ofdGT)else cache_behavs)
