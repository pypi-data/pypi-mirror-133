...
class EC2ElasticIP(GenericBaseModel):
 ...
 def cloudformation_type():
  return "AWS::EC2::EIP"
 def get_physical_resource_id(self,attribute=BHTkE,**kwargs):
...
class SecurityGroupInOrEgress(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  for perm in perms:
   if BHTkL(perm["IpProtocol"])!=BHTkL(rp["IpProtocol"]):
    continue
 ...
 @BHTkv
 def get_vpc_and_name_for_security_group(cls,group_id:BHTkL)->Tuple[BHTkL,BHTkL]:
  client=aws_stack.connect_to_service("ec2")
  groups=client.describe_security_groups(GroupIds=[group_id])["SecurityGroups"]
  vpc_id=groups and groups[0].get("VpcId")or BHTkE
  group_name=groups and groups[0]["GroupName"]or BHTkE
 ...
 def get_deploy_templates(cls):
  def create_params(params,**kwargs):
   ...
   source_group_id=result.pop("SourceSecurityGroupId",BHTkE)
   vpc_id=BHTkE
   ...
   dst_group_id=result.pop("DestinationSecurityGroupId",BHTkE)
   ...
   if result.get("IpProtocol"):
    result["IpProtocol"]=BHTkL(result.get("IpProtocol"))
   description=result.pop("Description",BHTkE)
   cidr_ipv6=result.pop("CidrIpv6",BHTkE)
