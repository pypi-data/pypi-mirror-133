...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[pdHTI])[0]
 def get_physical_resource_id(self,attribute=pdHTI,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if pdHTE(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return pdHTK(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @pdHTu
 def add_defaults(resource,stack_name:pdHTJ):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return pdHTK(SESReceiptRule,self).get_cfn_attribute(attribute)
 @pdHTU
