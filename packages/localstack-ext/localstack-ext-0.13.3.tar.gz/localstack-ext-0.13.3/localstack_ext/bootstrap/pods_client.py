...
class PodInfo:
 def __init__(self,name=aWTYE,pod_size=0):
  self.name=name
  self.pod_size=pod_size
  self.pod_size_compressed=0
  self.persisted_resource_names=[]
class CloudPodManager(aWTYv):
 BACKEND="_none_"
 def __init__(self,pod_name=aWTYE,config=aWTYE):
  self.pod_name=pod_name
  self._pod_config=config
 def init(self):
  raise aWTYn("Not implemented")
 def delete(self,remote:aWTYw)->aWTYw:
  raise aWTYn("Not implemented")
 def push(self,comment:aWTYu=aWTYE,three_way:aWTYw=aWTYf)->PodInfo:
  raise aWTYn("Not implemented")
 def push_overwrite(self,version:aWTYX,comment:aWTYu=aWTYE):
  pass
 def pull(self,inject_version_state:aWTYw=aWTYf,reset_state_before:aWTYw=aWTYf,lazy:aWTYw=aWTYf):
  raise aWTYn("Not implemented")
 def commit(self,message:aWTYu=aWTYE):
  raise aWTYn("Not implemented")
 def inject(self,version:aWTYX,reset_state:aWTYw)->aWTYw:
  raise aWTYn("Not implemented")
 def list_versions(self)->List[aWTYu]:
  raise aWTYn("Not implemented")
 def version_info(self,version:aWTYX):
  raise aWTYn("Not implemented")
 def version_metamodel(self,version:aWTYX)->Dict:
  raise aWTYn("Not implemented")
 def set_version(self,version:aWTYX,inject_version_state:aWTYw,reset_state:aWTYw,commit_before:aWTYw):
  raise aWTYn("Not implemented")
 def list_version_commits(self,version:aWTYX)->List[aWTYu]:
  raise aWTYn("Not implemented")
 def get_commit_diff(self,version:aWTYX,commit:aWTYX)->Dict:
  raise aWTYn("Not implemented")
 def register_remote(self,pod_name:aWTYu,ci_pod:aWTYw)->aWTYw:
  raise aWTYn("Not implemented")
 def rename_pod(self,current_pod_name,new_pod_name)->aWTYw:
  raise aWTYn("Not implemented")
 def list_pods(self,fetch_remote:aWTYw)->Set[aWTYu]:
  raise aWTYn("Not implemented")
 ...
 def get(cls,pod_name,pre_config=aWTYE):
  ...
  raise aWTYn('Unable to find Cloud Pod manager implementation type "%s"'%backend)
 def deploy_pod_into_instance(self,pod_path):
  delete_pod_zip=aWTYf
  if os.path.isdir(pod_path):
   ...
   for folder in PERSISTED_FOLDERS:
    ...
    cp_r(src_folder,tgt_folder,rm_dest_on_conflict=aWTYF)
  ...
  if result.status_code>=400:
   raise aWTYn("Unable to restore pod state via local pods management API %s (code %s): %s"%(url,result.status_code,result.content))
  if delete_pod_zip:
   rm_rf(pod_path)
  else:
   return pod_path
 @aWTYN
 def get_state_zip_from_instance(get_content=aWTYf):
  url=f"{get_pods_endpoint()}/state"
  result=requests.get(url)
  if result.status_code>=400:
   raise aWTYn("Unable to get local pod state via management API %s (code %s): %s"%(url,result.status_code,result.content))
  if get_content:
   return result.content
  zip_file=f"{new_tmp_file()}.zip"
  save_file(zip_file,result.content)
  return zip_file
 def get_pod_info(self,pod_data_dir:aWTYu=aWTYE):
...
class CloudPodManagerCPVCS(CloudPodManager):
 BACKEND="cpvcs"
 @aWTYN
 def parse_pod_name_from_qualifying_name(qualifying_name:aWTYu)->aWTYu:
 ...
 @aWTYN
 def _get_max_version_for_pod_from_platform(pod_name:aWTYu,auth_headers):
  ...
  remote_max_ver=aWTYX(content["max_ver"])
 ...
 def _upload_version_and_product_space(self,presigned_urls):
  ...
  with aWTYD(version_space_archive,"rb")as version_space_content:
  ...
  for version_no,urls in presigned_meta_state_urls.items():
   ...
   with aWTYD(meta_archive,"rb")as meta_archive_content:
   ...
   with aWTYD(state_archive,"rb")as state_archive_content:
    self.upload_content(state_presigned_url,state_archive_content)
 @aWTYN
 def _add_state_files_from_directory(service:aWTYu,path:aWTYu,root_dir:aWTYu,serialization:Serialization):
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
 ...
 def _add_state_to_cpvcs_store(self):
  ...
  with ZipFile(zip_file,"r")as state_zip:
   ...
   api_states_traverse(api_states_path=api_states_path,side_effect=CloudPodManagerCPVCS._add_state_files_func,mutables=aWTYE)
  ...
  rm_rf(zip_file)
  rm_rf(tmp_dir)
 def _pull_versions(self,auth_headers,required_versions:aWTYu):
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
 ...
 def _clone_pod(self,auth_headers,lazy:aWTYw=aWTYf):
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
 ...
 def delete(self,remote:aWTYw)->aWTYw:
  ...
  if remote:
   pass
  return aWTYf
 def _push_to_remote(self,url:aWTYu):
 ...
 def pull(self,inject_version_state:aWTYw=aWTYf,reset_state_before:aWTYw=aWTYf,lazy:aWTYw=aWTYf):
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
  auth_headers=get_auth_headers()
  if self.pod_name in cpvcs_api.list_locally_available_pods(show_remote_or_local=aWTYf):
   cpvcs_api.set_pod_context(self.pod_name)
   ...
   if remote_max_ver==current_max_ver:
    ...
    return
   if not lazy:
    required_versions=",".join(aWTYV(lambda ver:aWTYu(ver),aWTYs(current_max_ver+1,remote_max_ver+1)))
   else:
 ...
 def _download_version_product(self,version:aWTYX,retain:aWTYw=aWTYf)->Dict[aWTYu,aWTYu]:
 ...
 def _get_presigned_url_for_version_product(self,version:aWTYX)->Dict:
 ...
 def _inject_from_remote(self,version:aWTYX,retain:aWTYw=aWTYf)->aWTYw:
  from localstack_ext.bootstrap.cpvcs.utils.remote_utils import(extract_meta_and_state_archives)
  content=self._get_presigned_url_for_version_product(version=version)
  state_url=content.get("presigned_url_state")
  tmp_state_archive=new_tmp_file()
 ...
 def inject(self,version:aWTYX,reset_state:aWTYw)->aWTYw:
  ...
  if not cpvcs_api.config_context.pod_exists_locally(self.pod_name):
   ...
   if not state_archive_path and cpvcs_api.is_remotely_managed():
    ...
    product_archive_path=self._download_version_product(version=version,retain=aWTYF)
    if not product_archive_path:
     return aWTYf
    state_archive_path=cpvcs_api.PodFilePaths.get_version_state_archive(version)
  if reset_state:
   reset_local_state(reset_data_dir=aWTYF,exclude_from_reset=["dynamodb","kinesis","stepfunctions"])
 ...
 def version_metamodel(self,version:aWTYX)->Dict:
  ...
  if not result and cpvcs_api.is_remotely_managed():
   self._download_version_product(version=version,retain=aWTYF)
 ...
 def get_commit_diff(self,version:aWTYX,commit:aWTYX)->Dict:
  ...
  return commit_diff
 def register_remote(self,pod_name:aWTYu,ci_pod:aWTYw=aWTYf)->aWTYw:
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
  cpvcs_api.set_pod_context(self.pod_name)
 ...
 def list_pods(self,fetch_remote:aWTYw)->Set[aWTYu]:
  ...
  return result
 @aWTYy
 def upload_content(cls,presigned_url:aWTYu,zip_data_content):
  res=safe_requests.put(presigned_url,data=zip_data_content)
  if res.status_code>=400:
   raise aWTYn("Unable to upload pod state to S3 (code %s): %s"%(res.status_code,res.content))
  return res
 @aWTYN
 def create_platform_url(request:aWTYu)->aWTYu:
...
class CloudPodManagerManaged(CloudPodManager):
 BACKEND="managed"
 def push(self,comment:aWTYu=aWTYE,three_way:aWTYw=aWTYf)->PodInfo:
  zip_data_content=self.get_state_zip_from_instance(get_content=aWTYF)
  print('Pushing state of cloud pod "%s" to backend server (%s KB)'%(self.pod_name,format_number(aWTYq(zip_data_content)/1000.0)))
  self.push_content(self.pod_name,zip_data_content)
  print("Done.")
  result=self.get_pod_info()
  result.pod_size_compressed=aWTYq(zip_data_content)
  return result
 def pull(self,inject_version_state:aWTYw=aWTYf,reset_state_before:aWTYw=aWTYf,lazy:aWTYw=aWTYf):
  ...
  rm_rf(zip_path)
 @aWTYN
 def presigned_url(pod_name:aWTYu,mode:aWTYu)->aWTYu:
  ...
  if response.status_code>=400:
   raise aWTYn("Unable to get cloud pod presigned URL (code %s): %s"%(response.status_code,content))
 ...
 def push_content(cls,pod_name,zip_data_content):
  ...
  res=safe_requests.put(presigned_url,data=zip_data_content)
  if res.status_code>=400:
   raise aWTYn("Unable to push pod state to API (code %s): %s"%(res.status_code,res.content))
  return res
class CloudPodManagerGit(CloudPodManager):
 BACKEND="git"
 def push(self,comment:aWTYu=aWTYE,three_way:aWTYw=aWTYf):
  ...
  if branch not in repo:
   porcelain.branch_create(repo,branch,force=aWTYF)
 ...
 def pull(self,inject_version_state:aWTYw=aWTYf,reset_state_before:aWTYw=aWTYf,lazy:aWTYw=aWTYf):
  ...
  if remote_ref not in remote_refs:
   raise aWTYn('Unable to find branch "%s" in remote git repo'%branch)
  remote_location=self.pod_config.get("url")
  self.switch_branch(branch)
 ...
 def client(self):
  ...
  return client,path
 def local_repo(self):
  self.clone_dir=aWTYQ(self,"clone_dir",aWTYE)
...
class PodConfigManagerMeta(aWTYM):
 def __getattr__(cls,attr):
  def _call(*args,**kwargs):
   result=aWTYE
   for manager in cls.CHAIN:
    try:
     tmp=aWTYQ(manager,attr)(*args,**kwargs)
     if tmp:
      if not result:
       result=tmp
      elif aWTYt(tmp,aWTYB)and aWTYt(result,aWTYB):
       result.extend(tmp)
    except aWTYn:
     if LOG.isEnabledFor(logging.DEBUG):
      LOG.exception("error during PodConfigManager call chain")
   if result is not aWTYE:
    return result
   raise aWTYn('Unable to run operation "%s" for local or remote configuration'%attr)
  return _call
class PodConfigManager(aWTYv,metaclass=PodConfigManagerMeta):
 CHAIN=[]
 @aWTYy
 def pod_config(cls,pod_name):
  ...
  if not pod_config:
   raise aWTYn('Unable to find config for pod named "%s"'%pod_name)
  return pod_config[0]
class PodConfigManagerLocal(aWTYv):
 CONFIG_FILE=".localstack.yml"
 def list_pods(self):
  local_pods=self._load_config(safe=aWTYF).get("pods",{})
  local_pods=[{"pod_name":k,"state":"Local Only",**v}for k,v in local_pods.items()]
  existing_names=aWTYi([pod["pod_name"]for pod in local_pods])
  result=[pod for pod in local_pods if pod["pod_name"]not in existing_names]
  return result
 def store_pod_metadata(self,pod_name,metadata):
  pass
 def _load_config(self,safe=aWTYf):
  try:
   return yaml.safe_load(to_str(load_file(self.CONFIG_FILE)))
  except aWTYn:
   if safe:
    return{}
   raise aWTYn('Unable to find and parse config file "%s"'%self.CONFIG_FILE)
class PodConfigManagerRemote(aWTYv):
 def list_pods(self):
  result=[]
  auth_headers=get_auth_headers()
  ...
  content=response.content
  if response.status_code>=400:
   raise aWTYn("Unable to fetch list of pods from API (code %s): %s"%(response.status_code,content))
 ...
 def store_pod_metadata(self,pod_name,metadata):
  ...
  content=response.content
  if response.status_code>=400:
   raise aWTYn("Unable to store pod metadata in API (code %s): %s"%(response.status_code,content))
...
def init_cpvcs(pod_name:aWTYu,pre_config:Dict[aWTYu,aWTYu],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.init()
def delete_pod(pod_name:aWTYu,remote:aWTYw,pre_config:Dict[aWTYu,aWTYu])->aWTYw:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 result=backend.delete(remote=remote)
 return result
def register_remote(pod_name:aWTYu,pre_config:Dict[aWTYu,aWTYu],**kwargs)->aWTYw:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 result=backend.register_remote(pod_name=pod_name,ci_pod=pre_config.get("ci_pod",aWTYf))
 return result
def rename_pod(current_pod_name:aWTYu,new_pod_name:aWTYu,pre_config:Dict[aWTYu,aWTYu],**kwargs):
 ...
 return result
def list_pods_cpvcs(remote:aWTYw,pre_config:Dict[aWTYu,aWTYu],**kwargs)->List[aWTYu]:
 backend=CloudPodManager.get(pod_name="",pre_config=pre_config)
 result=backend.list_pods(fetch_remote=remote)
 return result
def commit_state(pod_name:aWTYu,pre_config:Dict[aWTYu,aWTYu],message:aWTYu=aWTYE,**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.commit(message=message)
def inject_state(pod_name:aWTYu,version:aWTYX,reset_state:aWTYw,pre_config:Dict[aWTYu,aWTYu],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 result=backend.inject(version=version,reset_state=reset_state)
 return result
def list_versions(pod_name:aWTYu,pre_config:Dict[aWTYu,aWTYu],**kwargs)->List[aWTYu]:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 versions=backend.list_versions()
 return versions
def get_version_info(version:aWTYX,pod_name:aWTYu,pre_config:Dict[aWTYu,aWTYu],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 info=backend.version_info(version=version)
 return info
def get_version_metamodel(version:aWTYX,pod_name:aWTYu,pre_config:Dict[aWTYu,aWTYu],**kwargs)->Dict:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 metamodel=backend.version_metamodel(version=version)
 return metamodel
def set_version(version:aWTYX,inject_version_state:aWTYw,reset_state:aWTYw,commit_before:aWTYw,pod_name:aWTYu,pre_config:Dict[aWTYu,aWTYu],**kwargs)->aWTYw:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 ...
 return success
def list_version_commits(version:aWTYX,pod_name:aWTYu,pre_config:Dict[aWTYu,aWTYu])->List[aWTYu]:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 commits=backend.list_version_commits(version=version)
 return commits
def get_commit_diff(version:aWTYX,commit:aWTYX,pod_name:aWTYu,pre_config:Dict[aWTYu,aWTYu])->Dict:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 commit_diff=backend.get_commit_diff(version=version,commit=commit)
 return commit_diff
def push_overwrite(version:aWTYX,pod_name:aWTYu,comment:aWTYu,pre_config:Dict[aWTYu,aWTYu]):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.push_overwrite(version=version,comment=comment)
def push_state(pod_name,pre_config=aWTYE,squash_commits=aWTYf,comment=aWTYE,three_way=aWTYf,**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
...
def pull_state(pod_name,inject_version_state=aWTYf,reset_state_before=aWTYf,lazy=aWTYf,**kwargs):
 pre_config=kwargs.get("pre_config",aWTYE)
 if not pod_name:
  raise aWTYn("Need to specify a pod name")
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.pull(inject_version_state=inject_version_state,reset_state_before=reset_state_before,lazy=lazy)
 print("Done.")
def reset_local_state(reset_data_dir=aWTYf,exclude_from_reset:List[aWTYu]=aWTYE):
 url=f"{get_pods_endpoint()}/state"
 if reset_data_dir:
  url+="/datadir"
 ...
 if result.status_code>=400:
  raise aWTYn("Unable to reset service state via local management API %s (code %s): %s"%(url,result.status_code,result.content))
 print("Done.")
def list_pods(args):
 return PodConfigManager.list_pods()
def get_data_dir_from_container()->aWTYu:
 try:
...
def get_persisted_resource_names(data_dir)->List[aWTYu]:
 names=[]
 ...
 with os.scandir(os.path.join(data_dir,"api_states"))as entries:
  for entry in entries:
   if entry.is_dir()and aWTYq(os.listdir(entry.path))>0:
    names.append(entry.name)
