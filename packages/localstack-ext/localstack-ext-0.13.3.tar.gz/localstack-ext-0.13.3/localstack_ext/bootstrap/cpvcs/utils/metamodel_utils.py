...
CAPTURE_METAMODEL_SEPARATELY=MPaYQ
...
class MetamodelDelta(SubtypesInstanceManager):
 def create_delta_for_state_files(self,sf1:StateFileRef,sf2:StateFileRef):
  ...
  if not sf1:
   backend2=load_persisted_object(config_context.get_obj_file_path(sf2.hash_ref))
   clazz=MPaYS(backend2)
   backend1=_infer_backend_init(clazz,sf2)
  else:
   backend1=load_persisted_object(config_context.get_obj_file_path(sf1.hash_ref))
   clazz=MPaYS(backend1)
   backend2=_infer_backend_init(clazz,sf1)
  diff=self.create_delta_json(backend1,backend2)
  return diff
 def create_delta_log(self,state_from:Set[StateFileRef],state_to:Set[StateFileRef])->MPaYw:
  ...
  def _create_sf_lookup(state_files:Set[StateFileRef])->Dict[MPaYw,StateFileRef]:
  ...
  for state_file_from in state_files_from_regular:
   ...
   if state_file_from.any_congruence(state_files_to_regular):
    ...
    diff_json=self.create_delta_for_state_files(state_file_from,MPaYj)
  ...
  for state_files_to in state_files_to_lookup.values():
   ...
   diff_json=self.create_delta_for_state_files(MPaYj,state_files_to)
...
class CommitMetamodelUtils:
 @MPaYW
 def create_metadata_archive(cls,version:Version,delete_reference:bool=MPaYQ,overwrite:bool=MPaYQ,metamodels_file:MPaYw=MPaYj):
 ...
 def get_metamodel_delta(cls,prev_metamodel:Dict,this_metamodel:Dict)->Dict:
  ...
  return result
 @MPaYW
 def create_delta_metamodel_file(cls,version:MPaYs,revision:MPaYs,store_to_zip=MPaYQ)->Tuple[Dict,Dict]:
 ...
 @MPaYW
 def reconstruct_metamodel(cls,version:MPaYs,revision:MPaYs=MPaYj)->Dict:
  result={}
  for rev in MPaYz(1,revision+1):
  ...
  return result
 @MPaYW
 def get_version_metamodel(cls,version:MPaYs,revision:MPaYs=MPaYj)->Dict:
 ...
 @MPaYW
 def get_commit_diff(cls,version_no:MPaYs,commit_no:MPaYs)->Optional[Dict]:
  ...
  return result
def _infer_backend_init(clazz:Type,sf:StateFileRef)->MPaYq:
 if MPaYr(clazz,MPaYb):
  return{}
 constructor=MPaYO(clazz,"__init__",MPaYj)
...
def _create_metamodel_helper(obj,width=25,visited:set=MPaYj):
 if obj is MPaYj:
  return obj
