...
def extract_meta_and_state_archives(meta_archives:Dict[pJVrK,pJVrK],state_archives:Dict[pJVrK,pJVrK]):
  ...
  for version_no,archive in product_space_archive.items():
   with zipfile.ZipFile(archive)as meta_zip:
    if product_space_archive==meta_archives:
     archive_dest=config_context.get_version_meta_archive_path(version=version_no,with_format=pJVrL)
    else:
     archive_dest=config_context.get_version_state_archive_path(version=version_no,with_format=pJVrL)
...
def register_remote(remote_info:Dict[pJVrK,pJVrK]):
 if config_context.is_remotly_managed():
  LOG.warning("Pod is already remotely managed")
  return
 with pJVrf(config_context.get_remote_info_path(),"w")as fp:
...
def merge_version_space(version_space_archive):
 ...
 for version_ref_file in os.listdir(remote_config_context.get_ver_refs_path()):
  ...
  with pJVrf(remote_version_ref_file_path,"r")as fp:
