...
def random_hash()->luGSh:
 return luGSs(random.getrandbits(160))
def compute_file_hash(file_path:luGSh,accum=luGSK)->Optional[luGSh]:
 try:
  with luGSt(file_path,"rb")as fp:
   if accum:
...
def compute_version_archive_hash(version_no:luGSI,state_archive:luGSh)->luGSh:
 ...
 m.update(luGSh(version_no).encode("utf-8"))
 rm_rf(tmp_state_dir)
 return m.hexdigest()
def compute_revision_hash(cpvcs_node:Revision)->luGSh:
 if not cpvcs_node.state_files:
  return random_hash()
 state_file_keys=luGSL(lambda state_file:state_file.hash_ref,cpvcs_node.state_files)
 m=hashlib.sha1()
 for key in state_file_keys:
  try:
   with luGSt(os.path.join(config_context.get_obj_store_path(),key),"rb")as fp:
    m.update(fp.read())
  except luGSQ as e:
   LOG.warning(f"Failed to open file and compute hash for {key}: {e}")
 if luGSp(cpvcs_node,Revision):
  m.update(cpvcs_node.rid.encode("utf-8"))
  m.update(luGSh(cpvcs_node.revision_number).encode("utf-8"))
 elif luGSp(cpvcs_node,Version):
  m.update(luGSh(cpvcs_node.version_number).encode("utf-8"))
 return m.hexdigest()
