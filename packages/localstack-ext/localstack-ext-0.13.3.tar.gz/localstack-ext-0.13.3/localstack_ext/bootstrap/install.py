...
def install_package(packages:Union[List[ciTRC],ciTRC],api_name:ciTRC):
 if not(is_debian()and in_docker()):
  ...
  return
 if ciTRz(packages,ciTRC):
  packages=packages.split()
 LOG.info("Downloading dependencies for %s API. This may take a while."%api_name)
 with INSTALL_LOCK:
  run(["apt-get","update"],shell=ciTRm)
  run(["apt-get","install","-y","--no-install-recommends"]+packages,shell=ciTRm)
def check_or_install(command:ciTRC,packages:Union[List[ciTRC],ciTRC],api:ciTRC):
...
def setup_ssl_cert():
 ...
 try:
  ...
  try:
   ...
   raise
def download_github_artifact(url:ciTRC,target_file:ciTRC,timeout:int=ciTRu):
 def do_download(url,print_error=ciTRm):
  try:
