...
SENT_EMAILS=[]
EMAIL_BLACKLIST=dIHbW()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",dIHbw(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=dIHbU,from_name=dIHbU,smtp_host=dIHbU,smtp_user=dIHbU,smtp_pass=dIHbU,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if dIHbh(recipients,dIHbJ)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=dIHbU):
 ...
 result["From"]=formataddr((dIHbw(Header(from_name,"utf-8")),from_email))
 return result
