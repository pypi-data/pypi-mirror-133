...
class DecryptionHandler:
 decryption_key:LgDpR
 def __init__(self,decryption_key:LgDpR):
  self.decryption_key=decryption_key
 def decrypt(self,content)->LgDpR:
...
class EncryptedFileFinder(MetaPathFinder):
 ...
 def find_spec(self,fullname,path,target=LgDpX):
  if path and not LgDpW(path,LgDpQ):
   path=LgDpQ(LgDpe(path,"_path",[]))
  if not path:
   return LgDpX
...
def patch_traceback_lines():
 if LgDpe(traceback.FrameSummary,"_ls_patch_applied",LgDpX):
  return
 @LgDpE
 def line(self):
  try:
 ...
 LgDpC(traceback.FrameSummary,"line",line)
 traceback.FrameSummary._ls_patch_applied=LgDpw
def patch_inspect_findsource():
 if LgDpe(inspect,"_ls_patch_applied",LgDpX):
  return
 ...
 inspect._ls_patch_applied=LgDpw
