...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return HSoUy if e else HSoUN
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->HSoUY:
...
@click.option("--revert",is_flag=HSoUy,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:HSoUc):
...
def _cpvcs_initialized(pod_name:HSoUs)->HSoUc:
...
@click.option("-n","--name",help="Name of the cloud pod",required=HSoUy)
def cmd_cpvcs_init(name:HSoUs):
...
@click.option("-n","--name",help="Name of the cloud pod",required=HSoUy)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=HSoUy,default=HSoUN)
def cmd_cpvcs_delete(name:HSoUs,remote:HSoUc):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=HSoUy)
def cmd_cpvcs_register_remote(name:HSoUs):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=HSoUy)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=HSoUy)
def cmd_cpvcs_rename(name:HSoUs,new_name:HSoUs):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=HSoUy)
def cmd_cpvcs_commit(message:HSoUs,name:HSoUs):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=HSoUy,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("--three-way",is_flag=HSoUy,default=HSoUN,help="")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=HSoUy)
def cmd_cpvcs_push(squash:HSoUc,message:HSoUs,name:HSoUs,three_way:HSoUc):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=HSoUy)
@click.option("-v","--version",type=HSoUl)
@click.option("-m","--message",required=HSoUN)
def cmd_cpvcs_push_overwrite(version:HSoUl,message:HSoUs,name:HSoUs):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=HSoUl,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=HSoUy,default=HSoUN,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=HSoUy)
def cmd_cpvcs_inject(version:HSoUl,reset:HSoUc,name:HSoUs):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--inject/--no-inject",default=HSoUy,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=HSoUy,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=HSoUy)
@click.option("--lazy/--eager",default=HSoUy,help="Will only fetch references to existing versions, i.e. version state is only downloaded when required")
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:HSoUs,inject:HSoUc,reset:HSoUc,lazy:HSoUc):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=HSoUy,default=HSoUN)
def cmd_cpvcs_list_pods(remote:HSoUc):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=HSoUy)
def cmd_cpvcs_versions(name:HSoUs):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=HSoUy,type=HSoUl)
@click.option("-n","--name",help="Name of the cloud pod",required=HSoUy)
def cmd_cpvcs_version_info(version:HSoUl,name:HSoUs):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=HSoUl,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=HSoUy)
def cmd_cpvcs_version_metamodel(version:HSoUl,name:HSoUs):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=HSoUy,type=HSoUl,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=HSoUy,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=HSoUy,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=HSoUN,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=HSoUy)
def cmd_cpvcs_set_version(version:HSoUl,inject:HSoUc,reset:HSoUc,commit_before:HSoUc,name:HSoUs):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=HSoUy)
def cmd_cpvcs_commits(version:HSoUl,name:HSoUs):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=HSoUy)
@click.option("--commit","-c",required=HSoUy)
@click.option("-n","--name",help="Name of the cloud pod",required=HSoUy)
def cmd_cpvcs_commit_diff(version:HSoUl,commit:HSoUl,name:HSoUs):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(HSoUY)
  status.stop()
...
def print_table(columns:Dict[HSoUs,HSoUs],rows:List[Dict[HSoUs,Any]],formatters:Dict[HSoUs,Callable[[Any],HSoUs]]=HSoUY):
 from rich.table import Table
 if formatters is HSoUY:
  formatters=HSoUD()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  HSoUe(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->HSoUY:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->HSoUY:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->HSoUY:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->HSoUY:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
