...
def api_key_configured()->rSgTU:
 return(rSgTv if os.environ.get("LOCALSTACK_API_KEY")and os.environ.get("LOCALSTACK_API_KEY").strip()else rSgTC)
...
def configure_pro_container(container:LocalstackContainer):
 try:
  ...
  if rSgTf([is_api_enabled(api)for api in EXTERNAL_PORT_APIS]):
