...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return pzlis if e else pzliV
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->pzliK:
...
@click.option("--revert",is_flag=pzlis,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:pzlia):
...
def _cpvcs_initialized(pod_name:pzliy)->pzlia:
...
@click.option("-n","--name",help="Name of the cloud pod",required=pzlis)
def cmd_cpvcs_init(name:pzliy):
...
@click.option("-n","--name",help="Name of the cloud pod",required=pzlis)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=pzlis,default=pzliV)
def cmd_cpvcs_delete(name:pzliy,remote:pzlia):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=pzlis)
def cmd_cpvcs_register_remote(name:pzliy):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=pzlis)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=pzlis)
def cmd_cpvcs_rename(name:pzliy,new_name:pzliy):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=pzlis)
def cmd_cpvcs_commit(message:pzliy,name:pzliy):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=pzlis,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("--three-way",is_flag=pzlis,default=pzliV,help="")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=pzlis)
def cmd_cpvcs_push(squash:pzlia,message:pzliy,name:pzliy,three_way:pzlia):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=pzlis)
@click.option("-v","--version",type=pzlid)
@click.option("-m","--message",required=pzliV)
def cmd_cpvcs_push_overwrite(version:pzlid,message:pzliy,name:pzliy):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=pzlid,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=pzlis,default=pzliV,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=pzlis)
def cmd_cpvcs_inject(version:pzlid,reset:pzlia,name:pzliy):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--inject/--no-inject",default=pzlis,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=pzlis,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=pzlis)
@click.option("--lazy/--eager",default=pzlis,help="Will only fetch references to existing versions, i.e. version state is only downloaded when required")
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:pzliy,inject:pzlia,reset:pzlia,lazy:pzlia):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=pzlis,default=pzliV)
def cmd_cpvcs_list_pods(remote:pzlia):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=pzlis)
def cmd_cpvcs_versions(name:pzliy):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=pzlis,type=pzlid)
@click.option("-n","--name",help="Name of the cloud pod",required=pzlis)
def cmd_cpvcs_version_info(version:pzlid,name:pzliy):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=pzlid,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=pzlis)
def cmd_cpvcs_version_metamodel(version:pzlid,name:pzliy):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=pzlis,type=pzlid,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=pzlis,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=pzlis,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=pzliV,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=pzlis)
def cmd_cpvcs_set_version(version:pzlid,inject:pzlia,reset:pzlia,commit_before:pzlia,name:pzliy):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=pzlis)
def cmd_cpvcs_commits(version:pzlid,name:pzliy):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=pzlis)
@click.option("--commit","-c",required=pzlis)
@click.option("-n","--name",help="Name of the cloud pod",required=pzlis)
def cmd_cpvcs_commit_diff(version:pzlid,commit:pzlid,name:pzliy):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(pzliK)
  status.stop()
...
def print_table(columns:Dict[pzliy,pzliy],rows:List[Dict[pzliy,Any]],formatters:Dict[pzliy,Callable[[Any],pzliy]]=pzliK):
 from rich.table import Table
 if formatters is pzliK:
  formatters=pzliU()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  pzliA(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->pzliK:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->pzliK:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->pzliK:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->pzliK:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
