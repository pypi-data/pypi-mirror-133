...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or CuWBR
 @CuWBI
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return CuWBh(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @CuWBp
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if CuWBt(origins,CuWBG)else origins.get("Items")
  origins_to_create=CuWBF([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if CuWBt(aliases,CuWBG)else aliases.get("Items")
  aliases_to_create=CuWBF(aliases)
  for distr in distrs:
   origins=CuWBF([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=CuWBF(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",CuWBw)
   config["Enabled"]=CuWBH(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if CuWBt(custom_errors,CuWBi)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=CuWBH(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":CuWBw}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=CuWBH(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=CuWBR):
    parent=config if parent is CuWBR else parent
    items=parent.get(key)or[]
    if CuWBt(items,CuWBi)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":CuWBf(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if CuWBt(cache_behavs,CuWBi)else cache_behavs)
