...
class EC2ElasticIP(GenericBaseModel):
 ...
 def cloudformation_type():
  return "AWS::EC2::EIP"
 def get_physical_resource_id(self,attribute=ompdw,**kwargs):
...
class SecurityGroupInOrEgress(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  for perm in perms:
   if ompdt(perm["IpProtocol"])!=ompdt(rp["IpProtocol"]):
    continue
 ...
 @ompdO
 def get_vpc_and_name_for_security_group(cls,group_id:ompdt)->Tuple[ompdt,ompdt]:
  client=aws_stack.connect_to_service("ec2")
  groups=client.describe_security_groups(GroupIds=[group_id])["SecurityGroups"]
  vpc_id=groups and groups[0].get("VpcId")or ompdw
  group_name=groups and groups[0]["GroupName"]or ompdw
 ...
 def get_deploy_templates(cls):
  def create_params(params,**kwargs):
   ...
   source_group_id=result.pop("SourceSecurityGroupId",ompdw)
   vpc_id=ompdw
   ...
   dst_group_id=result.pop("DestinationSecurityGroupId",ompdw)
   ...
   if result.get("IpProtocol"):
    result["IpProtocol"]=ompdt(result.get("IpProtocol"))
   description=result.pop("Description",ompdw)
   cidr_ipv6=result.pop("CidrIpv6",ompdw)
