...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[NYOQt])[0]
 def get_physical_resource_id(self,attribute=NYOQt,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if NYOQa(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return NYOQy(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @NYOQM
 def add_defaults(resource,stack_name:NYOQW):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return NYOQy(SESReceiptRule,self).get_cfn_attribute(attribute)
 @NYOQg
