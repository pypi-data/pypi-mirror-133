...
def api_key_configured()->DRhNv:
 return(DRhNp if os.environ.get("LOCALSTACK_API_KEY")and os.environ.get("LOCALSTACK_API_KEY").strip()else DRhNA)
...
def configure_pro_container(container:LocalstackContainer):
 try:
  ...
  if DRhNP([is_api_enabled(api)for api in EXTERNAL_PORT_APIS]):
