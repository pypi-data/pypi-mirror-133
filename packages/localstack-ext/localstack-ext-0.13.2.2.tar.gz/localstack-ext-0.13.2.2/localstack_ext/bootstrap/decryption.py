...
class DecryptionHandler:
 decryption_key:hYJSk
 def __init__(self,decryption_key:hYJSk):
  self.decryption_key=decryption_key
 def decrypt(self,content)->hYJSk:
...
class EncryptedFileFinder(MetaPathFinder):
 ...
 def find_spec(self,fullname,path,target=hYJSt):
  if path and not hYJSA(path,hYJSv):
   path=hYJSv(hYJSE(path,"_path",[]))
  if not path:
   return hYJSt
...
def patch_traceback_lines():
 if hYJSE(traceback.FrameSummary,"_ls_patch_applied",hYJSt):
  return
 @hYJSs
 def line(self):
  try:
 ...
 hYJSM(traceback.FrameSummary,"line",line)
 traceback.FrameSummary._ls_patch_applied=hYJSO
def patch_inspect_findsource():
 if hYJSE(inspect,"_ls_patch_applied",hYJSt):
  return
 ...
 inspect._ls_patch_applied=hYJSO
