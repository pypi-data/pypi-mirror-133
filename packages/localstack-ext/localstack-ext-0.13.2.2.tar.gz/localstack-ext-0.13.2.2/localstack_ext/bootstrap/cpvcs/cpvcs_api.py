...
def init(pod_name="My-Pod"):
 ...
 r0_hash=random_hash()
 v0_hash=random_hash()
 r0=Revision(hash_ref=r0_hash,parent_ptr=NIL_PTR,creator=config_context.get_context_user(),rid=short_uid(),revision_number=0,state_files=SBgnT())
 v0=Version(hash_ref=v0_hash,parent_ptr=NIL_PTR,creator=config_context.get_context_user(),comment="Init version",active_revision_ptr=r0_hash,outgoing_revision_ptrs={r0_hash},incoming_revision_ptr=SBgnz,state_files=SBgnT(),version_number=0)
 ...
 with SBgnO(config_context.get_head_path(),"w")as fp:
  fp.write(ver_symlink)
 with SBgnO(config_context.get_max_ver_path(),"w")as fp:
  fp.write(ver_symlink)
 with SBgnO(config_context.get_known_ver_path(),"w")as fp:
  fp.write(ver_symlink)
...
def init_remote(version_space_archive:SBgnk,meta_archives:Dict[SBgnk,SBgnk],state_archives:Dict[SBgnk,SBgnk],remote_info:Dict[SBgnk,SBgnk],pod_name:SBgnk):
...
def merge_from_remote(version_space_archive:SBgnk,meta_archives:Dict[SBgnk,SBgnk],state_archives:Dict[SBgnk,SBgnk]):
 merge_version_space(version_space_archive)
 extract_meta_and_state_archives(meta_archives=meta_archives,state_archives=state_archives)
def is_remotely_managed()->SBgnF:
 return config_context.is_remotly_managed()
def set_pod_context(pod_name:SBgnk):
 config_context.set_pod_context(pod_name)
def rename_pod(new_pod_name:SBgnk)->SBgnF:
...
def list_locally_available_pods(show_remote_or_local:SBgnF=SBgnp)->Set[SBgnk]:
 ...
 for available_pod in available_pods:
  ...
  result.add(pod_name)
 return result
def create_state_file_from_fs(path:SBgnk,file_name:SBgnk,service:SBgnk,region:SBgnk)->SBgnk:
 ...
 return key
def _create_state_file_from_in_memory_blob(blob)->SBgnk:
 ...
 return key
def _get_state_file_path(key:SBgnk)->SBgnk:
...
def _add_state_file_to_expansion_point(state_file:StateFileRef):
 ...
 updated_state_files=SBgnT(SBgnc(lambda sf:not sf.congruent(state_file),revision.state_files))
 updated_state_files.add(state_file)
 revision.state_files=updated_state_files
 object_storage.upsert_objects(revision)
def list_state_files(key:SBgnk)->Optional[SBgnk]:
...
def get_version_info(version_no:SBgns)->Dict[SBgnk,SBgnk]:
 ...
 return result
def commit(message:SBgnk=SBgnz)->Revision:
 ...
 curr_expansion_point_parent_state_files=SBgnT()
 if curr_expansion_point.parent_ptr!=NIL_PTR:
  referenced_by_version=SBgnz
 ...
 new_expansion_point=Revision(hash_ref=random_hash(),state_files=SBgnT(),parent_ptr=curr_expansion_point_hash,creator=curr_expansion_point.creator,rid=short_uid(),revision_number=curr_expansion_point.revision_number+1)
...
def create_version_space_archive()->SBgnk:
...
def get_max_version_no()->SBgns:
 with SBgnO(config_context.get_max_ver_path())as fp:
  return SBgns(os.path.basename(fp.readline()))
...
def push_overwrite(version:SBgns,comment:SBgnk)->SBgnF:
 ...
 CommitMetamodelUtils.create_metadata_archive(version_node,overwrite=SBgnp,metamodels_file=metamodels_file)
 version_node.comment=comment
 object_storage.upsert_objects(version_node)
 return SBgnp
def push(comment:SBgnk=SBgnz,three_way:SBgnF=SBgnP)->Version:
 expansion_point,head_version=_get_expansion_point_with_head()
 max_version=_get_max_version()
 new_active_revision=Revision(hash_ref=random_hash(),state_files=SBgnT(),parent_ptr=NIL_PTR,creator=expansion_point.creator,rid=short_uid(),revision_number=0)
 ...
 new_version=Version(hash_ref=compute_version_archive_hash(new_max_version_no,PodFilePaths.get_version_state_archive(new_max_version_no)),state_files=SBgnT(),parent_ptr=max_version.hash_ref,creator=expansion_point.creator,comment=comment,active_revision_ptr=new_active_revision.hash_ref,outgoing_revision_ptrs={new_active_revision.hash_ref},incoming_revision_ptr=expansion_point.hash_ref,version_number=new_max_version_no)
 if expansion_point.parent_ptr!=NIL_PTR:
  ...
  delta_log_ptr=create_delta_log(expansion_point.state_files,SBgnT())
 ...
 return new_version
def _create_state_directory(version_number:SBgns,state_file_refs:Set[StateFileRef],delete_files=SBgnP,archive=SBgnp):
...
def set_active_version(version_no:SBgns,commit_before=SBgnP)->SBgnF:
 ...
 return SBgnP
def _set_active_version(key:SBgnk):
 ...
 if current_head.hash_ref!=key and object_storage.version_exists(key):
  ...
  if requested_version.active_revision_ptr==NIL_PTR:
   new_path_root=Revision(hash_ref=random_hash(),state_files=SBgnT(),parent_ptr=NIL_PTR,creator=config_context.get_context_user(),rid=short_uid(),revision_number=0)
...
def get_version_by_number(version_no:SBgns)->Version:
 versions=load_version_references()
 version_ref=SBgna((version[1]for version in versions if version[0]==version_no),SBgnz)
 if not version_ref:
  LOG.warning(f"Could not find version number {version_no}")
  return
 return object_storage.get_version_by_key(version_ref)
def load_version_references()->List[Tuple[SBgns,SBgnk]]:
 result={}
 with SBgnO(config_context.get_known_ver_path(),"r")as vp:
  symlinks=vp.readlines()
  for symlink in symlinks:
   symlink=config_context.get_pod_absolute_path(symlink.rstrip())
   with SBgnO(symlink,"r")as sp:
    result[SBgns(os.path.basename(symlink))]=sp.readline()
 return SBgnA(result.items(),key=lambda x:x[0],reverse=SBgnp)
def list_versions()->List[SBgnk]:
 ...
 return result
def list_version_commits(version_no:SBgns)->List[SBgnk]:
 if version_no==-1:
 ...
 return result
def _update_head(new_head_ver_no,new_head_key)->SBgnk:
 with SBgnO(config_context.get_head_path(),"w")as fp:
  ver_symlink=config_context.create_version_symlink(VER_SYMLINK.format(ver_no=new_head_ver_no),new_head_key)
  fp.write(ver_symlink)
  return ver_symlink
def _update_max_ver(new_max_ver_no,new_max_ver_key)->SBgnk:
 with SBgnO(config_context.get_max_ver_path(),"w")as fp:
...
def _add_known_ver(new_ver_no,new_ver_key)->SBgnk:
 with SBgnO(config_context.get_known_ver_path(),"a")as fp:
...
def _is_special_case(obj)->SBgnF:
 return SBgnv(obj,(Queue,FakeBucket))
...
def _merge_two_state_dir_func(**kwargs):
 ...
 if os.path.isfile(dst_state_file_path):
  ...
  mkdir(dst_state_dir)
  persist_object(src_state,dst_state_file_path)
def _merge_expansion_point_with_max(three_way=SBgnP):
 ...
 common_ancestor_state_archive=SBgnz
 tmp_common_ancestor_state_dir=SBgnz
 ...
 new_max_version_states_dir=_create_state_directory(version_number=new_version_no,state_file_refs=expansion_point.state_files,archive=SBgnP)
...
def create_delta_log(state_from:Set[StateFileRef],state_to:Set[StateFileRef],diff_method:MetamodelDeltaMethod=MetamodelDeltaMethod.SIMPLE)->SBgnk:
 try:
