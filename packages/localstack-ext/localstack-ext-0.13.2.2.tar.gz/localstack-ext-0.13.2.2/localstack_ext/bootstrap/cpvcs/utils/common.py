...
class CPVCSConfigContext:
 ...
 def get_version_state_archive_path(self,version,with_format:AFvPr=AFvPI)->AFvPU:
 ...
 def update_ver_log(self,author:AFvPU,ver_no:AFvPO,rev_id:AFvPU,rev_no:AFvPO):
  with AFvPn(self.get_ver_log_path(),"a")as fp:
   fp.write(f"{VER_LOG_STRUCTURE.format(author=author, ver_no=ver_no, rev_rid_no=f'{rev_id}_{rev_no}')}\n")
 def create_version_symlink(self,name:AFvPU,key:AFvPU=AFvPc)->AFvPU:
  return self._create_symlink(name,key,self.get_ver_refs_path())
 def create_revision_symlink(self,name:AFvPU,key:AFvPU=AFvPc)->AFvPU:
  return self._create_symlink(name,key,self.get_rev_refs_path())
 def is_initialized(self)->AFvPr:
  return self.pod_root_dir and os.path.isdir(self.pod_root_dir)
 def _create_symlink(self,name:AFvPU,key:AFvPU,path:AFvPU)->AFvPU:
...
def zip_directories(zip_dest:AFvPU,directories:List[AFvPU])->AFvPU:
 ...
 return zip_dest
def add_file_to_archive(archive:AFvPU,entry_name:AFvPU,content:Union[AFvPU,AFvPQ]):
 with zipfile.ZipFile(archive,"a")as zip_file:
  zip_file.writestr(entry_name,content)
def read_file_from_archive(archive_path:AFvPU,file_name:AFvPU)->AFvPU:
 try:
