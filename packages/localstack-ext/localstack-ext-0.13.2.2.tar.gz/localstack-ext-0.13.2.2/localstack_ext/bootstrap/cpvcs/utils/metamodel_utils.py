...
CAPTURE_METAMODEL_SEPARATELY=dJWaC
...
class MetamodelDelta(SubtypesInstanceManager):
 def create_delta_for_state_files(self,sf1:StateFileRef,sf2:StateFileRef):
  ...
  if not sf1:
   backend2=load_persisted_object(config_context.get_obj_file_path(sf2.hash_ref))
   clazz=dJWaB(backend2)
   backend1=_infer_backend_init(clazz,sf2)
  else:
   backend1=load_persisted_object(config_context.get_obj_file_path(sf1.hash_ref))
   clazz=dJWaB(backend1)
   backend2=_infer_backend_init(clazz,sf1)
  diff=self.create_delta_json(backend1,backend2)
  return diff
 def create_delta_log(self,state_from:Set[StateFileRef],state_to:Set[StateFileRef])->dJWay:
  ...
  def _create_sf_lookup(state_files:Set[StateFileRef])->Dict[dJWay,StateFileRef]:
  ...
  for state_file_from in state_files_from_regular:
   ...
   if state_file_from.any_congruence(state_files_to_regular):
    ...
    diff_json=self.create_delta_for_state_files(state_file_from,dJWam)
  ...
  for state_files_to in state_files_to_lookup.values():
   ...
   diff_json=self.create_delta_for_state_files(dJWam,state_files_to)
...
class CommitMetamodelUtils:
 @dJWac
 def create_metadata_archive(cls,version:Version,delete_reference:bool=dJWaC,overwrite:bool=dJWaC,metamodels_file:dJWay=dJWam):
 ...
 def get_metamodel_delta(cls,prev_metamodel:Dict,this_metamodel:Dict)->Dict:
  ...
  return result
 @dJWac
 def create_delta_metamodel_file(cls,version:dJWas,revision:dJWas,store_to_zip=dJWaC)->Tuple[Dict,Dict]:
 ...
 @dJWac
 def reconstruct_metamodel(cls,version:dJWas,revision:dJWas=dJWam)->Dict:
  result={}
  for rev in dJWaV(1,revision+1):
  ...
  return result
 @dJWac
 def get_version_metamodel(cls,version:dJWas,revision:dJWas=dJWam)->Dict:
 ...
 @dJWac
 def get_commit_diff(cls,version_no:dJWas,commit_no:dJWas)->Optional[Dict]:
  ...
  return result
def _infer_backend_init(clazz:Type,sf:StateFileRef)->dJWaE:
 if dJWaS(clazz,dJWaX):
  return{}
 constructor=dJWaz(clazz,"__init__",dJWam)
...
def _create_metamodel_helper(obj,width=25,visited:set=dJWam):
 if obj is dJWam:
  return obj
