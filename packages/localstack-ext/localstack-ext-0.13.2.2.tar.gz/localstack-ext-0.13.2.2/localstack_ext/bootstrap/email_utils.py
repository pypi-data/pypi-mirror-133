...
SENT_EMAILS=[]
EMAIL_BLACKLIST=rIUOf()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",rIUOC(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=rIUOM,from_name=rIUOM,smtp_host=rIUOM,smtp_user=rIUOM,smtp_pass=rIUOM,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if rIUOD(recipients,rIUOv)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=rIUOM):
 ...
 result["From"]=formataddr((rIUOC(Header(from_name,"utf-8")),from_email))
 return result
