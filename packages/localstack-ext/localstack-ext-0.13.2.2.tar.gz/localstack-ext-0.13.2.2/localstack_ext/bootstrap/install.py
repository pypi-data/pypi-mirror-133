...
def install_package(packages:Union[List[Twfby],Twfby],api_name:Twfby):
 if not(is_debian()and in_docker()):
  ...
  return
 if TwfbH(packages,Twfby):
  packages=packages.split()
 LOG.info("Downloading dependencies for %s API. This may take a while."%api_name)
 with INSTALL_LOCK:
  run(["apt-get","update"],shell=TwfbX)
  run(["apt-get","install","-y","--no-install-recommends"]+packages,shell=TwfbX)
def check_or_install(command:Twfby,packages:Union[List[Twfby],Twfby],api:Twfby):
...
def setup_ssl_cert():
 ...
 try:
  ...
  try:
   ...
   raise
def download_github_artifact(url:Twfby,target_file:Twfby,timeout:int=Twfbt):
 def do_download(url,print_error=TwfbX):
  try:
