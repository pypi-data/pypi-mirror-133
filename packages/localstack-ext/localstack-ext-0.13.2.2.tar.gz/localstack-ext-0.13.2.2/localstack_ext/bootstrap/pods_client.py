...
class PodInfo:
 def __init__(self,name=oqpXw,pod_size=0):
  self.name=name
  self.pod_size=pod_size
  self.pod_size_compressed=0
  self.persisted_resource_names=[]
class CloudPodManager(oqpXS):
 BACKEND="_none_"
 def __init__(self,pod_name=oqpXw,config=oqpXw):
  self.pod_name=pod_name
  self._pod_config=config
 def init(self):
  raise oqpXV("Not implemented")
 def delete(self,remote:oqpXJ)->oqpXJ:
  raise oqpXV("Not implemented")
 def push(self,comment:oqpXx=oqpXw,three_way:oqpXJ=oqpXm)->PodInfo:
  raise oqpXV("Not implemented")
 def push_overwrite(self,version:oqpXr,comment:oqpXx=oqpXw):
  pass
 def pull(self,inject_version_state:oqpXJ=oqpXm,reset_state_before:oqpXJ=oqpXm,lazy:oqpXJ=oqpXm):
  raise oqpXV("Not implemented")
 def commit(self,message:oqpXx=oqpXw):
  raise oqpXV("Not implemented")
 def inject(self,version:oqpXr,reset_state:oqpXJ)->oqpXJ:
  raise oqpXV("Not implemented")
 def list_versions(self)->List[oqpXx]:
  raise oqpXV("Not implemented")
 def version_info(self,version:oqpXr):
  raise oqpXV("Not implemented")
 def version_metamodel(self,version:oqpXr)->Dict:
  raise oqpXV("Not implemented")
 def set_version(self,version:oqpXr,inject_version_state:oqpXJ,reset_state:oqpXJ,commit_before:oqpXJ):
  raise oqpXV("Not implemented")
 def list_version_commits(self,version:oqpXr)->List[oqpXx]:
  raise oqpXV("Not implemented")
 def get_commit_diff(self,version:oqpXr,commit:oqpXr)->Dict:
  raise oqpXV("Not implemented")
 def register_remote(self,pod_name:oqpXx,ci_pod:oqpXJ)->oqpXJ:
  raise oqpXV("Not implemented")
 def rename_pod(self,current_pod_name,new_pod_name)->oqpXJ:
  raise oqpXV("Not implemented")
 def list_pods(self,fetch_remote:oqpXJ)->Set[oqpXx]:
  raise oqpXV("Not implemented")
 ...
 def get(cls,pod_name,pre_config=oqpXw):
  ...
  raise oqpXV('Unable to find Cloud Pod manager implementation type "%s"'%backend)
 def deploy_pod_into_instance(self,pod_path):
  delete_pod_zip=oqpXm
  if os.path.isdir(pod_path):
   ...
   for folder in PERSISTED_FOLDERS:
    ...
    cp_r(src_folder,tgt_folder,rm_dest_on_conflict=oqpXz)
   pod_path=create_zip_file(tmpdir)
   rm_rf(tmpdir)
   delete_pod_zip=oqpXz
  zip_content=load_file(pod_path,mode="rb")
  url=get_pods_endpoint()
  result=requests.post(url,data=zip_content)
  if result.status_code>=400:
   raise oqpXV("Unable to restore pod state via local pods management API %s (code %s): %s"%(url,result.status_code,result.content))
  if delete_pod_zip:
   rm_rf(pod_path)
  else:
   return pod_path
 @oqpXN
 def get_state_zip_from_instance(get_content=oqpXm):
  url=f"{get_pods_endpoint()}/state"
  result=requests.get(url)
  if result.status_code>=400:
   raise oqpXV("Unable to get local pod state via management API %s (code %s): %s"%(url,result.status_code,result.content))
  if get_content:
   return result.content
  zip_file=f"{new_tmp_file()}.zip"
  save_file(zip_file,result.content)
  return zip_file
 def get_pod_info(self,pod_data_dir:oqpXx=oqpXw):
...
class CloudPodManagerCPVCS(CloudPodManager):
 BACKEND="cpvcs"
 @oqpXN
 def parse_pod_name_from_qualifying_name(qualifying_name:oqpXx)->oqpXx:
 ...
 @oqpXN
 def _get_max_version_for_pod_from_platform(pod_name:oqpXx,auth_headers):
  ...
  remote_max_ver=oqpXr(content["max_ver"])
 ...
 def _upload_version_and_product_space(self,presigned_urls):
  ...
  with oqpXc(version_space_archive,"rb")as version_space_content:
  ...
  for version_no,urls in presigned_meta_state_urls.items():
   ...
   with oqpXc(meta_archive,"rb")as meta_archive_content:
   ...
   with oqpXc(state_archive,"rb")as state_archive_content:
 ...
 def _add_state_to_cpvs_store(self):
  ...
  with ZipFile(zip_file,"r")as state_zip:
   ...
   api_states_traverse(api_states_path=api_states_path,side_effect=CloudPodManagerCPVCS._add_state_files_func,mutables=oqpXw)
  rm_rf(zip_file)
  rm_rf(tmp_dir)
 def _pull_versions(self,auth_headers,required_versions:oqpXx):
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
 ...
 def _clone_pod(self,auth_headers,lazy:oqpXJ=oqpXm):
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
 ...
 def delete(self,remote:oqpXJ)->oqpXJ:
  ...
  if remote:
   pass
  return oqpXm
 def _push_to_remote(self,url:oqpXx):
 ...
 def pull(self,inject_version_state:oqpXJ=oqpXm,reset_state_before:oqpXJ=oqpXm,lazy:oqpXJ=oqpXm):
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
  auth_headers=get_auth_headers()
  if self.pod_name in cpvcs_api.list_locally_available_pods(show_remote_or_local=oqpXm):
   cpvcs_api.set_pod_context(self.pod_name)
   ...
   if remote_max_ver==current_max_ver:
    ...
    return
   if not lazy:
    required_versions=",".join(oqpXP(lambda ver:oqpXx(ver),oqpXf(current_max_ver+1,remote_max_ver+1)))
   else:
 ...
 def _download_version_product(self,version:oqpXr,retain:oqpXJ=oqpXm)->Dict[oqpXx,oqpXx]:
 ...
 def _get_presigned_url_for_version_product(self,version:oqpXr)->Dict:
 ...
 def _inject_from_remote(self,version:oqpXr,retain:oqpXJ=oqpXm)->oqpXJ:
  from localstack_ext.bootstrap.cpvcs.utils.remote_utils import(extract_meta_and_state_archives)
  content=self._get_presigned_url_for_version_product(version=version)
  state_url=content.get("presigned_url_state")
  tmp_state_archive=new_tmp_file()
 ...
 def inject(self,version:oqpXr,reset_state:oqpXJ)->oqpXJ:
  ...
  if not cpvcs_api.config_context.pod_exists_locally(self.pod_name):
   ...
   if not state_archive_path and cpvcs_api.is_remotely_managed():
    ...
    product_archive_path=self._download_version_product(version=version,retain=oqpXz)
    if not product_archive_path:
     return oqpXm
    state_archive_path=cpvcs_api.PodFilePaths.get_version_state_archive(version)
  if reset_state:
   reset_local_state(reset_data_dir=oqpXz,exclude_from_reset=["dynamodb","kinesis","stepfunctions"])
 ...
 def version_metamodel(self,version:oqpXr)->Dict:
  ...
  if not result and cpvcs_api.is_remotely_managed():
   self._download_version_product(version=version,retain=oqpXz)
 ...
 def get_commit_diff(self,version:oqpXr,commit:oqpXr)->Dict:
  ...
  return commit_diff
 def register_remote(self,pod_name:oqpXx,ci_pod:oqpXJ=oqpXm)->oqpXJ:
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
  cpvcs_api.set_pod_context(self.pod_name)
 ...
 def list_pods(self,fetch_remote:oqpXJ)->Set[oqpXx]:
  ...
  return result
 @oqpXY
 def upload_content(cls,presigned_url:oqpXx,zip_data_content):
  res=safe_requests.put(presigned_url,data=zip_data_content)
  if res.status_code>=400:
   raise oqpXV("Unable to upload pod state to S3 (code %s): %s"%(res.status_code,res.content))
  return res
 @oqpXN
 def create_platform_url(request:oqpXx)->oqpXx:
...
class CloudPodManagerManaged(CloudPodManager):
 BACKEND="managed"
 def push(self,comment:oqpXx=oqpXw,three_way:oqpXJ=oqpXm)->PodInfo:
  zip_data_content=self.get_state_zip_from_instance(get_content=oqpXz)
  print('Pushing state of cloud pod "%s" to backend server (%s KB)'%(self.pod_name,format_number(oqpXU(zip_data_content)/1000.0)))
  self.push_content(self.pod_name,zip_data_content)
  print("Done.")
  result=self.get_pod_info()
  result.pod_size_compressed=oqpXU(zip_data_content)
  return result
 def pull(self,inject_version_state:oqpXJ=oqpXm,reset_state_before:oqpXJ=oqpXm,lazy:oqpXJ=oqpXm):
  ...
  rm_rf(zip_path)
 @oqpXN
 def presigned_url(pod_name:oqpXx,mode:oqpXx)->oqpXx:
  ...
  if response.status_code>=400:
   raise oqpXV("Unable to get cloud pod presigned URL (code %s): %s"%(response.status_code,content))
 ...
 def push_content(cls,pod_name,zip_data_content):
  ...
  res=safe_requests.put(presigned_url,data=zip_data_content)
  if res.status_code>=400:
   raise oqpXV("Unable to push pod state to API (code %s): %s"%(res.status_code,res.content))
  return res
class CloudPodManagerGit(CloudPodManager):
 BACKEND="git"
 def push(self,comment:oqpXx=oqpXw,three_way:oqpXJ=oqpXm):
  ...
  if branch not in repo:
   porcelain.branch_create(repo,branch,force=oqpXz)
 ...
 def pull(self,inject_version_state:oqpXJ=oqpXm,reset_state_before:oqpXJ=oqpXm,lazy:oqpXJ=oqpXm):
  ...
  if remote_ref not in remote_refs:
   raise oqpXV('Unable to find branch "%s" in remote git repo'%branch)
  remote_location=self.pod_config.get("url")
  self.switch_branch(branch)
 ...
 def client(self):
  ...
  return client,path
 def local_repo(self):
  self.clone_dir=oqpXG(self,"clone_dir",oqpXw)
...
class PodConfigManagerMeta(oqpXb):
 def __getattr__(cls,attr):
  def _call(*args,**kwargs):
   result=oqpXw
   for manager in cls.CHAIN:
    try:
     tmp=oqpXG(manager,attr)(*args,**kwargs)
     if tmp:
      if not result:
       result=tmp
      elif oqpXv(tmp,oqpXI)and oqpXv(result,oqpXI):
       result.extend(tmp)
    except oqpXV:
     if LOG.isEnabledFor(logging.DEBUG):
      LOG.exception("error during PodConfigManager call chain")
   if result is not oqpXw:
    return result
   raise oqpXV('Unable to run operation "%s" for local or remote configuration'%attr)
  return _call
class PodConfigManager(oqpXS,metaclass=PodConfigManagerMeta):
 CHAIN=[]
 @oqpXY
 def pod_config(cls,pod_name):
  ...
  if not pod_config:
   raise oqpXV('Unable to find config for pod named "%s"'%pod_name)
  return pod_config[0]
class PodConfigManagerLocal(oqpXS):
 CONFIG_FILE=".localstack.yml"
 def list_pods(self):
  local_pods=self._load_config(safe=oqpXz).get("pods",{})
  local_pods=[{"pod_name":k,"state":"Local Only",**v}for k,v in local_pods.items()]
  existing_names=oqpXC([pod["pod_name"]for pod in local_pods])
  result=[pod for pod in local_pods if pod["pod_name"]not in existing_names]
  return result
 def store_pod_metadata(self,pod_name,metadata):
  pass
 def _load_config(self,safe=oqpXm):
  try:
   return yaml.safe_load(to_str(load_file(self.CONFIG_FILE)))
  except oqpXV:
   if safe:
    return{}
   raise oqpXV('Unable to find and parse config file "%s"'%self.CONFIG_FILE)
class PodConfigManagerRemote(oqpXS):
 def list_pods(self):
  result=[]
  auth_headers=get_auth_headers()
  ...
  content=response.content
  if response.status_code>=400:
   raise oqpXV("Unable to fetch list of pods from API (code %s): %s"%(response.status_code,content))
 ...
 def store_pod_metadata(self,pod_name,metadata):
  ...
  content=response.content
  if response.status_code>=400:
   raise oqpXV("Unable to store pod metadata in API (code %s): %s"%(response.status_code,content))
...
def init_cpvcs(pod_name:oqpXx,pre_config:Dict[oqpXx,oqpXx],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.init()
def delete_pod(pod_name:oqpXx,remote:oqpXJ,pre_config:Dict[oqpXx,oqpXx])->oqpXJ:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 result=backend.delete(remote=remote)
 return result
def register_remote(pod_name:oqpXx,pre_config:Dict[oqpXx,oqpXx],**kwargs)->oqpXJ:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 result=backend.register_remote(pod_name=pod_name,ci_pod=pre_config.get("ci_pod",oqpXm))
 return result
def rename_pod(current_pod_name:oqpXx,new_pod_name:oqpXx,pre_config:Dict[oqpXx,oqpXx],**kwargs):
 ...
 return result
def list_pods_cpvcs(remote:oqpXJ,pre_config:Dict[oqpXx,oqpXx],**kwargs)->List[oqpXx]:
 backend=CloudPodManager.get(pod_name="",pre_config=pre_config)
 result=backend.list_pods(fetch_remote=remote)
 return result
def commit_state(pod_name:oqpXx,pre_config:Dict[oqpXx,oqpXx],message:oqpXx=oqpXw,**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.commit(message=message)
def inject_state(pod_name:oqpXx,version:oqpXr,reset_state:oqpXJ,pre_config:Dict[oqpXx,oqpXx],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 result=backend.inject(version=version,reset_state=reset_state)
 return result
def list_versions(pod_name:oqpXx,pre_config:Dict[oqpXx,oqpXx],**kwargs)->List[oqpXx]:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 versions=backend.list_versions()
 return versions
def get_version_info(version:oqpXr,pod_name:oqpXx,pre_config:Dict[oqpXx,oqpXx],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 info=backend.version_info(version=version)
 return info
def get_version_metamodel(version:oqpXr,pod_name:oqpXx,pre_config:Dict[oqpXx,oqpXx],**kwargs)->Dict:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 metamodel=backend.version_metamodel(version=version)
 return metamodel
def set_version(version:oqpXr,inject_version_state:oqpXJ,reset_state:oqpXJ,commit_before:oqpXJ,pod_name:oqpXx,pre_config:Dict[oqpXx,oqpXx],**kwargs)->oqpXJ:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 ...
 return success
def list_version_commits(version:oqpXr,pod_name:oqpXx,pre_config:Dict[oqpXx,oqpXx])->List[oqpXx]:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 commits=backend.list_version_commits(version=version)
 return commits
def get_commit_diff(version:oqpXr,commit:oqpXr,pod_name:oqpXx,pre_config:Dict[oqpXx,oqpXx])->Dict:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 commit_diff=backend.get_commit_diff(version=version,commit=commit)
 return commit_diff
def push_overwrite(version:oqpXr,pod_name:oqpXx,comment:oqpXx,pre_config:Dict[oqpXx,oqpXx]):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.push_overwrite(version=version,comment=comment)
def push_state(pod_name,pre_config=oqpXw,squash_commits=oqpXm,comment=oqpXw,three_way=oqpXm,**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
...
def pull_state(pod_name,inject_version_state=oqpXm,reset_state_before=oqpXm,lazy=oqpXm,**kwargs):
 pre_config=kwargs.get("pre_config",oqpXw)
 if not pod_name:
  raise oqpXV("Need to specify a pod name")
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.pull(inject_version_state=inject_version_state,reset_state_before=reset_state_before,lazy=lazy)
 print("Done.")
def reset_local_state(reset_data_dir=oqpXm,exclude_from_reset:List[oqpXx]=oqpXw):
 url=f"{get_pods_endpoint()}/state"
 if reset_data_dir:
  url+="/datadir"
 ...
 if result.status_code>=400:
  raise oqpXV("Unable to reset service state via local management API %s (code %s): %s"%(url,result.status_code,result.content))
 print("Done.")
def list_pods(args):
 return PodConfigManager.list_pods()
def get_data_dir_from_container()->oqpXx:
 try:
...
def get_persisted_resource_names(data_dir)->List[oqpXx]:
 names=[]
 ...
 with os.scandir(os.path.join(data_dir,"api_states"))as entries:
  for entry in entries:
   if entry.is_dir()and oqpXU(os.listdir(entry.path))>0:
    names.append(entry.name)
