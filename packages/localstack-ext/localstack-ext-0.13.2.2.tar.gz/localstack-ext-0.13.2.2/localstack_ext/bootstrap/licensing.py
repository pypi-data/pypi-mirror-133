...
class KeyActivationError(UqmBg):
 pass
...
class InvalidDecryptionKeyError(KeyActivationError):
 pass
def is_enterprise():
 return read_api_key(raise_if_missing=UqmBE)=="enterprise"
def read_api_key(raise_if_missing=UqmBK):
 key=(os.environ.get(ENV_LOCALSTACK_API_KEY)or "").strip()
 if not key and raise_if_missing:
  raise UqmBg("Unable to retrieve API key. Please configure $%s in your environment"%ENV_LOCALSTACK_API_KEY)
 return key
def truncate_api_key(api_key:UqmBy):
 return '"%s..."(%s)'%(api_key[:3],UqmBF(api_key))
def fetch_key()->UqmBO:
 ...
 if api_key=="test":
  ...
  key_base64=load_cached_key(api_key=api_key,check_timeout=UqmBE)
  decoded_key=base64.b64decode(key_base64)
  return decoded_key
 proxies=get_proxies()or UqmBP
 data={"api_key":api_key,"version":__version__}
 try:
  logging.getLogger("py.warnings").setLevel(logging.ERROR)
  result=requests.post("%s/activate"%config.API_URL,json.dumps(data),verify=UqmBE,proxies=proxies)
  ...
  if log_license_issues():
   api_key=UqmBy(api_key_configured()or "")
...
def cache_key_locally(api_key,key_b64):
 timestamp=UqmBy(UqmBx(now_utc()))
 key_raw=to_str(base64.b64decode(key_b64))
 for i in UqmBk(UqmBF(timestamp)):
  key_raw=str_insert(key_raw,i*2,timestamp[i])
 key_b64=to_str(base64.b64encode(to_bytes(key_raw)))
 cache=get_key_cache()
 cache.update({"timestamp":UqmBx(timestamp),"key_hash":md5(api_key),"key":key_b64})
 cache.save()
def load_cached_key(api_key:UqmBy,check_timeout:bool=UqmBK)->UqmBy:
 cache=get_key_cache()
 ...
 timestamp=UqmBy(cache["timestamp"])
 ...
 key_b64=to_str(base64.b64encode(to_bytes(key_raw)))
 return key_b64
def enable_file_decryption(key:UqmBO):
 ...
 try:
  ...
  if "import" not in to_str(file_content):
   raise UqmBY("Decryption resulted in invalid python file!")
 except UqmBg:
...
def prepare_environment():
 ...
 if not ENV_PREPARED.get("finalized"):
  try:
   key=fetch_key()
   if not key:
    raise UqmBg("Unable to fetch and validate API key from environment")
   if to_str(key)!="test":
...
def api_key_configured():
 return read_api_key(raise_if_missing=UqmBE)
def is_logged_in():
 return UqmBK if get_auth_cache().get("token")else UqmBE
def get_auth_headers()->Dict:
 ...
 api_key=read_api_key(raise_if_missing=UqmBE)
 if api_key:
  return{"ls-api-key":api_key,"ls-version":__version__}
 raise UqmBg("Please log in first")
