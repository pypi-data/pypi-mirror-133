...
class DecryptionHandler:
 decryption_key:nojEg
 def __init__(self,decryption_key:nojEg):
  self.decryption_key=decryption_key
 def decrypt(self,content)->nojEg:
...
class EncryptedFileFinder(MetaPathFinder):
 ...
 def find_spec(self,fullname,path,target=nojEB):
  if path and not nojEG(path,nojEH):
   path=nojEH(nojEL(path,"_path",[]))
  if not path:
   return nojEB
...
def patch_traceback_lines():
 if nojEL(traceback.FrameSummary,"_ls_patch_applied",nojEB):
  return
 @nojEi
 def line(self):
  try:
 ...
 nojEV(traceback.FrameSummary,"line",line)
 traceback.FrameSummary._ls_patch_applied=nojEa
def patch_inspect_findsource():
 if nojEL(inspect,"_ls_patch_applied",nojEB):
  return
 ...
 inspect._ls_patch_applied=nojEa
