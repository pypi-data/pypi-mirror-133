...
def install_package(packages:Union[List[GqIJS],GqIJS],api_name:GqIJS):
 if not(is_debian()and in_docker()):
  ...
  return
 if GqIJH(packages,GqIJS):
  packages=packages.split()
 LOG.info("Downloading dependencies for %s API. This may take a while."%api_name)
 with INSTALL_LOCK:
  run(["apt-get","update"],shell=GqIJr)
  run(["apt-get","install","-y","--no-install-recommends"]+packages,shell=GqIJr)
def check_or_install(command:GqIJS,packages:Union[List[GqIJS],GqIJS],api:GqIJS):
...
def setup_ssl_cert():
 ...
 try:
  ...
  try:
   ...
   raise
def download_github_artifact(url:GqIJS,target_file:GqIJS,timeout:int=GqIJn):
 def do_download(url,print_error=GqIJr):
  try:
