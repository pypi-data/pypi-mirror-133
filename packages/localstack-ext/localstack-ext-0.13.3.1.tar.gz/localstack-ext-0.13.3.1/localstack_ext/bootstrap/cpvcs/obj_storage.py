...
class StateFileLocator(abc.ABC):
 location="_none_"
 active_instance=WUFHj
 @abc.abstractmethod
 def get_state_file_location_by_key(self,key:WUFHh)->WUFHh:
  pass
 @WUFHa
 def get(cls,requested_file_locator:WUFHh):
...
class ObjectStorageProvider(abc.ABC):
 ...
 active_instance=WUFHj
 @WUFHa
 def get(cls,state_file_locator:WUFHh,requested_storage:WUFHh,serializers:Dict[WUFHh,CPVCSSerializer]):
  ...
  return cls.active_instance
 def __init__(self,state_file_locator:StateFileLocator,serializers:Dict[WUFHh,CPVCSSerializer]):
 ...
 def get_state_file_location_by_key(self,key:WUFHh)->WUFHh:
  pass
 @abc.abstractmethod
 def get_version_by_key(self,key:WUFHh)->Version:
  pass
 @abc.abstractmethod
 def get_revision_by_key(self,key:WUFHh)->Revision:
  pass
 @abc.abstractmethod
 def get_revision_or_version_by_key(self,key:WUFHh)->Union[Revision,Version]:
  pass
 @abc.abstractmethod
 def get_delta_file_by_key(self,key:WUFHh)->WUFHh:
  pass
 @abc.abstractmethod
 def upsert_objects(self,*args:CPVCSObj)->List[WUFHh]:
  pass
 @abc.abstractmethod
 def update_revision_key(self,old_key:WUFHh,new_key:WUFHh,referenced_by_version:WUFHh=WUFHj):
  pass
 @abc.abstractmethod
 def version_exists(self,key:WUFHh)->WUFHq:
  pass
 @abc.abstractmethod
 def merge_remote_into_local_version(self,remote_location:WUFHh,key:WUFHh):
  pass
 @abc.abstractmethod
 def _update_key(self,old_key:WUFHh,new_key:WUFHh,base_path:WUFHh)->WUFHq:
  pass
 @abc.abstractmethod
 def _serialize(self,*args:CPVCSObj)->List[WUFHh]:
  pass
 @abc.abstractmethod
 def _deserialize(self,key_serializer:Dict[WUFHh,WUFHh],remote_location:WUFHh=WUFHj)->List[CPVCSObj]:
  pass
class ObjectStorageLocal(ObjectStorageProvider):
 ...
 def _serialize(self,*args:CPVCSObj)->List[WUFHh]:
  ...
  for cpvcs_obj in args:
   if WUFHf(cpvcs_obj,Version):
    hash_refs.append(self.serializers["version"].store_obj(cpvcs_obj))
   elif WUFHf(cpvcs_obj,Revision):
    hash_refs.append(self.serializers["revision"].store_obj(cpvcs_obj))
  return hash_refs
 def _deserialize(self,key_serializer:Dict[WUFHh,WUFHh],remote_location:WUFHh=WUFHj)->List[Union[Revision,Version]]:
