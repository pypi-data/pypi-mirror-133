...
def random_hash()->Donjp:
 return DonjQ(random.getrandbits(160))
def compute_file_hash(file_path:Donjp,accum=DonjP)->Optional[Donjp]:
 try:
  with DonjM(file_path,"rb")as fp:
   if accum:
...
def compute_version_archive_hash(version_no:DonjN,state_archive:Donjp)->Donjp:
 ...
 m.update(Donjp(version_no).encode("utf-8"))
 rm_rf(tmp_state_dir)
 return m.hexdigest()
def compute_revision_hash(cpvcs_node:Revision)->Donjp:
 if not cpvcs_node.state_files:
  return random_hash()
 state_file_keys=Donjz(lambda state_file:state_file.hash_ref,cpvcs_node.state_files)
 m=hashlib.sha1()
 for key in state_file_keys:
  try:
   with DonjM(os.path.join(config_context.get_obj_store_path(),key),"rb")as fp:
    m.update(fp.read())
  except Donjd as e:
   LOG.warning(f"Failed to open file and compute hash for {key}: {e}")
 if Donjf(cpvcs_node,Revision):
  m.update(cpvcs_node.rid.encode("utf-8"))
  m.update(Donjp(cpvcs_node.revision_number).encode("utf-8"))
 elif Donjf(cpvcs_node,Version):
  m.update(Donjp(cpvcs_node.version_number).encode("utf-8"))
 return m.hexdigest()
