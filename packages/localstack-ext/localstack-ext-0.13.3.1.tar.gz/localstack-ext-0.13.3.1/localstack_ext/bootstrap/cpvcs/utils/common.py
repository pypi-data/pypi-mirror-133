...
class CPVCSConfigContext:
 ...
 def get_version_state_archive_path(self,version,with_format:VdOrk=VdOrN)->VdOrs:
 ...
 def update_ver_log(self,author:VdOrs,ver_no:VdOrp,rev_id:VdOrs,rev_no:VdOrp):
  with VdOrP(self.get_ver_log_path(),"a")as fp:
   fp.write(f"{VER_LOG_STRUCTURE.format(author=author, ver_no=ver_no, rev_rid_no=f'{rev_id}_{rev_no}')}\n")
 def create_version_symlink(self,name:VdOrs,key:VdOrs=VdOrm)->VdOrs:
  return self._create_symlink(name,key,self.get_ver_refs_path())
 def create_revision_symlink(self,name:VdOrs,key:VdOrs=VdOrm)->VdOrs:
  return self._create_symlink(name,key,self.get_rev_refs_path())
 def is_initialized(self)->VdOrk:
  return self.pod_root_dir and os.path.isdir(self.pod_root_dir)
 def _create_symlink(self,name:VdOrs,key:VdOrs,path:VdOrs)->VdOrs:
...
def zip_directories(zip_dest:VdOrs,directories:List[VdOrs])->VdOrs:
 ...
 return zip_dest
def add_file_to_archive(archive:VdOrs,entry_name:VdOrs,content:Union[VdOrs,VdOro]):
 with zipfile.ZipFile(archive,"a")as zip_file:
  zip_file.writestr(entry_name,content)
def read_file_from_archive(archive_path:VdOrs,file_name:VdOrs)->VdOrs:
 try:
