...
def extract_meta_and_state_archives(meta_archives:Dict[evHQk,evHQk],state_archives:Dict[evHQk,evHQk]):
  ...
  for version_no,archive in product_space_archive.items():
   with zipfile.ZipFile(archive)as meta_zip:
    if product_space_archive==meta_archives:
     archive_dest=config_context.get_version_meta_archive_path(version=version_no,with_format=evHQT)
    else:
     archive_dest=config_context.get_version_state_archive_path(version=version_no,with_format=evHQT)
...
def register_remote(remote_info:Dict[evHQk,evHQk]):
 if config_context.is_remotly_managed():
  LOG.warning("Pod is already remotely managed")
  return
 with evHQt(config_context.get_remote_info_path(),"w")as fp:
...
def merge_version_space(version_space_archive):
 ...
 for version_ref_file in os.listdir(remote_config_context.get_ver_refs_path()):
  ...
  with evHQt(remote_version_ref_file_path,"r")as fp:
