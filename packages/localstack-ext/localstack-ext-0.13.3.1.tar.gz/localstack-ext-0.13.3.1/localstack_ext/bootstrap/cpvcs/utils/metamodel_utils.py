...
CAPTURE_METAMODEL_SEPARATELY=nzMmO
...
class MetamodelDelta(SubtypesInstanceManager):
 def create_delta_for_state_files(self,sf1:StateFileRef,sf2:StateFileRef):
  ...
  if not sf1:
   backend2=load_persisted_object(config_context.get_obj_file_path(sf2.hash_ref))
   clazz=nzMms(backend2)
   backend1=_infer_backend_init(clazz,sf2)
  else:
   backend1=load_persisted_object(config_context.get_obj_file_path(sf1.hash_ref))
   clazz=nzMms(backend1)
   backend2=_infer_backend_init(clazz,sf1)
  diff=self.create_delta_json(backend1,backend2)
  return diff
 def create_delta_log(self,state_from:Set[StateFileRef],state_to:Set[StateFileRef])->nzMmk:
  ...
  def _create_sf_lookup(state_files:Set[StateFileRef])->Dict[nzMmk,StateFileRef]:
  ...
  for state_file_from in state_files_from_regular:
   ...
   if state_file_from.any_congruence(state_files_to_regular):
    ...
    diff_json=self.create_delta_for_state_files(state_file_from,nzMmw)
  ...
  for state_files_to in state_files_to_lookup.values():
   ...
   diff_json=self.create_delta_for_state_files(nzMmw,state_files_to)
...
class CommitMetamodelUtils:
 @nzMmt
 def create_metadata_archive(cls,version:Version,delete_reference:bool=nzMmO,overwrite:bool=nzMmO,metamodels_file:nzMmk=nzMmw):
 ...
 def get_metamodel_delta(cls,prev_metamodel:Dict,this_metamodel:Dict)->Dict:
  ...
  return result
 @nzMmt
 def create_delta_metamodel_file(cls,version:nzMmj,revision:nzMmj,store_to_zip=nzMmO)->Tuple[Dict,Dict]:
 ...
 @nzMmt
 def reconstruct_metamodel(cls,version:nzMmj,revision:nzMmj=nzMmw)->Dict:
  result={}
  for rev in nzMmc(1,revision+1):
  ...
  return result
 @nzMmt
 def get_version_metamodel(cls,version:nzMmj,revision:nzMmj=nzMmw)->Dict:
 ...
 @nzMmt
 def get_commit_diff(cls,version_no:nzMmj,commit_no:nzMmj)->Optional[Dict]:
  ...
  return result
def _infer_backend_init(clazz:Type,sf:StateFileRef)->nzMmA:
 if nzMmx(clazz,nzMmT):
  return{}
 constructor=nzMmQ(clazz,"__init__",nzMmw)
...
def _create_metamodel_helper(obj,width=25,visited:set=nzMmw):
 if obj is nzMmw:
  return obj
