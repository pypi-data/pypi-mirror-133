...
ROOT_DIR_LOOKUP={sMvaK(Serialization.KINESIS):KINESIS_DIR,sMvaK(Serialization.DDB):DYNAMODB_DIR,sMvaK(Serialization.MAIN):API_STATES_DIR}
def init(pod_name="My-Pod"):
 ...
 r0_hash=random_hash()
 v0_hash=random_hash()
 r0=Revision(hash_ref=r0_hash,parent_ptr=NIL_PTR,creator=config_context.get_context_user(),rid=short_uid(),revision_number=0,state_files=sMvaV())
 v0=Version(hash_ref=v0_hash,parent_ptr=NIL_PTR,creator=config_context.get_context_user(),comment="Init version",active_revision_ptr=r0_hash,outgoing_revision_ptrs={r0_hash},incoming_revision_ptr=sMvar,state_files=sMvaV(),version_number=0)
 ...
 with sMvah(config_context.get_head_path(),"w")as fp:
  fp.write(ver_symlink)
 with sMvah(config_context.get_max_ver_path(),"w")as fp:
  fp.write(ver_symlink)
 with sMvah(config_context.get_known_ver_path(),"w")as fp:
  fp.write(ver_symlink)
...
def init_remote(version_space_archive:sMvaK,meta_archives:Dict[sMvaK,sMvaK],state_archives:Dict[sMvaK,sMvaK],remote_info:Dict[sMvaK,sMvaK],pod_name:sMvaK):
...
def merge_from_remote(version_space_archive:sMvaK,meta_archives:Dict[sMvaK,sMvaK],state_archives:Dict[sMvaK,sMvaK]):
 merge_version_space(version_space_archive)
 extract_meta_and_state_archives(meta_archives=meta_archives,state_archives=state_archives)
def is_remotely_managed()->sMvaO:
 return config_context.is_remotly_managed()
def set_pod_context(pod_name:sMvaK):
 config_context.set_pod_context(pod_name)
def rename_pod(new_pod_name:sMvaK)->sMvaO:
...
def list_locally_available_pods(show_remote_or_local:sMvaO=sMvaf)->Set[sMvaK]:
 ...
 for available_pod in available_pods:
  ...
  result.add(pod_name)
 return result
def create_state_file_from_fs(path:sMvaK,file_name:sMvaK,service:sMvaK,region:sMvaK,root:sMvaK,serialization:Serialization)->sMvaK:
 ...
 return key
def _create_state_file_from_in_memory_blob(blob)->sMvaK:
 ...
 return key
def _get_state_file_path(key:sMvaK)->sMvaK:
...
def _add_state_file_to_expansion_point(state_file:StateFileRef):
 ...
 updated_state_files=sMvaV(sMvau(lambda sf:not sf.congruent(state_file),revision.state_files))
 updated_state_files.add(state_file)
 revision.state_files=updated_state_files
 object_storage.upsert_objects(revision)
def list_state_files(key:sMvaK)->Optional[sMvaK]:
...
def get_version_info(version_no:sMvae)->Dict[sMvaK,sMvaK]:
 ...
 return result
def commit(message:sMvaK=sMvar)->Revision:
 ...
 curr_expansion_point_parent_state_files=sMvaV()
 if curr_expansion_point.parent_ptr!=NIL_PTR:
  referenced_by_version=sMvar
 ...
 new_expansion_point=Revision(hash_ref=random_hash(),state_files=sMvaV(),parent_ptr=curr_expansion_point_hash,creator=curr_expansion_point.creator,rid=short_uid(),revision_number=curr_expansion_point.revision_number+1)
...
def create_version_space_archive()->sMvaK:
...
def get_max_version_no()->sMvae:
 with sMvah(config_context.get_max_ver_path())as fp:
  return sMvae(os.path.basename(fp.readline()))
...
def push_overwrite(version:sMvae,comment:sMvaK)->sMvaO:
 ...
 CommitMetamodelUtils.create_metadata_archive(version_node,overwrite=sMvaf,metamodels_file=metamodels_file)
 version_node.comment=comment
 object_storage.upsert_objects(version_node)
 return sMvaf
def push(comment:sMvaK=sMvar,three_way:sMvaO=sMvaj)->Version:
 expansion_point,head_version=_get_expansion_point_with_head()
 max_version=_get_max_version()
 new_active_revision=Revision(hash_ref=random_hash(),state_files=sMvaV(),parent_ptr=NIL_PTR,creator=expansion_point.creator,rid=short_uid(),revision_number=0)
 ...
 new_version=Version(hash_ref=compute_version_archive_hash(new_max_version_no,PodFilePaths.get_version_state_archive(new_max_version_no)),state_files=sMvaV(),parent_ptr=max_version.hash_ref,creator=expansion_point.creator,comment=comment,active_revision_ptr=new_active_revision.hash_ref,outgoing_revision_ptrs={new_active_revision.hash_ref},incoming_revision_ptr=expansion_point.hash_ref,version_number=new_max_version_no)
 if expansion_point.parent_ptr!=NIL_PTR:
  ...
  delta_log_ptr=create_delta_log(expansion_point.state_files,sMvaV())
 ...
 return new_version
def _get_dst_path_for_state_file(version_state_dir:sMvaK,state_file:StateFileRef):
 if state_file.serialization in[sMvaK(Serialization.KINESIS),sMvaK(Serialization.DDB)]:
 ...
 mkdir(dst_path)
 return dst_path
def _create_state_directory(version_number:sMvae,state_file_refs:Set[StateFileRef],delete_files=sMvaj,archive=sMvaf):
...
def set_active_version(version_no:sMvae,commit_before=sMvaj)->sMvaO:
 ...
 return sMvaj
def _set_active_version(key:sMvaK):
 ...
 if current_head.hash_ref!=key and object_storage.version_exists(key):
  ...
  if requested_version.active_revision_ptr==NIL_PTR:
   new_path_root=Revision(hash_ref=random_hash(),state_files=sMvaV(),parent_ptr=NIL_PTR,creator=config_context.get_context_user(),rid=short_uid(),revision_number=0)
...
def get_version_by_number(version_no:sMvae)->Version:
 versions=load_version_references()
 version_ref=sMvaP((version[1]for version in versions if version[0]==version_no),sMvar)
 if not version_ref:
  LOG.warning(f"Could not find version number {version_no}")
  return
 return object_storage.get_version_by_key(version_ref)
def load_version_references()->List[Tuple[sMvae,sMvaK]]:
 result={}
 with sMvah(config_context.get_known_ver_path(),"r")as vp:
  symlinks=vp.readlines()
  for symlink in symlinks:
   symlink=config_context.get_pod_absolute_path(symlink.rstrip())
   with sMvah(symlink,"r")as sp:
    result[sMvae(os.path.basename(symlink))]=sp.readline()
 return sMvaD(result.items(),key=lambda x:x[0],reverse=sMvaf)
def list_versions()->List[sMvaK]:
 ...
 return result
def list_version_commits(version_no:sMvae)->List[sMvaK]:
 if version_no==-1:
 ...
 return result
def _update_head(new_head_ver_no,new_head_key)->sMvaK:
 with sMvah(config_context.get_head_path(),"w")as fp:
  ver_symlink=config_context.create_version_symlink(VER_SYMLINK.format(ver_no=new_head_ver_no),new_head_key)
  fp.write(ver_symlink)
  return ver_symlink
def _update_max_ver(new_max_ver_no,new_max_ver_key)->sMvaK:
 with sMvah(config_context.get_max_ver_path(),"w")as fp:
...
def _add_known_ver(new_ver_no,new_ver_key)->sMvaK:
 with sMvah(config_context.get_known_ver_path(),"a")as fp:
...
def merge_expansion_point_with_max(three_way=sMvaj):
 ...
 for serialization_mechanism,new_version_state_files_path in new_version_tmp_dirs.items():
  try:
   merge_manager=CPVCSMergeManager.get(serialization_mechanism,raise_if_missing=sMvaf)
...
def create_delta_log(state_from:Set[StateFileRef],state_to:Set[StateFileRef],diff_method:MetamodelDeltaMethod=MetamodelDeltaMethod.SIMPLE)->sMvaK:
 try:
