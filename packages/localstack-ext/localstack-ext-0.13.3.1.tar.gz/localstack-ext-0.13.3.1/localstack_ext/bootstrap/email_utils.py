...
SENT_EMAILS=[]
EMAIL_BLACKLIST=VHksU()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",VHksf(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=VHksy,from_name=VHksy,smtp_host=VHksy,smtp_user=VHksy,smtp_pass=VHksy,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if VHksi(recipients,VHksh)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=VHksy):
 ...
 result["From"]=formataddr((VHksf(Header(from_name,"utf-8")),from_email))
 return result
