...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return iIBby if e else iIBbc
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->iIBbE:
...
@click.option("--revert",is_flag=iIBby,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:iIBbU):
...
def _cpvcs_initialized(pod_name:iIBbm)->iIBbU:
...
@click.option("-n","--name",help="Name of the cloud pod",required=iIBby)
def cmd_cpvcs_init(name:iIBbm):
...
@click.option("-n","--name",help="Name of the cloud pod",required=iIBby)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=iIBby,default=iIBbc)
def cmd_cpvcs_delete(name:iIBbm,remote:iIBbU):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=iIBby)
def cmd_cpvcs_register_remote(name:iIBbm):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=iIBby)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=iIBby)
def cmd_cpvcs_rename(name:iIBbm,new_name:iIBbm):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=iIBby)
def cmd_cpvcs_commit(message:iIBbm,name:iIBbm):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=iIBby,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("--three-way",is_flag=iIBby,default=iIBbc,help="")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=iIBby)
def cmd_cpvcs_push(squash:iIBbU,message:iIBbm,name:iIBbm,three_way:iIBbU):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=iIBby)
@click.option("-v","--version",type=iIBbJ)
@click.option("-m","--message",required=iIBbc)
def cmd_cpvcs_push_overwrite(version:iIBbJ,message:iIBbm,name:iIBbm):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=iIBbJ,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=iIBby,default=iIBbc,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=iIBby)
def cmd_cpvcs_inject(version:iIBbJ,reset:iIBbU,name:iIBbm):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--inject/--no-inject",default=iIBby,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=iIBby,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=iIBby)
@click.option("--lazy/--eager",default=iIBby,help="Will only fetch references to existing versions, i.e. version state is only downloaded when required")
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:iIBbm,inject:iIBbU,reset:iIBbU,lazy:iIBbU):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=iIBby,default=iIBbc)
def cmd_cpvcs_list_pods(remote:iIBbU):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=iIBby)
def cmd_cpvcs_versions(name:iIBbm):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=iIBby,type=iIBbJ)
@click.option("-n","--name",help="Name of the cloud pod",required=iIBby)
def cmd_cpvcs_version_info(version:iIBbJ,name:iIBbm):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=iIBbJ,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=iIBby)
def cmd_cpvcs_version_metamodel(version:iIBbJ,name:iIBbm):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=iIBby,type=iIBbJ,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=iIBby,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=iIBby,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=iIBbc,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=iIBby)
def cmd_cpvcs_set_version(version:iIBbJ,inject:iIBbU,reset:iIBbU,commit_before:iIBbU,name:iIBbm):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=iIBby)
def cmd_cpvcs_commits(version:iIBbJ,name:iIBbm):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=iIBby)
@click.option("--commit","-c",required=iIBby)
@click.option("-n","--name",help="Name of the cloud pod",required=iIBby)
def cmd_cpvcs_commit_diff(version:iIBbJ,commit:iIBbJ,name:iIBbm):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(iIBbE)
  status.stop()
...
def print_table(columns:Dict[iIBbm,iIBbm],rows:List[Dict[iIBbm,Any]],formatters:Dict[iIBbm,Callable[[Any],iIBbm]]=iIBbE):
 from rich.table import Table
 if formatters is iIBbE:
  formatters=iIBbs()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  iIBbO(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->iIBbE:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->iIBbE:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->iIBbE:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->iIBbE:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
