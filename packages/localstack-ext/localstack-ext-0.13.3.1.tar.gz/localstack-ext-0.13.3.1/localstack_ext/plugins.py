...
def api_key_configured()->xkrlE:
 return(xkrlo if os.environ.get("LOCALSTACK_API_KEY")and os.environ.get("LOCALSTACK_API_KEY").strip()else xkrlq)
...
def configure_pro_container(container:LocalstackContainer):
 try:
  ...
  if xkrlS([is_api_enabled(api)for api in EXTERNAL_PORT_APIS]):
