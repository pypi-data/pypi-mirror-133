...
class EC2ElasticIP(GenericBaseModel):
 ...
 def cloudformation_type():
  return "AWS::EC2::EIP"
 def get_physical_resource_id(self,attribute=ayAvJ,**kwargs):
...
class SecurityGroupInOrEgress(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  for perm in perms:
   if ayAvw(perm["IpProtocol"])!=ayAvw(rp["IpProtocol"]):
    continue
 ...
 @ayAvF
 def get_vpc_and_name_for_security_group(cls,group_id:ayAvw)->Tuple[ayAvw,ayAvw]:
  client=aws_stack.connect_to_service("ec2")
  groups=client.describe_security_groups(GroupIds=[group_id])["SecurityGroups"]
  vpc_id=groups and groups[0].get("VpcId")or ayAvJ
  group_name=groups and groups[0]["GroupName"]or ayAvJ
 ...
 def get_deploy_templates(cls):
  def create_params(params,**kwargs):
   ...
   source_group_id=result.pop("SourceSecurityGroupId",ayAvJ)
   vpc_id=ayAvJ
   ...
   dst_group_id=result.pop("DestinationSecurityGroupId",ayAvJ)
   ...
   if result.get("IpProtocol"):
    result["IpProtocol"]=ayAvw(result.get("IpProtocol"))
   description=result.pop("Description",ayAvJ)
   cidr_ipv6=result.pop("CidrIpv6",ayAvJ)
