...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or howBv
 @howBA
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return howBn(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @howBM
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if howBs(origins,howBp)else origins.get("Items")
  origins_to_create=howBm([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if howBs(aliases,howBp)else aliases.get("Items")
  aliases_to_create=howBm(aliases)
  for distr in distrs:
   origins=howBm([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=howBm(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",howBP)
   config["Enabled"]=howBe(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if howBs(custom_errors,howBX)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=howBe(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":howBP}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=howBe(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=howBv):
    parent=config if parent is howBv else parent
    items=parent.get(key)or[]
    if howBs(items,howBX)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":howBN(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if howBs(cache_behavs,howBX)else cache_behavs)
