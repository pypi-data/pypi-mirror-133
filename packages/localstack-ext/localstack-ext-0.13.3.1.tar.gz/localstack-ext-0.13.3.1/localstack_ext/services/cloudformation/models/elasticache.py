...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=dNMCj(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not dNMCX
   return result
  except dNMCU:
   props=self.props
   ...
   if parts[0]in candidates and dNMCS(parts)>1:
 ...
 def fetch_details(cls,cluster_name):
  ...
  return(match or[dNMCX])[0]
 @dNMCb
 def get_deploy_templates():
  return{"create":{"function":"create_cache_cluster","parameters":lambda_convert_types(lambda_rename_attributes({"ClusterName":"CacheClusterId","VpcSecurityGroupIds":"SecurityGroupIds"}),{"NumCacheNodes":dNMCV})},"delete":{"function":"delete_cache_cluster","parameters":{"CacheClusterId":"ClusterName"}}}
