...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[TYAuz])[0]
 def get_physical_resource_id(self,attribute=TYAuz,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if TYAuj(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return TYAux(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @TYAuF
 def add_defaults(resource,stack_name:TYAui):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return TYAux(SESReceiptRule,self).get_cfn_attribute(attribute)
 @TYAuw
