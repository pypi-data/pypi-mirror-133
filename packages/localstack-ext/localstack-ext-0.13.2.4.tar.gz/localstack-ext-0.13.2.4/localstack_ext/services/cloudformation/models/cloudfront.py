...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or ilOkR
 @ilOkJ
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return ilOkA(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @ilOkT
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if ilOkE(origins,ilOkB)else origins.get("Items")
  origins_to_create=ilOkw([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if ilOkE(aliases,ilOkB)else aliases.get("Items")
  aliases_to_create=ilOkw(aliases)
  for distr in distrs:
   origins=ilOkw([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=ilOkw(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",ilOkp)
   config["Enabled"]=ilOkC(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if ilOkE(custom_errors,ilOkU)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=ilOkC(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":ilOkp}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=ilOkC(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=ilOkR):
    parent=config if parent is ilOkR else parent
    items=parent.get(key)or[]
    if ilOkE(items,ilOkU)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":ilOkg(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if ilOkE(cache_behavs,ilOkU)else cache_behavs)
