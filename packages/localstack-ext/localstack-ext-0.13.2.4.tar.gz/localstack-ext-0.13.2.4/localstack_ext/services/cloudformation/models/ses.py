...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[oTkcV])[0]
 def get_physical_resource_id(self,attribute=oTkcV,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if oTkcY(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return oTkcE(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @oTkcz
 def add_defaults(resource,stack_name:oTkcj):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return oTkcE(SESReceiptRule,self).get_cfn_attribute(attribute)
 @oTkcg
