...
class EC2ElasticIP(GenericBaseModel):
 ...
 def cloudformation_type():
  return "AWS::EC2::EIP"
 def get_physical_resource_id(self,attribute=oAVlv,**kwargs):
...
class SecurityGroupInOrEgress(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  for perm in perms:
   if oAVlf(perm["IpProtocol"])!=oAVlf(rp["IpProtocol"]):
    continue
 ...
 @oAVli
 def get_vpc_and_name_for_security_group(cls,group_id:oAVlf)->Tuple[oAVlf,oAVlf]:
  client=aws_stack.connect_to_service("ec2")
  groups=client.describe_security_groups(GroupIds=[group_id])["SecurityGroups"]
  vpc_id=groups and groups[0].get("VpcId")or oAVlv
  group_name=groups and groups[0]["GroupName"]or oAVlv
 ...
 def get_deploy_templates(cls):
  def create_params(params,**kwargs):
   ...
   source_group_id=result.pop("SourceSecurityGroupId",oAVlv)
   vpc_id=oAVlv
   ...
   dst_group_id=result.pop("DestinationSecurityGroupId",oAVlv)
   ...
   if result.get("IpProtocol"):
    result["IpProtocol"]=oAVlf(result.get("IpProtocol"))
   description=result.pop("Description",oAVlv)
   cidr_ipv6=result.pop("CidrIpv6",oAVlv)
