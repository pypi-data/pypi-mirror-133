...
def install_package(packages:Union[List[WhHlM],WhHlM],api_name:WhHlM):
 if not(is_debian()and in_docker()):
  ...
  return
 if WhHls(packages,WhHlM):
  packages=packages.split()
 LOG.info("Downloading dependencies for %s API. This may take a while."%api_name)
 with INSTALL_LOCK:
  run(["apt-get","update"],shell=WhHlt)
  run(["apt-get","install","-y","--no-install-recommends"]+packages,shell=WhHlt)
def check_or_install(command:WhHlM,packages:Union[List[WhHlM],WhHlM],api:WhHlM):
...
def setup_ssl_cert():
 ...
 try:
  ...
  try:
   ...
   raise
def download_github_artifact(url:WhHlM,target_file:WhHlM,timeout:int=WhHlT):
 def do_download(url,print_error=WhHlt):
  try:
