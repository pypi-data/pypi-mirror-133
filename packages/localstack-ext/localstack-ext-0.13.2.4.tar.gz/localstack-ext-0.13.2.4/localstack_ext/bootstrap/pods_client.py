...
class PodInfo:
 def __init__(self,name=xFLdv,pod_size=0):
  self.name=name
  self.pod_size=pod_size
  self.pod_size_compressed=0
  self.persisted_resource_names=[]
class CloudPodManager(xFLdU):
 BACKEND="_none_"
 def __init__(self,pod_name=xFLdv,config=xFLdv):
  self.pod_name=pod_name
  self._pod_config=config
 def init(self):
  raise xFLdD("Not implemented")
 def delete(self,remote:xFLdG)->xFLdG:
  raise xFLdD("Not implemented")
 def push(self,comment:xFLdE=xFLdv,three_way:xFLdG=xFLdV)->PodInfo:
  raise xFLdD("Not implemented")
 def push_overwrite(self,version:xFLdy,comment:xFLdE=xFLdv):
  pass
 def pull(self,inject_version_state:xFLdG=xFLdV,reset_state_before:xFLdG=xFLdV,lazy:xFLdG=xFLdV):
  raise xFLdD("Not implemented")
 def commit(self,message:xFLdE=xFLdv):
  raise xFLdD("Not implemented")
 def inject(self,version:xFLdy,reset_state:xFLdG)->xFLdG:
  raise xFLdD("Not implemented")
 def list_versions(self)->List[xFLdE]:
  raise xFLdD("Not implemented")
 def version_info(self,version:xFLdy):
  raise xFLdD("Not implemented")
 def version_metamodel(self,version:xFLdy)->Dict:
  raise xFLdD("Not implemented")
 def set_version(self,version:xFLdy,inject_version_state:xFLdG,reset_state:xFLdG,commit_before:xFLdG):
  raise xFLdD("Not implemented")
 def list_version_commits(self,version:xFLdy)->List[xFLdE]:
  raise xFLdD("Not implemented")
 def get_commit_diff(self,version:xFLdy,commit:xFLdy)->Dict:
  raise xFLdD("Not implemented")
 def register_remote(self,pod_name:xFLdE,ci_pod:xFLdG)->xFLdG:
  raise xFLdD("Not implemented")
 def rename_pod(self,current_pod_name,new_pod_name)->xFLdG:
  raise xFLdD("Not implemented")
 def list_pods(self,fetch_remote:xFLdG)->Set[xFLdE]:
  raise xFLdD("Not implemented")
 ...
 def get(cls,pod_name,pre_config=xFLdv):
  ...
  raise xFLdD('Unable to find Cloud Pod manager implementation type "%s"'%backend)
 def deploy_pod_into_instance(self,pod_path):
  delete_pod_zip=xFLdV
  if os.path.isdir(pod_path):
   ...
   for folder in PERSISTED_FOLDERS:
    ...
    cp_r(src_folder,tgt_folder,rm_dest_on_conflict=xFLdM)
   pod_path=create_zip_file(tmpdir)
   rm_rf(tmpdir)
   delete_pod_zip=xFLdM
  zip_content=load_file(pod_path,mode="rb")
  url=get_pods_endpoint()
  result=requests.post(url,data=zip_content)
  if result.status_code>=400:
   raise xFLdD("Unable to restore pod state via local pods management API %s (code %s): %s"%(url,result.status_code,result.content))
  if delete_pod_zip:
   rm_rf(pod_path)
  else:
   return pod_path
 @xFLdi
 def get_state_zip_from_instance(get_content=xFLdV):
  url=f"{get_pods_endpoint()}/state"
  result=requests.get(url)
  if result.status_code>=400:
   raise xFLdD("Unable to get local pod state via management API %s (code %s): %s"%(url,result.status_code,result.content))
  if get_content:
   return result.content
  zip_file=f"{new_tmp_file()}.zip"
  save_file(zip_file,result.content)
  return zip_file
 def get_pod_info(self,pod_data_dir:xFLdE=xFLdv):
...
class CloudPodManagerCPVCS(CloudPodManager):
 BACKEND="cpvcs"
 @xFLdi
 def parse_pod_name_from_qualifying_name(qualifying_name:xFLdE)->xFLdE:
 ...
 @xFLdi
 def _get_max_version_for_pod_from_platform(pod_name:xFLdE,auth_headers):
  ...
  remote_max_ver=xFLdy(content["max_ver"])
 ...
 def _upload_version_and_product_space(self,presigned_urls):
  ...
  with xFLdB(version_space_archive,"rb")as version_space_content:
  ...
  for version_no,urls in presigned_meta_state_urls.items():
   ...
   with xFLdB(meta_archive,"rb")as meta_archive_content:
   ...
   with xFLdB(state_archive,"rb")as state_archive_content:
    self.upload_content(state_presigned_url,state_archive_content)
 @xFLdi
 def _add_state_files_from_directory(service:xFLdE,path:xFLdE,root_dir:xFLdE,serialization:Serialization):
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
 ...
 def _add_state_to_cpvcs_store(self):
  ...
  with ZipFile(zip_file,"r")as state_zip:
   ...
   api_states_traverse(api_states_path=api_states_path,side_effect=CloudPodManagerCPVCS._add_state_files_func,mutables=xFLdv)
  ...
  rm_rf(zip_file)
  rm_rf(tmp_dir)
 def _pull_versions(self,auth_headers,required_versions:xFLdE):
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
 ...
 def _clone_pod(self,auth_headers,lazy:xFLdG=xFLdV):
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
 ...
 def delete(self,remote:xFLdG)->xFLdG:
  ...
  if remote:
   pass
  return xFLdV
 def _push_to_remote(self,url:xFLdE):
 ...
 def pull(self,inject_version_state:xFLdG=xFLdV,reset_state_before:xFLdG=xFLdV,lazy:xFLdG=xFLdV):
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
  auth_headers=get_auth_headers()
  if self.pod_name in cpvcs_api.list_locally_available_pods(show_remote_or_local=xFLdV):
   cpvcs_api.set_pod_context(self.pod_name)
   ...
   if remote_max_ver==current_max_ver:
    ...
    return
   if not lazy:
    required_versions=",".join(xFLdh(lambda ver:xFLdE(ver),xFLdk(current_max_ver+1,remote_max_ver+1)))
   else:
 ...
 def _download_version_product(self,version:xFLdy,retain:xFLdG=xFLdV)->Dict[xFLdE,xFLdE]:
 ...
 def _get_presigned_url_for_version_product(self,version:xFLdy)->Dict:
 ...
 def _inject_from_remote(self,version:xFLdy,retain:xFLdG=xFLdV)->xFLdG:
  from localstack_ext.bootstrap.cpvcs.utils.remote_utils import(extract_meta_and_state_archives)
  content=self._get_presigned_url_for_version_product(version=version)
  state_url=content.get("presigned_url_state")
  tmp_state_archive=new_tmp_file()
 ...
 def inject(self,version:xFLdy,reset_state:xFLdG)->xFLdG:
  ...
  if not cpvcs_api.config_context.pod_exists_locally(self.pod_name):
   ...
   if not state_archive_path and cpvcs_api.is_remotely_managed():
    ...
    product_archive_path=self._download_version_product(version=version,retain=xFLdM)
    if not product_archive_path:
     return xFLdV
    state_archive_path=cpvcs_api.PodFilePaths.get_version_state_archive(version)
  if reset_state:
   reset_local_state(reset_data_dir=xFLdM,exclude_from_reset=["dynamodb","kinesis","stepfunctions"])
 ...
 def version_metamodel(self,version:xFLdy)->Dict:
  ...
  if not result and cpvcs_api.is_remotely_managed():
   self._download_version_product(version=version,retain=xFLdM)
 ...
 def get_commit_diff(self,version:xFLdy,commit:xFLdy)->Dict:
  ...
  return commit_diff
 def register_remote(self,pod_name:xFLdE,ci_pod:xFLdG=xFLdV)->xFLdG:
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
  cpvcs_api.set_pod_context(self.pod_name)
 ...
 def list_pods(self,fetch_remote:xFLdG)->Set[xFLdE]:
  ...
  return result
 @xFLdR
 def upload_content(cls,presigned_url:xFLdE,zip_data_content):
  res=safe_requests.put(presigned_url,data=zip_data_content)
  if res.status_code>=400:
   raise xFLdD("Unable to upload pod state to S3 (code %s): %s"%(res.status_code,res.content))
  return res
 @xFLdi
 def create_platform_url(request:xFLdE)->xFLdE:
...
class CloudPodManagerManaged(CloudPodManager):
 BACKEND="managed"
 def push(self,comment:xFLdE=xFLdv,three_way:xFLdG=xFLdV)->PodInfo:
  zip_data_content=self.get_state_zip_from_instance(get_content=xFLdM)
  print('Pushing state of cloud pod "%s" to backend server (%s KB)'%(self.pod_name,format_number(xFLdt(zip_data_content)/1000.0)))
  self.push_content(self.pod_name,zip_data_content)
  print("Done.")
  result=self.get_pod_info()
  result.pod_size_compressed=xFLdt(zip_data_content)
  return result
 def pull(self,inject_version_state:xFLdG=xFLdV,reset_state_before:xFLdG=xFLdV,lazy:xFLdG=xFLdV):
  ...
  rm_rf(zip_path)
 @xFLdi
 def presigned_url(pod_name:xFLdE,mode:xFLdE)->xFLdE:
  ...
  if response.status_code>=400:
   raise xFLdD("Unable to get cloud pod presigned URL (code %s): %s"%(response.status_code,content))
 ...
 def push_content(cls,pod_name,zip_data_content):
  ...
  res=safe_requests.put(presigned_url,data=zip_data_content)
  if res.status_code>=400:
   raise xFLdD("Unable to push pod state to API (code %s): %s"%(res.status_code,res.content))
  return res
class CloudPodManagerGit(CloudPodManager):
 BACKEND="git"
 def push(self,comment:xFLdE=xFLdv,three_way:xFLdG=xFLdV):
  ...
  if branch not in repo:
   porcelain.branch_create(repo,branch,force=xFLdM)
 ...
 def pull(self,inject_version_state:xFLdG=xFLdV,reset_state_before:xFLdG=xFLdV,lazy:xFLdG=xFLdV):
  ...
  if remote_ref not in remote_refs:
   raise xFLdD('Unable to find branch "%s" in remote git repo'%branch)
  remote_location=self.pod_config.get("url")
  self.switch_branch(branch)
 ...
 def client(self):
  ...
  return client,path
 def local_repo(self):
  self.clone_dir=xFLdj(self,"clone_dir",xFLdv)
...
class PodConfigManagerMeta(xFLdI):
 def __getattr__(cls,attr):
  def _call(*args,**kwargs):
   result=xFLdv
   for manager in cls.CHAIN:
    try:
     tmp=xFLdj(manager,attr)(*args,**kwargs)
     if tmp:
      if not result:
       result=tmp
      elif xFLdm(tmp,xFLdX)and xFLdm(result,xFLdX):
       result.extend(tmp)
    except xFLdD:
     if LOG.isEnabledFor(logging.DEBUG):
      LOG.exception("error during PodConfigManager call chain")
   if result is not xFLdv:
    return result
   raise xFLdD('Unable to run operation "%s" for local or remote configuration'%attr)
  return _call
class PodConfigManager(xFLdU,metaclass=PodConfigManagerMeta):
 CHAIN=[]
 @xFLdR
 def pod_config(cls,pod_name):
  ...
  if not pod_config:
   raise xFLdD('Unable to find config for pod named "%s"'%pod_name)
  return pod_config[0]
class PodConfigManagerLocal(xFLdU):
 CONFIG_FILE=".localstack.yml"
 def list_pods(self):
  local_pods=self._load_config(safe=xFLdM).get("pods",{})
  local_pods=[{"pod_name":k,"state":"Local Only",**v}for k,v in local_pods.items()]
  existing_names=xFLdl([pod["pod_name"]for pod in local_pods])
  result=[pod for pod in local_pods if pod["pod_name"]not in existing_names]
  return result
 def store_pod_metadata(self,pod_name,metadata):
  pass
 def _load_config(self,safe=xFLdV):
  try:
   return yaml.safe_load(to_str(load_file(self.CONFIG_FILE)))
  except xFLdD:
   if safe:
    return{}
   raise xFLdD('Unable to find and parse config file "%s"'%self.CONFIG_FILE)
class PodConfigManagerRemote(xFLdU):
 def list_pods(self):
  result=[]
  auth_headers=get_auth_headers()
  ...
  content=response.content
  if response.status_code>=400:
   raise xFLdD("Unable to fetch list of pods from API (code %s): %s"%(response.status_code,content))
 ...
 def store_pod_metadata(self,pod_name,metadata):
  ...
  content=response.content
  if response.status_code>=400:
   raise xFLdD("Unable to store pod metadata in API (code %s): %s"%(response.status_code,content))
...
def init_cpvcs(pod_name:xFLdE,pre_config:Dict[xFLdE,xFLdE],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.init()
def delete_pod(pod_name:xFLdE,remote:xFLdG,pre_config:Dict[xFLdE,xFLdE])->xFLdG:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 result=backend.delete(remote=remote)
 return result
def register_remote(pod_name:xFLdE,pre_config:Dict[xFLdE,xFLdE],**kwargs)->xFLdG:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 result=backend.register_remote(pod_name=pod_name,ci_pod=pre_config.get("ci_pod",xFLdV))
 return result
def rename_pod(current_pod_name:xFLdE,new_pod_name:xFLdE,pre_config:Dict[xFLdE,xFLdE],**kwargs):
 ...
 return result
def list_pods_cpvcs(remote:xFLdG,pre_config:Dict[xFLdE,xFLdE],**kwargs)->List[xFLdE]:
 backend=CloudPodManager.get(pod_name="",pre_config=pre_config)
 result=backend.list_pods(fetch_remote=remote)
 return result
def commit_state(pod_name:xFLdE,pre_config:Dict[xFLdE,xFLdE],message:xFLdE=xFLdv,**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.commit(message=message)
def inject_state(pod_name:xFLdE,version:xFLdy,reset_state:xFLdG,pre_config:Dict[xFLdE,xFLdE],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 result=backend.inject(version=version,reset_state=reset_state)
 return result
def list_versions(pod_name:xFLdE,pre_config:Dict[xFLdE,xFLdE],**kwargs)->List[xFLdE]:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 versions=backend.list_versions()
 return versions
def get_version_info(version:xFLdy,pod_name:xFLdE,pre_config:Dict[xFLdE,xFLdE],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 info=backend.version_info(version=version)
 return info
def get_version_metamodel(version:xFLdy,pod_name:xFLdE,pre_config:Dict[xFLdE,xFLdE],**kwargs)->Dict:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 metamodel=backend.version_metamodel(version=version)
 return metamodel
def set_version(version:xFLdy,inject_version_state:xFLdG,reset_state:xFLdG,commit_before:xFLdG,pod_name:xFLdE,pre_config:Dict[xFLdE,xFLdE],**kwargs)->xFLdG:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 ...
 return success
def list_version_commits(version:xFLdy,pod_name:xFLdE,pre_config:Dict[xFLdE,xFLdE])->List[xFLdE]:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 commits=backend.list_version_commits(version=version)
 return commits
def get_commit_diff(version:xFLdy,commit:xFLdy,pod_name:xFLdE,pre_config:Dict[xFLdE,xFLdE])->Dict:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 commit_diff=backend.get_commit_diff(version=version,commit=commit)
 return commit_diff
def push_overwrite(version:xFLdy,pod_name:xFLdE,comment:xFLdE,pre_config:Dict[xFLdE,xFLdE]):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.push_overwrite(version=version,comment=comment)
def push_state(pod_name,pre_config=xFLdv,squash_commits=xFLdV,comment=xFLdv,three_way=xFLdV,**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
...
def pull_state(pod_name,inject_version_state=xFLdV,reset_state_before=xFLdV,lazy=xFLdV,**kwargs):
 pre_config=kwargs.get("pre_config",xFLdv)
 if not pod_name:
  raise xFLdD("Need to specify a pod name")
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.pull(inject_version_state=inject_version_state,reset_state_before=reset_state_before,lazy=lazy)
 print("Done.")
def reset_local_state(reset_data_dir=xFLdV,exclude_from_reset:List[xFLdE]=xFLdv):
 url=f"{get_pods_endpoint()}/state"
 if reset_data_dir:
  url+="/datadir"
 ...
 if result.status_code>=400:
  raise xFLdD("Unable to reset service state via local management API %s (code %s): %s"%(url,result.status_code,result.content))
 print("Done.")
def list_pods(args):
 return PodConfigManager.list_pods()
def get_data_dir_from_container()->xFLdE:
 try:
...
def get_persisted_resource_names(data_dir)->List[xFLdE]:
 names=[]
 ...
 with os.scandir(os.path.join(data_dir,"api_states"))as entries:
  for entry in entries:
   if entry.is_dir()and xFLdt(os.listdir(entry.path))>0:
    names.append(entry.name)
