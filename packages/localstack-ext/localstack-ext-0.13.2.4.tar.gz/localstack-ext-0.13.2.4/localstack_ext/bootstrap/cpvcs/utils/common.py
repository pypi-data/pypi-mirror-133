...
class CPVCSConfigContext:
 ...
 def get_version_state_archive_path(self,version,with_format:QjkpI=Qjkpi)->Qjkpr:
 ...
 def update_ver_log(self,author:Qjkpr,ver_no:Qjkps,rev_id:Qjkpr,rev_no:Qjkps):
  with QjkpD(self.get_ver_log_path(),"a")as fp:
   fp.write(f"{VER_LOG_STRUCTURE.format(author=author, ver_no=ver_no, rev_rid_no=f'{rev_id}_{rev_no}')}\n")
 def create_version_symlink(self,name:Qjkpr,key:Qjkpr=QjkpT)->Qjkpr:
  return self._create_symlink(name,key,self.get_ver_refs_path())
 def create_revision_symlink(self,name:Qjkpr,key:Qjkpr=QjkpT)->Qjkpr:
  return self._create_symlink(name,key,self.get_rev_refs_path())
 def is_initialized(self)->QjkpI:
  return self.pod_root_dir and os.path.isdir(self.pod_root_dir)
 def _create_symlink(self,name:Qjkpr,key:Qjkpr,path:Qjkpr)->Qjkpr:
...
def zip_directories(zip_dest:Qjkpr,directories:List[Qjkpr])->Qjkpr:
 ...
 return zip_dest
def add_file_to_archive(archive:Qjkpr,entry_name:Qjkpr,content:Union[Qjkpr,Qjkpz]):
 with zipfile.ZipFile(archive,"a")as zip_file:
  zip_file.writestr(entry_name,content)
def read_file_from_archive(archive_path:Qjkpr,file_name:Qjkpr)->Qjkpr:
 try:
