...
def random_hash()->YIutQ:
 return YIutH(random.getrandbits(160))
def compute_file_hash(file_path:YIutQ,accum=YIutv)->Optional[YIutQ]:
 try:
  with YIutw(file_path,"rb")as fp:
   if accum:
...
def compute_version_archive_hash(version_no:YIuto,state_archive:YIutQ)->YIutQ:
 ...
 m.update(YIutQ(version_no).encode("utf-8"))
 rm_rf(tmp_state_dir)
 return m.hexdigest()
def compute_revision_hash(cpvcs_node:Revision)->YIutQ:
 if not cpvcs_node.state_files:
  return random_hash()
 state_file_keys=YIutk(lambda state_file:state_file.hash_ref,cpvcs_node.state_files)
 m=hashlib.sha1()
 for key in state_file_keys:
  try:
   with YIutw(os.path.join(config_context.get_obj_store_path(),key),"rb")as fp:
    m.update(fp.read())
  except YIutS as e:
   LOG.warning(f"Failed to open file and compute hash for {key}: {e}")
 if YIutJ(cpvcs_node,Revision):
  m.update(cpvcs_node.rid.encode("utf-8"))
  m.update(YIutQ(cpvcs_node.revision_number).encode("utf-8"))
 elif YIutJ(cpvcs_node,Version):
  m.update(YIutQ(cpvcs_node.version_number).encode("utf-8"))
 return m.hexdigest()
