...
CAPTURE_METAMODEL_SEPARATELY=PTcqE
...
class MetamodelDelta(SubtypesInstanceManager):
 def create_delta_for_state_files(self,sf1:StateFileRef,sf2:StateFileRef):
  ...
  if not sf1:
   backend2=load_persisted_object(config_context.get_obj_file_path(sf2.hash_ref))
   clazz=PTcqF(backend2)
   backend1=_infer_backend_init(clazz,sf2)
  else:
   backend1=load_persisted_object(config_context.get_obj_file_path(sf1.hash_ref))
   clazz=PTcqF(backend1)
   backend2=_infer_backend_init(clazz,sf1)
  diff=self.create_delta_json(backend1,backend2)
  return diff
 def create_delta_log(self,state_from:Set[StateFileRef],state_to:Set[StateFileRef])->PTcqS:
  ...
  def _create_sf_lookup(state_files:Set[StateFileRef])->Dict[PTcqS,StateFileRef]:
  ...
  for state_file_from in state_files_from_regular:
   ...
   if state_file_from.any_congruence(state_files_to_regular):
    ...
    diff_json=self.create_delta_for_state_files(state_file_from,PTcqp)
  ...
  for state_files_to in state_files_to_lookup.values():
   ...
   diff_json=self.create_delta_for_state_files(PTcqp,state_files_to)
...
class CommitMetamodelUtils:
 @PTcqN
 def create_metadata_archive(cls,version:Version,delete_reference:bool=PTcqE,overwrite:bool=PTcqE,metamodels_file:PTcqS=PTcqp):
 ...
 def get_metamodel_delta(cls,prev_metamodel:Dict,this_metamodel:Dict)->Dict:
  ...
  return result
 @PTcqN
 def create_delta_metamodel_file(cls,version:PTcqI,revision:PTcqI,store_to_zip=PTcqE)->Tuple[Dict,Dict]:
 ...
 @PTcqN
 def reconstruct_metamodel(cls,version:PTcqI,revision:PTcqI=PTcqp)->Dict:
  result={}
  for rev in PTcqo(1,revision+1):
  ...
  return result
 @PTcqN
 def get_version_metamodel(cls,version:PTcqI,revision:PTcqI=PTcqp)->Dict:
 ...
 @PTcqN
 def get_commit_diff(cls,version_no:PTcqI,commit_no:PTcqI)->Optional[Dict]:
  ...
  return result
def _infer_backend_init(clazz:Type,sf:StateFileRef)->PTcqC:
 if PTcqa(clazz,PTcqQ):
  return{}
 constructor=PTcqj(clazz,"__init__",PTcqp)
...
def _create_metamodel_helper(obj,width=25,visited:set=PTcqp):
 if obj is PTcqp:
  return obj
