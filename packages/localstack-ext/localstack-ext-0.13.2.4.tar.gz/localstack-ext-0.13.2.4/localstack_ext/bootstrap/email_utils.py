...
SENT_EMAILS=[]
EMAIL_BLACKLIST=etCHJ()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",etCHE(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=etCHk,from_name=etCHk,smtp_host=etCHk,smtp_user=etCHk,smtp_pass=etCHk,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if etCHW(recipients,etCHy)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=etCHk):
 ...
 result["From"]=formataddr((etCHE(Header(from_name,"utf-8")),from_email))
 return result
