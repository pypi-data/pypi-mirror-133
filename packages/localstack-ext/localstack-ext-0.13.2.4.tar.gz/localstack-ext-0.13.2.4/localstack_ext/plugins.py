...
from localstack.utils.docker_utils import VolumeBind
...
def api_key_configured()->vtGez:
 return(vtGeP if os.environ.get("LOCALSTACK_API_KEY")and os.environ.get("LOCALSTACK_API_KEY").strip()else vtGeC)
...
def configure_pro_container(container:LocalstackContainer):
 try:
  ...
  if vtGeu([is_api_enabled(api)for api in EXTERNAL_PORT_APIS]):
...
def init_pro_extensions():
 from localstack_ext.services.eks.eks_starter import add_missing_ec2_images
