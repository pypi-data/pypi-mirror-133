...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return BtHfp if e else BtHfh
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->BtHfL:
...
@click.option("--revert",is_flag=BtHfp,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:BtHfX):
...
def _cpvcs_initialized(pod_name:BtHfY)->BtHfX:
...
@click.option("-n","--name",help="Name of the cloud pod",required=BtHfp)
def cmd_cpvcs_init(name:BtHfY):
...
@click.option("-n","--name",help="Name of the cloud pod",required=BtHfp)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=BtHfp,default=BtHfh)
def cmd_cpvcs_delete(name:BtHfY,remote:BtHfX):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=BtHfp)
def cmd_cpvcs_register_remote(name:BtHfY):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=BtHfp)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=BtHfp)
def cmd_cpvcs_rename(name:BtHfY,new_name:BtHfY):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=BtHfp)
def cmd_cpvcs_commit(message:BtHfY,name:BtHfY):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=BtHfp,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("--three-way",is_flag=BtHfp,default=BtHfh,help="")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=BtHfp)
def cmd_cpvcs_push(squash:BtHfX,message:BtHfY,name:BtHfY,three_way:BtHfX):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=BtHfp)
@click.option("-v","--version",type=BtHfc)
@click.option("-m","--message",required=BtHfh)
def cmd_cpvcs_push_overwrite(version:BtHfc,message:BtHfY,name:BtHfY):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=BtHfc,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=BtHfp,default=BtHfh,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=BtHfp)
def cmd_cpvcs_inject(version:BtHfc,reset:BtHfX,name:BtHfY):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--inject/--no-inject",default=BtHfp,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=BtHfp,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=BtHfp)
@click.option("--lazy/--eager",default=BtHfp,help="Will only fetch references to existing versions, i.e. version state is only downloaded when required")
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:BtHfY,inject:BtHfX,reset:BtHfX,lazy:BtHfX):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=BtHfp,default=BtHfh)
def cmd_cpvcs_list_pods(remote:BtHfX):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=BtHfp)
def cmd_cpvcs_versions(name:BtHfY):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=BtHfp,type=BtHfc)
@click.option("-n","--name",help="Name of the cloud pod",required=BtHfp)
def cmd_cpvcs_version_info(version:BtHfc,name:BtHfY):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=BtHfc,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=BtHfp)
def cmd_cpvcs_version_metamodel(version:BtHfc,name:BtHfY):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=BtHfp,type=BtHfc,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=BtHfp,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=BtHfp,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=BtHfh,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=BtHfp)
def cmd_cpvcs_set_version(version:BtHfc,inject:BtHfX,reset:BtHfX,commit_before:BtHfX,name:BtHfY):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=BtHfp)
def cmd_cpvcs_commits(version:BtHfc,name:BtHfY):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=BtHfp)
@click.option("--commit","-c",required=BtHfp)
@click.option("-n","--name",help="Name of the cloud pod",required=BtHfp)
def cmd_cpvcs_commit_diff(version:BtHfc,commit:BtHfc,name:BtHfY):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(BtHfL)
  status.stop()
...
def print_table(columns:Dict[BtHfY,BtHfY],rows:List[Dict[BtHfY,Any]],formatters:Dict[BtHfY,Callable[[Any],BtHfY]]=BtHfL):
 from rich.table import Table
 if formatters is BtHfL:
  formatters=BtHfv()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  BtHfD(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->BtHfL:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->BtHfL:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->BtHfL:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->BtHfL:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
