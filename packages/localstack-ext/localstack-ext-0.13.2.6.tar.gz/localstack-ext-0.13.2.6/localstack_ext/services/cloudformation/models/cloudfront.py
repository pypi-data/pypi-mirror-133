...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or Gcsnf
 @Gcsni
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return Gcsng(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @Gcsnq
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if GcsnX(origins,GcsnK)else origins.get("Items")
  origins_to_create=GcsnU([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if GcsnX(aliases,GcsnK)else aliases.get("Items")
  aliases_to_create=GcsnU(aliases)
  for distr in distrs:
   origins=GcsnU([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=GcsnU(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",Gcsny)
   config["Enabled"]=GcsnN(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if GcsnX(custom_errors,Gcsnt)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=GcsnN(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":Gcsny}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=GcsnN(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=Gcsnf):
    parent=config if parent is Gcsnf else parent
    items=parent.get(key)or[]
    if GcsnX(items,Gcsnt)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":Gcsno(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if GcsnX(cache_behavs,Gcsnt)else cache_behavs)
