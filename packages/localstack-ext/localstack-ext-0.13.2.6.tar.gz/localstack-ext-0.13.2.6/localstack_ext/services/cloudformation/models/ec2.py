...
class EC2ElasticIP(GenericBaseModel):
 ...
 def cloudformation_type():
  return "AWS::EC2::EIP"
 def get_physical_resource_id(self,attribute=LyirJ,**kwargs):
...
class SecurityGroupInOrEgress(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  for perm in perms:
   if Lyirn(perm["IpProtocol"])!=Lyirn(rp["IpProtocol"]):
    continue
 ...
 @Lyire
 def get_vpc_and_name_for_security_group(cls,group_id:Lyirn)->Tuple[Lyirn,Lyirn]:
  client=aws_stack.connect_to_service("ec2")
  groups=client.describe_security_groups(GroupIds=[group_id])["SecurityGroups"]
  vpc_id=groups and groups[0].get("VpcId")or LyirJ
  group_name=groups and groups[0]["GroupName"]or LyirJ
 ...
 def get_deploy_templates(cls):
  def create_params(params,**kwargs):
   ...
   source_group_id=result.pop("SourceSecurityGroupId",LyirJ)
   vpc_id=LyirJ
   ...
   dst_group_id=result.pop("DestinationSecurityGroupId",LyirJ)
   ...
   if result.get("IpProtocol"):
    result["IpProtocol"]=Lyirn(result.get("IpProtocol"))
   description=result.pop("Description",LyirJ)
   cidr_ipv6=result.pop("CidrIpv6",LyirJ)
