...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[SIobx])[0]
 def get_physical_resource_id(self,attribute=SIobx,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if SIobg(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return SIobu(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @SIobj
 def add_defaults(resource,stack_name:SIobO):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return SIobu(SESReceiptRule,self).get_cfn_attribute(attribute)
 @SIoby
