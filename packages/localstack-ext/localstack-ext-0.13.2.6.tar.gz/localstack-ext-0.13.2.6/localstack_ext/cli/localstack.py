...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return BmqXM if e else BmqXr
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->BmqXs:
...
@click.option("--revert",is_flag=BmqXM,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:BmqXD):
...
def _cpvcs_initialized(pod_name:BmqXn)->BmqXD:
...
@click.option("-n","--name",help="Name of the cloud pod",required=BmqXM)
def cmd_cpvcs_init(name:BmqXn):
...
@click.option("-n","--name",help="Name of the cloud pod",required=BmqXM)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=BmqXM,default=BmqXr)
def cmd_cpvcs_delete(name:BmqXn,remote:BmqXD):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=BmqXM)
def cmd_cpvcs_register_remote(name:BmqXn):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=BmqXM)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=BmqXM)
def cmd_cpvcs_rename(name:BmqXn,new_name:BmqXn):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=BmqXM)
def cmd_cpvcs_commit(message:BmqXn,name:BmqXn):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=BmqXM,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("--three-way",is_flag=BmqXM,default=BmqXr,help="")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=BmqXM)
def cmd_cpvcs_push(squash:BmqXD,message:BmqXn,name:BmqXn,three_way:BmqXD):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=BmqXM)
@click.option("-v","--version",type=BmqXp)
@click.option("-m","--message",required=BmqXr)
def cmd_cpvcs_push_overwrite(version:BmqXp,message:BmqXn,name:BmqXn):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=BmqXp,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=BmqXM,default=BmqXr,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=BmqXM)
def cmd_cpvcs_inject(version:BmqXp,reset:BmqXD,name:BmqXn):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--inject/--no-inject",default=BmqXM,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=BmqXM,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=BmqXM)
@click.option("--lazy/--eager",default=BmqXM,help="Will only fetch references to existing versions, i.e. version state is only downloaded when required")
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:BmqXn,inject:BmqXD,reset:BmqXD,lazy:BmqXD):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=BmqXM,default=BmqXr)
def cmd_cpvcs_list_pods(remote:BmqXD):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=BmqXM)
def cmd_cpvcs_versions(name:BmqXn):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=BmqXM,type=BmqXp)
@click.option("-n","--name",help="Name of the cloud pod",required=BmqXM)
def cmd_cpvcs_version_info(version:BmqXp,name:BmqXn):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=BmqXp,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=BmqXM)
def cmd_cpvcs_version_metamodel(version:BmqXp,name:BmqXn):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=BmqXM,type=BmqXp,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=BmqXM,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=BmqXM,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=BmqXr,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=BmqXM)
def cmd_cpvcs_set_version(version:BmqXp,inject:BmqXD,reset:BmqXD,commit_before:BmqXD,name:BmqXn):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=BmqXM)
def cmd_cpvcs_commits(version:BmqXp,name:BmqXn):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=BmqXM)
@click.option("--commit","-c",required=BmqXM)
@click.option("-n","--name",help="Name of the cloud pod",required=BmqXM)
def cmd_cpvcs_commit_diff(version:BmqXp,commit:BmqXp,name:BmqXn):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(BmqXs)
  status.stop()
...
def print_table(columns:Dict[BmqXn,BmqXn],rows:List[Dict[BmqXn,Any]],formatters:Dict[BmqXn,Callable[[Any],BmqXn]]=BmqXs):
 from rich.table import Table
 if formatters is BmqXs:
  formatters=BmqXN()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  BmqXa(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->BmqXs:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->BmqXs:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->BmqXs:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->BmqXs:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
