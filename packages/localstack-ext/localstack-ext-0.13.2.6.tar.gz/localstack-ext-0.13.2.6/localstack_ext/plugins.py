...
def api_key_configured()->jDJRO:
 return(jDJRy if os.environ.get("LOCALSTACK_API_KEY")and os.environ.get("LOCALSTACK_API_KEY").strip()else jDJRY)
...
def configure_pro_container(container:LocalstackContainer):
 try:
  ...
  if jDJRa([is_api_enabled(api)for api in EXTERNAL_PORT_APIS]):
