...
class CPVCSConfigContext:
 ...
 def get_version_state_archive_path(self,version,with_format:hCtrO=hCtrm)->hCtrA:
 ...
 def update_ver_log(self,author:hCtrA,ver_no:hCtrG,rev_id:hCtrA,rev_no:hCtrG):
  with hCtrM(self.get_ver_log_path(),"a")as fp:
   fp.write(f"{VER_LOG_STRUCTURE.format(author=author, ver_no=ver_no, rev_rid_no=f'{rev_id}_{rev_no}')}\n")
 def create_version_symlink(self,name:hCtrA,key:hCtrA=hCtrs)->hCtrA:
  return self._create_symlink(name,key,self.get_ver_refs_path())
 def create_revision_symlink(self,name:hCtrA,key:hCtrA=hCtrs)->hCtrA:
  return self._create_symlink(name,key,self.get_rev_refs_path())
 def is_initialized(self)->hCtrO:
  return self.pod_root_dir and os.path.isdir(self.pod_root_dir)
 def _create_symlink(self,name:hCtrA,key:hCtrA,path:hCtrA)->hCtrA:
...
def zip_directories(zip_dest:hCtrA,directories:List[hCtrA])->hCtrA:
 ...
 return zip_dest
def add_file_to_archive(archive:hCtrA,entry_name:hCtrA,content:Union[hCtrA,hCtrH]):
 with zipfile.ZipFile(archive,"a")as zip_file:
  zip_file.writestr(entry_name,content)
def read_file_from_archive(archive_path:hCtrA,file_name:hCtrA)->hCtrA:
 try:
