...
CAPTURE_METAMODEL_SEPARATELY=vSmRI
...
class MetamodelDelta(SubtypesInstanceManager):
 def create_delta_for_state_files(self,sf1:StateFileRef,sf2:StateFileRef):
  ...
  if not sf1:
   backend2=load_persisted_object(config_context.get_obj_file_path(sf2.hash_ref))
   clazz=vSmRk(backend2)
   backend1=_infer_backend_init(clazz,sf2)
  else:
   backend1=load_persisted_object(config_context.get_obj_file_path(sf1.hash_ref))
   clazz=vSmRk(backend1)
   backend2=_infer_backend_init(clazz,sf1)
  diff=self.create_delta_json(backend1,backend2)
  return diff
 def create_delta_log(self,state_from:Set[StateFileRef],state_to:Set[StateFileRef])->vSmRb:
  ...
  def _create_sf_lookup(state_files:Set[StateFileRef])->Dict[vSmRb,StateFileRef]:
  ...
  for state_file_from in state_files_from_regular:
   ...
   if state_file_from.any_congruence(state_files_to_regular):
    ...
    diff_json=self.create_delta_for_state_files(state_file_from,vSmRn)
  ...
  for state_files_to in state_files_to_lookup.values():
   ...
   diff_json=self.create_delta_for_state_files(vSmRn,state_files_to)
...
class CommitMetamodelUtils:
 @vSmRi
 def create_metadata_archive(cls,version:Version,delete_reference:bool=vSmRI,overwrite:bool=vSmRI,metamodels_file:vSmRb=vSmRn):
 ...
 def get_metamodel_delta(cls,prev_metamodel:Dict,this_metamodel:Dict)->Dict:
  ...
  return result
 @vSmRi
 def create_delta_metamodel_file(cls,version:vSmRx,revision:vSmRx,store_to_zip=vSmRI)->Tuple[Dict,Dict]:
 ...
 @vSmRi
 def reconstruct_metamodel(cls,version:vSmRx,revision:vSmRx=vSmRn)->Dict:
  result={}
  for rev in vSmRX(1,revision+1):
  ...
  return result
 @vSmRi
 def get_version_metamodel(cls,version:vSmRx,revision:vSmRx=vSmRn)->Dict:
 ...
 @vSmRi
 def get_commit_diff(cls,version_no:vSmRx,commit_no:vSmRx)->Optional[Dict]:
  ...
  return result
def _infer_backend_init(clazz:Type,sf:StateFileRef)->vSmRy:
 if vSmRP(clazz,vSmRw):
  return{}
 constructor=vSmRU(clazz,"__init__",vSmRn)
...
def _create_metamodel_helper(obj,width=25,visited:set=vSmRn):
 if obj is vSmRn:
  return obj
