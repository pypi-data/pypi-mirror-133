...
def random_hash()->YySkh:
 return YySkb(random.getrandbits(160))
def compute_file_hash(file_path:YySkh,accum=YySkA)->Optional[YySkh]:
 try:
  with YySkl(file_path,"rb")as fp:
   if accum:
...
def compute_version_archive_hash(version_no:YySki,state_archive:YySkh)->YySkh:
 ...
 m.update(YySkh(version_no).encode("utf-8"))
 rm_rf(tmp_state_dir)
 return m.hexdigest()
def compute_revision_hash(cpvcs_node:Revision)->YySkh:
 if not cpvcs_node.state_files:
  return random_hash()
 state_file_keys=YySkJ(lambda state_file:state_file.hash_ref,cpvcs_node.state_files)
 m=hashlib.sha1()
 for key in state_file_keys:
  try:
   with YySkl(os.path.join(config_context.get_obj_store_path(),key),"rb")as fp:
    m.update(fp.read())
  except YySkN as e:
   LOG.warning(f"Failed to open file and compute hash for {key}: {e}")
 if YySkG(cpvcs_node,Revision):
  m.update(cpvcs_node.rid.encode("utf-8"))
  m.update(YySkh(cpvcs_node.revision_number).encode("utf-8"))
 elif YySkG(cpvcs_node,Version):
  m.update(YySkh(cpvcs_node.version_number).encode("utf-8"))
 return m.hexdigest()
