...
def install_package(packages:Union[List[jOILa],jOILa],api_name:jOILa):
 if not(is_debian()and in_docker()):
  ...
  return
 if jOILl(packages,jOILa):
  packages=packages.split()
 LOG.info("Downloading dependencies for %s API. This may take a while."%api_name)
 with INSTALL_LOCK:
  run(["apt-get","update"],shell=jOILr)
  run(["apt-get","install","-y","--no-install-recommends"]+packages,shell=jOILr)
def check_or_install(command:jOILa,packages:Union[List[jOILa],jOILa],api:jOILa):
...
def setup_ssl_cert():
 ...
 try:
  ...
  try:
   ...
   raise
def download_github_artifact(url:jOILa,target_file:jOILa,timeout:int=jOILK):
 def do_download(url,print_error=jOILr):
  try:
