...
class AuthProvider(TavLJ):
 ...
 @TavLi
 def get(provider,raise_error=TavLt):
...
class AuthProviderInternal(AuthProvider):
 ...
 def get_user_for_token(self,token):
  raise TavLK("Not implemented")
def get_auth_cache()->FileMappedDocument:
 return FileMappedDocument(os.path.join(dirs.cache,AUTH_CACHE_FILE),mode=0o600)
def login(provider,username=TavLf):
 auth_provider=AuthProvider.get(provider)
 if not auth_provider:
  providers=TavLy(AuthProvider.providers().keys())
  raise TavLK('Unknown provider "%s", should be one of %s'%(provider,providers))
 username,password,headers=auth_provider.read_credentials(username)
 print("Verifying credentials ... (this may take a few moments)")
 token=auth_provider.get_or_create_token(username,password,headers)
 if not token:
  raise TavLK("Unable to verify login credentials - please try again")
