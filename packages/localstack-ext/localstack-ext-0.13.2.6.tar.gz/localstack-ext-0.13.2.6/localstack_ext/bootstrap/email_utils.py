...
SENT_EMAILS=[]
EMAIL_BLACKLIST=LdXHz()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",LdXHt(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=LdXHI,from_name=LdXHI,smtp_host=LdXHI,smtp_user=LdXHI,smtp_pass=LdXHI,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if LdXHx(recipients,LdXHy)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=LdXHI):
 ...
 result["From"]=formataddr((LdXHt(Header(from_name,"utf-8")),from_email))
 return result
