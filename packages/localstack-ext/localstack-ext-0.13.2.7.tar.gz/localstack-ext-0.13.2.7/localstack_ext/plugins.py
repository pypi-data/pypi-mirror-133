...
def api_key_configured()->htMpj:
 return(htMpV if os.environ.get("LOCALSTACK_API_KEY")and os.environ.get("LOCALSTACK_API_KEY").strip()else htMpe)
...
def configure_pro_container(container:LocalstackContainer):
 try:
  ...
  if htMpI([is_api_enabled(api)for api in EXTERNAL_PORT_APIS]):
