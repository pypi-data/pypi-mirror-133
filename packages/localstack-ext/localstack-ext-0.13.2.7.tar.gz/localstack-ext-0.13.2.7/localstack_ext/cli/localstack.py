...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return azJKU if e else azJKl
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->azJKP:
...
@click.option("--revert",is_flag=azJKU,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:azJKR):
...
def _cpvcs_initialized(pod_name:azJKy)->azJKR:
...
@click.option("-n","--name",help="Name of the cloud pod",required=azJKU)
def cmd_cpvcs_init(name:azJKy):
...
@click.option("-n","--name",help="Name of the cloud pod",required=azJKU)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=azJKU,default=azJKl)
def cmd_cpvcs_delete(name:azJKy,remote:azJKR):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=azJKU)
def cmd_cpvcs_register_remote(name:azJKy):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=azJKU)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=azJKU)
def cmd_cpvcs_rename(name:azJKy,new_name:azJKy):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=azJKU)
def cmd_cpvcs_commit(message:azJKy,name:azJKy):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=azJKU,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("--three-way",is_flag=azJKU,default=azJKl,help="")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=azJKU)
def cmd_cpvcs_push(squash:azJKR,message:azJKy,name:azJKy,three_way:azJKR):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=azJKU)
@click.option("-v","--version",type=azJKn)
@click.option("-m","--message",required=azJKl)
def cmd_cpvcs_push_overwrite(version:azJKn,message:azJKy,name:azJKy):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=azJKn,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=azJKU,default=azJKl,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=azJKU)
def cmd_cpvcs_inject(version:azJKn,reset:azJKR,name:azJKy):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--inject/--no-inject",default=azJKU,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=azJKU,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=azJKU)
@click.option("--lazy/--eager",default=azJKU,help="Will only fetch references to existing versions, i.e. version state is only downloaded when required")
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:azJKy,inject:azJKR,reset:azJKR,lazy:azJKR):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=azJKU,default=azJKl)
def cmd_cpvcs_list_pods(remote:azJKR):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=azJKU)
def cmd_cpvcs_versions(name:azJKy):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=azJKU,type=azJKn)
@click.option("-n","--name",help="Name of the cloud pod",required=azJKU)
def cmd_cpvcs_version_info(version:azJKn,name:azJKy):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=azJKn,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=azJKU)
def cmd_cpvcs_version_metamodel(version:azJKn,name:azJKy):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=azJKU,type=azJKn,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=azJKU,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=azJKU,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=azJKl,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=azJKU)
def cmd_cpvcs_set_version(version:azJKn,inject:azJKR,reset:azJKR,commit_before:azJKR,name:azJKy):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=azJKU)
def cmd_cpvcs_commits(version:azJKn,name:azJKy):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=azJKU)
@click.option("--commit","-c",required=azJKU)
@click.option("-n","--name",help="Name of the cloud pod",required=azJKU)
def cmd_cpvcs_commit_diff(version:azJKn,commit:azJKn,name:azJKy):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(azJKP)
  status.stop()
...
def print_table(columns:Dict[azJKy,azJKy],rows:List[Dict[azJKy,Any]],formatters:Dict[azJKy,Callable[[Any],azJKy]]=azJKP):
 from rich.table import Table
 if formatters is azJKP:
  formatters=azJKi()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  azJKC(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->azJKP:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->azJKP:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->azJKP:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->azJKP:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
