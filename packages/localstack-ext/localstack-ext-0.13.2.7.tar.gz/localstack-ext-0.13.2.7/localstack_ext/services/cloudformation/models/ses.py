...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[pnuwU])[0]
 def get_physical_resource_id(self,attribute=pnuwU,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if pnuwB(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return pnuwX(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @pnuwg
 def add_defaults(resource,stack_name:pnuwV):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return pnuwX(SESReceiptRule,self).get_cfn_attribute(attribute)
 @pnuwe
