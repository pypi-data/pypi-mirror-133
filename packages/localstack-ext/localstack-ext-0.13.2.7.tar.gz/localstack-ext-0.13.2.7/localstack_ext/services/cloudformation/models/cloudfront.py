...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or RAaKo
 @RAaKz
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return RAaKh(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @RAaKV
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if RAaKD(origins,RAaKL)else origins.get("Items")
  origins_to_create=RAaKF([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if RAaKD(aliases,RAaKL)else aliases.get("Items")
  aliases_to_create=RAaKF(aliases)
  for distr in distrs:
   origins=RAaKF([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=RAaKF(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",RAaKP)
   config["Enabled"]=RAaKH(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if RAaKD(custom_errors,RAaKx)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=RAaKH(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":RAaKP}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=RAaKH(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=RAaKo):
    parent=config if parent is RAaKo else parent
    items=parent.get(key)or[]
    if RAaKD(items,RAaKx)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":RAaKN(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if RAaKD(cache_behavs,RAaKx)else cache_behavs)
