...
class EC2ElasticIP(GenericBaseModel):
 ...
 def cloudformation_type():
  return "AWS::EC2::EIP"
 def get_physical_resource_id(self,attribute=XCObI,**kwargs):
...
class SecurityGroupInOrEgress(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  for perm in perms:
   if XCObl(perm["IpProtocol"])!=XCObl(rp["IpProtocol"]):
    continue
 ...
 @XCObw
 def get_vpc_and_name_for_security_group(cls,group_id:XCObl)->Tuple[XCObl,XCObl]:
  client=aws_stack.connect_to_service("ec2")
  groups=client.describe_security_groups(GroupIds=[group_id])["SecurityGroups"]
  vpc_id=groups and groups[0].get("VpcId")or XCObI
  group_name=groups and groups[0]["GroupName"]or XCObI
 ...
 def get_deploy_templates(cls):
  def create_params(params,**kwargs):
   ...
   source_group_id=result.pop("SourceSecurityGroupId",XCObI)
   vpc_id=XCObI
   ...
   dst_group_id=result.pop("DestinationSecurityGroupId",XCObI)
   ...
   if result.get("IpProtocol"):
    result["IpProtocol"]=XCObl(result.get("IpProtocol"))
   description=result.pop("Description",XCObI)
   cidr_ipv6=result.pop("CidrIpv6",XCObI)
