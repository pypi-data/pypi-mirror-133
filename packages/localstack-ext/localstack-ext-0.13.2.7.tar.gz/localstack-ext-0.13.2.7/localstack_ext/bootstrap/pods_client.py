...
class PodInfo:
 def __init__(self,name=leCaN,pod_size=0):
  self.name=name
  self.pod_size=pod_size
  self.pod_size_compressed=0
  self.persisted_resource_names=[]
class CloudPodManager(leCaO):
 BACKEND="_none_"
 def __init__(self,pod_name=leCaN,config=leCaN):
  self.pod_name=pod_name
  self._pod_config=config
 def init(self):
  raise leCat("Not implemented")
 def delete(self,remote:leCak)->leCak:
  raise leCat("Not implemented")
 def push(self,comment:leCaH=leCaN,three_way:leCak=leCap)->PodInfo:
  raise leCat("Not implemented")
 def push_overwrite(self,version:leCac,comment:leCaH=leCaN):
  pass
 def pull(self,inject_version_state:leCak=leCap,reset_state_before:leCak=leCap,lazy:leCak=leCap):
  raise leCat("Not implemented")
 def commit(self,message:leCaH=leCaN):
  raise leCat("Not implemented")
 def inject(self,version:leCac,reset_state:leCak)->leCak:
  raise leCat("Not implemented")
 def list_versions(self)->List[leCaH]:
  raise leCat("Not implemented")
 def version_info(self,version:leCac):
  raise leCat("Not implemented")
 def version_metamodel(self,version:leCac)->Dict:
  raise leCat("Not implemented")
 def set_version(self,version:leCac,inject_version_state:leCak,reset_state:leCak,commit_before:leCak):
  raise leCat("Not implemented")
 def list_version_commits(self,version:leCac)->List[leCaH]:
  raise leCat("Not implemented")
 def get_commit_diff(self,version:leCac,commit:leCac)->Dict:
  raise leCat("Not implemented")
 def register_remote(self,pod_name:leCaH,ci_pod:leCak)->leCak:
  raise leCat("Not implemented")
 def rename_pod(self,current_pod_name,new_pod_name)->leCak:
  raise leCat("Not implemented")
 def list_pods(self,fetch_remote:leCak)->Set[leCaH]:
  raise leCat("Not implemented")
 ...
 def get(cls,pod_name,pre_config=leCaN):
  ...
  raise leCat('Unable to find Cloud Pod manager implementation type "%s"'%backend)
 def deploy_pod_into_instance(self,pod_path):
  delete_pod_zip=leCap
  if os.path.isdir(pod_path):
   ...
   for folder in PERSISTED_FOLDERS:
    ...
    cp_r(src_folder,tgt_folder,rm_dest_on_conflict=leCaK)
   pod_path=create_zip_file(tmpdir)
   rm_rf(tmpdir)
   delete_pod_zip=leCaK
  zip_content=load_file(pod_path,mode="rb")
  url=get_pods_endpoint()
  result=requests.post(url,data=zip_content)
  if result.status_code>=400:
   raise leCat("Unable to restore pod state via local pods management API %s (code %s): %s"%(url,result.status_code,result.content))
  if delete_pod_zip:
   rm_rf(pod_path)
  else:
   return pod_path
 @leCaq
 def get_state_zip_from_instance(get_content=leCap):
  url=f"{get_pods_endpoint()}/state"
  result=requests.get(url)
  if result.status_code>=400:
   raise leCat("Unable to get local pod state via management API %s (code %s): %s"%(url,result.status_code,result.content))
  if get_content:
   return result.content
  zip_file=f"{new_tmp_file()}.zip"
  save_file(zip_file,result.content)
  return zip_file
 def get_pod_info(self,pod_data_dir:leCaH=leCaN):
...
class CloudPodManagerCPVCS(CloudPodManager):
 BACKEND="cpvcs"
 @leCaq
 def parse_pod_name_from_qualifying_name(qualifying_name:leCaH)->leCaH:
 ...
 @leCaq
 def _get_max_version_for_pod_from_platform(pod_name:leCaH,auth_headers):
  ...
  remote_max_ver=leCac(content["max_ver"])
 ...
 def _upload_version_and_product_space(self,presigned_urls):
  ...
  with leCar(version_space_archive,"rb")as version_space_content:
  ...
  for version_no,urls in presigned_meta_state_urls.items():
   ...
   with leCar(meta_archive,"rb")as meta_archive_content:
   ...
   with leCar(state_archive,"rb")as state_archive_content:
    self.upload_content(state_presigned_url,state_archive_content)
 @leCaq
 def _add_state_files_from_directory(service:leCaH,path:leCaH,root_dir:leCaH,serialization:Serialization):
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
 ...
 def _add_state_to_cpvcs_store(self):
  ...
  with ZipFile(zip_file,"r")as state_zip:
   ...
   api_states_traverse(api_states_path=api_states_path,side_effect=CloudPodManagerCPVCS._add_state_files_func,mutables=leCaN)
  ...
  rm_rf(zip_file)
  rm_rf(tmp_dir)
 def _pull_versions(self,auth_headers,required_versions:leCaH):
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
 ...
 def _clone_pod(self,auth_headers,lazy:leCak=leCap):
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
 ...
 def delete(self,remote:leCak)->leCak:
  ...
  if remote:
   pass
  return leCap
 def _push_to_remote(self,url:leCaH):
 ...
 def pull(self,inject_version_state:leCak=leCap,reset_state_before:leCak=leCap,lazy:leCak=leCap):
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
  auth_headers=get_auth_headers()
  if self.pod_name in cpvcs_api.list_locally_available_pods(show_remote_or_local=leCap):
   cpvcs_api.set_pod_context(self.pod_name)
   ...
   if remote_max_ver==current_max_ver:
    ...
    return
   if not lazy:
    required_versions=",".join(leCaj(lambda ver:leCaH(ver),leCaJ(current_max_ver+1,remote_max_ver+1)))
   else:
 ...
 def _download_version_product(self,version:leCac,retain:leCak=leCap)->Dict[leCaH,leCaH]:
 ...
 def _get_presigned_url_for_version_product(self,version:leCac)->Dict:
 ...
 def _inject_from_remote(self,version:leCac,retain:leCak=leCap)->leCak:
  from localstack_ext.bootstrap.cpvcs.utils.remote_utils import(extract_meta_and_state_archives)
  content=self._get_presigned_url_for_version_product(version=version)
  state_url=content.get("presigned_url_state")
  tmp_state_archive=new_tmp_file()
 ...
 def inject(self,version:leCac,reset_state:leCak)->leCak:
  ...
  if not cpvcs_api.config_context.pod_exists_locally(self.pod_name):
   ...
   if not state_archive_path and cpvcs_api.is_remotely_managed():
    ...
    product_archive_path=self._download_version_product(version=version,retain=leCaK)
    if not product_archive_path:
     return leCap
    state_archive_path=cpvcs_api.PodFilePaths.get_version_state_archive(version)
  if reset_state:
   reset_local_state(reset_data_dir=leCaK,exclude_from_reset=["dynamodb","kinesis","stepfunctions"])
 ...
 def version_metamodel(self,version:leCac)->Dict:
  ...
  if not result and cpvcs_api.is_remotely_managed():
   self._download_version_product(version=version,retain=leCaK)
 ...
 def get_commit_diff(self,version:leCac,commit:leCac)->Dict:
  ...
  return commit_diff
 def register_remote(self,pod_name:leCaH,ci_pod:leCak=leCap)->leCak:
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
  cpvcs_api.set_pod_context(self.pod_name)
 ...
 def list_pods(self,fetch_remote:leCak)->Set[leCaH]:
  ...
  return result
 @leCav
 def upload_content(cls,presigned_url:leCaH,zip_data_content):
  res=safe_requests.put(presigned_url,data=zip_data_content)
  if res.status_code>=400:
   raise leCat("Unable to upload pod state to S3 (code %s): %s"%(res.status_code,res.content))
  return res
 @leCaq
 def create_platform_url(request:leCaH)->leCaH:
...
class CloudPodManagerManaged(CloudPodManager):
 BACKEND="managed"
 def push(self,comment:leCaH=leCaN,three_way:leCak=leCap)->PodInfo:
  zip_data_content=self.get_state_zip_from_instance(get_content=leCaK)
  print('Pushing state of cloud pod "%s" to backend server (%s KB)'%(self.pod_name,format_number(leCaD(zip_data_content)/1000.0)))
  self.push_content(self.pod_name,zip_data_content)
  print("Done.")
  result=self.get_pod_info()
  result.pod_size_compressed=leCaD(zip_data_content)
  return result
 def pull(self,inject_version_state:leCak=leCap,reset_state_before:leCak=leCap,lazy:leCak=leCap):
  ...
  rm_rf(zip_path)
 @leCaq
 def presigned_url(pod_name:leCaH,mode:leCaH)->leCaH:
  ...
  if response.status_code>=400:
   raise leCat("Unable to get cloud pod presigned URL (code %s): %s"%(response.status_code,content))
 ...
 def push_content(cls,pod_name,zip_data_content):
  ...
  res=safe_requests.put(presigned_url,data=zip_data_content)
  if res.status_code>=400:
   raise leCat("Unable to push pod state to API (code %s): %s"%(res.status_code,res.content))
  return res
class CloudPodManagerGit(CloudPodManager):
 BACKEND="git"
 def push(self,comment:leCaH=leCaN,three_way:leCak=leCap):
  ...
  if branch not in repo:
   porcelain.branch_create(repo,branch,force=leCaK)
 ...
 def pull(self,inject_version_state:leCak=leCap,reset_state_before:leCak=leCap,lazy:leCak=leCap):
  ...
  if remote_ref not in remote_refs:
   raise leCat('Unable to find branch "%s" in remote git repo'%branch)
  remote_location=self.pod_config.get("url")
  self.switch_branch(branch)
 ...
 def client(self):
  ...
  return client,path
 def local_repo(self):
  self.clone_dir=leCaI(self,"clone_dir",leCaN)
...
class PodConfigManagerMeta(leCau):
 def __getattr__(cls,attr):
  def _call(*args,**kwargs):
   result=leCaN
   for manager in cls.CHAIN:
    try:
     tmp=leCaI(manager,attr)(*args,**kwargs)
     if tmp:
      if not result:
       result=tmp
      elif leCaE(tmp,leCaT)and leCaE(result,leCaT):
       result.extend(tmp)
    except leCat:
     if LOG.isEnabledFor(logging.DEBUG):
      LOG.exception("error during PodConfigManager call chain")
   if result is not leCaN:
    return result
   raise leCat('Unable to run operation "%s" for local or remote configuration'%attr)
  return _call
class PodConfigManager(leCaO,metaclass=PodConfigManagerMeta):
 CHAIN=[]
 @leCav
 def pod_config(cls,pod_name):
  ...
  if not pod_config:
   raise leCat('Unable to find config for pod named "%s"'%pod_name)
  return pod_config[0]
class PodConfigManagerLocal(leCaO):
 CONFIG_FILE=".localstack.yml"
 def list_pods(self):
  local_pods=self._load_config(safe=leCaK).get("pods",{})
  local_pods=[{"pod_name":k,"state":"Local Only",**v}for k,v in local_pods.items()]
  existing_names=leCad([pod["pod_name"]for pod in local_pods])
  result=[pod for pod in local_pods if pod["pod_name"]not in existing_names]
  return result
 def store_pod_metadata(self,pod_name,metadata):
  pass
 def _load_config(self,safe=leCap):
  try:
   return yaml.safe_load(to_str(load_file(self.CONFIG_FILE)))
  except leCat:
   if safe:
    return{}
   raise leCat('Unable to find and parse config file "%s"'%self.CONFIG_FILE)
class PodConfigManagerRemote(leCaO):
 def list_pods(self):
  result=[]
  auth_headers=get_auth_headers()
  ...
  content=response.content
  if response.status_code>=400:
   raise leCat("Unable to fetch list of pods from API (code %s): %s"%(response.status_code,content))
 ...
 def store_pod_metadata(self,pod_name,metadata):
  ...
  content=response.content
  if response.status_code>=400:
   raise leCat("Unable to store pod metadata in API (code %s): %s"%(response.status_code,content))
...
def init_cpvcs(pod_name:leCaH,pre_config:Dict[leCaH,leCaH],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.init()
def delete_pod(pod_name:leCaH,remote:leCak,pre_config:Dict[leCaH,leCaH])->leCak:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 result=backend.delete(remote=remote)
 return result
def register_remote(pod_name:leCaH,pre_config:Dict[leCaH,leCaH],**kwargs)->leCak:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 result=backend.register_remote(pod_name=pod_name,ci_pod=pre_config.get("ci_pod",leCap))
 return result
def rename_pod(current_pod_name:leCaH,new_pod_name:leCaH,pre_config:Dict[leCaH,leCaH],**kwargs):
 ...
 return result
def list_pods_cpvcs(remote:leCak,pre_config:Dict[leCaH,leCaH],**kwargs)->List[leCaH]:
 backend=CloudPodManager.get(pod_name="",pre_config=pre_config)
 result=backend.list_pods(fetch_remote=remote)
 return result
def commit_state(pod_name:leCaH,pre_config:Dict[leCaH,leCaH],message:leCaH=leCaN,**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.commit(message=message)
def inject_state(pod_name:leCaH,version:leCac,reset_state:leCak,pre_config:Dict[leCaH,leCaH],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 result=backend.inject(version=version,reset_state=reset_state)
 return result
def list_versions(pod_name:leCaH,pre_config:Dict[leCaH,leCaH],**kwargs)->List[leCaH]:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 versions=backend.list_versions()
 return versions
def get_version_info(version:leCac,pod_name:leCaH,pre_config:Dict[leCaH,leCaH],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 info=backend.version_info(version=version)
 return info
def get_version_metamodel(version:leCac,pod_name:leCaH,pre_config:Dict[leCaH,leCaH],**kwargs)->Dict:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 metamodel=backend.version_metamodel(version=version)
 return metamodel
def set_version(version:leCac,inject_version_state:leCak,reset_state:leCak,commit_before:leCak,pod_name:leCaH,pre_config:Dict[leCaH,leCaH],**kwargs)->leCak:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 ...
 return success
def list_version_commits(version:leCac,pod_name:leCaH,pre_config:Dict[leCaH,leCaH])->List[leCaH]:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 commits=backend.list_version_commits(version=version)
 return commits
def get_commit_diff(version:leCac,commit:leCac,pod_name:leCaH,pre_config:Dict[leCaH,leCaH])->Dict:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 commit_diff=backend.get_commit_diff(version=version,commit=commit)
 return commit_diff
def push_overwrite(version:leCac,pod_name:leCaH,comment:leCaH,pre_config:Dict[leCaH,leCaH]):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.push_overwrite(version=version,comment=comment)
def push_state(pod_name,pre_config=leCaN,squash_commits=leCap,comment=leCaN,three_way=leCap,**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
...
def pull_state(pod_name,inject_version_state=leCap,reset_state_before=leCap,lazy=leCap,**kwargs):
 pre_config=kwargs.get("pre_config",leCaN)
 if not pod_name:
  raise leCat("Need to specify a pod name")
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.pull(inject_version_state=inject_version_state,reset_state_before=reset_state_before,lazy=lazy)
 print("Done.")
def reset_local_state(reset_data_dir=leCap,exclude_from_reset:List[leCaH]=leCaN):
 url=f"{get_pods_endpoint()}/state"
 if reset_data_dir:
  url+="/datadir"
 ...
 if result.status_code>=400:
  raise leCat("Unable to reset service state via local management API %s (code %s): %s"%(url,result.status_code,result.content))
 print("Done.")
def list_pods(args):
 return PodConfigManager.list_pods()
def get_data_dir_from_container()->leCaH:
 try:
...
def get_persisted_resource_names(data_dir)->List[leCaH]:
 names=[]
 ...
 with os.scandir(os.path.join(data_dir,"api_states"))as entries:
  for entry in entries:
   if entry.is_dir()and leCaD(os.listdir(entry.path))>0:
    names.append(entry.name)
