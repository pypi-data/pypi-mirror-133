...
def install_package(packages:Union[List[EQCJF],EQCJF],api_name:EQCJF):
 if not(is_debian()and in_docker()):
  ...
  return
 if EQCJW(packages,EQCJF):
  packages=packages.split()
 LOG.info("Downloading dependencies for %s API. This may take a while."%api_name)
 with INSTALL_LOCK:
  run(["apt-get","update"],shell=EQCJb)
  run(["apt-get","install","-y","--no-install-recommends"]+packages,shell=EQCJb)
def check_or_install(command:EQCJF,packages:Union[List[EQCJF],EQCJF],api:EQCJF):
...
def setup_ssl_cert():
 ...
 try:
  ...
  try:
   ...
   raise
def download_github_artifact(url:EQCJF,target_file:EQCJF,timeout:int=EQCJz):
 def do_download(url,print_error=EQCJb):
  try:
