...
CAPTURE_METAMODEL_SEPARATELY=NBAdP
...
class MetamodelDelta(SubtypesInstanceManager):
 def create_delta_for_state_files(self,sf1:StateFileRef,sf2:StateFileRef):
  ...
  if not sf1:
   backend2=load_persisted_object(config_context.get_obj_file_path(sf2.hash_ref))
   clazz=NBAdy(backend2)
   backend1=_infer_backend_init(clazz,sf2)
  else:
   backend1=load_persisted_object(config_context.get_obj_file_path(sf1.hash_ref))
   clazz=NBAdy(backend1)
   backend2=_infer_backend_init(clazz,sf1)
  diff=self.create_delta_json(backend1,backend2)
  return diff
 def create_delta_log(self,state_from:Set[StateFileRef],state_to:Set[StateFileRef])->NBAdj:
  ...
  def _create_sf_lookup(state_files:Set[StateFileRef])->Dict[NBAdj,StateFileRef]:
  ...
  for state_file_from in state_files_from_regular:
   ...
   if state_file_from.any_congruence(state_files_to_regular):
    ...
    diff_json=self.create_delta_for_state_files(state_file_from,NBAdq)
  ...
  for state_files_to in state_files_to_lookup.values():
   ...
   diff_json=self.create_delta_for_state_files(NBAdq,state_files_to)
...
class CommitMetamodelUtils:
 @NBAdz
 def create_metadata_archive(cls,version:Version,delete_reference:bool=NBAdP,overwrite:bool=NBAdP,metamodels_file:NBAdj=NBAdq):
 ...
 def get_metamodel_delta(cls,prev_metamodel:Dict,this_metamodel:Dict)->Dict:
  ...
  return result
 @NBAdz
 def create_delta_metamodel_file(cls,version:NBAde,revision:NBAde,store_to_zip=NBAdP)->Tuple[Dict,Dict]:
 ...
 @NBAdz
 def reconstruct_metamodel(cls,version:NBAde,revision:NBAde=NBAdq)->Dict:
  result={}
  for rev in NBAdu(1,revision+1):
  ...
  return result
 @NBAdz
 def get_version_metamodel(cls,version:NBAde,revision:NBAde=NBAdq)->Dict:
 ...
 @NBAdz
 def get_commit_diff(cls,version_no:NBAde,commit_no:NBAde)->Optional[Dict]:
  ...
  return result
def _infer_backend_init(clazz:Type,sf:StateFileRef)->NBAdJ:
 if NBAdb(clazz,NBAdL):
  return{}
 constructor=NBAdE(clazz,"__init__",NBAdq)
...
def _create_metamodel_helper(obj,width=25,visited:set=NBAdq):
 if obj is NBAdq:
  return obj
