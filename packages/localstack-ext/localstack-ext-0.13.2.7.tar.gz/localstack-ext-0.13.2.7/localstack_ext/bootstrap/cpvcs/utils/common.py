...
class CPVCSConfigContext:
 ...
 def get_version_state_archive_path(self,version,with_format:SPAlQ=SPAlC)->SPAlw:
 ...
 def update_ver_log(self,author:SPAlw,ver_no:SPAlx,rev_id:SPAlw,rev_no:SPAlx):
  with SPAlX(self.get_ver_log_path(),"a")as fp:
   fp.write(f"{VER_LOG_STRUCTURE.format(author=author, ver_no=ver_no, rev_rid_no=f'{rev_id}_{rev_no}')}\n")
 def create_version_symlink(self,name:SPAlw,key:SPAlw=SPAln)->SPAlw:
  return self._create_symlink(name,key,self.get_ver_refs_path())
 def create_revision_symlink(self,name:SPAlw,key:SPAlw=SPAln)->SPAlw:
  return self._create_symlink(name,key,self.get_rev_refs_path())
 def is_initialized(self)->SPAlQ:
  return self.pod_root_dir and os.path.isdir(self.pod_root_dir)
 def _create_symlink(self,name:SPAlw,key:SPAlw,path:SPAlw)->SPAlw:
...
def zip_directories(zip_dest:SPAlw,directories:List[SPAlw])->SPAlw:
 ...
 return zip_dest
def add_file_to_archive(archive:SPAlw,entry_name:SPAlw,content:Union[SPAlw,SPAlT]):
 with zipfile.ZipFile(archive,"a")as zip_file:
  zip_file.writestr(entry_name,content)
def read_file_from_archive(archive_path:SPAlw,file_name:SPAlw)->SPAlw:
 try:
