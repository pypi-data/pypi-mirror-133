...
ROOT_DIR_LOOKUP={rPDKo(Serialization.KINESIS):KINESIS_DIR,rPDKo(Serialization.DDB):DYNAMODB_DIR,rPDKo(Serialization.MAIN):API_STATES_DIR}
def init(pod_name="My-Pod"):
 ...
 r0_hash=random_hash()
 v0_hash=random_hash()
 r0=Revision(hash_ref=r0_hash,parent_ptr=NIL_PTR,creator=config_context.get_context_user(),rid=short_uid(),revision_number=0,state_files=rPDKp())
 v0=Version(hash_ref=v0_hash,parent_ptr=NIL_PTR,creator=config_context.get_context_user(),comment="Init version",active_revision_ptr=r0_hash,outgoing_revision_ptrs={r0_hash},incoming_revision_ptr=rPDKL,state_files=rPDKp(),version_number=0)
 ...
 with rPDKF(config_context.get_head_path(),"w")as fp:
  fp.write(ver_symlink)
 with rPDKF(config_context.get_max_ver_path(),"w")as fp:
  fp.write(ver_symlink)
 with rPDKF(config_context.get_known_ver_path(),"w")as fp:
  fp.write(ver_symlink)
...
def init_remote(version_space_archive:rPDKo,meta_archives:Dict[rPDKo,rPDKo],state_archives:Dict[rPDKo,rPDKo],remote_info:Dict[rPDKo,rPDKo],pod_name:rPDKo):
...
def merge_from_remote(version_space_archive:rPDKo,meta_archives:Dict[rPDKo,rPDKo],state_archives:Dict[rPDKo,rPDKo]):
 merge_version_space(version_space_archive)
 extract_meta_and_state_archives(meta_archives=meta_archives,state_archives=state_archives)
def is_remotely_managed()->rPDKQ:
 return config_context.is_remotly_managed()
def set_pod_context(pod_name:rPDKo):
 config_context.set_pod_context(pod_name)
def rename_pod(new_pod_name:rPDKo)->rPDKQ:
...
def list_locally_available_pods(show_remote_or_local:rPDKQ=rPDKY)->Set[rPDKo]:
 ...
 for available_pod in available_pods:
  ...
  result.add(pod_name)
 return result
def create_state_file_from_fs(path:rPDKo,file_name:rPDKo,service:rPDKo,region:rPDKo,root:rPDKo,serialization:Serialization)->rPDKo:
 ...
 return key
def _create_state_file_from_in_memory_blob(blob)->rPDKo:
 ...
 return key
def _get_state_file_path(key:rPDKo)->rPDKo:
...
def _add_state_file_to_expansion_point(state_file:StateFileRef):
 ...
 updated_state_files=rPDKp(rPDKT(lambda sf:not sf.congruent(state_file),revision.state_files))
 updated_state_files.add(state_file)
 revision.state_files=updated_state_files
 object_storage.upsert_objects(revision)
def list_state_files(key:rPDKo)->Optional[rPDKo]:
...
def get_version_info(version_no:rPDKe)->Dict[rPDKo,rPDKo]:
 ...
 return result
def commit(message:rPDKo=rPDKL)->Revision:
 ...
 curr_expansion_point_parent_state_files=rPDKp()
 if curr_expansion_point.parent_ptr!=NIL_PTR:
  referenced_by_version=rPDKL
 ...
 new_expansion_point=Revision(hash_ref=random_hash(),state_files=rPDKp(),parent_ptr=curr_expansion_point_hash,creator=curr_expansion_point.creator,rid=short_uid(),revision_number=curr_expansion_point.revision_number+1)
...
def create_version_space_archive()->rPDKo:
...
def get_max_version_no()->rPDKe:
 with rPDKF(config_context.get_max_ver_path())as fp:
  return rPDKe(os.path.basename(fp.readline()))
...
def push_overwrite(version:rPDKe,comment:rPDKo)->rPDKQ:
 ...
 CommitMetamodelUtils.create_metadata_archive(version_node,overwrite=rPDKY,metamodels_file=metamodels_file)
 version_node.comment=comment
 object_storage.upsert_objects(version_node)
 return rPDKY
def push(comment:rPDKo=rPDKL,three_way:rPDKQ=rPDKa)->Version:
 expansion_point,head_version=_get_expansion_point_with_head()
 max_version=_get_max_version()
 new_active_revision=Revision(hash_ref=random_hash(),state_files=rPDKp(),parent_ptr=NIL_PTR,creator=expansion_point.creator,rid=short_uid(),revision_number=0)
 ...
 new_version=Version(hash_ref=compute_version_archive_hash(new_max_version_no,PodFilePaths.get_version_state_archive(new_max_version_no)),state_files=rPDKp(),parent_ptr=max_version.hash_ref,creator=expansion_point.creator,comment=comment,active_revision_ptr=new_active_revision.hash_ref,outgoing_revision_ptrs={new_active_revision.hash_ref},incoming_revision_ptr=expansion_point.hash_ref,version_number=new_max_version_no)
 if expansion_point.parent_ptr!=NIL_PTR:
  ...
  delta_log_ptr=create_delta_log(expansion_point.state_files,rPDKp())
 ...
 return new_version
def _get_dst_path_for_state_file(version_state_dir:rPDKo,state_file:StateFileRef):
 if state_file.serialization in[rPDKo(Serialization.KINESIS),rPDKo(Serialization.DDB)]:
 ...
 mkdir(dst_path)
 return dst_path
def _create_state_directory(version_number:rPDKe,state_file_refs:Set[StateFileRef],delete_files=rPDKa,archive=rPDKY):
...
def set_active_version(version_no:rPDKe,commit_before=rPDKa)->rPDKQ:
 ...
 return rPDKa
def _set_active_version(key:rPDKo):
 ...
 if current_head.hash_ref!=key and object_storage.version_exists(key):
  ...
  if requested_version.active_revision_ptr==NIL_PTR:
   new_path_root=Revision(hash_ref=random_hash(),state_files=rPDKp(),parent_ptr=NIL_PTR,creator=config_context.get_context_user(),rid=short_uid(),revision_number=0)
...
def get_version_by_number(version_no:rPDKe)->Version:
 versions=load_version_references()
 version_ref=rPDKR((version[1]for version in versions if version[0]==version_no),rPDKL)
 if not version_ref:
  LOG.warning(f"Could not find version number {version_no}")
  return
 return object_storage.get_version_by_key(version_ref)
def load_version_references()->List[Tuple[rPDKe,rPDKo]]:
 result={}
 with rPDKF(config_context.get_known_ver_path(),"r")as vp:
  symlinks=vp.readlines()
  for symlink in symlinks:
   symlink=config_context.get_pod_absolute_path(symlink.rstrip())
   with rPDKF(symlink,"r")as sp:
    result[rPDKe(os.path.basename(symlink))]=sp.readline()
 return rPDKI(result.items(),key=lambda x:x[0],reverse=rPDKY)
def list_versions()->List[rPDKo]:
 ...
 return result
def list_version_commits(version_no:rPDKe)->List[rPDKo]:
 if version_no==-1:
 ...
 return result
def _update_head(new_head_ver_no,new_head_key)->rPDKo:
 with rPDKF(config_context.get_head_path(),"w")as fp:
  ver_symlink=config_context.create_version_symlink(VER_SYMLINK.format(ver_no=new_head_ver_no),new_head_key)
  fp.write(ver_symlink)
  return ver_symlink
def _update_max_ver(new_max_ver_no,new_max_ver_key)->rPDKo:
 with rPDKF(config_context.get_max_ver_path(),"w")as fp:
...
def _add_known_ver(new_ver_no,new_ver_key)->rPDKo:
 with rPDKF(config_context.get_known_ver_path(),"a")as fp:
...
def merge_expansion_point_with_max(three_way=rPDKa):
 ...
 for serialization_mechanism,new_version_state_files_path in new_version_tmp_dirs.items():
  try:
   merge_manager=CPVCSMergeManager.get(serialization_mechanism,raise_if_missing=rPDKY)
...
def create_delta_log(state_from:Set[StateFileRef],state_to:Set[StateFileRef],diff_method:MetamodelDeltaMethod=MetamodelDeltaMethod.SIMPLE)->rPDKo:
 try:
