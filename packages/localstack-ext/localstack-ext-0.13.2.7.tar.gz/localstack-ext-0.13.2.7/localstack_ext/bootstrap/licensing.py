...
class KeyActivationError(YcWmv):
 pass
...
class InvalidDecryptionKeyError(KeyActivationError):
 pass
def is_enterprise():
 return read_api_key(raise_if_missing=YcWms)=="enterprise"
def read_api_key(raise_if_missing=YcWmB):
 key=(os.environ.get(ENV_LOCALSTACK_API_KEY)or "").strip()
 if not key and raise_if_missing:
  raise YcWmv("Unable to retrieve API key. Please configure $%s in your environment"%ENV_LOCALSTACK_API_KEY)
 return key
def truncate_api_key(api_key:YcWmI):
 return '"%s..."(%s)'%(api_key[:3],YcWmq(api_key))
def fetch_key()->YcWmu:
 ...
 if api_key=="test":
  ...
  key_base64=load_cached_key(api_key=api_key,check_timeout=YcWms)
  decoded_key=base64.b64decode(key_base64)
  return decoded_key
 proxies=get_proxies()or YcWmF
 data={"api_key":api_key,"version":__version__}
 try:
  logging.getLogger("py.warnings").setLevel(logging.ERROR)
  result=requests.post("%s/activate"%config.API_URL,json.dumps(data),verify=YcWms,proxies=proxies)
  ...
  if log_license_issues():
   api_key=YcWmI(api_key_configured()or "")
...
def cache_key_locally(api_key,key_b64):
 timestamp=YcWmI(YcWmO(now_utc()))
 key_raw=to_str(base64.b64decode(key_b64))
 for i in YcWmt(YcWmq(timestamp)):
  key_raw=str_insert(key_raw,i*2,timestamp[i])
 key_b64=to_str(base64.b64encode(to_bytes(key_raw)))
 cache=get_key_cache()
 cache.update({"timestamp":YcWmO(timestamp),"key_hash":md5(api_key),"key":key_b64})
 cache.save()
def load_cached_key(api_key:YcWmI,check_timeout:bool=YcWmB)->YcWmI:
 cache=get_key_cache()
 ...
 timestamp=YcWmI(cache["timestamp"])
 ...
 key_b64=to_str(base64.b64encode(to_bytes(key_raw)))
 return key_b64
def enable_file_decryption(key:YcWmu):
 ...
 try:
  ...
  if "import" not in to_str(file_content):
   raise YcWmo("Decryption resulted in invalid python file!")
 except YcWmv:
...
def prepare_environment():
 ...
 if not ENV_PREPARED.get("finalized"):
  try:
   key=fetch_key()
   if not key:
    raise YcWmv("Unable to fetch and validate API key from environment")
   if to_str(key)!="test":
...
def api_key_configured():
 return read_api_key(raise_if_missing=YcWms)
def is_logged_in():
 return YcWmB if get_auth_cache().get("token")else YcWms
def get_auth_headers()->Dict:
 ...
 api_key=read_api_key(raise_if_missing=YcWms)
 if api_key:
  return{"ls-api-key":api_key,"ls-version":__version__}
 raise YcWmv("Please log in first")
