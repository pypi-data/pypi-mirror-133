...
SENT_EMAILS=[]
EMAIL_BLACKLIST=tVLuR()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",tVLuW(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=tVLuM,from_name=tVLuM,smtp_host=tVLuM,smtp_user=tVLuM,smtp_pass=tVLuM,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if tVLuw(recipients,tVLus)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=tVLuM):
 ...
 result["From"]=formataddr((tVLuW(Header(from_name,"utf-8")),from_email))
 return result
