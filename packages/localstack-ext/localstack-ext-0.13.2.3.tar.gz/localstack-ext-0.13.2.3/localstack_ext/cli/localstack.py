...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return vFiKD if e else vFiKB
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->vFiKg:
...
@click.option("--revert",is_flag=vFiKD,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:vFiKR):
...
def _cpvcs_initialized(pod_name:vFiKk)->vFiKR:
...
@click.option("-n","--name",help="Name of the cloud pod",required=vFiKD)
def cmd_cpvcs_init(name:vFiKk):
...
@click.option("-n","--name",help="Name of the cloud pod",required=vFiKD)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=vFiKD,default=vFiKB)
def cmd_cpvcs_delete(name:vFiKk,remote:vFiKR):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=vFiKD)
def cmd_cpvcs_register_remote(name:vFiKk):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=vFiKD)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=vFiKD)
def cmd_cpvcs_rename(name:vFiKk,new_name:vFiKk):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=vFiKD)
def cmd_cpvcs_commit(message:vFiKk,name:vFiKk):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=vFiKD,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("--three-way",is_flag=vFiKD,default=vFiKB,help="")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=vFiKD)
def cmd_cpvcs_push(squash:vFiKR,message:vFiKk,name:vFiKk,three_way:vFiKR):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=vFiKD)
@click.option("-v","--version",type=vFiKT)
@click.option("-m","--message",required=vFiKB)
def cmd_cpvcs_push_overwrite(version:vFiKT,message:vFiKk,name:vFiKk):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=vFiKT,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=vFiKD,default=vFiKB,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=vFiKD)
def cmd_cpvcs_inject(version:vFiKT,reset:vFiKR,name:vFiKk):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--inject/--no-inject",default=vFiKD,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=vFiKD,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=vFiKD)
@click.option("--lazy/--eager",default=vFiKD,help="Will only fetch references to existing versions, i.e. version state is only downloaded when required")
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:vFiKk,inject:vFiKR,reset:vFiKR,lazy:vFiKR):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=vFiKD,default=vFiKB)
def cmd_cpvcs_list_pods(remote:vFiKR):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=vFiKD)
def cmd_cpvcs_versions(name:vFiKk):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=vFiKD,type=vFiKT)
@click.option("-n","--name",help="Name of the cloud pod",required=vFiKD)
def cmd_cpvcs_version_info(version:vFiKT,name:vFiKk):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=vFiKT,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=vFiKD)
def cmd_cpvcs_version_metamodel(version:vFiKT,name:vFiKk):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=vFiKD,type=vFiKT,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=vFiKD,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=vFiKD,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=vFiKB,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=vFiKD)
def cmd_cpvcs_set_version(version:vFiKT,inject:vFiKR,reset:vFiKR,commit_before:vFiKR,name:vFiKk):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=vFiKD)
def cmd_cpvcs_commits(version:vFiKT,name:vFiKk):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=vFiKD)
@click.option("--commit","-c",required=vFiKD)
@click.option("-n","--name",help="Name of the cloud pod",required=vFiKD)
def cmd_cpvcs_commit_diff(version:vFiKT,commit:vFiKT,name:vFiKk):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def print_table(columns:Dict[vFiKk,vFiKk],rows:List[Dict[vFiKk,Any]],formatters:Dict[vFiKk,Callable[[Any],vFiKk]]=vFiKg):
 from rich.table import Table
 if formatters is vFiKg:
  formatters=vFiKN()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  vFiKQ(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->vFiKg:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->vFiKg:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->vFiKg:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->vFiKg:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
