...
def api_key_configured()->yNlSh:
 return(yNlSI if os.environ.get("LOCALSTACK_API_KEY")and os.environ.get("LOCALSTACK_API_KEY").strip()else yNlSb)
...
def configure_pro_container(container:LocalstackContainer):
 try:
  ...
  if yNlSk([is_api_enabled(api)for api in EXTERNAL_PORT_APIS]):
