...
SENT_EMAILS=[]
EMAIL_BLACKLIST=DOKGU()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",DOKGP(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=DOKGn,from_name=DOKGn,smtp_host=DOKGn,smtp_user=DOKGn,smtp_pass=DOKGn,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if DOKGR(recipients,DOKGT)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=DOKGn):
 ...
 result["From"]=formataddr((DOKGP(Header(from_name,"utf-8")),from_email))
 return result
