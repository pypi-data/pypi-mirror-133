...
def install_package(packages:Union[List[IEPSb],IEPSb],api_name:IEPSb):
 if not(is_debian()and in_docker()):
  ...
  return
 if IEPSJ(packages,IEPSb):
  packages=packages.split()
 LOG.info("Downloading dependencies for %s API. This may take a while."%api_name)
 with INSTALL_LOCK:
  run(["apt-get","update"],shell=IEPSs)
  run(["apt-get","install","-y","--no-install-recommends"]+packages,shell=IEPSs)
def check_or_install(command:IEPSb,packages:Union[List[IEPSb],IEPSb],api:IEPSb):
...
def setup_ssl_cert():
 ...
 try:
  ...
  try:
   ...
   raise
def download_github_artifact(url:IEPSb,target_file:IEPSb,timeout:int=IEPSq):
 def do_download(url,print_error=IEPSs):
  try:
