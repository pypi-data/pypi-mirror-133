...
class PodInfo:
 def __init__(self,name=mEVia,pod_size=0):
  self.name=name
  self.pod_size=pod_size
  self.pod_size_compressed=0
  self.persisted_resource_names=[]
class CloudPodManager(mEViF):
 BACKEND="_none_"
 def __init__(self,pod_name=mEVia,config=mEVia):
  self.pod_name=pod_name
  self._pod_config=config
 def init(self):
  raise mEViz("Not implemented")
 def delete(self,remote:mEVio)->mEVio:
  raise mEViz("Not implemented")
 def push(self,comment:mEViB=mEVia,three_way:mEVio=mEViC)->PodInfo:
  raise mEViz("Not implemented")
 def push_overwrite(self,version:mEVis,comment:mEViB=mEVia):
  pass
 def pull(self,inject_version_state:mEVio=mEViC,reset_state_before:mEVio=mEViC,lazy:mEVio=mEViC):
  raise mEViz("Not implemented")
 def commit(self,message:mEViB=mEVia):
  raise mEViz("Not implemented")
 def inject(self,version:mEVis,reset_state:mEVio)->mEVio:
  raise mEViz("Not implemented")
 def list_versions(self)->List[mEViB]:
  raise mEViz("Not implemented")
 def version_info(self,version:mEVis):
  raise mEViz("Not implemented")
 def version_metamodel(self,version:mEVis)->Dict:
  raise mEViz("Not implemented")
 def set_version(self,version:mEVis,inject_version_state:mEVio,reset_state:mEVio,commit_before:mEVio):
  raise mEViz("Not implemented")
 def list_version_commits(self,version:mEVis)->List[mEViB]:
  raise mEViz("Not implemented")
 def get_commit_diff(self,version:mEVis,commit:mEVis)->Dict:
  raise mEViz("Not implemented")
 def register_remote(self,pod_name:mEViB,ci_pod:mEVio)->mEVio:
  raise mEViz("Not implemented")
 def rename_pod(self,current_pod_name,new_pod_name)->mEVio:
  raise mEViz("Not implemented")
 def list_pods(self,fetch_remote:mEVio)->Set[mEViB]:
  raise mEViz("Not implemented")
 ...
 def get(cls,pod_name,pre_config=mEVia):
  ...
  raise mEViz('Unable to find Cloud Pod manager implementation type "%s"'%backend)
 def deploy_pod_into_instance(self,pod_path):
  delete_pod_zip=mEViC
  if os.path.isdir(pod_path):
   ...
   for folder in PERSISTED_FOLDERS:
    ...
    cp_r(src_folder,tgt_folder,rm_dest_on_conflict=mEViG)
   pod_path=create_zip_file(tmpdir)
   rm_rf(tmpdir)
   delete_pod_zip=mEViG
  zip_content=load_file(pod_path,mode="rb")
  url=get_pods_endpoint()
  result=requests.post(url,data=zip_content)
  if result.status_code>=400:
   raise mEViz("Unable to restore pod state via local pods management API %s (code %s): %s"%(url,result.status_code,result.content))
  if delete_pod_zip:
   rm_rf(pod_path)
  else:
   return pod_path
 @mEVit
 def get_state_zip_from_instance(get_content=mEViC):
  url=f"{get_pods_endpoint()}/state"
  result=requests.get(url)
  if result.status_code>=400:
   raise mEViz("Unable to get local pod state via management API %s (code %s): %s"%(url,result.status_code,result.content))
  if get_content:
   return result.content
  zip_file=f"{new_tmp_file()}.zip"
  save_file(zip_file,result.content)
  return zip_file
 def get_pod_info(self,pod_data_dir:mEViB=mEVia):
...
class CloudPodManagerCPVCS(CloudPodManager):
 BACKEND="cpvcs"
 @mEVit
 def parse_pod_name_from_qualifying_name(qualifying_name:mEViB)->mEViB:
 ...
 @mEVit
 def _get_max_version_for_pod_from_platform(pod_name:mEViB,auth_headers):
  ...
  remote_max_ver=mEVis(content["max_ver"])
 ...
 def _upload_version_and_product_space(self,presigned_urls):
  ...
  with mEViW(version_space_archive,"rb")as version_space_content:
  ...
  for version_no,urls in presigned_meta_state_urls.items():
   ...
   with mEViW(meta_archive,"rb")as meta_archive_content:
   ...
   with mEViW(state_archive,"rb")as state_archive_content:
 ...
 def _add_state_to_cpvs_store(self):
  ...
  with ZipFile(zip_file,"r")as state_zip:
   ...
   api_states_traverse(api_states_path=api_states_path,side_effect=CloudPodManagerCPVCS._add_state_files_func,mutables=mEVia)
  rm_rf(zip_file)
  rm_rf(tmp_dir)
 def _pull_versions(self,auth_headers,required_versions:mEViB):
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
 ...
 def _clone_pod(self,auth_headers,lazy:mEVio=mEViC):
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
 ...
 def delete(self,remote:mEVio)->mEVio:
  ...
  if remote:
   pass
  return mEViC
 def _push_to_remote(self,url:mEViB):
 ...
 def pull(self,inject_version_state:mEVio=mEViC,reset_state_before:mEVio=mEViC,lazy:mEVio=mEViC):
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
  auth_headers=get_auth_headers()
  if self.pod_name in cpvcs_api.list_locally_available_pods(show_remote_or_local=mEViC):
   cpvcs_api.set_pod_context(self.pod_name)
   ...
   if remote_max_ver==current_max_ver:
    ...
    return
   if not lazy:
    required_versions=",".join(mEVif(lambda ver:mEViB(ver),mEViN(current_max_ver+1,remote_max_ver+1)))
   else:
 ...
 def _download_version_product(self,version:mEVis,retain:mEVio=mEViC)->Dict[mEViB,mEViB]:
 ...
 def _get_presigned_url_for_version_product(self,version:mEVis)->Dict:
 ...
 def _inject_from_remote(self,version:mEVis,retain:mEVio=mEViC)->mEVio:
  from localstack_ext.bootstrap.cpvcs.utils.remote_utils import(extract_meta_and_state_archives)
  content=self._get_presigned_url_for_version_product(version=version)
  state_url=content.get("presigned_url_state")
  tmp_state_archive=new_tmp_file()
 ...
 def inject(self,version:mEVis,reset_state:mEVio)->mEVio:
  ...
  if not cpvcs_api.config_context.pod_exists_locally(self.pod_name):
   ...
   if not state_archive_path and cpvcs_api.is_remotely_managed():
    ...
    product_archive_path=self._download_version_product(version=version,retain=mEViG)
    if not product_archive_path:
     return mEViC
    state_archive_path=cpvcs_api.PodFilePaths.get_version_state_archive(version)
  if reset_state:
   reset_local_state(reset_data_dir=mEViG,exclude_from_reset=["dynamodb","kinesis","stepfunctions"])
 ...
 def version_metamodel(self,version:mEVis)->Dict:
  ...
  if not result and cpvcs_api.is_remotely_managed():
   self._download_version_product(version=version,retain=mEViG)
 ...
 def get_commit_diff(self,version:mEVis,commit:mEVis)->Dict:
  ...
  return commit_diff
 def register_remote(self,pod_name:mEViB,ci_pod:mEVio=mEViC)->mEVio:
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
  cpvcs_api.set_pod_context(self.pod_name)
 ...
 def list_pods(self,fetch_remote:mEVio)->Set[mEViB]:
  ...
  return result
 @mEViJ
 def upload_content(cls,presigned_url:mEViB,zip_data_content):
  res=safe_requests.put(presigned_url,data=zip_data_content)
  if res.status_code>=400:
   raise mEViz("Unable to upload pod state to S3 (code %s): %s"%(res.status_code,res.content))
  return res
 @mEVit
 def create_platform_url(request:mEViB)->mEViB:
...
class CloudPodManagerManaged(CloudPodManager):
 BACKEND="managed"
 def push(self,comment:mEViB=mEVia,three_way:mEVio=mEViC)->PodInfo:
  zip_data_content=self.get_state_zip_from_instance(get_content=mEViG)
  print('Pushing state of cloud pod "%s" to backend server (%s KB)'%(self.pod_name,format_number(mEVic(zip_data_content)/1000.0)))
  self.push_content(self.pod_name,zip_data_content)
  print("Done.")
  result=self.get_pod_info()
  result.pod_size_compressed=mEVic(zip_data_content)
  return result
 def pull(self,inject_version_state:mEVio=mEViC,reset_state_before:mEVio=mEViC,lazy:mEVio=mEViC):
  ...
  rm_rf(zip_path)
 @mEVit
 def presigned_url(pod_name:mEViB,mode:mEViB)->mEViB:
  ...
  if response.status_code>=400:
   raise mEViz("Unable to get cloud pod presigned URL (code %s): %s"%(response.status_code,content))
 ...
 def push_content(cls,pod_name,zip_data_content):
  ...
  res=safe_requests.put(presigned_url,data=zip_data_content)
  if res.status_code>=400:
   raise mEViz("Unable to push pod state to API (code %s): %s"%(res.status_code,res.content))
  return res
class CloudPodManagerGit(CloudPodManager):
 BACKEND="git"
 def push(self,comment:mEViB=mEVia,three_way:mEVio=mEViC):
  ...
  if branch not in repo:
   porcelain.branch_create(repo,branch,force=mEViG)
 ...
 def pull(self,inject_version_state:mEVio=mEViC,reset_state_before:mEVio=mEViC,lazy:mEVio=mEViC):
  ...
  if remote_ref not in remote_refs:
   raise mEViz('Unable to find branch "%s" in remote git repo'%branch)
  remote_location=self.pod_config.get("url")
  self.switch_branch(branch)
 ...
 def client(self):
  ...
  return client,path
 def local_repo(self):
  self.clone_dir=mEVir(self,"clone_dir",mEVia)
...
class PodConfigManagerMeta(mEVie):
 def __getattr__(cls,attr):
  def _call(*args,**kwargs):
   result=mEVia
   for manager in cls.CHAIN:
    try:
     tmp=mEVir(manager,attr)(*args,**kwargs)
     if tmp:
      if not result:
       result=tmp
      elif mEViT(tmp,mEViL)and mEViT(result,mEViL):
       result.extend(tmp)
    except mEViz:
     if LOG.isEnabledFor(logging.DEBUG):
      LOG.exception("error during PodConfigManager call chain")
   if result is not mEVia:
    return result
   raise mEViz('Unable to run operation "%s" for local or remote configuration'%attr)
  return _call
class PodConfigManager(mEViF,metaclass=PodConfigManagerMeta):
 CHAIN=[]
 @mEViJ
 def pod_config(cls,pod_name):
  ...
  if not pod_config:
   raise mEViz('Unable to find config for pod named "%s"'%pod_name)
  return pod_config[0]
class PodConfigManagerLocal(mEViF):
 CONFIG_FILE=".localstack.yml"
 def list_pods(self):
  local_pods=self._load_config(safe=mEViG).get("pods",{})
  local_pods=[{"pod_name":k,"state":"Local Only",**v}for k,v in local_pods.items()]
  existing_names=mEViY([pod["pod_name"]for pod in local_pods])
  result=[pod for pod in local_pods if pod["pod_name"]not in existing_names]
  return result
 def store_pod_metadata(self,pod_name,metadata):
  pass
 def _load_config(self,safe=mEViC):
  try:
   return yaml.safe_load(to_str(load_file(self.CONFIG_FILE)))
  except mEViz:
   if safe:
    return{}
   raise mEViz('Unable to find and parse config file "%s"'%self.CONFIG_FILE)
class PodConfigManagerRemote(mEViF):
 def list_pods(self):
  result=[]
  auth_headers=get_auth_headers()
  ...
  content=response.content
  if response.status_code>=400:
   raise mEViz("Unable to fetch list of pods from API (code %s): %s"%(response.status_code,content))
 ...
 def store_pod_metadata(self,pod_name,metadata):
  ...
  content=response.content
  if response.status_code>=400:
   raise mEViz("Unable to store pod metadata in API (code %s): %s"%(response.status_code,content))
...
def init_cpvcs(pod_name:mEViB,pre_config:Dict[mEViB,mEViB],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.init()
def delete_pod(pod_name:mEViB,remote:mEVio,pre_config:Dict[mEViB,mEViB])->mEVio:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 result=backend.delete(remote=remote)
 return result
def register_remote(pod_name:mEViB,pre_config:Dict[mEViB,mEViB],**kwargs)->mEVio:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 result=backend.register_remote(pod_name=pod_name,ci_pod=pre_config.get("ci_pod",mEViC))
 return result
def rename_pod(current_pod_name:mEViB,new_pod_name:mEViB,pre_config:Dict[mEViB,mEViB],**kwargs):
 ...
 return result
def list_pods_cpvcs(remote:mEVio,pre_config:Dict[mEViB,mEViB],**kwargs)->List[mEViB]:
 backend=CloudPodManager.get(pod_name="",pre_config=pre_config)
 result=backend.list_pods(fetch_remote=remote)
 return result
def commit_state(pod_name:mEViB,pre_config:Dict[mEViB,mEViB],message:mEViB=mEVia,**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.commit(message=message)
def inject_state(pod_name:mEViB,version:mEVis,reset_state:mEVio,pre_config:Dict[mEViB,mEViB],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 result=backend.inject(version=version,reset_state=reset_state)
 return result
def list_versions(pod_name:mEViB,pre_config:Dict[mEViB,mEViB],**kwargs)->List[mEViB]:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 versions=backend.list_versions()
 return versions
def get_version_info(version:mEVis,pod_name:mEViB,pre_config:Dict[mEViB,mEViB],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 info=backend.version_info(version=version)
 return info
def get_version_metamodel(version:mEVis,pod_name:mEViB,pre_config:Dict[mEViB,mEViB],**kwargs)->Dict:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 metamodel=backend.version_metamodel(version=version)
 return metamodel
def set_version(version:mEVis,inject_version_state:mEVio,reset_state:mEVio,commit_before:mEVio,pod_name:mEViB,pre_config:Dict[mEViB,mEViB],**kwargs)->mEVio:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 ...
 return success
def list_version_commits(version:mEVis,pod_name:mEViB,pre_config:Dict[mEViB,mEViB])->List[mEViB]:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 commits=backend.list_version_commits(version=version)
 return commits
def get_commit_diff(version:mEVis,commit:mEVis,pod_name:mEViB,pre_config:Dict[mEViB,mEViB])->Dict:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 commit_diff=backend.get_commit_diff(version=version,commit=commit)
 return commit_diff
def push_overwrite(version:mEVis,pod_name:mEViB,comment:mEViB,pre_config:Dict[mEViB,mEViB]):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.push_overwrite(version=version,comment=comment)
def push_state(pod_name,pre_config=mEVia,squash_commits=mEViC,comment=mEVia,three_way=mEViC,**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
...
def pull_state(pod_name,inject_version_state=mEViC,reset_state_before=mEViC,lazy=mEViC,**kwargs):
 pre_config=kwargs.get("pre_config",mEVia)
 if not pod_name:
  raise mEViz("Need to specify a pod name")
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.pull(inject_version_state=inject_version_state,reset_state_before=reset_state_before,lazy=lazy)
 print("Done.")
def reset_local_state(reset_data_dir=mEViC,exclude_from_reset:List[mEViB]=mEVia):
 url=f"{get_pods_endpoint()}/state"
 if reset_data_dir:
  url+="/datadir"
 ...
 if result.status_code>=400:
  raise mEViz("Unable to reset service state via local management API %s (code %s): %s"%(url,result.status_code,result.content))
 print("Done.")
def list_pods(args):
 return PodConfigManager.list_pods()
def get_data_dir_from_container()->mEViB:
 try:
...
def get_persisted_resource_names(data_dir)->List[mEViB]:
 names=[]
 ...
 with os.scandir(os.path.join(data_dir,"api_states"))as entries:
  for entry in entries:
   if entry.is_dir()and mEVic(os.listdir(entry.path))>0:
    names.append(entry.name)
