...
class StateFileLocator(abc.ABC):
 location="_none_"
 active_instance=XYfBp
 @abc.abstractmethod
 def get_state_file_location_by_key(self,key:XYfBD)->XYfBD:
  pass
 @XYfBb
 def get(cls,requested_file_locator:XYfBD):
...
class ObjectStorageProvider(abc.ABC):
 ...
 active_instance=XYfBp
 @XYfBb
 def get(cls,state_file_locator:XYfBD,requested_storage:XYfBD,serializers:Dict[XYfBD,CPVCSSerializer]):
  ...
  return cls.active_instance
 def __init__(self,state_file_locator:StateFileLocator,serializers:Dict[XYfBD,CPVCSSerializer]):
 ...
 def get_state_file_location_by_key(self,key:XYfBD)->XYfBD:
  pass
 @abc.abstractmethod
 def get_version_by_key(self,key:XYfBD)->Version:
  pass
 @abc.abstractmethod
 def get_revision_by_key(self,key:XYfBD)->Revision:
  pass
 @abc.abstractmethod
 def get_revision_or_version_by_key(self,key:XYfBD)->Union[Revision,Version]:
  pass
 @abc.abstractmethod
 def get_delta_file_by_key(self,key:XYfBD)->XYfBD:
  pass
 @abc.abstractmethod
 def upsert_objects(self,*args:CPVCSObj)->List[XYfBD]:
  pass
 @abc.abstractmethod
 def update_revision_key(self,old_key:XYfBD,new_key:XYfBD,referenced_by_version:XYfBD=XYfBp):
  pass
 @abc.abstractmethod
 def version_exists(self,key:XYfBD)->XYfBK:
  pass
 @abc.abstractmethod
 def merge_remote_into_local_version(self,remote_location:XYfBD,key:XYfBD):
  pass
 @abc.abstractmethod
 def _update_key(self,old_key:XYfBD,new_key:XYfBD,base_path:XYfBD)->XYfBK:
  pass
 @abc.abstractmethod
 def _serialize(self,*args:CPVCSObj)->List[XYfBD]:
  pass
 @abc.abstractmethod
 def _deserialize(self,key_serializer:Dict[XYfBD,XYfBD],remote_location:XYfBD=XYfBp)->List[CPVCSObj]:
  pass
class ObjectStorageLocal(ObjectStorageProvider):
 ...
 def _serialize(self,*args:CPVCSObj)->List[XYfBD]:
  ...
  for cpvcs_obj in args:
   if XYfBI(cpvcs_obj,Version):
    hash_refs.append(self.serializers["version"].store_obj(cpvcs_obj))
   elif XYfBI(cpvcs_obj,Revision):
    hash_refs.append(self.serializers["revision"].store_obj(cpvcs_obj))
  return hash_refs
 def _deserialize(self,key_serializer:Dict[XYfBD,XYfBD],remote_location:XYfBD=XYfBp)->List[Union[Revision,Version]]:
