...
def random_hash()->SkuRA:
 return SkuRV(random.getrandbits(160))
def compute_file_hash(file_path:SkuRA,accum=SkuRr)->Optional[SkuRA]:
 try:
  with SkuRm(file_path,"rb")as fp:
   if accum:
...
def compute_version_archive_hash(version_no:SkuRU,state_archive:SkuRA)->SkuRA:
 ...
 m.update(SkuRA(version_no).encode("utf-8"))
 rm_rf(tmp_state_dir)
 return m.hexdigest()
def compute_revision_hash(cpvcs_node:Revision)->SkuRA:
 if not cpvcs_node.state_files:
  return random_hash()
 state_file_keys=SkuRh(lambda state_file:state_file.hash_ref,cpvcs_node.state_files)
 m=hashlib.sha1()
 for key in state_file_keys:
  try:
   with SkuRm(os.path.join(config_context.get_obj_store_path(),key),"rb")as fp:
    m.update(fp.read())
  except SkuRl as e:
   LOG.warning(f"Failed to open file and compute hash for {key}: {e}")
 if SkuRc(cpvcs_node,Revision):
  m.update(cpvcs_node.rid.encode("utf-8"))
  m.update(SkuRA(cpvcs_node.revision_number).encode("utf-8"))
 elif SkuRc(cpvcs_node,Version):
  m.update(SkuRA(cpvcs_node.version_number).encode("utf-8"))
 return m.hexdigest()
