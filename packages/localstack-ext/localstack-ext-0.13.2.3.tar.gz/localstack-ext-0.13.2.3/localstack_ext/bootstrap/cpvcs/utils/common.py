...
class CPVCSConfigContext:
 ...
 def get_version_state_archive_path(self,version,with_format:rDFwT=rDFwj)->rDFwn:
 ...
 def update_ver_log(self,author:rDFwn,ver_no:rDFwJ,rev_id:rDFwn,rev_no:rDFwJ):
  with rDFwf(self.get_ver_log_path(),"a")as fp:
   fp.write(f"{VER_LOG_STRUCTURE.format(author=author, ver_no=ver_no, rev_rid_no=f'{rev_id}_{rev_no}')}\n")
 def create_version_symlink(self,name:rDFwn,key:rDFwn=rDFwL)->rDFwn:
  return self._create_symlink(name,key,self.get_ver_refs_path())
 def create_revision_symlink(self,name:rDFwn,key:rDFwn=rDFwL)->rDFwn:
  return self._create_symlink(name,key,self.get_rev_refs_path())
 def is_initialized(self)->rDFwT:
  return self.pod_root_dir and os.path.isdir(self.pod_root_dir)
 def _create_symlink(self,name:rDFwn,key:rDFwn,path:rDFwn)->rDFwn:
...
def zip_directories(zip_dest:rDFwn,directories:List[rDFwn])->rDFwn:
 ...
 return zip_dest
def add_file_to_archive(archive:rDFwn,entry_name:rDFwn,content:Union[rDFwn,rDFwU]):
 with zipfile.ZipFile(archive,"a")as zip_file:
  zip_file.writestr(entry_name,content)
def read_file_from_archive(archive_path:rDFwn,file_name:rDFwn)->rDFwn:
 try:
