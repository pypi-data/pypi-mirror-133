...
CAPTURE_METAMODEL_SEPARATELY=UVGDw
...
class MetamodelDelta(SubtypesInstanceManager):
 def create_delta_for_state_files(self,sf1:StateFileRef,sf2:StateFileRef):
  ...
  if not sf1:
   backend2=load_persisted_object(config_context.get_obj_file_path(sf2.hash_ref))
   clazz=UVGDW(backend2)
   backend1=_infer_backend_init(clazz,sf2)
  else:
   backend1=load_persisted_object(config_context.get_obj_file_path(sf1.hash_ref))
   clazz=UVGDW(backend1)
   backend2=_infer_backend_init(clazz,sf1)
  diff=self.create_delta_json(backend1,backend2)
  return diff
 def create_delta_log(self,state_from:Set[StateFileRef],state_to:Set[StateFileRef])->UVGDa:
  ...
  def _create_sf_lookup(state_files:Set[StateFileRef])->Dict[UVGDa,StateFileRef]:
  ...
  for state_file_from in state_files_from_regular:
   ...
   if state_file_from.any_congruence(state_files_to_regular):
    ...
    diff_json=self.create_delta_for_state_files(state_file_from,UVGDn)
  ...
  for state_files_to in state_files_to_lookup.values():
   ...
   diff_json=self.create_delta_for_state_files(UVGDn,state_files_to)
...
class CommitMetamodelUtils:
 @UVGDy
 def create_metadata_archive(cls,version:Version,delete_reference:bool=UVGDw,overwrite:bool=UVGDw,metamodels_file:UVGDa=UVGDn):
 ...
 def get_metamodel_delta(cls,prev_metamodel:Dict,this_metamodel:Dict)->Dict:
  ...
  return result
 @UVGDy
 def create_delta_metamodel_file(cls,version:UVGDk,revision:UVGDk,store_to_zip=UVGDw)->Tuple[Dict,Dict]:
 ...
 @UVGDy
 def reconstruct_metamodel(cls,version:UVGDk,revision:UVGDk=UVGDn)->Dict:
  result={}
  for rev in UVGDx(1,revision+1):
  ...
  return result
 @UVGDy
 def get_version_metamodel(cls,version:UVGDk,revision:UVGDk=UVGDn)->Dict:
 ...
 @UVGDy
 def get_commit_diff(cls,version_no:UVGDk,commit_no:UVGDk)->Optional[Dict]:
  ...
  return result
def _infer_backend_init(clazz:Type,sf:StateFileRef)->UVGDt:
 if UVGDT(clazz,UVGDv):
  return{}
 constructor=UVGDP(clazz,"__init__",UVGDn)
...
def _create_metamodel_helper(obj,width=25,visited:set=UVGDn):
 if obj is UVGDn:
  return obj
