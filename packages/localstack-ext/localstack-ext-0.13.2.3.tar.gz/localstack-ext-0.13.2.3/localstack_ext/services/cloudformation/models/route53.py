...
def canonicalize_name(name:BDkVs)->BDkVs:
 if name[-1]!=".":
  return f"{name}."
 return name
class Route53HostedZone(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(result or[BDkVn])[0]
 def get_physical_resource_id(self,attribute=BDkVn,**kwargs):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return BDkVb(Route53HostedZone,self).get_cfn_attribute(attribute_name)
 @BDkVP
