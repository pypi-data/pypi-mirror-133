...
class EC2ElasticIP(GenericBaseModel):
 ...
 def cloudformation_type():
  return "AWS::EC2::EIP"
 def get_physical_resource_id(self,attribute=NARar,**kwargs):
...
class SecurityGroupInOrEgress(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  for perm in perms:
   if NARaJ(perm["IpProtocol"])!=NARaJ(rp["IpProtocol"]):
    continue
 ...
 @NARaL
 def get_vpc_and_name_for_security_group(cls,group_id:NARaJ)->Tuple[NARaJ,NARaJ]:
  client=aws_stack.connect_to_service("ec2")
  groups=client.describe_security_groups(GroupIds=[group_id])["SecurityGroups"]
  vpc_id=groups and groups[0].get("VpcId")or NARar
  group_name=groups and groups[0]["GroupName"]or NARar
 ...
 def get_deploy_templates(cls):
  def create_params(params,**kwargs):
   ...
   source_group_id=result.pop("SourceSecurityGroupId",NARar)
   vpc_id=NARar
   ...
   dst_group_id=result.pop("DestinationSecurityGroupId",NARar)
   ...
   if result.get("IpProtocol"):
    result["IpProtocol"]=NARaJ(result.get("IpProtocol"))
   description=result.pop("Description",NARar)
   cidr_ipv6=result.pop("CidrIpv6",NARar)
