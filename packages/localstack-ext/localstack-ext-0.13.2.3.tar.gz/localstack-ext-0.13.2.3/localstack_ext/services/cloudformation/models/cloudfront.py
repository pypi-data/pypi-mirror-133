...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or HJbCk
 @HJbCw
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return HJbCB(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @HJbCF
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if HJbCs(origins,HJbCr)else origins.get("Items")
  origins_to_create=HJbCo([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if HJbCs(aliases,HJbCr)else aliases.get("Items")
  aliases_to_create=HJbCo(aliases)
  for distr in distrs:
   origins=HJbCo([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=HJbCo(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",HJbCW)
   config["Enabled"]=HJbCx(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if HJbCs(custom_errors,HJbCi)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=HJbCx(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":HJbCW}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=HJbCx(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=HJbCk):
    parent=config if parent is HJbCk else parent
    items=parent.get(key)or[]
    if HJbCs(items,HJbCi)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":HJbCG(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if HJbCs(cache_behavs,HJbCi)else cache_behavs)
