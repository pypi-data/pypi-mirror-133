...
class Ice_d18O():
    ...
    def forward(self, nproc=None):
        ...
        # sensor model
        d18O_ice = ice_sensor(self.pobj.clim[self.pr_name].time, self.pobj.clim[self.d18O_name].da.values, self.pobj.clim[pr_name].da.values)
...
class Lake_VarveThickness():
    ...
    '''
    def __init__(self, pobj, tas_name='model_tas', H=None, shape=None, mean=None, SNR=None, seed=None):
        self.pobj = pobj
        self.H = H
        self.shape = shape
        self.mean = mean
    ...
    def forward(self):
        ...
        # run the model
        if self.pobj.lat > 0:
            tas_ann = self.pobj.clim[self.tas_name].annualize(months=[6, 7, 8])
        else:
            tas_ann = self.pobj.clim[self.tas_name].annualize(months=[12, 1, 2])
...
class Coral_SrCa:
    ...
    '''
    def __init__(self, pobj, tos_name='model_tos', b=10.553, a=None, seed=0):
        self.pobj = pobj
    ...
    def forward(self):
        ...
        SrCa = self.a*self.pobj.clim[self.tos_name].da.values + self.b

        pp = ProxyRecord(
            pid=f'pseudo_{self.pobj.pid}',
            time=self.pobj.clim[self.tos_name].time,
            value=SrCa,
            lat=self.pobj.lat,
            lon=self.pobj.lon,
            ptype=self.pobj.ptype,
