import glob
import os
from p2k.climate import ClimateField, ClimateDataset
import xarray as xr
import pandas as pd
import numpy as np
import copy
...
class ProxyRecord:
    ...
    def get_clim(self, fields, tag=None, verbose=True, **kwargs):
        ''' Get the nearest climate from cliamte fields

        Parameters
        ----------
        field : p2k.climate.ClimateField or p2k.climate.ClimateDataset
            the climate field

        field_name : str
            the string name for the field
        '''
    ...
    def plot(self, mute=False, figsize=[12, 4], legend=False, ms=200, stock_img=True, edge_clr='w',
        ...
        ax['ts'].set_title(f'{self.pid} @ (lat:{self.lat}, lon:{self.lon}) | Type: {self.ptype}')
        if legend:
            ax['ts'].legend()
