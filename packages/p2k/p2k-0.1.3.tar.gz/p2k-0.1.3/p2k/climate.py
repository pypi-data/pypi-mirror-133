...
class ClimateField:
    ...
    def wrap_lon(self, mode='360', lon_name='lon'):
        if mode == '360':
            ...
            tmp_da = self.assign_coords({lon_name: ((self.da[lon_name]+180) % 360)-180})
        else:
    ...
    def load_nc(self, path, time_name='time', lat_name='lat', lon_name='lon', **kwargs):
    ...
    def plot(self, it=0, mute=False, **kwargs):
...
class ClimateDataset:
    ...
    def load_nc(self, path, time_name='time', lat_name='lat', lon_name='lon', vn=None, **kwargs):
        new = self.copy()
        fields = {}
