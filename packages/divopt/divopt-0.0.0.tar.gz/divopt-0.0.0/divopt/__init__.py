...
class OptunaStudy:
	...
	def optimize(self, target_func, n_trials):
		...
		# 初回空打ちで情報を取得
		info = get_init_info(target_func)
		# optuna様のtarget_funcを呼ぶラッパーを生成
		wrapper_func = gen_wrapper_func(target_func, info)
		# 最適化 [divopt]
		best_x = optimize(
			func = wrapper_func,	# 最適化対象関数
			dim_n = info["dim_n"],	# 入力の次元数
			lim_range = info["lim_range"],	# 各軸の範囲
			fin_judge = ["rep", n_trials],	# 終了条件 (指定: [mode, value])
			direction = self.direction	# 最大化 / 最小化
		)
		# best_paramsを束縛する
		self.best_params = {
			info["param_names"][i]: best_x[i]
			for i in range(info["dim_n"])
		}
		self.optimize_done_flag = False
