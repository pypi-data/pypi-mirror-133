...
class OCIFileSystem(AbstractFileSystem):
    ...
    def url(self, path, expires, access_type="ObjectRead", name=None, **kwargs):
        ...
        access_type: str (defaults to "ObjectRead")
            The operation that can be performed on this resource.
            Allowed values for this property are: “ObjectRead”, “ObjectWrite”,
            “ObjectReadWrite”, “AnyObjectWrite”
        name: str (defaults to ocifs generated name)
            A user-specified name for the pre-authenticated request.
            Names can be helpful in managing pre-authenticated requests.
            Avoid entering confidential information.
        """
        bucket, namespace, key = self.split_path(path)
        if name is None:
            name = f"Unnamed PAR for {key} created using ocifs"
        try:
            par_details = self._call_oci(
                CreatePreauthenticatedRequestDetails,
                is_detail_method=True,
                object_name=key,
            ...
            return self._call_oci(
                self.oci_client.create_preauthenticated_request,
                namespace_name=namespace,
                bucket_name=bucket,
