...
def A246028(n):
    return sum(int(not (~(n - k) & 2 * k) | (~n & k)) for k in range(n + 1))
...
def A333196(n):
    ...
    return (
        ...
        if len(fs) == 0
            ...
            mul, (p ** (fs[p] // n + 1 if fs[p] % n else fs[p] // n) for p in fs)
        )
    )
...
def A329722(n):
    return sum(int(not (~(n + 2 * k) & 2 * n - k) | (~n & k)) for k in range(n + 1))
...
def A118548_gen():
    return (
        n
        for n in (x ** 2 for x in range(1, 10 ** 6))
