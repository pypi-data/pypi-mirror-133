"""Tests for trading.datasets_core.exchange.ftx."""
...
from trading.datasets_core.exchange import FTXExchange
...
class TestExchangeFTX:
    ...
    def test_initialization(self, exchange):
        ...
        assert isinstance(exchange, FTXExchange)

    def test_generate_fetch_ohlcv_params(self, exchange):
        expected_output = {
            "start_time": 1293836400,
            "end_time": 1294196400,
        }
    ...
    def test_successful_fetch_ohlcv(self, exchange):
        expected_output = [
            (1609459200000, 28965.0, 29014.5, 28960.0, 29014.5, 146879.57105),
            (1609459260000, 29014.5, 29070.5, 29014.5, 29065.5, 527600.784),
            (1609459320000, 29065.5, 29066.0, 29018.0, 29041.5, 181341.49355),
        ]
    ...
    def test_successful_fetch_named_tuple_ohlcv(self, exchange):
        ...
        assert ohlcvs[0].open == 28965.0
        assert ohlcvs[0].high == 29014.5
        assert ohlcvs[0].low == 28960.0
        assert ohlcvs[0].close == 29014.5
        assert ohlcvs[0].volume == 146879.57105

        assert ohlcvs[1].timestamp == 1609459260000
        assert ohlcvs[1].open == 29014.5
        assert ohlcvs[1].high == 29070.5
        assert ohlcvs[1].low == 29014.5
        assert ohlcvs[1].close == 29065.5
        assert ohlcvs[1].volume == 527600.784

        assert ohlcvs[2].timestamp == 1609459320000
        assert ohlcvs[2].open == 29065.5
        assert ohlcvs[2].high == 29066.0
        assert ohlcvs[2].low == 29018.0
        assert ohlcvs[2].close == 29041.5
        assert ohlcvs[2].volume == 181341.49355
