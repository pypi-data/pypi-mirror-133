...
"""
from hyService.cloudService import Debug
from hyService.utils import GetConfig
from hycv.vision import resize_image_esp
import os
import cv2
import dlib
import numpy as np
...
class FaceDetection68(Debug, FaceConfig68):
    """
    ...
    def __init__(self, model_path:str=None, *args, **kwargs):
        ...
        self.predictor = dlib.shape_predictor(self.model_path)  # 输入模型,构建特征提取器

    def draw_face(self, img:np.ndarray, dets:dlib.rectangles, *args, **kwargs):
    ...
    def draw_points(self, img:np.ndarray, landmarks:np.matrix, if_num:int=False, *args, **kwargs):
        """
    ...
    def facesPoints(self, img:np.ndarray, esp:int=None, det_num:int=1,*args, **kwargs):
        """
        :param img: 输入的是人脸检测的图,必须是3通道或者灰度图
        :param onlyOne: 如果设置为True,会检测是否只是一张人脸,如果有多张人脸,则会报错
        ...
        # dlib的人脸检测装置
        if esp is not None:
            img = resize_image_esp(input_image=img, esp=esp)
        dets = self.detector(img, det_num)
        # self.draw_face(img, dets)
        font_color = "green" if len(dets) == 1 else "red"
        self.debug_print("Number of faces detected: {}".format(len(dets)), font_color=font_color)
        landmarkList = []
        pointsList = []
        for d in dets:
        ...
        self.debug_print("Key point detection SUCCESS.", font_color="green")
        return dets, landmarkList, pointsList

    def facePoints(self, img:np.ndarray, det_num:int=1, *args, **kwargs):
        """
...
class PoseEstimator68(object):
    ...
    本类将实现但输入图的人脸姿态检测
    """
    def __init__(self, img:np.ndarray, params_path:str=None):
    ...
    def rot_params_rm(R):
        ...
        yaw = 180*atan2(-R[1][0], R[0][0])/pi + 360
...
if __name__ == "__main__":
    # 示例
    input_image = cv2.imread("./test.jpg")  # 读取一张图片, 必须是三通道或者灰度图
    fd68 = FaceDetection68()  # 初始化人脸关键点检测类
    dets_, landmark_, point_list_ = fd68.facePoints(input_image)  # 输入图片. 检测单张人脸
    # dets_, landmark_, point_list_ = fd68.facesPoints(input_image)  # 输入图片. 检测多张人脸
    pe = PoseEstimator68(input_image)
