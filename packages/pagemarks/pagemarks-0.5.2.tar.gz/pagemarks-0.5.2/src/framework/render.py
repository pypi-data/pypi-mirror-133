...
from framework.data import Bookmark, PagemarksCollection
...
class GenshiBookmarkContext(object):
    ...
    def build_tag_data(self, tags: [str]) -> [TagData]:
        result: [TagData] = []
        for tag in tags:
            tag_data = TagData(tag)
            tag_data.link = escape(self.baseurl + '/index.html?q=[' + tag + ']')
...
class GenshiCollectionContext(object):
    baseurl: str
    title: str
    pinned_filters: [FilterData]


    def __init__(self, base_url: str, coll_data: PagemarksCollection, **data):
        super().__init__(**data)
        self.title = coll_data.title if coll_data.title is not None else 'Pagemarks Collection'
        self.baseurl = base_url
        self.pinned_filters = []
        for qf in coll_data.pinned_filters:
