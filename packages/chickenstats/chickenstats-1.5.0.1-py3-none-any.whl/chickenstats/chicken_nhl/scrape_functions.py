...
from .chicken_nhl.player_names import correct_names_dict, goalie_names, api_names_dict

############################################## Requests functions & classes ##############################################

# These functions & the timeout class are used for scraping throughout

class TimeoutHTTPAdapter(HTTPAdapter):
    
    def __init__(self, *args, **kwargs):
        
        # Can change the default timeout here
        self.timeout = 2.5
...
def s_session():
    ...
    retry = Retry(total = 5, backoff_factor = 2, status_forcelist=[401, 403, 404, 408, 429, 500, 502, 503, 504])
    
    adapter = TimeoutHTTPAdapter(max_retries = retry, timeout = 2.5)
...
def scrape_schedule(seasons, game_types = ['R', 'P'], today_only = False, final_only = False, live_only = False, _print = True):
    ...
    for season_idx, season in enumerate(seasons):
        ...
        ## Removing unfinished/unplayed games from scheduule information. By default live games are included
        if final_only == True:
...
def scrape_api_game_info(game_ids, response_data = None, _print = True, session = None):
    '''
...
def scrape_html_shifts(game_ids, _print = True, roster_data = pd.DataFrame(), game_data = pd.DataFrame(), session = None, pbp = False):
    '''
    ...
    for game_id_idx, game_id in enumerate(game_ids):
        ...
        if game_data.empty:
            ...
            game_info = scrape_api_game_info(game_id, _print = False, session = s)
        ...
        for team, url in urls_dict.items():
            ...
            team_name = soup.find('td', {'align':'center', 'class':'teamHeading + border'}).get_text()
        ...
        mask = shifts.start_time != shifts.end_time
    ...
    if shifts_concat == []:
        ...
        columns = ['season', 'game_id', 'team', 'venue', 'player_name', 'player_jersey', 'shift_number', 'period',
                   ...
                   'end_time_seconds', 'duration_seconds', 'api_name', 'goalie']
    ...
    if changes_concat == []:
        ...
        columns = ['season', 'game_id', 'team', 'venue', 'period', 'time', 'period_seconds',  'game_seconds', 'number_on',
...
def scrape_api_events(game_ids, response_data = None, game_data = pd.DataFrame(), session = None, _print = True):
    ...
    for game_id_idx, game_id in enumerate(game_ids):
        ...
        column_names = {'result_event': 'event_type_name', 'result_eventCode': 'event_id_nhl', 'result_eventTypeId': 'event_type', 
                        ...
                        'result_secondaryType': 'event_detail', 'result_strength_code': 'goal_strength_code', 'result_strength_name': 'goal_strength_name',
    ...
    if concat_list == []:
        ...
        columns = ['season', 'game_id', 'play_id', 'event_idx', 'event_idx_nhl', 'event_id_nhl', 'event_type', 'period', 'period_type', 'period_seconds',
                   ...
                   'event_detail', 'goal_strength_code', 'goal_strength_name', 'goal_gwg', 'goal_empty_net', 'penalty_severity', 'penalty_minutes', 'version']
...
def scrape_html_events(game_ids, _print = True, roster = pd.DataFrame(), game_data = pd.DataFrame(), session = None):
    ...
    ## IMPORTANT
    new_teams_dict = {'L.A': 'LAK', 'N.J': 'NJD', 'S.J': 'SJS', 'T.B': 'TBL'}
    ...
    for game_id_idx, game_id in enumerate(game_ids):
        ...
        for team_type, team_name in teams_dict.items():

            events_df[team_type + '_skaters'] = events_df[team_type + '_skaters'].str.replace('\n', '')

            events_df[team_type + '_team'] = team_name

            events_df[team_type + '_team_abbreviated'] = events_df[team_type + '_skaters'][0].split(' ')[0]
        ...
        events_df['event_team'] = events_df.description.astype(str).str.split(' ').str[0].astype(str).str.split('\xa0').str[0]
        ...
        home_team_name = events_df.home_team_abbreviated.iloc[0]
        away_team_name = events_df.away_team_abbreviated.iloc[0]
        
        teams_list = [home_team_name, away_team_name]

        events_df.event_team = np.where(np.isin(events_df.event_team, teams_list), events_df.event_team, np.nan)

        cond_1 = events_df.event_team == ' '
        value_1 = np.nan

        cond_2 = events_df.event_team == home_team_name
        value_2 = away_team_name

        cond_3 = events_df.event_team == away_team_name
        value_3 = home_team_name
        ...
        #events_df['event_player_str'] = events_df.description.str.findall('(#)(\d\d)|(#)(\d)|(-) (\d\d)|(-) (\d)').astype(str).str\
        #                                            .replace('#', '', regex = False).str.replace(',', '', regex = False).str\
        #                                            .replace('-', '', regex = False).str.replace("'", '', regex = False).str\
        #                                            .replace('(', '', regex = False).str.replace(')', '', regex = False).str\
        ...
        events_df['event_player_str'] = events_df.description.str.findall('\#\d*').str.join(' ').str.replace('#', '')    
        ...
        #events_df.event_player_2 = np.where(events_df.description.str.contains('Drawn By'), 
        #                                    events_df.description.astype(str).str\
        #                                        .split('Drawn By').astype(str).str[1].str\
        #                                        .split('#').astype(str).str[1].str\
        #                                        .split(' ').astype(str).str[0].str.strip(), 
        #                                    events_df.event_player_2)

        events_df.event_player_3 = np.where(events_df.description.str.contains('Served By'), '\xa0', events_df.event_player_3)

        conditions = [(~pd.isna(events_df.event_player_1) & (events_df.event_player_1 != '') & (events_df.event == 'FAC')), 
                      (~pd.isna(events_df.event_player_1) & (events_df.event_player_1 != '') & (events_df.event != 'FAC'))]

        values = [events_df.away_team_abbreviated + events_df.event_player_1.astype(str), 
                  events_df.event_team + events_df.event_player_1.astype(str)]

        events_df.event_player_1 = np.select(conditions, values, events_df.event_player_1)

        event_list = ['BLOCK', 'HIT', 'PENL']

        conditions = [(~pd.isna(events_df.event_player_2) & (events_df.event_player_2 != '') & (events_df.event == 'FAC')),
                      (~pd.isna(events_df.event_player_2) & (events_df.event_player_2 != '') & (events_df.event != 'FAC') &
                       (events_df.event.isin(event_list))),
                      (~pd.isna(events_df.event_player_2) & (events_df.event_player_2 != '') & (events_df.event != 'FAC') &
                       (~events_df.event.isin(event_list)))]

        values = [events_df.home_team_abbreviated + events_df.event_player_2.astype(str),
                  events_df.opp_team + events_df.event_player_2.astype(str),
                  events_df.event_team + events_df.event_player_2.astype(str)]

        events_df.event_player_2 = np.select(conditions, values, events_df.event_player_2)

        events_df.event_player_3 = np.where((~pd.isna(events_df.event_player_3) & (events_df.event_player_3 != '')), 
                                            events_df.event_team + events_df.event_player_3.astype(str), 
        ...
        mask = np.logical_and(events_df.event_team == home_team_name, events_df.event == 'FAC')
        
        events_df.update(events_df.loc[mask].rename(players_dict, axis = 1))
        
        #conditions = [np.logical_and(events_df.event_team == home_team_name, events_df.event == 'FAC'), 
        #              np.logical_and(events_df.event_team == away_team_name, events_df.event == 'FAC')]
        
        #values = [events_df.event_player_2, events_df.event_player_1]
        
        #fo_winner = np.select(conditions, values, events_df.event_player_1)
        
        #conditions = [np.logical_and(events_df.event_team == home_team_name, events_df.event == 'FAC'), 
        #              np.logical_and(events_df.event_team == away_team_name, events_df.event == 'FAC')]
        
        #values = [events_df.event_player_1, events_df.event_player_2]
        
        #fo_loser = np.select(conditions, values, events_df.event_player_2)
        
        #events_df.event_player_1 = np.where(events_df.event == 'FAC', fo_winner, events_df.event_player_1)
        #events_df.event_player_2 = np.where(events_df.event == 'FAC', fo_loser, events_df.event_player_2)
        ...
        roster_df = roster_df[roster_df.player_status != 'scratch'].copy()
            
        roster_df['team_abbreviated'] = np.where(roster_df.team == 'home', home_team_name, away_team_name)
        
        roster_df['teamnum'] = roster_df.team_abbreviated + roster_df.player_jersey.astype(str)
        
        name_dict = dict(zip(roster_df.teamnum, roster_df.player_name))
        
        api_name_dict = dict(zip(roster_df.teamnum, roster_df.api_name))
        
        players = ['event_player_' + str(x) for x in list(range(1, 4))]
        
        for player in players:
            
            events_df[player + '_api_name'] = events_df[player].map(api_name_dict).fillna(events_df[player])
            
            events_df[player] = events_df[player].map(correct_names_dict).fillna(events_df[player])
        ...
        events_df.event_player_1 = np.where(np.logical_or(events_df.description.str.contains('team', case = False), 
                                                          events_df.description.str.contains('bench', case = False)),
                                            'BENCH', events_df.event_player_1)
        
        events_df['event_zone'] = events_df.description.str.extract("([a-zA-Z]{3}\\.\\s*[zZ]one)", expand = False)\
                                        .str.replace('. zone', '', case = False).str.upper()
        ...
        home_penalty = events_df.home_skater_count > events_df.home_skater_count.shift(-1)
        away_penalty = events_df.away_skater_count > events_df.away_skater_count.shift(-1)
        
        cond_1 = ((events_df.event == 'PENL') & (events_df.event_team == '') &
                  (events_df.description.str.contains('bench', case = False) &
                   home_penalty))
        
        value_1 = events_df.home_team_abbreviated
        
        cond_2 = ((events_df.event == 'PENL') & (events_df.event_team == '') &
                  (events_df.description.str.contains('bench', case = False) &
                   away_penalty))
        
        value_2 = events_df.away_team_abbreviated
        
        conditions = [cond_1, cond_2]
        values = [value_1, value_2]
        
        events_df.event_team = np.select(conditions, values, events_df.event_team)
    ...
    columns = ['season', 'game_id', 'event_idx', 'period', 'period_seconds', 'game_seconds', 'event',
               'description', 'event_team', 'event_player_1', 'event_player_2', 'event_player_3', 'opp_team', 
               'home_team', 'home_team_abbreviated', 'home_skaters', 'away_team', 'away_team_abbreviated', 'away_skaters',
               'strength', 'home_skater_count', 'away_skater_count', 'time', 'original_time', 'game_date', 'event_player_1_api_name', 
               'event_player_2_api_name', 'event_player_3_api_name', 'event_player_str']
...
def scrape_pbp(game_ids, _print = True):
    '''
    ...
    for game_id_idx, game_id in enumerate(game_ids):
        ...
        game_info = scrape_api_game_info(game_id, response = live_response, _print = False, session = live_response_session)
        
        ## Important dictionaries of team information
        ...
        api_events = scrape_api_events(game_id, response = live_response, game_data = game_info, session = live_response_session, _print = False)
        ...
        shifts_dict = scrape_html_shifts(game_id, roster = roster_df, game_data = game_info, _print = False, session = html_shifts_session, pbp = True)
        ...
        keep_list = ['game_id', 'period', 'game_seconds', 'event_type', 'event_team_tri_code', 'event_player_1_api_name',
                     'coords_x', 'coords_y', 'penalty_severity', 'penalty_minutes', 'datetime', 'event_detail', 'version']\
        ...
        team_type_dict = dict(zip(game_info.team_tri_code, game_info.status))
        
        team_type_dict_rev = dict(zip(game_info.status, game_info.team_tri_code))

        pbp_df['is_home'] = pbp_df.event_team.map(team_type_dict)

        conditions = [pbp_df.is_home == 'home', pbp_df.is_home == 'away']

        values = [1, 0]

        pbp_df.is_home = np.select(conditions, values, pbp_df.is_home)
        ...
        for team in teams_list:
            ...
            for player in player_list:
                ...
                #vector = pd.DataFrame(np.cumsum(np.where(np.isin(player, pbp_df.players_on_api.str.join(', ')), 1, 0) - 
                #                                np.where(np.isin(player, pbp_df.players_on_api.str.join(', ')), 1, 0)))
            ...
            players_on = pd.DataFrame((matrix == 1).apply(lambda x: tuple(matrix.columns[x.tolist()].to_list()), axis = 1))

            on_df = players_on[0].apply(pd.Series)

            on_df[f'{team}_on'] = players_on[0]
        ...
        pbp_df['game_date'] = game_info.game_date.iloc[0]
        
        weird_list = ['PSTR', 'PEND', 'SOC', 'GEND']
        
        conditions = [np.isin(pbp_df.event, CORSI_EVENTS),
                      np.isin(pbp_df.event, ['PSTR', 'PEND', 'SOC', 'GEND']),
                      pbp_df.event == 'PENL',
                      pbp_df.event == 'CHL']
          
        values = [pbp_df.description.str.extract(",\\s*([a-zA-Z|-]+),", expand = False),#.replace("(,|\\s)", '', regex = True), 
                  pbp_df.description.str.extract("([0-9]+:[0-9]+)\\s*([A-Z])+", expand = False),
                  pbp_df.description.str.extract("([0-9]+)\\s([a-z]*)", expand = False),#.replace("([(]|[)]|\\s*)", '', regex = True),
                  pbp_df.description.str.extract("-([a-zA-Z\\s])+-", expand = False)]#.replace("(\\s*-|-\\s*)", '', regex = True)]
        
        #pbp_df.event_detail = np.select(conditions, values, np.nan)
        
        pbp_df.event_detail = np.where(np.isin(pbp_df.event, CORSI_EVENTS),
                                       pbp_df.description.str.extract(",\\s*([a-zA-Z|-]+),", expand = False),
                                       pbp_df.event_detail)
        
        #pbp_df.event_detail = np.where(pbp_df.isin(weird_list), 
        #                               pbp_df.description.str.extract("([0-9]+:[0-9]+)\\s*([A-Z])+", expand = False), 
        #                               pbp_df.event_detail)
        
        #pbp_df.event_detail = np.where(np.equal(pbp_df.event, 'PENL'), 
        #                               pbp_df.description.str.extract("([0-9]+)\\s([a-z]*)", expand = False), 
        #                               pbp_df.event_detail)
        
        #pbp_df.event_detail = np.where(np.equal(pbp_df.event, 'CHL'), 
        #                               pbp_df.description.str.extract("-([a-zA-Z\\s])+-", expand = False), 
        #                               pbp_df.event_detail)
        ...
        for team_type, team_name in team_type_dict_rev.items():
            
            pbp_df[team_type + '_score'] = np.where(np.logical_and(pbp_df.event_team == team_name, pbp_df.event == 'GOAL'), 1, 0)
        ...
        pbp_df['strength_state'] = np.where(pbp_df.is_home == 0,
                                            pbp_df['away_on_num'].astype(str) + 'v' + pbp_df['home_on_num'].astype(str), 
                                            pbp_df['home_on_num'].astype(str) + 'v' + pbp_df['away_on_num'].astype(str))
        ...
        is_penalty_shot = pbp_df['description'].str.lower().str.contains('penalty shot')
        ...
        for x in list(range(1, 7)):
            ...
            pbp_df['zone_start'] = np.where(np.logical_and(near_faceoff, same_time),
                                            pbp_df.event_zone.shift(-x),
                                            pbp_df.zone_start)
    ...
    if concat_list == []:
        ...
        column_names = {'home_team': 'home_team_name', 'away_team': 'away_team_name',
                        'home_team_abbreviated': 'home_team', 'away_team_abbrevaited': 'away_team'}
        
        pbp_df.rename(columns = column_names, inplace = True)
        
        columns = ['season', 'game_date', 'game_id', 'event_idx','period', 'time', 'event_team', 'event', 'description', 
                   'event_zone', 'opp_team', 'event_player_1', 'event_player_2', 'event_player_3', 'coords_x', 'coords_y',
                   'strength_state', 'score_state', 'event_distance', 'event_angle', 'pbp_distance', 'is_home', 'is_goal',
                   'home_skaters', 'home_on', 'home_goalie', 'home_on_1', 'home_on_1_pos', 'home_on_2', 'home_on_2_pos',
                   'home_on_3','home_on_4', 'home_on_4_pos', 'home_on_5', 'home_on_5_pos', 'home_on_6', 'home_on_6_pos',
                   ...
                   'away_on_8', 'away_on_8_pos', 'home_empty_net', 'away_empty_net', 'period_seconds', 'game_seconds',
                   'home_team', 'home_team_name', 'away_team', 'away_team_name', 'strength', 'penalty_severity', 'penalty_minutes', 
                   ...
                   'home_zone', 'event_detail', 'zone_start', 'face_idx', 'shift_idx', 'pen_idx']
...
def scrape_api_rosters(game_ids, response_data = None, session = None, _print = True):
    ...
    for game_id_idx, game_id in enumerate(game_ids):
        ...
        game_info = scrape_api_game_info(game_id, response = response, session = s, _print = False)
