...
class Database:
    ...
    def all(self, table):
        ...
        for row in self.conn.execute(sql).fetchall():
            ...
            for field, value in zip(fields, row):
                # ToDo it could not be used isinstance?
    ...
    def get(self, table, id):
        ...
        if row is None:
            raise Exception(
                f"{table.__name__} instance with id {id} does not exist")
...
class Table:
    ...
    def _get_insert_sql(self):
        ...
        sql = INSERT_SQL.format(name=cls.__name__.lower(), fields=fields,
    ...
    def _get_select_where_sql(cls, id):
        ...
        sql = SELECT_WHERE_SQL.format(name=cls.__name__.lower(),
                                      fields=", ".join(fields))
        params = [id]
