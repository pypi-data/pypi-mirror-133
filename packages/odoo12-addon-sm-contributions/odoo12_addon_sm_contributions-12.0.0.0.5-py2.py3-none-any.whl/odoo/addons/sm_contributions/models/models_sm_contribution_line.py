        ...
        if active_contribution_lines.exists():
          for line in active_contribution_lines:
            ...
            if related_member.id != False:
              # not grouped
              ir_model_data = self.env['ir.model.data'].search([('model','=','account.journal'),('name','=','account_journal_sm_contributions')])
              if not ir_model_data.exists():
              ...
              invoice = self.env['account.invoice'].create({
                ...
                'journal_id': ir_model_data.res_id,
