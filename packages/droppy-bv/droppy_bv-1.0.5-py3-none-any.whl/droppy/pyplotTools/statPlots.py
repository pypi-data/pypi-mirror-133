...
def rpPlot(data, duration, frozenDist=None, ax=None,
    ...
    **kwargs : TYPE
    ...
    if label is not None :
        data_kwargs["label"] = labelFit
        logger.warning("label argument to distPlot is deprecated, use fit_kwargs")
...
def distPlot(data, frozenDist=None, ax=None,
    ...
    if frozenDist is not None:
        if v_range is None :
            v_range = np.linspace( np.min(data), np.max(data) , 300 )
        ax.plot( v_range , frozenDist.sf(v_range) * conv, **fit_kwargs_)
        # ax.plot( frozenDist.isf(prob), prob*conv, **fit_kwargs_)
...
def distPlot_bins(edges, count, frozenDist, fit_kwargs={}, data_kwargs = {} , noData = False, ax = None ) :
...
def distPlot_bins_pdf(edges, count, frozenDist, label=None, labelFit=None, marker="+",
                      noData = False,
                      fit_args = {},
                      data_kwargs = {}, ax = None ):
    ...
    fit_kwargs_ = { "color": "darkorange", "marker" : "" }
    fit_kwargs_.update(data_kwargs)
    ...
    if frozenDist is not None:
        c = np.arange( edges[0] , edges[1] , 200)
        ax.plot( c , frozenDist.pdf(c), **fit_args)
