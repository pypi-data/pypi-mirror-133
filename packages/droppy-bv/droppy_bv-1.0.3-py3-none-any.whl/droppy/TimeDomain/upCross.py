...
from droppy.pyplotTools.statPlots import distPlot, rpPlot
...
class UpCrossAnalysis( pd.DataFrame ):
    ...
    def plotReturnPeriod( self , ax=None, data = "Maximum", duration = None, **kwargs ):
        if duration is None :
            duration = self.attrs["se"].max() - self.attrs["se"].min()
    ...
    def getWithMinSpacing(self , spacing, correct = True) :
        ...
        from droppy.TimeDomain.decluster import minSpacingFilter_array
