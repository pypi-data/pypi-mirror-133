...
@torch.jit.script
def _script_viterbi_decode(emissions: torch.FloatTensor, mask: torch.LongTensor, start_transitions: torch.FloatTensor, end_transitions: torch.FloatTensor, transitions: torch.FloatTensor):
    """Use torch.jit.script wrap this decode

    Args:
        emissions: logits, max_len*batch_size*num_tags
        mask: max_len*batch_size, mask==0 means padding
        start_transitions: `ConditionalRandomField.start_transitions`
        end_transitions: `ConditionalRandomField.end_transitions`
        transitions: `ConditionalRandomField.transitions`

    Returns: 
        The batch best root batch*max_len
    ...
    for i in range(1, seq_length):
        ...
        # shape: (batch_size, num_tags)
        score = torch.where(mask.type(torch.bool)[i].unsqueeze(1), next_score, score)
...
class ConditionalRandomField(Module):
    ...
    def forward(self, logits: torch.FloatTensor, mask: torch.LongTensor):
        ...
        return self._script_viterbi_decode(logits, mask)
    ...
    def _script_viterbi_decode(self, emissions: torch.FloatTensor,
                        mask: torch.LongTensor) -> torch.Tensor:
        """predict step, get the best path
        ...
        assert emissions.size(2) == self.num_tags
        assert mask[0].all()
        return _script_viterbi_decode(emissions, mask, self.start_transitions, self.end_transitions, self.transitions)
