...
#               ###################################################
              ...
              if dirPath==QQ:
                # import os
                # initQQ = os.path.basename(dirPath)
                # # os.environ["HOME"]
                # if initQQ in os.environ.keys():
                #     initData = os.environ[initQQ]      
...
main(["install","'''+nameA+'=='+str(nameB)+'''"])

#### import os
#### os.system("pip install '''+nameA+'''=='''+str(nameB)+''' ")
...
'''
                ...
                # os.system("echo >> "+dirPath+f"{ os.sep }__init__.py") 
                # os.system("echo exec()a>> "+dirPath+f"{ os.sep }__init__.py") 

                open(dirPath+f"{ os.sep }__init__.py","a+").write(text)
          else:
                print("警告: 參數或型別 出現問題") 
...
main(["install","git+https://pypi:nJa4Pym6eSez-Axzg9Qb@gitlab.com/moon-start/'''+nameA+'''/@v12.11#egg='''+nameA+'''" ])
# import os
