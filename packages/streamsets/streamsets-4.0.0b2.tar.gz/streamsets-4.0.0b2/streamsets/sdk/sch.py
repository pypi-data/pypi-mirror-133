...
from .sch_models import (ActionAudits, AdminTool, Alert, ApiCredentialBuilder, ApiCredentials,
                         ...
                         OrganizationBuilder, Organizations, Pipeline,
...
class ControlHub:
    ...
    Args:
        server_url (:obj:`str`): ControlHub server base URL.
    ...
    def get_pipeline_builder(self, engine_type, engine_url, fragment=False):
        """Get a pipeline builder instance with which a pipeline can be created.

        Args:
            engine_type (:obj:`str`): The type of pipeline that will be created. The options are ``'data_collector'`` or
            ``'transformer'``.
            engine_url (:obj:`str`): The URL of the Data Collector or Transformer in which to author the pipeline.
        ...
        if engine_type == 'data_collector':
            authoring_engine = self.data_collectors.get(url=engine_url)
        elif engine_type == 'transformer':
            authoring_engine = self.transformers.get(url=engine_url)
        else:
            raise ValueError("The type of the pipeline must be either 'data_collector' or 'transformer'.")
    ...
    def add_environment(self, environment):
        ...
        if isinstance(environment, AWSEnvironment):
            ...
            fields = ['credentialsType', 'credentials', 'projectId', 'vpcId']
            created_dict = {field: environment._data.get(field, None) for field in fields}
    ...
    def add_deployment(self, deployment):
        ...
        if isinstance(deployment, SelfManagedDeployment):
            ...
            fields = ['blockProjectSshKeys', 'desiredInstances', 'instanceServiceAccountEmail', 'machineType',
                      'tags', 'publicSshKey', 'region', 'trackingUrl', 'resourceLabels', 'zones']
            created_dict = {field: deployment._data.get(field, None) for field in fields}
            fetched_data.update(created_dict)
