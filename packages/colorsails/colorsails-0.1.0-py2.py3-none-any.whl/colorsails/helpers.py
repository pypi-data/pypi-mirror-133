...
from .sails import get_cmap, parse_name_postfix, colorspace
import colorspacious



def apply_sail(data_0, data_1, cmap, vmin=None, vmax = None, vmin_0 = None, vmax_0 = None, vmin_1 = None, vmax_1 = None, clip = None,
    ...
    '''
        ...
        Args:
            ...
            l, rot, J, sat, limit_sat, a, b: arguments passed to sails.get_cmap() when a colormap is called by name, see sails.get_cmap() for explanation
        ...
        Returns:
            ...
            and sail is a colorsails.helpers.Sail object.
    ...
    if clip is None:
        if type(cmap) == type('') and cmap in ['disk','cone','funnel']:
            ...
            clip = 'box'
    sail = Sail(cmap, l=l, rot = rot, J = J, sat = sat, limit_sat = limit_sat, a = a, b = b)
    cmap = sail.cmap
    if vmin_0 is None:
        vmin_0 = vmin
    if vmin_1 is None:
    ...
    if vmin_0 is None:
        sail.vmin_0 = np.min(data_0)
    else:
        sail.vmin_0 = vmin_0
    if vmax_0 is None:
        sail.vmax_0 = np.max(data_0)
    else:
        sail.vmax_0 = vmax_0

    if vmin_1 is None:
        sail.vmin_1 = np.min(data_1)
    else:
        sail.vmin_1 = vmin_1
    if vmax_1 is None:
        sail.vmax_1 = np.max(data_1)
    else:
        sail.vmax_1 = vmax_1

    l = cmap.shape[0]
    if clip == 'box':
        colorax_0 = np.array(l*(data_0-sail.vmin_0)/(sail.vmax_0-sail.vmin_0), dtype = int)
        colorax_1 = np.array(l*(data_1-sail.vmin_1)/(sail.vmax_1-sail.vmin_1), dtype = int)
        ...
        rgb = sail.cmap[colorax_0,colorax_1]

    if clip == 'square':

        # get a and b values of data (y and x coordinates in the cmap)
        # also calculate the phi and rin the cmap space
        a = 2*(data_0-sail.vmin_0)/(sail.vmax_0-sail.vmin_0)-1 # -1 -> 1 for vmin to vmax
        b = 2*(data_1-sail.vmin_1)/(sail.vmax_1-sail.vmin_1)-1 # -1 -> 1 for vmin to vmax
        phi = np.arctan2(a,b)
        ...
        rgb[mask] = sail.cmap[loc_a,loc_b]
        ...
        a_img = (data_0-sail.vmin_0)/(sail.vmax_0-sail.vmin_0) # 0 -> 1 for vmin to vmax
        b_img = (data_1-sail.vmin_1)/(sail.vmax_1-sail.vmin_1) # 0 -> 1 for vmin to vmax
        ...
        colorax_0 = np.array(0.999999999*l*(data_0-sail.vmin_0)/(sail.vmax_0-sail.vmin_0), dtype = int)
        colorax_1 = np.array(0.999999999*l*(data_1-sail.vmin_1)/(sail.vmax_1-sail.vmin_1), dtype = int)
...
class Sail:
    ...
    In normal use class objects of this class will be generated by apply_sail(), and not generated.
    May be called directly to inspect colormaps using the member funciton Sail.eval() without having any data, as needed for apply_sail().
    Member variables self.vmin_0, self.vmin_1, self.vmax_0, self.vmax_1 are set when invoked by apply_sail() and determine the limts.

    Args:
        cmap:
            numpy array of shape (l,l,3) with rgb values
            or string for a valid colormap available with sails.get_cmap()

        l, rot, J, sat, limit_sat, a, b: passed to sails.get_cmap() if cmap is a string, otherwise ignored
