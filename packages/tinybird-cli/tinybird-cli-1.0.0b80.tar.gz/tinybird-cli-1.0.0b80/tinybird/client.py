...
def parse_error_response(response):
    try:
        ...
        return response['error'] if 'error' in response else json.dumps(response, indent=4)
    except json.decoder.JSONDecodeError:
        return 'Server error, cannot parse response'
...
class TinyB(object):
    ...
    async def datasource_create_from_url(self, table_name, url, mode='create', status_callback=None, sql_condition=None, format='csv'):
        params = {
            'name': table_name,
            'url': url,
            'mode': mode,
    ...
    async def datasource_append_data(self, datasource_name, f, mode='append', status_callback=None, sql_condition=None, format='csv'):
        params = {
