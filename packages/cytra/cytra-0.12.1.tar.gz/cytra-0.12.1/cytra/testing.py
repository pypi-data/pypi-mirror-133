...
class TestingApp:
    ...
    def __enter__(self):
        if self.app.config.get("sqlalchemy"):
        ...
        metadata.drop_all(bind=self.db.get_bind())
        metadata.create_all(bind=self.db.get_bind())
        return self
