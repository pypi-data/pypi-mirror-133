...
class MyGame(arcade.Window):
    ...
    def setup(self):
        ...
        # Sprite lists
        self.coin_list1 = arcade.SpriteList(use_spatial_hash=USE_SPATIAL_HASHING)
        self.coin_list2 = arcade.SpriteList(use_spatial_hash=USE_SPATIAL_HASHING)
    ...
    def on_draw(self):
        ...
        output = f"Sprite count: {len(self.coin_list1)+len(self.coin_list2):,}"
    ...
    def update(self, delta_time):
        ...
        coin_hit_list1 = arcade.check_for_collision_with_list(self.player, self.coin_list1, method=1)
        coin_hit_list2 = arcade.check_for_collision_with_list(self.player, self.coin_list1, method=2)
        coin_hit_list3 = arcade.check_for_collision_with_list(self.player, self.coin_list1, method=3)
        if len(coin_hit_list1) != len(coin_hit_list2) != len(coin_hit_list3):
            print(f"1 => {len(coin_hit_list1)=}, {len(coin_hit_list2)=}, {len(coin_hit_list3)}=")
        for coin in coin_hit_list1:
            coin.center_x = random.randrange(SCREEN_WIDTH)
            coin.center_y = random.randrange(SCREEN_HEIGHT)

        coin_hit_list1 = arcade.check_for_collision_with_list(self.player, self.coin_list2, method=1)
        coin_hit_list2 = arcade.check_for_collision_with_list(self.player, self.coin_list2, method=2)
        coin_hit_list3 = arcade.check_for_collision_with_list(self.player, self.coin_list2, method=3)
        if len(coin_hit_list1) != len(coin_hit_list2) != len(coin_hit_list3):
            print(f"2 => {len(coin_hit_list1)=}, {len(coin_hit_list2)=}, {len(coin_hit_list3)}=")
        for coin in coin_hit_list1:
            coin.center_x = random.randrange(SCREEN_WIDTH)
            coin.center_y = random.randrange(SCREEN_HEIGHT)
        ...
        if total_program_time > self.last_fps_reading:
            ...
            if total_program_time > 5:
                ...
                if total_program_time % 2 == 1:
                    ...
                    output = f"{total_program_time}, {len(self.coin_list1) + len(self.coin_list2)}, {self.fps.get_fps():.1f}, " \
                             f"{self.processing_time:.4f}, {self.draw_time:.4f}\n"
                    print(output, end="")
                    self.results_file.write(output)

                    if len(self.coin_list1) + len(self.coin_list2) >= STOP_COUNT:
                        self.results_file.close()
                        pyglet.app.exit()
                        return

                    # Take timings
                    print(f"{total_program_time}, {len(self.coin_list1) + len(self.coin_list2)}, {self.fps.get_fps():.1f}, "
                          f"{self.processing_time:.4f}, {self.draw_time:.4f}")
                    self.sprite_count_list.append(len(self.coin_list1) + len(self.coin_list2))
