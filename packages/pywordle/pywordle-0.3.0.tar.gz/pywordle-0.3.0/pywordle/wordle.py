...
def get_words_txt() -> Generator[str, None, None]:
    """
    Read the words from the words.en.txt file.
    It is now a ressource provided by the package.
    """
    wordsfilename = f"words.{'large' if ARGS.large else 'small'}.en.txt"
...
def save_words(words: Dict[str, Optional[int]]) -> None:
    ...
    with open(join(WORDS_DIR, WORDS_FOUND_FILE), mode="w", encoding="utf_8") as jsonfile:
        dump(words, jsonfile, indent=4)


def get_words() -> Dict[str, Optional[int]]:
    """
    ...
    try:
        with open(join(WORDS_DIR, WORDS_FOUND_FILE), mode="r", encoding="utf_8") as jsonfile:
            words: Dict[str, Optional[int]] = loads(jsonfile.read())
    except FileNotFoundError:
        # If not found we initialize it with the content of the text file
        words = {word: None for word in get_words_txt() if len(word) == 5}
        save_words(words)
...
def main() -> None:
    ...
    if ARGS.minfrequency:
        ...
        selected = filterwords(selected, lambda word, frequency: bool(frequency >= ARGS.minfrequency))
