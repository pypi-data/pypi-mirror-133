...
class ImplCompute(ImplBase):
    ...
    def get_runs_by_box(self, run_names, workspace=None):
        ...
        mongo = self.store.get_database()
        fields_dict = {"box_name": 1, "compute": 1, "ws_name": 1, "run_name": 1}
    ...
    def cancel_run(self, run_names, workspace):
        ...
        for box_name, runs in runs_by_box.items():
            ...
            ws_name = workspace    # first_run["ws_name"]
    ...
    @command(kwgroup="cancel", kwhelp="cancels active runs, specified by name, job, or target", help="cancels the specified jobs and their active or queued runs")
    ...
    @option("node-repeat", type=int, help="the number of times to repeat each hyperparameter set on each node")
