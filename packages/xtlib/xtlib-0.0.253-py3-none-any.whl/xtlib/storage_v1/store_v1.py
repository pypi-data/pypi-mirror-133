...
from xtlib.storage_v1 import mongo_v1
...
from .store_objects_v1 import StoreBlobObjs
...
def store_from_context(context):
    store = StoreV1(context.store_creds, provider_code_path=context.store_code_path, mongo_conn_str=context.mongo_conn_str)
...
class StoreV1():
    ...
    def __init__(self, storage_creds=None, config=None, max_retries=25, run_cache_dir=None, feedback_enabled=True,  db_creds=None, 
        ...
        if db_creds:
            mongo_conn_str = db_creds["connection-string"]
        ...
        self.database = mongo_v1.MongoDB(mongo_conn_str, run_cache_dir) if mongo_conn_str else None
        self.db_conn_str = mongo_conn_str
        self.run_cache_dir = run_cache_dir
        self.provider_code_path = provider_code_path
        self.storage_creds = storage_creds
        self.format = constants.STORAGE_FORMAT_V1
        ...
        self.helper.validate_storage_and_mongo(self.database)
    ...
    def delete_workspace(self, ws_name):
        ...
        if result:
            ...
            self.database.delete_workspace_if_needed(ws_name)
    ...
    def get_workspace_names(self, use_mongo=False):
        ''' return the names of all workspaces that are currently defined in the XT Store.
        '''
        if use_mongo:
            # use mongo for more faster workspace names 
    ...
    def start_child_run(self, ws_name, parent_run_name, exper_name=None, cmd_line_args=None, all_args=None, description=None, 
        ...
        run_num = run_helper.get_run_number(child_name)
        ...
        self.database.create_db_run(dd)
    ...
    def start_run_core(self, ws_name, run_name, exper_name=None, description=None, username=None,
            ...
            sku=None, search_style=None, tag_dict=None):
    ...
    def rollup_and_end_run(self, ws_name, run_name, aggregate_dest, dest_name, status, exit_code, primary_metric, maximize_metric, 
        report_rollup, use_last_end_time=False, job_id=None):
    ...
    def log_run_event(self, ws_name, run_name, event_name, data_dict=None, event_time=None, job_id=None):
        ...
        self.database.process_run_event(ws_name, run_name, record_dict)
    ...
    def get_ws_runs(self, ws_name, filter_dict=None, include_log_records=False):
        return self.database.get_ws_runs(ws_name, filter_dict, include_log_records)
    ...
    def get_all_runs(self, aggregator_dest, ws_name, job_or_exper_name, filter_dict=None, fields_dict=None, use_cache=False, 
        ...
        '''
        returns a list of all runs in the specified workspace.
        '''
    ...
    def download_all_runs_blobs(self, ws_name, aggregator_dest, job_or_exper_name, dest_fn, 
        ...
        if aggregator_dest == "job":
            self.download_file_from_job(ws_name, job_or_exper_name, constants.ALL_RUNS_FN, dest_fn)
    ...
    def wrapup_run(self, ws_name, run_name, aggregate_dest, dest_name, status, exit_code, primary_metric, maximize_metric, 
        ...
        self.database.run_exit(ws_name, run_name, status=None, exit_code=None, start_time=start_time)
    ...
    def update_run_at_end(self, ws_name, run_name, status, exit_code, end_time, log_records, hparams, metrics):
        return self.database.update_run_at_end(ws_name, run_name, status, exit_code, end_time, log_records, hparams, metrics)
    ...
    def upload_file_to_run(self, ws_name, run_name, run_fn, source_fn, job_id=None):
    ...
    def download_file_from_run(self, ws_name, run_name, run_fn, dest_fn, job_id=None):
        ...
        '''
        dest_fn = os.path.abspath(dest_fn)      # ensure it has a directory specified
        print("download_file_from_run: dest_fn=", dest_fn)
        rf = self.run_files(ws_name, run_name, use_blobs=True)
        return rf.download_file(run_fn, dest_fn)

    def download_files_from_run(self, ws_name, run_name, run_wildcard, dest_folder, job_id=None):
    ...
    def delete_run_subfolder_and_files(self, ws_name, run_name, subfolder_name):
        '''delete all files under the specified folder_name.
        '''
        rf = self.run_files(ws_name, run_name, use_blobs=True)
        return rf.delete_files(subfolder_name + "/**")
    ...
    #---- JOBS ----
    # all JOB APIs should include "ws_name", for compatibility with V2 Store API.  The
    # "ws_name" is not used by the V1 Store.
    ...
    def create_job(self, ws_name):
        ...
        job_id = self.database.get_next_job_id(default_next=default_next)

        job_name = "job{}".format(job_id)
        return job_name

    def get_job_secret(self, ws_name, job_id):
        records = self.database.get_info_for_jobs( filter_dict={"_id": job_id}, fields_dict={"job_secret": 1} )
    ...
    def does_job_exist(self, ws_name, job_name):
        '''return True if the job exists.
        '''
        #return self.helper.does_job_file_exist(job_name, job_path)
        jf = self.job_files(ws_name, job_name, use_blobs=True)
        return jf.does_file_exist(constants.JOB_LOG)
...
def build_store_utils_v1():
    # update PROPERTIES of store_utils for version 1 values
    store_utils.DATA_STORE_ROOT = make_share_name("data")
    store_utils.MODELS_STORE_ROOT = make_share_name("models")
    store_utils.STORAGE_FORMAT = constants.STORAGE_FORMAT_V1
...
def is_share_name(name):
    return isinstance(name, str) and name.startswith("xts-")

def get_run_path(job_id, run_name):
    path = constants.RUNS_DIR + "/" + run_name
    return path

def get_jobs_container(ws_name):
    return constants.INFO_CONTAINER_V1    

def make_id(ws_name, obj_name):
    if "/" in obj_name:
        id = obj_name.split("/", 1)[1]
    else:
        id = obj_name
    return id
