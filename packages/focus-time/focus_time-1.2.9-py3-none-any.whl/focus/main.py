...
# -----DAY FUNCTIONS-------------------------------------------------------------------------------
    ...
    def report_hours_of_focus_remaining(self):
        ...
        self.session_num += 1
        print(f"\n   You only have {np.round(self.hours_of_focus_left*(1 + 59*less_than_two_hours_left),1)} {'minutes'*less_than_two_hours_left + 'hours'*(not less_than_two_hours_left)} to go. When you're ready, let's start session number {self.session_num}!\n")
