...
class BOS():
    ...
    def put_Object(self, file_name, object_key):
        ...
        try:
            ...
            response = self.bos_client.put_object_from_file(
                self.bucket_name, object_key, file_name)
    ...
    def delete_Object(self, object_key):
        ...
        try:
            if isinstance(object_key, str):
                self.bos_client.delete_object(self.bucket_name,
                                              object_key)  # 删除单个文件
            elif isinstance(object_key, list):
                self.bos_client.delete_multiple_objects(
                    self.bucket_name, object_key)  # 批量删除列表内的文件
            return 200
        except Exception as err:
            print(err)  # 异常处理
    ...
    def head_Object(self, object_key):
        ...
        """
        try:
            response = self.bos_client.get_object_meta_data(
    ...
    def get_Object(self, object_key, save_dir):
        ...
        Returns:
            [int]: [200-成功;!=200 失败]
        """
        file_name = save_dir + os.sep + object_key.replace(
            '/', os.sep)  # 下载文件本地绝对路径拼接
        os.makedirs(os.path.dirname(file_name), exist_ok=True)  # 创建文件目录
        try:
            response = self.bos_client.get_object_to_file(
                self.bucket_name, object_key, file_name)
            if response.metadata.content_md5 == md5file(file_name).decode(
                    'utf-8'):
                return 200
            else:
                return -1
