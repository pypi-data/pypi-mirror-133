    ...
    def defined_opcodes(self) -> List[Opcode]:
        ...
        for plugin in self.plugins.values():
            ...
            for opcodename in plugin.opcodes:
                ...
                opcodes.append(Opcode(name=opcodename, plugin=plugin.name, abstract=manpage.abstract,
                                      syntaxes=manpage.syntaxes, installed=pluginstalled))
        opcodes.sort(key=lambda opc: opc.name.lower())
        self._cache['opcodes'] = opcodes
        return opcodes

    def serialize(self, outfile: str = _MAININDEX_PICKLE_FILE):
        import pickle
        db = self.opcodes_by_name()
...
def cmd_info(idx: MainIndex, args) -> bool:
    ...
    d = {
        ...
        'installed-plugins': [plugin for plugin in idx.plugins.values()
                              if idx.is_plugin_installed()]
    }
