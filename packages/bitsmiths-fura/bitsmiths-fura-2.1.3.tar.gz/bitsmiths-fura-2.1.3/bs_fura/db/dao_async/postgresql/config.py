...
from bs_fura.db.tables.config_key import tConfigKey
...
class dConfig:
    ...
    async def lock_one_with(self, _rec: tConfig, _lock4update: mettle.db.DBLock, _expected: bool = True) -> bool:
        """
    ...
    async def lock_one_by_key(self, _key: tConfigKey, _lock4update: mettle.db.DBLock, _expected: bool = True) -> bool:
        """
        ...
        :return: True if selected, False if not found and _expected is set to False
        """
        with dConfigLockOne(self._dbcon) as _qry:
            ...
            while _dex >= 0:
                try:
                    ...
                    if not await _qry.fetch():
                        ...
                        raise mettle.lib.xMettle("LockOne failed to return a record (dConfig)")
                    ...
                    if _dex == 0:
                        raise mettle.lib.xMettle("Could not lock Config record after %d attempt(s) at %d mili second intervals!" % (
_lock4update.retrys, _lock4update.mili_seconds), errCode = mettle.lib.xMettle.eCode.DBLockNoWaitFailed)
    ...
    async def try_select_one_with(self, _rec: tConfig) -> bool:
        """
    ...
    async def try_select_one_by_key(self, _key: tConfigKey) -> bool:
        """
        ...
        """
        with dConfigSelectOne(self._dbcon) as _qry:
            await _qry.exec(_key)
            if not await _qry.fetch():
                return False
    ...
    async def select_one(self) -> "dConfig":
        """
        ...
        if not await self.try_select_one():
            raise mettle.lib.xMettle("SelectOne failed to return a record (dConfig)")

        return self

    async def select_one_with(self, _rec: tConfig) -> "dConfig":
        """
    ...
    async def select_one_by_key(self, _key: tConfigKey) -> "dConfig":
        """
        Attempt to select the record with it's primary key.
    ...
    async def update(self, irec: tConfig = None):
        """
        ...
        with dConfigUpdate(self._dbcon) as _qry:
    ...
    async def insert(self, irec: tConfig = None):
        """
        ...
        with dConfigInsert(self._dbcon) as _qry:
            await _qry.exec(self.rec)
    ...
    async def delete_one(self, irec: tConfigKey = None):
        """
        ...
        with dConfigDeleteOne(self._dbcon) as _qry:
            await _qry.exec(_key)
