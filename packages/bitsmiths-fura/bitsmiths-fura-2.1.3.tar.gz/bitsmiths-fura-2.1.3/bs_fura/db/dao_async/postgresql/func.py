...
from bs_fura.db.tables.func_key import tFuncKey
...
class dFunc:
    ...
    async def lock_one_with(self, _rec: tFunc, _lock4update: mettle.db.DBLock, _expected: bool = True) -> bool:
        """
    ...
    async def lock_one_by_key(self, _key: tFuncKey, _lock4update: mettle.db.DBLock, _expected: bool = True) -> bool:
        """
        ...
        :return: True if selected, False if not found and _expected is set to False
        """
        with dFuncLockOne(self._dbcon) as _qry:
            ...
            while _dex >= 0:
                try:
                    ...
                    if not await _qry.fetch():
                        ...
                        raise mettle.lib.xMettle("LockOne failed to return a record (dFunc)")
                    ...
                    if _dex == 0:
                        raise mettle.lib.xMettle("Could not lock Func record after %d attempt(s) at %d mili second intervals!" % (
_lock4update.retrys, _lock4update.mili_seconds), errCode = mettle.lib.xMettle.eCode.DBLockNoWaitFailed)
    ...
    async def try_select_one_with(self, _rec: tFunc) -> bool:
        """
    ...
    async def try_select_one_by_key(self, _key: tFuncKey) -> bool:
        """
        ...
        """
        with dFuncSelectOne(self._dbcon) as _qry:
            await _qry.exec(_key)
            if not await _qry.fetch():
                return False
    ...
    async def select_one(self) -> "dFunc":
        """
        ...
        if not await self.try_select_one():
            raise mettle.lib.xMettle("SelectOne failed to return a record (dFunc)")

        return self

    async def select_one_with(self, _rec: tFunc) -> "dFunc":
        """
    ...
    async def select_one_by_key(self, _key: tFuncKey) -> "dFunc":
        """
        Attempt to select the record with it's primary key.
    ...
    async def update_deft(self,
        ...
        """
        self.rec.id = id
        self.rec.funcgrp_id = funcgrp_id
        self.rec.descr = descr
        self.rec.action = action
        self.rec.oride = oride
        self.rec.audit_lvl = audit_lvl
        self.rec.modified_by = modified_by

        await self.update()

    async def update(self, irec: tFunc = None):
        """
        ...
        with dFuncUpdate(self._dbcon) as _qry:
    ...
    async def insert(self, irec: tFunc = None):
        """
        ...
        with dFuncInsert(self._dbcon) as _qry:
            await _qry.exec(self.rec)
    ...
    async def delete_one(self, irec: tFuncKey = None):
        """
        ...
        with dFuncDeleteOne(self._dbcon) as _qry:
            await _qry.exec(_key)
