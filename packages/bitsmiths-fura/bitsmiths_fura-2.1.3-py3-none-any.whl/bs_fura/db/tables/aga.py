...
class tAga(mettle.io.ISerializable):

    def __init__(self,
                 site_id: int = 0,
                 site_code: str = '',
                 tm_stamp: datetime.datetime = None,
                 usr_id: str = '',
                 eausr_id: str = '',
                 role_id: str = '',
                 func_id: str = '',
    ...
    def _get_davs(self, dvc=None, for_ins: bool = False, key_only: bool = False):
        tAga._cache_davs(dvc, for_ins, key_only)
    ...
    def __repr__(self) -> str:
        return '<Aga [site_id:%r, site_code:%.32s, tm_stamp:%s, usr_id:%.32s, eausr_id:%.32s, role_id:%.32s, func_id:%.32s, action:%.32s, src:%.32s]>' % (
            self.site_id, self.site_code, self.tm_stamp, self.usr_id, self.eausr_id, self.role_id, self.func_id, self.action, self.src)
    ...
    class List(list, mettle.io.ISerializable):
        ...
        def __repr__(self) -> str:
            return '<tAga.List[%d]>' % (len(self))

        def _get_davs(self, dvc=None, for_ins: bool = False, key_only: bool = False):
            return tAga._cache_davs(dvc, for_ins, key_only)
