...
class tSiteCfg(mettle.io.ISerializable, mettle.db.IPrimaryKey):
    ...
    def _copy_from(self, rec: "tSiteCfg"):
        """
    ...
    def _write_pk(self, _key: "tSiteCfg"):
        """
    ...
    def _read_pk(self, _key: "tSiteCfg"):
        """
    ...
    def _cache_davs(dvc=None, for_ins: bool = False, key_only: bool = False):
        ...
        dvc.add_targ("value", mettle.lib.Dav(mettle.lib.Dav.eDavType.Max, 4096))
        dvc.add_targ("modified_by", mettle.lib.Dav(mettle.lib.Dav.eDavType.NotNull))
        dvc.add_targ("modified_by", mettle.lib.Dav(mettle.lib.Dav.eDavType.Max, 64))

        return dvc

    def _get_davs(self, dvc=None, for_ins: bool = False, key_only: bool = False):
        tSiteCfg._cache_davs(dvc, for_ins, key_only)

    def _write_key(self, _key: tSiteCfgKey):
        """
    ...
    def _read_key(self, _key: tSiteCfgKey):
        """
    ...
    def __repr__(self) -> str:
        return '<SiteCfg [site_id:%r, id:%.32s, descr:%.32s, value:%.32s, modified_by:%.32s, tm_stamp:%s]>' % (
            self.site_id, self.id, self.descr, self.value, self.modified_by, self.tm_stamp)
    ...
    class List(list, mettle.io.ISerializable):
        ...
        def __repr__(self) -> str:
            return '<tSiteCfg.List[%d]>' % (len(self))

        def _get_davs(self, dvc=None, for_ins: bool = False, key_only: bool = False):
            return tSiteCfg._cache_davs(dvc, for_ins, key_only)
