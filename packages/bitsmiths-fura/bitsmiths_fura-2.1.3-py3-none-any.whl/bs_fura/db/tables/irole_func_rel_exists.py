...
class iRoleFuncRelExists(mettle.io.ISerializable):
    ...
    def _copy_from(self, rec: "iRoleFuncRelExists"):
        """
    ...
    def _cache_davs(dvc=None):
        ...
        dvc.add_targ("funcId", mettle.lib.Dav(mettle.lib.Dav.eDavType.NotNull))
        dvc.add_targ("funcId", mettle.lib.Dav(mettle.lib.Dav.eDavType.Max, 128))
        dvc.add_targ("roleId", mettle.lib.Dav(mettle.lib.Dav.eDavType.NotNull))
        dvc.add_targ("roleId", mettle.lib.Dav(mettle.lib.Dav.eDavType.Max, 128))

        return dvc

    def _get_davs(self, dvc=None):
        iRoleFuncRelExists._cache_davs(dvc)

    def __repr__(self) -> str:
        return '<RoleFuncRel [site_id:%r, func_id:%.32s, role_id:%.32s]>' % (
            self.site_id, self.func_id, self.role_id)
    ...
    class List(list, mettle.io.ISerializable):
        ...
        def _name(self) -> str:
            ...
            :return: The name
            """
            return 'iRoleFuncRelExists.List'
        ...
        def __repr__(self) -> str:
            return '<iRoleFuncRelExists.List[%d]>' % (len(self))

        def _get_davs(self, dvc=None):
            return iRoleFuncRelExists._cache_davs(dvc)
