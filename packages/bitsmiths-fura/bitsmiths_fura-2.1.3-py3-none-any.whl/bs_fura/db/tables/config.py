...
class tConfig(mettle.io.ISerializable, mettle.db.IPrimaryKey):
    ...
    def _copy_from(self, rec: "tConfig"):
        """
    ...
    def _write_pk(self, _key: "tConfig"):
        """
    ...
    def _read_pk(self, _key: "tConfig"):
        """
    ...
    def _get_davs(self, dvc=None, for_ins: bool = False, key_only: bool = False):
        tConfig._cache_davs(dvc, for_ins, key_only)

    def _write_key(self, _key: tConfigKey):
        """
    ...
    def _read_key(self, _key: tConfigKey):
        """
    ...
    def __repr__(self) -> str:
        return '<Config [id:%.32s, descr:%.32s, value:%.32s, modified_by:%.32s, tm_stamp:%s]>' % (
            self.id, self.descr, self.value, self.modified_by, self.tm_stamp)
    ...
    class List(list, mettle.io.ISerializable):
        ...
        def __repr__(self) -> str:
            return '<tConfig.List[%d]>' % (len(self))

        def _get_davs(self, dvc=None, for_ins: bool = False, key_only: bool = False):
            return tConfig._cache_davs(dvc, for_ins, key_only)
