...
class CKStore(RecordStoreWithInitialization):
    ...
    def _on_assign(consumer: Consumer, partitions: list[TopicPartition]) -> None:
        partitions_str = CKStore._partitions_str(partitions)
        ...
        with self.initialization_lock:
            ...
            if consumer_topics:
                self.consumer.subscribe(
                    consumer_topics,
                    on_assign=CKStore._on_assign,
                    on_revoke=CKStore._on_revoke,
                    on_lost=CKStore._on_lost,
                )
    ...
    def read(
        ...
        try:
            partitions_str = CKStore._partitions_str(self.consumer.assignment())
            ...
            raise MessagingError(f"Error consuming messages from kafka.") from e

        for message in messages:
            if message.error():
                logger.error(
                    f"Message has error: {message.error()}. "
                    f"Message was {message.topic()}[{message.partition()}]:{message.offset()}."
                )
                continue
            logger.debug(f"New record read: {message.topic()}[{message.partition()}]:{message.offset()}")
            try:
                ...
                raise MessagingError(f"Error decoding message {message.topic()}:{message.offset()}.") from e
            logger.info(f"New record {record.qualified_name} from kafka message {message.topic()}:{message.offset()}.")
            yield record
            if auto_ack:
                try:
                    self.consumer.commit(message=message, asynchronous=False)
                except Exception as e:
                    raise MessagingError(f"Error committing message {message.topic()}:{message.offset()}.") from e
    ...
    def ack(self, timeout_ms=None) -> None:
        ...
        if self.in_transaction:
            ...
            try:
                ...
                raise MessagingError(f"Error sending offsets to transaction.") from e
        else:
