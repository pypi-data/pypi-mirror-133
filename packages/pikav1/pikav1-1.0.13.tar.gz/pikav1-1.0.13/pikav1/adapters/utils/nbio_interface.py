"""Non-blocking I/O interface for pikav1 connection adapters.

I/O interface expected by `pikav1.adapters.base_connection.BaseConnection`
...
Furthermore, the API caters to the needs of pikav1 core and lack of generalization
...
class AbstractIOServices(pikav1.compat.AbstractBase):
    """Interface to I/O services required by `pikav1.adapters.BaseConnection` and
    related utilities.
...
class AbstractFileDescriptorServices(pikav1.compat.AbstractBase):
...
class AbstractTimerReference(pikav1.compat.AbstractBase):
...
class AbstractIOReference(pikav1.compat.AbstractBase):
...
class AbstractStreamProtocol(pikav1.compat.AbstractBase):
...
class AbstractStreamTransport(pikav1.compat.AbstractBase):
