...
import pikav1.tcp_socket_opts
from pikav1 import __version__
...
class AMQPConnector(object):
    ...
    def __init__(self, conn_factory, nbio):
        ...
        :param callable conn_factory: A function that takes
            `pikav1.connection.Parameters` as its only arg and returns a brand new
            `pikav1.connection.Connection`-based adapter instance each time it is
            called. The factory must instantiate the connection with
            `internal_connection_workflow=False`.
        :param pikav1.adapters.utils.nbio_interface.AbstractIOServices nbio:

        """
        self._conn_factory = conn_factory
        self._nbio = nbio
        self._addr_record = None  # type: tuple
        self._conn_params = None  # type: pikav1.connection.Parameters
        self._on_done = None  # will be provided via start()
        # TCP connection timeout
        # pylint: disable=C0301
        self._tcp_timeout_ref = None  # type: pikav1.adapters.utils.nbio_interface.AbstractTimerReference
        # Overall TCP/[SSL]/AMQP timeout
        self._stack_timeout_ref = None  # type: pikav1.adapters.utils.nbio_interface.AbstractTimerReference
        # Current task
        self._task_ref = None  # type: pikav1.adapters.utils.nbio_interface.AbstractIOReference
        self._sock = None  # type: socket.socket
        self._amqp_conn = None  # type: pikav1.connection.Connection
    ...
    def start(self, addr_record, conn_params, on_done):
        ...
        :param pikav1.connection.Parameters conn_params:
        :param callable on_done: Function to call upon completion of the
            workflow: `on_done(pikav1.connection.Connection | BaseException)`. If
        ...
        self._sock.setsockopt(pikav1.compat.SOL_TCP, socket.TCP_NODELAY, 1)
        pikav1.tcp_socket_opts.set_sock_opts(self._conn_params.tcp_options,
                                             self._sock)
    ...
    def abort(self):
        ...
        NOTE: we can't cancel/close synchronously because aborting pikav1
    ...
    def _report_completion_and_cleanup(self, result):
        ...
        :param pikav1.connection.Connection | BaseException result: value to pass
    ...
    def _on_amqp_handshake_done(self, connection, error=None):
        ...
        :param pikav1.connection.Connection connection:
...
class AbstractAMQPConnectionWorkflow(pikav1.compat.AbstractBase):
    ...
    def start(self, connection_configs, connector_factory, native_loop,
        ...
        :param sequence connection_configs: A sequence of one or more
            `pikav1.connection.Parameters`-based objects. Will attempt to connect
        ...
        :param callable on_done: Function to call upon completion of the
            workflow:
            `on_done(pikav1.connection.Connection |
...
class AMQPConnectionWorkflow(AbstractAMQPConnectionWorkflow):
    ...
    The workflow:
        while not success and retries remain:
            1. For each given config (pikav1.connection.Parameters object):
    ...
    def __init__(self, _until_first_amqp_attempt=False):
        ...
        :param bool _until_first_amqp_attempt: INTERNAL USE ONLY; ends workflow
            ...
            `pikav1.connection.Connection` enables this because it's not
        ...
        # pylint: disable=C0301
        self._nbio = None  # type: pikav1.adapters.utils.nbio_interface.AbstractIOServices
    ...
    def set_io_services(self, nbio):
        """Called by the conneciton adapter only on pikav1's
    ...
    def _report_completion_and_cleanup(self, result):
        ...
        :param pikav1.connection.Connection | AMQPConnectionWorkflowFailed result:
    ...
    def _on_connector_done(self, conn_or_exc):
        ...
        :param pikav1.connection.Connection | BaseException conn_or_exc: See
        ...
        if isinstance(conn_or_exc, BaseException):
            ...
            if isinstance(conn_or_exc, AMQPConnectorAborted):
                ...
                if isinstance(conn_or_exc.exception,
                              pikav1.exceptions.ConnectionOpenAborted):
