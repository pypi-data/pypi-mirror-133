...
from pikav1.adapters.utils.nbio_interface import (AbstractIOReference,
                                                  AbstractStreamTransport)
import pikav1.compat
import pikav1.diagnostic_utils
...
_log_exceptions = pikav1.diagnostic_utils.create_log_exception_decorator(_LOGGER)  # pylint: disable=C0103
...
def _retry_on_sigint(func):
    ...
    def retry_sigint_wrap(*args, **kwargs):
        ...
        while True:
            ...
            except pikav1.compat.SOCKET_ERROR as error:
...
class SocketConnectionMixin(object):
    """Implements
    `pikav1.adapters.utils.nbio_interface.AbstractIOServices.connect_socket()`
    on top of
    `pikav1.adapters.utils.nbio_interface.AbstractFileDescriptorServices` and
    basic `pikav1.adapters.utils.nbio_interface.AbstractIOServices`.
...
class _AsyncSocketConnector(object):
    ...
    def _start_async(self):
        ...
        except (Exception, pikav1.compat.SOCKET_ERROR) as error:  # pylint: disable=W0703
            if (isinstance(error, pikav1.compat.SOCKET_ERROR) and
    ...
    def _on_writable(self):
        ...
        if not error_code:
            ...
            result = pikav1.compat.SOCKET_ERROR(error_code, error_msg)
...
class _AsyncTransportBase(  # pylint: disable=W0223
    ...
    def __init__(self, sock, protocol, nbio):
        ...
        :param pikav1.adapters.utils.nbio_interface.AbstractStreamProtocol protocol:
    ...
    def get_protocol(self):
        ...
        :rtype: pikav1.adapters.utils.nbio_interface.AbstractStreamProtocol
        """
