...
:py:class:`~pikav1.adapters.blocking_connection.BlockingConnection`
and the :class:`~pikav1.adapters.blocking_connection.BlockingChannel`
classes.
...
class _ConnectionBlockedUnblockedEvtBase(object):
    ...
    def __init__(self, callback, method_frame):
        ...
        :param pikav1.frame.Method method_frame: with method_frame.method of type
          `pikav1.spec.Connection.Blocked` or `pikav1.spec.Connection.Unblocked`
        """
        self._callback = callback
...
class BlockingConnection(object):
    ...
    :class:`Channel <pikav1.channel.Channel>` based communication for the
    ...
    exception`. See `pikav1.connection.ConnectionParameters` documentation to
    ...
    def __init__(self, parameters=None, _impl_class=None):
        ...
        :param None | pikav1.connection.Parameters | sequence parameters:
    ...
    def _create_connection(self, configs, impl_class):
        ...
        :param None | pikav1.connection.Parameters | sequence configs: Connection
        ...
        if configs is None:
            configs = (pikav1.connection.Parameters(),)

        if isinstance(configs, pikav1.connection.Parameters):
            configs = (configs,)
    ...
    def _on_connection_blocked(self, user_callback, _impl, method_frame):
        ...
        :param pikav1.frame.Method method_frame: method frame having `method`
            member of type `pikav1.spec.Connection.Blocked`
        """
    ...
    def add_on_connection_blocked_callback(self, callback):
        ...
        :param callable callback: Callback to call on `Connection.Blocked`,
            ...
            frame's `method` member is of type `pikav1.spec.Connection.Blocked`
    ...
    def add_on_connection_unblocked_callback(self, callback):
        ...
        :param callable callback: Callback to call on Connection.Unblocked`,
            having the signature `callback(connection, pikav1.frame.Method)`,
            where connection is the `BlockingConnection` instance and the method
             frame's `method` member is of type `pikav1.spec.Connection.Unblocked`
    ...
    def close(self, reply_code=200, reply_text='Normal shutdown'):
        ...
        :raises pikav1.exceptions.ConnectionWrongStateError: if called on a closed
    ...
    def process_data_events(self, time_limit=0):
        ...
        :param float time_limit: suggested upper bound on processing time in
            ...
            compatibility. This parameter is NEW in pikav1 0.10.0.
        """
        with self._acquire_event_dispatch() as dispatch_acquired:
    ...
    def channel(self, channel_number=None):
        ...
        :rtype: pikav1.adapters.blocking_connection.BlockingChannel
        """
...
class _ConsumerCancellationEvt(_ChannelPendingEvt):
    ...
    def __init__(self, method_frame):
        """
        :param pikav1.frame.Method method_frame: method frame with method of type
            `spec.Basic.Cancel`
        """
...
class _ReturnedMessageEvt(_ChannelPendingEvt):
    ...
    def __init__(self, callback, channel, method, properties, body):
        ...
        :param pikav1.spec.Basic.Return method:
        :param pikav1.spec.BasicProperties properties:
        :param bytes body:
        """
...
class BlockingChannel(object):
    ...
    :py:class:`~pikav1.channel.Channel` class. In addition,
    ...
    Example of creating a BlockingChannel::
        ...
        connection = pikav1.BlockingConnection()
    ...
    _RxMessageArgs = namedtuple(
        ...
        [
            'channel',  # implementation pikav1.Channel instance
            'method',  # Basic.GetOk
    ...
    def __init__(self, channel_impl, connection):
        ...
        :param pikav1.channel.Channel channel_impl: Channel implementation object
        ...
        self._impl.add_callback(
            ...
            replies=[pikav1.spec.Basic.ConsumeOk],
            one_shot=False)
        ...
        self._impl.add_callback(
            ...
            replies=[pikav1.spec.Basic.GetEmpty],
            one_shot=False)
    ...
    def _on_puback_message_returned(self, channel, method, properties, body):
        ...
        :param pikav1.spec.Basic.Return method:
        :param pikav1.spec.BasicProperties properties: message properties
        :param bytes body: returned message body; empty string if no body
        """
        assert channel is self._impl, (channel.channel_number,
                                       self.channel_number)

        assert isinstance(method, pikav1.spec.Basic.Return), method
        assert isinstance(properties, pikav1.spec.BasicProperties), (properties)
    ...
    def _on_channel_closed(self, _channel, reason):
        ...
        See `pikav1.Channel.add_on_close_callback()` for additional documentation.

        :param pikav1.Channel _channel: (unused)
    ...
    def _on_consumer_cancelled_by_broker(self, method_frame):
        ...
        :param pikav1.frame.Method method_frame: method frame with the
    ...
    def add_on_cancel_callback(self, callback):
        ...
        :param callable callback: a callable for handling broker's Basic.Cancel
            ...
            where method_frame is of type `pikav1.frame.Method` with method of
    ...
    def basic_consume(self,
        ...
        :rtype: str
        :raises pikav1.exceptions.DuplicateConsumerTag: if consumer with given
    ...
    def basic_cancel(self, consumer_tag):
        ...
        :returns: (NEW IN pikav1 0.10.0) empty sequence for a auto_ack=False
    ...
    def start_consuming(self):
        ...
        pikav1 callback, because dispatching `basic_consume` callbacks from this
        context would constitute recursion.

        :raises pikav1.exceptions.ReentrancyError: if called from the scope of a
    ...
    def basic_publish(self,
        ...
        if self._delivery_confirmation:
            ...
            with self._message_confirmation_result:
                ...
                if isinstance(conf_method, pikav1.spec.Basic.Nack):
                    ...
                    assert isinstance(conf_method,
                                      pikav1.spec.Basic.Ack), (conf_method)
    ...
    def exchange_declare(self,
        ...
        :rtype: `pikav1.frame.Method` having `method` attribute of type
