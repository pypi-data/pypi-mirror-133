...
from pikav1.adapters.utils.selector_ioloop_adapter import (
...
class SelectConnection(BaseConnection):
    ...
    def __init__(
        ...
        :param pikav1.connection.Parameters parameters: Connection parameters
    ...
    def create_connection(cls,
                          ...
                          workflow=None):
        """Implement
        :py:classmethod:`pikav1.adapters.BaseConnection.create_connection()`.
        ...
        def connection_factory(params):
            """Connection factory."""
            if params is None:
                raise ValueError('Expected pikav1.connection.Parameters '
...
class IOLoop(AbstractSelectorIOLoop):
    """I/O loop implementation that picks a suitable poller (`select`,
     ...
     Implements the
     `pikav1.adapters.utils.selector_ioloop_adapter.AbstractSelectorIOLoop`
     interface.
    ...
    def process_timeouts(self):
        ...
        for _ in pikav1.compat.xrange(len(self._callbacks)):
...
class _PollerBase(pikav1.compat.AbstractBase):  # pylint: disable=R0902
    ...
    def wake_threadsafe(self):
        ...
        with self._waking_mutex:
            ...
            except pikav1.compat.SOCKET_ERROR as err:
    ...
    def start(self):
        ...
        recursively (pikav1/pikav1#1095)
    ...
    def _dispatch_fd_events(self, fd_event_map):
        ...
        for fileno in pikav1.compat.dictkeys(fd_event_map):
    ...
    def _get_interrupt_pair():
        ...
        """
        return pikav1.compat._nonblocking_socketpair()  # pylint: disable=W0212
...
class PollPoller(_PollerBase):
    ...
    def poll(self):
        ...
        for fileno, event in events:
            ...
            if (event & select.POLLHUP) and pikav1.compat.ON_OSX:
