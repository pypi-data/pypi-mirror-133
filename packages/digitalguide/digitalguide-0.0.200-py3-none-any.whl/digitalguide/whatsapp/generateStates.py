...
import yaml
from digitalguide.pattern import (EMOJI_PATTERN, JA_PATTERN, NEIN_PATTERN,
            ...
            if handler["handler"] == "PollAnswerHandler":
                ...
                if handler["filter"] == "regex":
                    if handler["regex"] == "EMOJI_PATTERN":
                        ...
                        newHandler = MessageHandler(RegexFilter(
                            ZURUECK_PATTERN), actions[handler["action"]])
                    else:
                        newHandler = MessageHandler(RegexFilter(
                            handler["regex"]), actions[handler["action"]])
    ...
    def __call__(self, update: WhatsAppUpdate):
        try:
            ...
            token_response = req.post(
                config["rasa"]["url"] + "/api/auth", data=payload)
