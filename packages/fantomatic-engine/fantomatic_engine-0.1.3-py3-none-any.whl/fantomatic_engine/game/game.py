...
class Game:
    def __init__(self, config: GameConfig, win_data: dict):
        self.config = config
        ...
        }, light_resource) if not light_resource.is_none else None
        ...
        self.menu_items_descriptors = (
            ...
            {
                "text":  t("Select directory") + t(": ↩") + " | " + t("Cancel") + ": " + t("Escape key"),
                "read_only": True,
                "small": True,
                "cond": lambda: self.menu_state == "config" and self.menu.browse_files,
            },
