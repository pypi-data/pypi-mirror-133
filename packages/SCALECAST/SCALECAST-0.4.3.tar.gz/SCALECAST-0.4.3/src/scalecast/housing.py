...
lstm_grid = GridGenerator.get_expanded_lstm_grid()
lstm_grid['verbose'] = [1]
f.set_estimator('lstm')
f.ingest_grid(lstm_grid)
f.tune()
f.auto_forecast(call_me='lstm_expanded',dynamic_testing=False)

f.pop_using_criterion('AnyLevelPrediction','>',500)
f.pop_using_criterion('AnyLevelPrediction','<',0)
...
f.all_validation_grids_to_excel(sort_by_metric_value=True)
f.export_Xvars_df().to_csv('Xvars.csv',index=False)