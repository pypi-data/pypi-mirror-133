...
class Human:
    ...
    @staticmethod
    def calculate_mouth(ai_crop_point_dict):
        """
        1.根据 眼镜 鼻子 计算嘴巴的位置
        2.根据眼睛鼻子判断 头顶是否要删除
        3.根据脚踝判断 脚底是否要删除
        ...
        if 'eye' not in ai_crop_point_dict.keys() and 'nose' not in ai_crop_point_dict.keys() \
                and 'human_top' in ai_crop_point_dict.keys():
    ...
    @staticmethod
    def calculate_mandible_crop_point(face, human_keypoint_list):
        """
        在recognition face 内
        置信度小于0.8不可信
    ...
    def add_new_crop_point(self, recognition, human_pose):
        ...
        for i, human in enumerate(human_bodies_list):
            if len(human_pose_result[i]['keypoints']) == 17:
                ...
                human_pose_result[i]['keypoints'].insert(self.HUMAN_BODY_KEY_POINTS.index('waist'),
                                                         human['keypoints'][6])
                ...
                # 嘴巴点 在 keypoints index 3 4
                # 下巴点 在 bbox index  2 3
                human_pose_result[i]['keypoints'].insert(self.HUMAN_BODY_KEY_POINTS.index('mandible'),
