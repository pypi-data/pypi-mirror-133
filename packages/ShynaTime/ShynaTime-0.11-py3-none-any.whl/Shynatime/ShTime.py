...
class ClassTime:
    ...
    # Todo: try to do it with regex instead of if else. That will be better option
    def check_am_pm(self, text_string):
        self.string_value = text_string
        try:
            if str(self.string_value).lower().__contains__('a.m.') or str(self.string_value).lower().__contains__('p.m.'):
                return self.string_value
            elif str(self.string_value).lower().__contains__('a.m') or str(self.string_value).lower().__contains__('p.m'):
                return self.string_value
    ...
    # Todo: As per time digit return time instead of just the digit
    ...
    def convert_time_zone(self, from_zone, time_value):
        self.string_value = time_value
        from_zone = tz.gettz(from_zone)
        to_zone = tz.gettz('Asia/Kolkata')
        self.string_value = datetime.strptime(str(self.string_value).replace('T', ' ').strip('Z').split('+')[0].split('.')[0],
                                       '%Y-%m-%d %H:%M:%S')
        self.string_value = self.string_value.replace(tzinfo=from_zone)
        self.string_value = self.string_value.astimezone(to_zone)
        self.string_value = str(self.string_value).split('+')[0]
        return self.string_value
