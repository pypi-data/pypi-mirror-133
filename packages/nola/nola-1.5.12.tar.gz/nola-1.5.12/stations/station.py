...
def timeliness(data_df, truth_df, expected_df):
    ...
    for obsv, vals in data_df.iterrows():
        ...
        if len(not_na) == 0:
            ...
            truth_val = truth_df.loc[obsv]
            expected_dt = expected_df.loc[obsv]

            dct[obsv] = time_till_metrics(vals, expected_dt, truth_val)

    return pd.DataFrame.from_dict(dct, orient='rows')
...
def accuracy(data_df, truth_df, expected_df):
    ...
    for obsv, vals in data_df.iterrows():
        ...
        if len(not_na) == 0:
            ...
            dct[obsv] = accuracy_metrics(vals, expected_dt, truth_val)
...
def completness(data_df, truth_df, expected_df):
    pass
