...
class EvaluationPhase(ExtensibleModel):
    ...
    def clean(self):
        ...
        if self.registration_date_end >= self.evaluation_date_start:
            raise ValidationError(_("The registration has to be finished before the evaluation."))
...
class EvaluationKeyPair(models.Model):
    ...
    def decrypt(self, ciphertext: Union[str, bytes], password: str) -> str:
...
class EvaluationResult(ExtensibleModel):
    ...
    result = models.TextField(verbose_name=_("Encrypted result"))  # HERE SOME MAGIC
    ...
    def store_result(self, result: Union[str, int], commit: bool = False):
        ...
        message = str(result)
        ciphertext = self.group.registration.keys.encrypt(message)
    ...
    def get_result(self, password: str) -> Union[str, int]:
        result = self.group.registration.keys.decrypt(self.result, password)
