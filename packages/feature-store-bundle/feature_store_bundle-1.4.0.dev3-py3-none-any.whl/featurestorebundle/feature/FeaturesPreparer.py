...
class FeaturesPreparer:
    ...
    def __prepare_base_dataframe(self, features_storage: FeaturesStorage, feature_store: DataFrame, rainbow_table: DataFrame) -> DataFrame:
        ...
        # TODO if collect run_date.disttinct == jen jedno datum
        ...
        joined_results = (
            ...
            .withColumn("new_columns_temp", f.flatten(f.filter("temp", lambda x: x.isNotNull())))
            # .withColumn("new_columns", f.flatten(f.collect_set("new_columns").over(window)))
            .select(
                entity.id_column,
                entity.time_column,
                *feature_list.get_names(),
        ...
        new_columns = (
            joined_results.select(entity.time_column, "new_columns_temp")
            .groupBy(entity.time_column)
            .agg( f.flatten(f.collect_set("new_columns_temp")).alias("new_columns"))
        )

        joined_results = joined_results.join(new_columns, on=entity.time_column).select(
            entity.id_column,
            entity.time_column,
            *feature_list.get_names(),
            "new_columns",
