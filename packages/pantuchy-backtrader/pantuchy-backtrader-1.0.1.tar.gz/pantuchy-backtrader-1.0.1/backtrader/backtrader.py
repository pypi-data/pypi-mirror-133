...
class OHLCData:
	"""
	Args:
		tsformat (str): Possible values: D, s, ms, us, ns
	"""
	symbol: str
	files: list
	timestamp: int
...
class Indicator:
	"""
	Args:
		timeframe (str): Possible values: 1m, 5m, 15m, 30m, 1h, 2h, 3h, 4h, 6h, 12h, 1D, 1W
		data_key (str): Indicators that requires multiple sets of prices (open, high, close) will ignore this argument.
	"""
	name: str
	tag: str
	timeframe: str
	...
	def set_strategy(self, strategy):
		...
		if self.__strategy.indicators is not None and len(self.__strategy.indicators) > 0:
			for ind in self.__strategy.indicators:
				...
				if ind.name == "EMA":
					if len(resampled[ind.data_key]) < ind.period:
						raise Exception(f"OHLC data is not enough for period {ind.period}.")

					values = numpy.array(resampled[ind.data_key], dtype=float)
					nans = numpy.isnan(values)
					df[ind.tag] = pd.Series(talib.EMA(values[~nans], timeperiod=ind.period), index=indexes[~nans])
				elif ind.name == "CCI":
