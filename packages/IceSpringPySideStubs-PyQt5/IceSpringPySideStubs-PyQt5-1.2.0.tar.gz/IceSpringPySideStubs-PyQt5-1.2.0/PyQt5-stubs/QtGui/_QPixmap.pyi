    ...
    **QImageWriter** .
    ...
    def detach(self) -> None:
        ...
        **void QPixmap::detach()**
        ...
        A pixmap is automatically detached by Qt whenever its contents are about
        to change. This is done in almost all **QPixmap**  member functions that
        modify the pixmap (**fill** (), **fromImage** (), **load** (), etc.),
        and in **QPainter::begin** () on a pixmap.

        There are two exceptions in which detach() must be called explicitly,
        that is when calling the handle() or the x11PictureHandle() function
        (only available on X11). Otherwise, any modifications done using system
        calls, will be performed on the shared data.

        The detach() function returns immediately if there is just a single
