...
class TeamsHandler:
    ...
    def add(self, incoming: Union[UUID, List[UUID]], sources: Union[UUID, List[UUID]]):
        ...
        for to_add in incoming:
            self._teams.setdefault(to_add, []).extend(sources)
    ...
    def filter_to_source(self, source_id: Union[str, UUID]) -> 'TeamsHandler':
        ...
        return TeamsHandler(
            teams={k: v for k, v in self._teams if source_id in search_uuid}
        )
    ...
    def to_dict(self) -> dict:
        ...
        if len(self._teams) > 0:
            result["T"] = {k.__str__(): v for k, v in self._teams.items()}
        return result
