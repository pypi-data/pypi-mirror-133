...
from slapp_py.helpers.dict_helper import first_key


class FriendCodeHandler:
    _codes: Dict[FriendCode, List[UUID]]

    def __init__(self, codes: Optional[Dict[FriendCode, List[UUID]]] = None):
    ...
    def add(self, incoming: Union[FriendCode, List[FriendCode]], sources: Union[UUID, List[UUID]]):
        ...
        for to_add in incoming:
            if to_add.no_code:
                continue
            self._codes.setdefault(to_add, []).extend(sources)

    def merge(self, handler: 'FriendCodeHandler'):
        for key, value in handler._codes.items():
            self.add(key, value)

    def filter_to_source(self, source_id: Union[str, UUID]) -> 'FriendCodeHandler':
        search_uuid = source_id if isinstance(source_id, UUID) else UUID(source_id)
        return FriendCodeHandler(
            codes={k: v for k, v in self._codes if source_id in search_uuid}
        )

    def get_sources_for_code(self, fc: FriendCode) -> List[UUID]:
        """Gets UUIDs for the code, or empty if not found."""
    ...
    def get_codes_sourced(self) -> Dict[FriendCode, List[UUID]]:
    ...
    def from_dict(obj: dict) -> 'FriendCodeHandler':
        ...
        try:
            ...
            for key, value in val_dict.items():
                result.add(FriendCode(key), value)
            return result
    ...
    def to_dict(self) -> dict:
        ...
        if len(self._codes) > 0:
            result["C"] = {k.to_int(): v for k, v in self._codes.items()}
        return result
