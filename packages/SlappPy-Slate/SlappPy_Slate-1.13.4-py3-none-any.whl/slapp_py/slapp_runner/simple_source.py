...
class SimpleSource:
    ...
    def initialise(source_name: Optional[str]) -> Tuple[Optional[str], Optional[str], Optional[str], Optional[str]]:
        """Initialise the simple sources's (date, organiser, tournament name, url)."""
        if not source_name:
        ...
        if id_at_the_end_match:
            # We always want to grab the last match as the id is at the end of the sources name.
    ...
    def get_linked_date_display(self) -> str:
        """Return a markdown link with the truncated sources date if available,
    ...
    def get_linked_name_display(self) -> str:
        """Return a markdown link with the truncated sources name (date-name) if available,
        otherwise return its truncated_name only."""
        link = self.url
        text = self.truncated_name
        return f"[{text}]({link})" if link else text
    ...
    def truncated_name(self):
        """The sources with the id removed and truncated down."""
