...
def load_sources_yaml_file(path: Optional[str] = None):
    """
    Load the yaml sources file - this is in the format of one sources file to load per line.
    :param path: The sources file path. Do not specify to use the latest default.
    :return: The contents of the sources file - expected a list of sources file paths.
...
def generate_new_sources_files(battlefy_ids: Collection[str], skip_redownload: bool = False) -> Tuple[str, str]:
    ...
    # and if the sources is new since the current sources (true) or was present in the last sources file (false)
...
def add_tourney_placement_to_source(tourney_id: str,
    ...
    # Find the suitable sources in the latest sources snapshot
    source = get_source_by_tourney_id(tourney_id, sources)
    if not source:
        print(f"Could not find a sources in the snapshot that matches {tourney_id=}. Not adding.")
        return False

    if source.brackets and source.brackets[0].is_valid and source.brackets[-1].is_valid:
        print(f"Brackets already exist for this sources. Skipping.")
        return False
