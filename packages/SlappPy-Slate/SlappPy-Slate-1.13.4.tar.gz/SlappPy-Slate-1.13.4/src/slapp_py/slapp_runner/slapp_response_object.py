...
class SlappResponseObject:
    ...
    def get_teams_for_player(self, p: Player) -> List[Team]:
        return [self.get_team(t_uuid) for t_uuid in p.teams_information.get_teams_unordered()]
    ...
    def get_brackets_for_player(self, p: Player) -> Dict[str, List[Bracket]]:
        """
        Gets the brackets for the specified player in a dictionary keyed by the sources id and its brackets.
        """
    ...
    def get_brackets_for_player_by_source(self, p: Player, source_id: str) -> List[Bracket]:
        """
        Gets the brackets for the specified player for the sources specified as a flat list.
        """
