...
class ModelTracker(TrackingBase):
    ...
    def save_model(self, model, model_name, type_="sklearn", preprocessor=None):
        """Saves model to project s3 directory.
        
        Args:
            model: Model to save.
            model_name: Name of file to save model to.
        ...
        self._set_storage_location()
        try:  # add in ability to save tensorflow files
            if type_ == "sklearn":
                model_name = f"{model_name}.joblib"
                joblib.dump(model, model_name)

                model_path = f"{self.bucket_prefix}/model/{model_name}"
                tracking_utils.save_to_s3(self.s3_bucket, model_name, model_path)

            if preprocessor is not None:
                transformer_name = f"{preprocessor.__class__.__name__}.joblib"
                joblib.dump(preprocessor, transformer_name)
                transformer_path = (
                    f"{self.bucket_prefix}/preprocessor{transformer_name}"
                )
                tracking_utils.save_to_s3(
                    self.s3_bucket, transformer_name, transformer_path
                )
            ...
            if os.path.exists(f"requirements.txt"):
                tracking_utils.save_to_s3(
                    self.s3_bucket,
                    "requirements.txt",
                    f"{self.bucket_prefix}/requirements.txt",
                )
