...
def install():
    ...
    def _fast_falcon_api(this: API, env, start_response):
        ...
        with context.new_entry_span(op=functions.getUri(str(req.url.split("?")[0])), carrier=carrier) as span:
            ...
            # functions.log("falcon url:%s,op:%s", str(req.url), span.op)
            ...
            functions.log("resp_status:%s", str(resp_status[0]))
        ...
        if name.startswith("HTTP_"):
            ...
            if header_key == "FAST-TRACKER":
                ...
                if len(value_arr) > 0:
                    ...
                    # functions.log("header_key:%s,value:%s",header_key,headers[header_key])
                    if len(value_arr) >= 2:
                        headers["parent_id"] = lang.b64decode(value_arr[1])
                        # functions.log("header_key:%s,value:%s",header_key,headers["parent_id"])
            else:
                headers[header_key] = value
                # functions.log("header_key:%s,value:%s",header_key,value)
...
def parse_status(status_str):
    ...
    :return:
    """
    functions.log("status_str:%s",status_str)
