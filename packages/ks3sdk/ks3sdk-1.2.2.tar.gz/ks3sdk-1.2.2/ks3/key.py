...
class Key(object):
    ...
    def get_presigned_url(self, expires_in, headers=None, expires_in_absolute=False, version_id=None, domain=False):
        ...
        url = self.bucket.connection.generate_url(expires_in, 'PUT',
                                                   self.bucket.name, self.name,
                                                   headers=headers, expires_in_absolute=expires_in_absolute, version_id=version_id)
        if domain is True :
           url = url.replace('http://'+self.bucket.name+'.','http://')
           url = url.replace('https://'+self.bucket.name+'.','https://')
    ...
    def set_contents_from_filename(self, filename, headers=None, replace=True,
        ...
        with open(filename, 'rb') as fp:
            return self.set_contents_from_file(fp, headers, replace, cb,
                                               num_cb, policy, md5,
                                               reduced_redundancy,
                                               encrypt_key=encrypt_key, calc_encrypt_md5=calc_encrypt_md5,query_args= query_args)
    ...
    def get_contents_to_filename(self, filename, byte_range=None, headers=None,
        ...
        try:
            with open(filename, 'wb') as fp:
                self.get_contents_to_file(fp, byte_range, headers, cb, num_cb,
                                          torrent=torrent,
                                          version_id=version_id,
