...
class Connect4(commands.Cog):
    ...
    def has_won_connect4(self, board, is_bot=False):
        ...
        if not sum([i.count(' ') for i in board]):
        # num = 0
        # for row in board:
        #     for column in row:
        #         if column != " ":
    ...
    def minimax_connect4(self, board, depth, isMaximizing, difficulty):
        """The minimax algorithm for connect4"""
        ...
        if isMaximizing:
            ...
            for x, y in self.get_empty_connect4(board):
                board[x][y] = "b"
                score = self.minimax_connect4(board, depth + 1, False, difficulty)
                board[x][y] = " "
                if score > bestScore:
                    bestScore = score
            return bestScore
            ...
            for x, y in self.get_empty_connect4(board):
                board[x][y] = "r"
                score = self.minimax_connect4(board, depth + 1, True, difficulty)
                board[x][y] = " "
                if score < bestScore:
                    bestScore = score
            return bestScore
    ...
    async def make_bot_move_connect4(self, board, difficulty):
        ...
        if difficulty == 0:
            ...
            for x, y in self.get_empty_connect4(board):
                board[x][y] = "b"
                thing = functools.partial(self.minimax_connect4, board, 0, False, difficulty)
    ...
    async def connect4(self, ctx, member: discord.Member = None):
        ...
        if not member:
            ...
            turn = ctx.author
            await ctx.send("Choose ai difficulty level: 0-3 (the bigger the number the longer it takes to make moves but also the smarter the ai will be)")
            ...
            if not int(message.content) in range(4):
                return await ctx.send("That's not a valid difficulty level")
            e = discord.Embed(
                title="Connect4",
            ...
            while True:
                ...
                if turn == ctx.author:
                    ...
                    board = await self.make_bot_move_connect4(board, int(message.content)+3 if int(message.content) != 0 else 0)
