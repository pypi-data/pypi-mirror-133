...
class Battleships(commands.Cog):
    ...
    async def battleship(self, ctx, member: discord.Member):
        """Each player hides ships on their grid. Players take turns calling out row and column coordinates on the other player's grid in an attempt to identify a square that contains a ship."""
        if member.bot:
