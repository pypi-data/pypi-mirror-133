...
class Bitkub:
    ...
    def ticker(self, sym: str = None):  # get ticker
        ...
        if sym is not None:
            sym = str.upper(sym)
            response_url = response_url + f"symbol={sym}&"
    ...
    def trades(self, sym: str, lmt: int):  # get trades
        ...
        lmt = str(lmt)
        sym = str.upper(sym)
        response_url = response_url + f"symbol={sym}&lmt={lmt}"
        response = requests.get(response_url).json()
        return response
    ...
    def tradingview(self, symbol: str, resolution: str, frm: int, to: int):  # get tradingview
        ...
        response_url = response_url + f"symbol={symbol}&resolution={resolution}&from={frm}&to={to}"
        response = requests.get(response_url).json()
        return response
    ...
    def cancel_order(self, sym: str = None, id: int = None, sd: str = None, hash: str = None, ts: int = None):  # cancel order
        ...
        if hash is not None:
            ...
            if sym or id or sd is None:
                raise Exception("symbol, id, sd is required or use hash inserted")
            else:
                sym = str.upper(sym)
                data.update({'symbol': sym, 'id': id, 'sd': sd})
        if ts is not None:
            data.update({'ts': ts})
        else:
            data.update({'ts': self.servertime()})
