...
ToDo
1. Silence the scripting implementations and write output to log file.
2. Write the number of tests and time to the ini and then add it to the readme.rst
3. Warning in documentation about Dropbox grabbing files
4. Upfront tasks
   4.1 Create the following accounts
   - Github
   - ReadTheDocs https://docs.readthedocs.io/en/stable/tutorial/
   - CodeCov
5. Discuss the usage of templates i.e. where the default templates are and
   how to use a custom template.
6. Rename all the templates and read the template name from the pimt.ini
7. Read the module | package setting from the commandline rather than the
   ini file.  Reading it from the ini forces the creation off 2 ini files, one
   for packages and the other for modules
8. Check if it is necessary for the pre-commit onto run before CI process
   on GitHUb and fix if necessary.
9. Add Black-badge https://black.readthedocs.io/en/stable/
...
Conventions
1.  Release note title name = "Ver n.n.n"
2.  Release tag name = "Tag n.n.n'
3.  GitHub enhancement branch name = "Enhancement_nnnn" where nnnn is the
    GitHub issue number with leading zeros.
4.  Color Coding
...
1. Virtual Environments:
    ...
    -   It (obviously) assume the PackageIt.create_venv work correctly and
        is tested properly.
    -   Tests rely on the virtual environment installed for PackageIt.
    -   Fresh virtual environments will only be created if:
        -   The test or any methods it calls will alter the virtual
            environment or
        -   It requires to operate specifically in the virtual environment
            for the package begin created.
"""
...
import sys
import toml
from beetools import beeutils, beescript, beevenv
from beetools.beearchiver import Archiver, msg_error, msg_info, msg_milestone
...
class PackageIt:
    ...
    def create_github_release_yml(self):
        ...
        if self.project_gh_enable and self.project_pypi_publishing == "GitHub":
            ...
            test_token_name = "TEST_PYPI_API_TOKEN"
            if self.project_pypi_repository == "pypi":
    ...
    def create_scaffolding(self):
        ...
        success = True
        print(msg_milestone("Create project scaffolding..."))
    ...
    def create_source_code_py(self):
        ...
        if not dir_contents:
            if self.project_type == "Module":
                shutil.copy(self.templ_dir / "templ_module.py", dest_pth)
            elif self.project_type == "Package":
                shutil.copy(self.templ_dir / "templ_package.py", dest_pth)
            contents = dest_pth.read_text().format(
                self.project_name,
                "{}",
                self.project_name.lower(),
    ...
    def get_github_repo_tags(self):
        print(msg_milestone("Get GitHub repo tags..."))
        tags = []
            ...
            while not commit_success:
                try:
                    ...
                    if retry_cntr < retry_max:
                        ...
                        print(msg_error(err))
                        print(msg_error("System will now terminate!"))
                        sys.exit()
        return success
    ...
    def github_sync_release_notes(self):
        ...
        print(msg_milestone("Synchronise release notes..."))
        tag_black_list = []
    ...
    def init_git(self):
        ...
        success = False
        if self.project_git_enable:
            print(msg_milestone("Initialize Git..."))
            try:
    ...
    def init_github(self):
        ...
        if self.project_git_enable and self.project_gh_enable:
            print(msg_milestone("Initialize GitHub..."))
            self.github = Github(login_or_token=self.project_gh_token)
            if not self.project_gh_wf_dir.is_dir():
    ...
    def marry_git_github(self):
        ...
        if self.project_git_enable and self.project_gh_enable:
            ...
            try:
                ...
                print(msg_info('Head "master" already exists.'))
                print(msg_info(err))
    ...
    def zip_project(self):
        print(msg_milestone("Archive {} files...").format(self.project_name))
        arc = Archiver(
            self.project_header_description,
