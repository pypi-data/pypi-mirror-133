...
trace
~~~~~
Module contains a trace to analyze, process, visualize time-discrete signals.
...
def _pairwise(
    trace: Union[Iterator, List, Trace]) -> Iterator[Tuple[Number, Number]]:
...
class Trace:
    ...
    def from_dict(cls, key: str, data: DataFrame, **kwargs) -> Trace:
        ...
        :attr:`samples` from the value of the the *data* dictionary item selected
        by the *key*.
    ...
    def lag(self, ratio: Union[int, float], **kwargs) -> Trace:
        """ First-order lag over the signal :attr:`samples` of the
        trace.
        ...
        :keyword float preset: optional preset value of the first-order lag.
            Default is ``0.0``.
        :keyword str label: optional trace label to set
        """
        ...
        return replace(self,
                       label=kwargs.get('label', f'{self.label}:lag'),
                       samples=samples)
