...
statemachine
~~~~~~~~~~~~
Module contains a state machine to analyze, process, visualize time-discrete
...
class Statemachine(MutableMapping):
    """ State machine data class to :attr:`evaluate` the state transitions
    between :attr:`states` of a state machine.

    A *state* of the state machine is defined by a tuple pair, consisting of
    a unique integer *number* of the state, and the *label* of the state.

    The :attr:`states` of a state machine must define a consecutive, ascending
    ...
    #: List with the state labels of the state machine
    ...
    def create(cls,
               ...
               **kwargs) -> Statemachine:
        """ Creates either a state machine with the number of *states* without
        labeling them, or a state machine with states from an iterable containing
    ...
    @property
    def numbers(self):
        """ List with the state numbers of the state machine."""
    ...
    def zeroed_matrix(self) -> List[List[int, ...]]:
        """ Returns the zeroed transition matrix of the state machine.
        """
    ...
    def evaluate(self, signal: Optional[Trace] = None) -> Statemachine:
        ...
        state machine in the :attr:`signal`.
    ...
    def data(self) -> List[List[str, int, ...]]:
        ...
        between the :attr:`states` of the state machine.
        """
        # table data
        data = list()
        # add column labels
