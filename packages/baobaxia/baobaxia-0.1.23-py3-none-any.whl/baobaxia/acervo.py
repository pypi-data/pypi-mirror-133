...
api.add_api_route('/{balaio}/{mucua}/acervo/midia',
                  post_midia,
...
api.add_api_route('/{balaio}/{mucua}/acervo/midia/{path:path}',
                  put_midia,
                  response_model=Midia,
                  methods=['PUT'],
...
api.add_api_route('/{balaio}/{mucua}/acervo/upload/{path:path}', upload_midia,
...
api.add_api_route('/{balaio}/{mucua}/acervo/download/{path:path}',  
                  download_midia,
                  methods=['GET'],
                  summary='Retornar o arquivo de uma mídia')

async def download_thumbnail(balaio: str,
                             mucua: str,
                             path: Path,
                             token: Optional[str] = Header(None)):
    try:
        ...
        if len(saber.content) == 1:
            if saber.tipo == MidiaTipo.imagem:
                ...
                source = cv2.imread(str(base_path / saber.path / saber.content[0]))
                height, width, channels = source.shape
                scale = THUMBNAIL_WIDTH / width
                thumbnail_height = int(height * scale)
                target = cv2.resize(src=source, dsize=(THUMBNAIL_WIDTH, thumbnail_height), interpolation=cv2.INTER_LINEAR)
                cv2.imwrite(str(base_path / saber.path / thumbnail), target)
                saber.content.append(thumbnail)
                if token is not None:
                    api.baobaxia.put_midia(balaio, mucua, saber, token)
            elif saber.tipo == MidiaTipo.video:
                cam = cv2.VideoCapture(str(base_path / saber.path / saber.content[0]))
                source = None
                currentframe = 0
                while True:
                    ret, frame = cam.read()
                ...
                raise HTTPException(status_code=404, detail='Acervo não possui miniatura')
...
api.add_api_route('/{balaio}/{mucua}/acervo/thumbnail/{path:path}',
    download_thumbnail,
    methods=['GET'],
...
api.add_api_route('/{balaio}/{mucua}/acervo/publish/{path:path}',
    publish_midia,
    methods=['PUT'],
...
api.add_api_route('/{balaio}/{mucua}/acervo/find', find_midias,
...
api.add_api_route('/{balaio}/{mucua}/acervo/tipos_por_content_type',
...
async def get_top_tags(*,
                       ...
                       size: int = 10):
    try:
        api.baobaxia._check_cache(Midia, 'midia', saberes_patterns, balaio, mucua)
        tags = api.baobaxia.indexes[balaio][mucua]['midia']['tags']
        counters = []
        for tag in tags:
        ...
        return sorted(counters, key=lambda counter: counter.count, reverse=True)[:size] 
    except (SessionNotFound, SessionExpired):
        traceback.print_exc()
        raise HTTPException(status_code=401, detail="Token inválido")
    except Exception as e:
...
api.add_api_route('/{balaio}/{mucua}/acervo/top_tags',
                  get_top_tags,
