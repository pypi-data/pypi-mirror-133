...
class BaobaxiaAPI(FastAPI):
    ...
    def __init__(
            ...
            root_path: str = "",
            **kwargs: Any) -> None:
        super().__init__(prefix=prefix, tags=tags, root_path=root_path, **kwargs)
        ...
        super().add_api_route(
            '/{balaio}/{mucua}/keepalive',
            self.keep_alive,
            methods=['GET'],
            summary='Mantém ativa a sessão do mocambola')
        super().add_api_route(
            '/{balaio}/{mucua}/recover',
            self.recover,
            methods=['POST'],
            response_model=str,
        ...
        super().add_api_route(
            '/{balaio}/{mucua}/mocambola/{username}',
            self.get_mocambola,
            methods=['GET'],
            response_model=Mocambola,
        ...
        super().add_api_route(
            '/{balaio}/{mucua}/mocambola/password',
            self.set_password,
            methods=['POST'],
    ...
    def add_saberes_api(self, model: type, **kwargs):
        ...
        if 'url_path' in kwargs:
            url_path = '/{balaio}/{mucua}/'+kwargs['url_path']
        else:
            url_path = '/{balaio}/{mucua}/'+field_name
    ...
    async def authenticate(self, *,
        ...
        try:
            result = self.baobaxia.authenticate(
                balaio, mucua, username, password)
            return result
    ...
    async def recover(self, *,
        ...
        return self.baobaxia.authenticate(
            balaio, mucua, username,
            recovery_answer = recovery_answer)

    async def list_balaios(self, token: str = Header(None)):
        try:
    ...
    async def post_balaio(self,
                          ...
                          token: str = Header(...)):
        try:
            return self.baobaxia.put_balaio(name=name, default_mucua=default_mucua, token=token)
        except (SessionNotFound, SessionExpired):
            traceback.print_exc()
            raise HTTPException(status_code=401, detail="Token inválido")
        except Exception as e:
            traceback.print_exc()
            raise HTTPException(status_code=500, detail=str(e))
    async def get_balaio(self, balaio: str, token: str = Header(None)):
        try:
            return self.baobaxia.get_balaio(balaio, token=token)
        except (SessionNotFound, SessionExpired):
            traceback.print_exc()
            raise HTTPException(status_code=401, detail="Token inválido")
        except Exception as e:
            traceback.print_exc()
            raise HTTPException(status_code=500, detail=str(e))
    async def put_balaio(self, balaio: str,
                         name: str = Form(...), 
                         default_mucua: str = Form(...),
                         token: str = Header(...)):
        try:
            ...
            traceback.print_exc()
            raise HTTPException(status_code=500, detail=str(e))
    async def del_balaio(self, balaio: str, token: str = Header(...)):
        try:
            self.baobaxia.del_balaio(balaio, token=token)
            return {'detail': 'success'}
        except (SessionNotFound, SessionExpired):
            traceback.print_exc()
            raise HTTPException(status_code=401, detail="Token inválido")
    ...
    async def list_mucuas(self, balaio: str, token: str = Header(None)):
        try:
            return self.baobaxia.list_mucuas(balaio, token=token)
        except (SessionNotFound, SessionExpired):
            traceback.print_exc()
            raise HTTPException(status_code=401, detail="Token inválido")
        except Exception as e:
    ...
    async def post_mucua(self,
                         ...
                         token: str = Header(...)):
        try:
            return self.baobaxia.put_mucua(balaio_slug=balaio, name=name, token=token)
        except (SessionNotFound, SessionExpired):
            traceback.print_exc()
            raise HTTPException(status_code=401, detail="Token inválido")
        except Exception as e:
    ...
    async def get_mucua(self,
        ...
        try:
            return self.baobaxia.get_mucua(balaio, mucua, token=token)
        except (SessionNotFound, SessionExpired):
            traceback.print_exc()
            raise HTTPException(status_code=401, detail="Token inválido")
        except Exception as e:
    ...
    async def del_mucua(self,
                        ...
                        token: str = Header(...)):
        try:
            self.baobaxia.del_mucua(balaio, mucua, token=token)
            return {'detail': 'success'}
        except (SessionNotFound, SessionExpired):
            traceback.print_exc()
            raise HTTPException(status_code=401, detail="Token inválido")
