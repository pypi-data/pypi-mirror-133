...
class ApiManager:
    ...
    def delete_source(self, org_id, source_id):
    ...
    def graphs_list(self, org):
        """
        Return the list of graphs on a given org
        :param org: Org ID that wants to be queried
        :return: Json list with graph on the org
        """
        return self.graphs.get_graphs_list(org_id=org)

    def regular_pipeline_duplicate(self, org_id, pipeline_id, new_name):
        """
    ...
    def export_function(self, org_id, function):
        """
        Create a json structure with all the components needed
    ...
    def proceses_importing_pipeline_file(self, org_id, content, skip, handleduplicates="clone", function=False,
                                         loaded=False):
        """
    ...
    def get_all_hierarchies_display(self, org):
        """
        Method to retrieve all the organization on a given org
        :param org: Organization id where the org is stored
        :return:
        """
        columns = [
            "id", "name"
        ...
        raw_data = self.get_all_hierarchies(org=org)
    ...
    def get_single_hierarchy(self, org, hierarchy):
        """
    ...
    def get_all_hierarchies(self, org):
        """
        Method to retrieve all the organization on a given org
        :param org: Organization id where the org is stored
        :return:
    ...
    def create_hierarchy(self, org, name, levels=None, attribute_names=None, private=False):
        """
        Method to create a new hierarchy on the given org
        :param org: Organization id where the org would be saved
    ...
    def export_hierarchy(self, org, hierarchy):
        """
        Method to extract the necessary information to export a hierarchy
        :param org: organization id where the hierarchy currently is stored
    ...
    def import_hierarchy(self, org, content):
        """
        Method to import a hierarchy, this should be used with the output of the export hierarchy
        :param org: organization id where the new hierarchy will be saved
