...
try:
    ...
    from ._schemas import KwargRelation
except ImportError:
    from _utils import Decorators
    from _schemas import KwargRelation
...
class OAuthClient(OAuthBase):
    ...
    @Decorators.merge_arguments(rels=[KwargRelation(
        inner="extra_params",
        outer="params"
    )])
...
class AsyncOAuthClient(OAuthBase):
    ...
    @Decorators.async_merge_arguments(rels=[KwargRelation(
        inner="extra_params",
        outer="params"
    )])
    ...
    async def get(
        ...
        **kwargs
        ...
        # update params
        if params:
            params = {k: str(v) for k,v in params.items() if v != None}
