...
class APIClient:
    ...
    def fetch_database(self, database_id):
        url = f'{BACKEND_URL}/operations/databases-instances/{database_id}/'
        headers = {'Authorization': f'Token {self.token}'}
        response = requests.get(url, headers=headers)
        response.raise_for_status()
        return response.json()
    ...
    @api_exception_wrapper
    def create_volume(self, project_id, name):
    ...
    def create_domain(self, service_id, base, https_active=True, https_redirect_code=None):
        ...
        data = {
            ...
            'https_redirect_code': https_redirect_code
        }
        response = requests.post(url, headers=headers, json=data)
        response.raise_for_status()
        return response.json()
    ...
    @api_exception_wrapper
    def list_services(self, project_id, type_id=None, search=None, limit=20, offset=0):
        url = f'{BACKEND_URL}/operations/services/'
        headers = {'Authorization': f'Token {self.token}'}
        params = {
            'project': project_id,
