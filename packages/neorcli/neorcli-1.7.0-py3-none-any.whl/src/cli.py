...
create_parser.add_argument('project_id', help='Project ID to create service in', type=int)
create_parser.add_argument('service_name', help='Name of service to create')
create_parser.add_argument('--base_service_id', help='Base service ID to use for service creation', type=int)
create_parser.add_argument('--test-domain', help='Test domain to use for service creation', required=False)
create_parser.add_argument('--no-new-image', help='Do not create a new image', action='store_true', default=False)
create_parser.add_argument('--no-deploy', help='Do not deploy service', action='store_true', default=False)
create_parser.add_argument('--share-storage', help='Share storage with base service', action='store_true',
                           default=False)

create_db = subparsers.add_parser('create-db', help='Create a database')

create_db.add_argument('project_id', help='Project ID to create database in', type=int)
create_db.add_argument('db_name', help='Name of database to create')
create_db.add_argument('--type-id', help='Type ID of database to create', type=int)
create_db.add_argument('--image-id', help='Image ID of database to create', type=int)
create_db.add_argument('--quota-id', help='Quota ID of database to create', type=int)
...
def handle_create(args):
    ...
    test_domain = args.test_domain or f"{args.service_name}-{hex(uuid.uuid4().int)[:6]}"
        ...
        if args.share_storage is False:
            ...
            for volume_relation in volume_relations:
                volume_relation['volume'] = client.create_volume(
                    ...
                    name=f"{args.service_name}-vol-{hex(uuid.uuid4().int)[:6]}",
                )
...
def handle_create_db(args):
    ...
    volumes = client.list_defaults(
        ...
        field_name='volumes',
        service_type_codename=type_['codename']
    )['results'] or []
    ...
    print(f"{db['name']} database has been created with ID {db['id']}")
    if volumes:
        print(f"Creating {len(volumes)} volumes")
    for volume in volumes:
        volume = json.loads(volume['value'])
        volume_id = client.create_volume(args.project_id, volume['name'])['id']
        print(f"Attaching volume {volume['name']} to {db['name']}")
        client.create_deployment_volume(
            volume_id=volume_id,
            deployment_id=db['id'],
        ...
        print(f"Volume {volume['name']} attached")
