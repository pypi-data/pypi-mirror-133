...
def print_dataset_storage_usage_summary(path_list):
  ...
  for se in se_list:
    ...
    stat_dict.update({se:{'size':0.,'nFiles':0}})
...
Usage:
   %s <datasetName (may contain wild cards) or ascii file with a list of datasets> <options>
""" % Script.scriptName)
    ...
    for dataset_name in dataset_list:
      ...
      for metadata in mq:
        if metadata in fileMetaFields:
          ...
          gLogger.error('Cannot provide storage report per SE for dataset:', dataset_name )
          DIRAC.exit(-1)
      if long:
      ...
      if SEUsage:  
        if 'outputType' in mq:
          ...
          gLogger.error('Error getting SEUsage, no outputType metadata defined')
          gLogger.error('for dataset %s', name)
          DIRAC.exit(-1)
        result = fc.findDirectoriesByMetadata(mq)
        for k, path in result['Value'].items():
          if os.path.basename(path) == outputType:
  ...
  if SEUsage:
    gLogger.notice('Storage usage per SE for all datasets\n')
