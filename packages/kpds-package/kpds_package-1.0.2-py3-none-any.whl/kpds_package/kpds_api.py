...
def kpds_ticker_raw_data(ticker,**param):
    ...
    if param.get("excel") == "o":
        ...
        writer = pd.ExcelWriter('raw_data.xlsx', engine='openpyxl')
...
def kpds_ticker_raw_data_merge(ticker, **param):
    ...
    if( (param.get("merge_type")).lower() == "none"):
        ...
        writer = pd.ExcelWriter('raw_data_merge_'+param.get("merge_type")+'.xlsx', engine='openpyxl')
        ...
        for i in range(len(split_data)):
            ...
            if param.get("excel") == "o":
                ...
                info_data = info_data.head(6)
                info_data.to_excel(writer, startcol= i * 3)
              
                df.to_excel(writer, startrow=len(info_data)+2, startcol= i * 3, index=False)
                writer.save()
                print(f"{data['TICKER']} 데이터 저장 완료")
    else:
        # STRING -> JSON
        data = json.loads(output.text)
