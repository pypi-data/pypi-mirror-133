...
import warnings
from io import open
from airtest.core.api import G, auto_setup, log, stop_app, shell
...
def run_air(devices, case, app_name, log_path, case_path, base_dir, static_dir='https://cdn.jsdelivr.net/gh/openutx/static/'):
    ...
    if device_platform().lower() in 'ios':
        ...
        dev = model + ' ios {}'.format(build_version)
        ...
        dev = model + ' Android {}'.format(build_version)
    else:
    ...
    if f.endswith(".air"):
        ...
        print(script)
        log = os.path.join(log_path + '/' + dev + '/' + airName.replace('.air', ''))
