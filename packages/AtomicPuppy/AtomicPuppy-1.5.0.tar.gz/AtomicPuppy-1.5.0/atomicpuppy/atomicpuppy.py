...
class Event:
    ...
    def __init__(
      self, id, type, data, stream, sequence, metadata=None, correlation_id=None,
      published_sequence=None, published_stream=None, link_metadata=None):
        self.id = id
        self.type = type
        self.data = data
        self.stream = stream
        ...
        self.published_sequence = published_sequence
        self.published_stream = published_stream
...
class Page:
    ...
    def _make_event(self, e):
        ...
        try:
            link_metadata = json.loads(e["linkMetaData"])
        except KeyError:
            link_metadata = None
        ...
        published_sequence=e["eventNumber"]
        published_stream=e["streamId"]
...
class StreamReader:
    ...
    def __init__(self, queue, stream_name, instance_name, max_time, subscriptions_store, session, nosleep=False):
        self._fetcher = StreamFetcher(None, nosleep=nosleep, session=session, max_time=max_time)
        self._queue = queue
...
class EventFinder:
    ...
    def __init__(self, fetcher, stream_name, instance_name, head_uri):
...
class StreamFetcher:
    ...
    def __init__(self, policy, session, max_time, nosleep=False):
        self._policy = policy
        self._log = logging.getLogger(__name__)
    ...
    async def sleep(self, delay):
        if self._nosleep:
            return
        self._sleep = asyncio.futures.Future()
...
class EventRaiser:
    ...
    def __init__(self, queue, counter, callback, exception_handler=None):
        self._queue = queue
                ...
                try:
                    ...
                    if self._exception_handler is None:
                        self._logger.exception(
                            "Failed to persist last read event with {}".format(
                                type(self._counter)))
                    else:
                        self._exception_handler(
                            {"exception": e,
                             "atomicpuppy_cause": ExceptionCause.counter})
...
class StreamConfigReader:
    ...
    def read(self, config_file):
        ...
        return SubscriptionConfig(streams=streams,
                                  ...
                                  max_time=cfg.get("max_time") or 60,
