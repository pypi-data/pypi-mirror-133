...
class SocketHandler:
    ...
    def reconnect_handler(self):
        ...
        while True:
            if self.debug:
                print(
                    f"[socket][reconnect_handler] socketDelay : {self.socketDelay}")

            if self.socketDelay >= self.socketDelayFetch and self.active:
                if self.debug:
                    print(
                        f"[socket][reconnect_handler] socketDelay >= {self.socketDelayFetch}, Reconnecting Socket")
            ...
            if not self.reconnect:
                if self.debug:
                    print(
                        f"[socket][reconnect_handler] reconnect is False, breaking")
                break
    ...
    def get_websocket_url(self):
        ...
        response = requests.get(
            "https://aminoapps.com/api/chat/web-socket-url", headers=headers)
    ...
    def close(self):
        ...
        try:
            ...
            if self.debug:
                print(
                    f"[socket][close] Error while closing Socket : {closeError}")
