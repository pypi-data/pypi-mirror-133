...
class Features:
    ...
    def getting_model_data(self, df, column_ds, column_y, country=''):
        """
        :param df: dataframe
        :param country: string
        :param column_ds: string
        :param column_y: string
        :return: dataframe
        """
        print("Preparing expected column names and getting model data...")
        columns = [column_ds, column_y]
        df_ = df[df.country == country].reset_index(drop=True) if country != '' else df
        return df_[columns] \
            .rename(columns={column_ds: 'ds', column_y: 'y'}) \
