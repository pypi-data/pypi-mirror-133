...
class MySQLDB:
    ...
    def getting_raw_data_from_mysql(self,starting_date, finishing_date, date_type, schema, metric_type, conn_mysql):
        """ Get the raw data from mysql with given parameters.
        Args:
            starting_date (str): Start from this time ex. '2020-09-01'
            finishing_date (str): Finish time ex '2021-09-01'
            date_type (str): Should be 'daily' or 'hourly'
            schema (str): Table name
            metric_type (str): A filtering parameter for 'type' column.
        ...
        if starting_date is None:
            starting_date = '2020-09-01'

        if finishing_date is None:
            if date_type == 'daily':
                finishing_date = datetime.strftime((datetime.now() - timedelta(days=1)), '%Y-%m-%d')
            else:
                finishing_date = datetime.strftime((datetime.now()), '%Y-%m-%d %H:00:00')
        ...
        query = """
                SELECT *
                FROM (
                         SELECT DATE_FORMAT(count_datetime, '{0}')  AS datetime,
                                type,
                                country,
                                SUM(count)                          AS count
                         FROM {1}
                         WHERE count_datetime >= '{2}' 
                           AND count_datetime <= '{3}' 
                           AND type = '{4}'
                         GROUP BY datetime, type, country
                         UNION
                         SELECT DATE_FORMAT(count_datetime, '{0}')  AS datetime,
                                type,
                                'ZZ'                                AS country,
        ...
        """.format(date_format, schema, starting_date, finishing_date, metric_type)
        try:
...
class File:
    ...
    def read_from_csv(file_path, filter=None):
        """
