...
class Catalog(PersistentMapping):
    ...
    def search(self, handler, secure=False, sortBy=None, sortOrder='asc',
        ...
        #            | (descending).
        #  - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        # maxResults | If specified, the search results (after potentially being
        #            | sorted) will be truncated to this number.
        #  - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        ...
        r = self.getSorted(r, sortBy, sortOrder)
        # Truncate the result if required
        if r and maxResults is not None:
            r = r[:maxResults]
        return r
