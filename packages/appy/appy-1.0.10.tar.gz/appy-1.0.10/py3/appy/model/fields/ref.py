...
E_VAL_KO    = '"emptyIndexValue" can only hold these values: %s.'
...
class Ref(Field):
    ...
    pxObjectTitle = Px('''
     <x var="navInfo=ifield.getNavInfo(initiator.o, batch.start+currentNumber,
      ...
      <x if="not selectable">::ifield.getSupTitle(initiator.o, o,
                                                  navInfo) or ''</x>
      <x>::ui.Title.get(o, mode=titleMode, nav=navInfo, target=target,
                        page=pageName, popup=popup, selectJs=selectJs)</x>
      <x if="not selectable">
       <span style=":'display:inline' if showSubTitles else 'display:none'"
     ...
     <div if="ifield.showActions"
      ...
      <x if="editable and create != 'noForm'">
       <a if="not locked"
        ...
        <img src=":svg('edit')" class="iconS" title=":_('object_edit')"/>
       </a>
      ...
      <img var="mayDeleteViaField=inPickList or ifield.delete;
        ...
        if="not locked and mayEdit and mayDeleteViaField and guard.mayDelete(o)"
     ...
     <x if="mayAdd and not inPickList">
      <form class=":inMenu and 'addFormMenu' or 'addForm'"
       ...
       <input if="not createFromSearch"
        type=":'button' if field.addConfirm or create=='noForm' else 'submit'"
        var="addLabel=_(field.addLabel);
             label=inMenu and tiedClassLabel or addLabel;
             css=ui.Button.getCss(label)" class=":css"
        style=":url(field.addIcon, bg='18px 18px',
                    ram=field.addIcon.endswith('.svg')) if addIcon else ''"
    ...
    pxSortIcons = Px('''
     <x if="changeOrder and (len(objects) &gt; 1) and \
      ...
      <img src=":svg('asc')" class="clickable iconS"
     ...
     <table if="objects" id=":collapse.id" style=":collapse.style"
      ...
      <tr if="field.showHeaders">
       <th for="column in columns" width=":column.width" align=":column.align"
        ...
        <x if="column.header">
         <img if="isCB" src=":svg('checkAll')" title=":_('check_uncheck')"
    ...
    pxList = Px('''
     <div id=":hook"
      ...
      <!-- Top controls -->
      <div if="(layout=='view' or mayAdd or mayLink) and field.showControls"
           class="topSpaceS">
       ...
       <input if="objects and field.queryable" type="button"
              ...
              value=":label" class=":css" style=":svg('search', bg='18px 18px')"
              onclick=":'goto(%s)' % 
    ...
    pxViewMenus = Px('''
     <x var2="inMenu=True">
      ...
      <div for="menu in field.getLinkedObjectsByMenu(o, objects)"
           class="inline"
           style=":'padding-right:4px' if not loop.menu.last else ''">
        ...
        <div class="dropdown" style="width:150px">
         <div for="tied in menu.objects"
              var2="batch=field.getBatchFor(str(o.iid), menu.objects);
                    tiedId=tied.iid"
              class=":'' if loop.tied.first else 'refMenuItem'">
          ...
          <x if="guard.mayAct(tied)"
             var2="ifield=field; io=o; o=tied;
                   currentNumber=0">:ifield.pxObjectActions</x>
         </div>
        </div>
       </div>
    ...
    pxFilterSelect = Px('''
     <select var="name=field.name;
      ...
      <option if="emptyVal is not None" value=":emptyVal"
    ...
    def __init__(self, class_=None, attribute=None, validator=None,
      ...
      separator=None, rowAlign='top', showControls=True, actions=None):
        ...
        # ----------------------------------------------------------------------
        # 1 | results of the first Search from the dict will be shown, instead
        ...
        #   | the list. This allows to produce alternate or partial views of
        #   | the referred objects, via searches and their related facilities,
    ...
    def initPx(self, o, req, c):
        ...
        if not c.ajaxSingle:
            r.backHook = '%d_%s'%(o.iid,r.name) if r.layout == 'cell' else None
    ...
    def checkParameters(self):
        ...
        validVals = Ref.validIndexEmptyValues
        if self.emptyIndexValue not in validVals:
            raise Exception(E_VAL_KO % ', '.join([str(v) for v in validVals]))
    ...
    def getLinkStyle(self, icon, layout, url):
        ...
        return '%s; background-position: 10%% 50%%; float: %s' % \
               (url(icon, bg='18px 18px', ram=icon.endswith('.svg')), align)
    ...
    def getAjaxDataRow(self, o, ohook, parentHook, **params):
