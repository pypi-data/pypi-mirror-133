...
class UART:
  def __init__(self, board=None, tty_name="", bus_num=0, baud_rate=9600):
...
class Board:
  ...
  def begin(self):
    ...
    if self.boardname in ["MICROBIT-V1", "MICROBIT-V2"]:
      if self.boardname == "MICROBIT-V1":
        self.microbitV1 = True
      if self.boardname == "MICROBIT-V2":
        self.microbitV2 = True
      self.boardname = "MICROBIT"
...
#          port_list_2 = [x[0:2] for x in port_list_2]
      ...
      print("[37] Burn done")
      self.detect_firmata()
    if self.boardname == "UNIHIKER": #Degug 后期需要删除
      time.sleep(0.01)
    self.board = pymata4.Pymata4(com_port=self.port, arduino_wait=2, baud_rate=115200, name = self.boardname)
  ...
  def detect_firmata(self):
    ...
    if len(res) < 3:
      ...
      minor=0
    if (major,minor) == (2,6):
      if self.boardname in ["HANDPY" , "MICROBIT", "UNIHIKER"]:
        ser.read(ser.in_waiting)
        reset_buf=bytearray(b"\xf0\x0d\x55\xf7")
        ser.write(reset_buf)
        reset = ser.read(10)
      if self.is_PinPong:
        ser.write(bytearray(b'\xf0v\x10\x00\x00\x00\x00\x00\x00\x00\xf7'))   #debug pinpong bread motor stop
        ser.write(bytearray(b'\xf0v\x10\x00\x02\x00\x00\x00\x00\x00\xf7'))
    ser.close()
