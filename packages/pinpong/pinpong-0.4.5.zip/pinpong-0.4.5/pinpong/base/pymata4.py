...
#            self.set_flag()
        ...
        try:
            ...
            if not self.ip_address and "/dev/ttyS" not in self.com_port:
...
#        print("i2c write:",data)
    ...
    def uart_readchar(self, bus_num):
        if self.dfrobot_dispatch[PrivateConstants.SUB_MESSAGE_UART_READ][bus_num] != []:
            data = self.dfrobot_dispatch[PrivateConstants.SUB_MESSAGE_UART_READ][bus_num].pop(0)
        else:
            data = None
        return data

    def uart_readline(self, bus_num):
        pass
    ...
    def uart_readinto(self, bus_num, buf):
        if self.dfrobot_dispatch[PrivateConstants.SUB_MESSAGE_UART_READ][bus_num] != []:
            for i in range(len(buf)):
                buf[i] = self.dfrobot_dispatch[PrivateConstants.SUB_MESSAGE_UART_READ][bus_num].pop(0)
        return buf
    ...
    def set_pin_mode_DS18B20(self, pin_number, callback=None):
        if callback:
            ...
            self.dfrobot_dispatch[PrivateConstants.SUB_MESSAGE_18B20][pin_number] = None
...
#        time.sleep(0.1)
    ...
    def set_pin_mode_uart(self, bus_number, baud_rate):
        ...
        data = [PrivateConstants.UART_CONFIG, bus_number,
                ...
                lo&0x7F, (lo>>7 & 0x7F), PrivateConstants.MODE_8N1]
        self._send_sysex(PrivateConstants.DFROBOT_MESSAGE, data)
        #print("pin mode uart:",data)
...
#            if value == (~temp & 0xff):
            ...
            if self.uart_mark:
                self.dfrobot_dispatch[PrivateConstants.SUB_MESSAGE_UART_READ][bus_num] = []
                self.uart_mark = False
#            num = data[2]/2
            num = data[2]
            ret_value=[]
            for i in range(num):
                self.dfrobot_dispatch[PrivateConstants.SUB_MESSAGE_UART_READ][bus_num].append(data[i+3])
#              ret_value.push(data[3+i*2] + (data[3+i*2+1]<<7))
#            self.dfrobot_dispatch[PrivateConstants.SUB_MESSAGE_UART_READ][bus_num] = ret_value
