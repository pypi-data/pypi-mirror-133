...
class ID809():
    ...
    def get_enroll_count(self):
        ...
        header = self.pack(self.CMD_TYPE, self.CMD_GET_ENROLL_COUNT, data, 4)
        self.sendPacket(header)
        if self.ISIIC == True:
            time.sleep(0.08)
        ret = self.responsePayload()
    ...
    def get_emptyID(self):
        ...
        data[0] = 1
        data[2] = self.FINGERPRINT_CAPACITY
        header = self.pack(self.CMD_TYPE, self.CMD_GET_EMPTY_ID, data, 4)
        self.sendPacket(header)
        if self.ISIIC == True:
            time.sleep(0.1)
        ret = self.responsePayload()
    ...
    def get_brokenID(self):
        ...
        data[0] = 1
        data[2] = self.FINGERPRINT_CAPACITY
        header = self.pack(self.CMD_TYPE, self.CMD_GET_BROKEN_ID, data, 4)
        self.sendPacket(header)
        if self.ISIIC == True:
            time.sleep(0.05)
        ret = self.responsePayload()
    ...
    def ctrlLED(self, mode, color, blinkCount):
        data = [0,0,0,0]
        data[0] = mode
        data[1] = color
        data[2] = color
        data[2] = blinkCount
        header = self.pack(self.CMD_TYPE, self.CMD_SLED_CTRL, data, 4)
        self.sendPacket(header)
        if self.ISIIC == True:
            time.sleep(0.05)
        ret = self.responsePayload()
    ...
    def get_enrolledID_list(self):
        header = self.pack(self.CMD_TYPE, self.CMD_GET_ENROLLED_ID_LIST, [], 0)
        self.sendPacket(header)
        if self.ISIIC == True:
            time.sleep(0.12)
        ret = self.responsePayload()
    ...
    def get_moduleSN(self):
        header = self.pack(self.CMD_TYPE, self.CMD_GET_MODULE_SN, [], 0)
        self.sendPacket(header)
        if self.ISIIC == True:
            time.sleep(0.1)
        ret = self.responsePayload()
    ...
    def get_param(self, data):
        header = self.pack(self.CMD_TYPE, self.CMD_GET_PARAM, data, 1)
        self.sendPacket(header)
        if self.ISIIC == True:
            time.sleep(0.05)
        ret = self.responsePayload()
    ...
    def set_moduleSN(self, val):
        ...
        if len(val) > self.MODULE_SN_SIZE:
            ...
            return
        header = self.pack(self.CMD_TYPE, self.CMD_SET_MODULE_SN, data, 2)
        self.sendPacket(header)
        if self.ISIIC == True:
            time.sleep(0.1)
        ret = self.responsePayload()
        ...
        for i in range(16-len(val)):
            SN.append(0)
        head = self.pack(self.DATA_TYPE, self.CMD_SET_MODULE_SN, SN, self.MODULE_SN_SIZE)
    ...
    def set_param(self, data):
        header = self.pack(self.CMD_TYPE, self.CMD_SET_PARAM, data, 5)
        self.sendPacket(header)
        if self.ISIIC == True:
            time.sleep(0.24)
        ret = self.responsePayload()
    ...
    def generate(self, RamBufferID):
        data = [0,0]
        data[0] = RamBufferID
        header = self.pack(self.CMD_TYPE, self.CMD_GENERATE, data, 2)
        self.sendPacket(header)
        if self.ISIIC == True:
            time.sleep(0.36)
        ret = self.responsePayload()
    ...
    def del_fingerprint(self, id):
        ...
        if id == self.DELALL:
            ...
            data[0] = id
            data[2] = id
        header = self.pack(self.CMD_TYPE, self.CMD_DEL_CHAR, data, 4)
        self.sendPacket(header)
        if self.ISIIC == True:
            time.sleep(0.36)
        ret = self.responsePayload()
    ...
    def getImage(self):
        header = self.pack(self.CMD_TYPE, self.CMD_GET_IMAGE, [], 0)
        self.sendPacket(header)
        if self.ISIIC == True:
            time.sleep(0.36)
        ret = self.responsePayload()
    ...
    def search(self):
        if self._state == 1:
            ...
            self._number = 0
            header = self.pack(self.CMD_TYPE, self.CMD_SEARCH, data, 6)
            self.sendPacket(header)
            if self.ISIIC == True:
                time.sleep(0.36)
            ret = self.responsePayload()
    ...
    def store_fingerprint(self, id):
        ...
        self._number = 0
        data[0] = id
        header = self.pack(self.CMD_TYPE, self.CMD_STORE_CHAR, data, 4)
        self.sendPacket(header)
        if self.ISIIC == True:
            time.sleep(0.36)
        ret = self.responsePayload()
    ...
    def merge(self):
        data = [0,0,0]
        data[2] = self._number
        header = self.pack(self.CMD_TYPE, self.CMD_MERGE, data, 3)
        self.sendPacket(header)
        if self.ISIIC ==True:
            time.sleep(0.36)
        ret = self.responsePayload()
    ...
    def detect_finger(self):
        header = self.pack(self.CMD_TYPE, self.CMD_FINGER_DETECT, [], 0)
        self.sendPacket(header)
        if self.ISIIC == True:
            time.sleep(0.24)
        ret = self.responsePayload()
    ...
    def isConnected(self):
        header = self.pack(self.CMD_TYPE, self.CMD_TEST_CONNECTION, 0, 0)
        self.sendPacket(header)
        if self.ISIIC == True:
            time.sleep(0.05)
        ret = self.responsePayload()
    ...
    def responsePayload(self , val=[]):
        ...
        if ret != self.ERR_SUCCESS:
            ...
            ret = self.ERR_ID809
        elif self.getRcmCKS(packet) != cks:
    ...
    def readPrefix(self, header):
        ...
        header[2] = self.readN(1)[0]
        header[3] = self.readN(1)[0]
        RCM = self.readN(2)
        LEN = self.readN(2)
        RET = self.readN(2)
        header[4] = RCM[0]
    ...
    def readN(self, lens):
        return self.read_to_addr(lens)


    def sendPacket(self, pack):
        while 0xee != self.read_to_addr(1)[0]:
            time.sleep(0.01)
        self.write_to_addr(pack)

    def pack(self, type, cmd, payload, lens):
