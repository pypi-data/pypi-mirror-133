...
class QPrinter(PySide2.QtGui.QPagedPaintDevice):
    ...
    **See also** **QPrintDialog**  and **Qt Print Support** .
    ...
    def setPageMargins(self, margins: PySide2.QtCore.QMarginsF) -> bool:
        """
        https://doc.qt.io/qt-5/qprinter.html#setPageMargins-1

        **bool QPrinter::setPageMargins(const QMarginsF & margins ,
        QPageLayout::Unit units )**

        Set the page margins to **margins** in the given **units**. If **units**
        are not provided then the current units are used.

        If in Full Page mode then no check is performed on the **margins** set,
        otherwise the **margins** must fall within the printable area for the
        page size on the current printer.
    ...
    def setPageSize(self, arg__1: PySide2.QtGui.QPageSize) -> bool:
        ...
        **bool QPrinter::setPageSize(const QPageSize & pageSize )**
        ...
        If the **pageSize** is not valid for the current printer then the page
        size will not be changed.

        Changing the page size may affect the current page margins if they fall
        outside the printable margins for the new page size on the current
        printer.
