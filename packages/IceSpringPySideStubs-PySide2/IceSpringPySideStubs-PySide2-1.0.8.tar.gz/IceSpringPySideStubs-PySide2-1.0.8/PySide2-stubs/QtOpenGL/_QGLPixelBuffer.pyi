...
class QGLPixelBuffer(PySide2.QtGui.QPaintDevice):
    ...
    **Detailed Description**
    ...
    Rendering into a pbuffer is normally done using full hardware acceleration.
    This can be significantly faster than rendering into a **QPixmap** .

    There are three approaches to using this class:

    1.  **We can draw into the pbuffer and convert it to a**QImage**  using
    **toImage** ().** This is normally much faster than calling
    **QGLWidget::renderPixmap** ().
      2. **We can draw into the pbuffer and copy
    the contents into an OpenGL texture using**updateDynamicTexture** ().** This
    allows us to create dynamic textures and works on all systems with pbuffer
    support.
      3. **On systems that support it, we can bind the pbuffer to an
    OpenGL texture.** The texture is then updated automatically when the pbuffer
    contents change, eliminating the need for additional copy operations. This
    is supported only on Windows and macOS systems that provide the
    `render_texture` extension. Note that under Windows, a multi-sampled pbuffer
    can't be used in conjunction with the `render_texture` extension. If a
    multi-sampled pbuffer is requested under Windows, the `render_texture`
    extension is turned off for that pbuffer.

    **Note:** This class has been deprecated, use **QOpenGLFramebufferObject**
    for offscreen rendering.

    **Threading**

    As of Qt 4.8, it's possible to render into a QGLPixelBuffer using a
    **QPainter**  in a separate thread. Note that OpenGL 2.0 or OpenGL ES 2.0 is
    required for this to work.

    Pbuffers are provided by the OpenGL `pbuffer` extension; call
    ...
    def __init__(
        ...
        **QGLPixelBuffer::QGLPixelBuffer(const QSize & size , const QGLFormat &
        format = QGLFormat::defaultFormat(), QGLWidget * shareWidget =
        nullptr)**

        Constructs an OpenGL pbuffer of the given **size**. If no **format** is
        specified, the **default format**  is used. If the **shareWidget**
        parameter points to a valid QGLWidget, the pbuffer will share its
        context with **shareWidget**.

        If you intend to bind this pbuffer as a dynamic texture, the width and
        height components of `size` must be powers of two (e.g., 512 x 128).
    ...
    def __init__(
        ...
        """
        https://doc.qt.io/qt-5/qglpixelbuffer.html#QGLPixelBuffer-1

        **QGLPixelBuffer::QGLPixelBuffer(int width , int height , const
        QGLFormat & format = QGLFormat::defaultFormat(), QGLWidget * shareWidget
        = nullptr)**

        This is an overloaded function.

        Constructs an OpenGL pbuffer with the **width** and **height**. If no
        **format** is specified, the **default format**  is used. If the
        **shareWidget** parameter points to a valid QGLWidget, the pbuffer will
        share its context with **shareWidget**.
    ...
    def bindTexture(self, fileName: str) -> int:
        ...
        **GLuint QGLPixelBuffer::bindTexture(const QImage & image , GLenum
    ...
    def bindTexture(self, image: PySide2.QtGui.QImage, target: int = ...) -> int:
        """
        https://doc.qt.io/qt-5/qglpixelbuffer.html#bindTexture-1

        **GLuint QGLPixelBuffer::bindTexture(const QPixmap & pixmap , GLenum
        target = GL_TEXTURE_2D)**
    ...
    def bindTexture(self, pixmap: PySide2.QtGui.QPixmap, target: int = ...) -> int:
        """
        https://doc.qt.io/qt-5/qglpixelbuffer.html#bindTexture-2

        **GLuint QGLPixelBuffer::bindTexture(const QString & fileName )**

        This is an overloaded function.

        Reads the DirectDrawSurface (DDS) compressed file **fileName** and
    ...
    def bindToDynamicTexture(self, texture: int) -> bool:
        ...
        **bool QGLPixelBuffer::bindToDynamicTexture(GLuint texture_id )**
        ...
        Binds the texture specified by **texture_id** to this pbuffer. Returns
        `true` on success; otherwise returns `false`.

        The texture must be of the same size and format as the pbuffer.

        To unbind the texture, call **releaseFromDynamicTexture** (). While the
        texture is bound, it is updated automatically when the pbuffer contents
        change, eliminating the need for additional copy operations.
        ...
        **Warning:** This function uses the `render_texture` extension, which is
        currently not supported under X11. An alternative that works on all
        systems (including X11) is to manually copy the pbuffer contents to a
        texture using **updateDynamicTexture** ().

        **Warning:** For the bindToDynamicTexture() call to succeed on the
        macOS, the pbuffer needs a shared context, i.e. the **QGLPixelBuffer**
        must be created with a share widget.

        **See also** **generateDynamicTexture** () and
    ...
    def doneCurrent(self) -> bool:
        ...
        **bool QGLPixelBuffer::doneCurrent()**
        ...
        Makes no context the current OpenGL context. Returns `true` on success;
    ...
    def drawTexture(
        ...
        **void QGLPixelBuffer::drawTexture(const QRectF & target , GLuint
    ...
    def drawTexture(
        ...
        """
        https://doc.qt.io/qt-5/qglpixelbuffer.html#drawTexture-1

        **void QGLPixelBuffer::drawTexture(const QPointF & point , GLuint
        textureId , GLenum textureTarget = GL_TEXTURE_2D)**

        Draws the given texture, **textureId** , at the given **point** in
        OpenGL model space. The textureTarget parameter should be a 2D texture
        target.
    ...
    def format(self) -> PySide2.QtOpenGL.QGLFormat:
        ...
        **QGLFormat QGLPixelBuffer::format() const**
        ...
        Returns the format of the pbuffer. The format may be different from the
    ...
    def generateDynamicTexture(self) -> int:
        ...
        **GLuint QGLPixelBuffer::generateDynamicTexture() const**
        ...
        pbuffer, and returns the texture's ID. This can be used in conjunction
        with **bindToDynamicTexture** () and **updateDynamicTexture** ().
    ...
    def hasOpenGLPbuffers() -> bool:
        ...
        **[static] bool QGLPixelBuffer::hasOpenGLPbuffers()**
        ...
        Returns `true` if the OpenGL `pbuffer` extension is present on this
    ...
    def makeCurrent(self) -> bool:
        ...
        **bool QGLPixelBuffer::makeCurrent()**
        ...
        Makes this pbuffer the current OpenGL rendering context. Returns true on
    ...
    def updateDynamicTexture(self, texture_id: int) -> None:
        ...
        **QGLPixelBuffer**  pbuffer(...);
        ...
        `render_texture` extension is to use **bindToDynamicTexture** () to get
        dynamic updates of the texture.
