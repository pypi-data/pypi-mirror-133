...
class QOpenGLWidget(PySide2.QtWidgets.QWidget):
    ...
    **Relation to QGLWidget**
    ...
    Due to being backed by a framebuffer object, the behavior of QOpenGLWidget is very similar to **QOpenGLWindow**  with the update behavior set to `PartialUpdateBlit` or `PartialUpdateBlend`. This means that the contents are preserved between **paintGL** () calls so that incremental rendering is possible. With QGLWidget (and naturally **QOpenGLWindow**  with the default update behavior) this is usually not the case because swapping the buffers leaves the back buffer with undefined contents.

    **Note:** Most applications do not need incremental rendering because they will render everything in the view on every paint call. In this case it is important to call glClear() as early as possible in **paintGL** (). This helps mobile GPUs that use a tile-based architecture to recognize that the tile buffer does not need to be reloaded with the framebuffer\'s previous contents. Omitting the clear call can lead to significant performance drops on such systems.
    ...
    **Differences to QGLWidget**
    ...
    Besides the main conceptual difference of being backed by a framebuffer object, there are a number of smaller, internal differences between QOpenGLWidget and the older QGLWidget:

    * OpenGL state when invoking **paintGL** (). QOpenGLWidget sets up the viewport via glViewport(). It does not perform any clearing.
      * Clearing when starting to paint via **QPainter** . Unlike regular widgets, QGLWidget defaulted to a value of `true` for **autoFillBackground** . It then performed clearing to the palette\'s background color every time **QPainter::begin** () was used. QOpenGLWidget does not follow this: **autoFillBackground**  defaults to false, like for any other widget. The only exception is when being used as a viewport for other widgets like **QGraphicsView** . In such a case **autoFillBackground**  will be automatically set to true to ensure compatibility with QGLWidget-based viewports.
