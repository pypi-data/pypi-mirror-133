    ...
    **See also** **QSqlDatabase** , **QSqlQueryModel** , **QSqlTableModel** , and **QVariant** .
    ...
    def isNull(self, field: int) -> bool:
        ...
        **bool QSqlQuery::isNull(int field ) const**
        ...
        Returns `true` if the query is not **active** , the query is not positioned on a valid record, there is no such **field** , or the **field** is null; otherwise `false`. Note that for some drivers, isNull() will not return accurate information until after an attempt is made to retrieve data.
    ...
    def prepare(self, query: str) -> bool:
        ...
        **bool QSqlQuery::prepare(const QString & query )**
        ...
        Portability notes: Some databases choose to delay preparing a query until it is executed the first time. In this case, preparing a syntactically wrong query succeeds, but every consecutive **exec** () will fail. When the database does not support named placeholders directly, the placeholder can only contain characters in the range [a-zA-Z0-9**_** ].
