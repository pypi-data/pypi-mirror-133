...
class QRegExp(object):
    ...
    **Detailed Description**
    ...
    Search and ReplaceA regexp can replace all occurrences of a substring with a different substring, e.g., replace all occurrences of **&** with **& amp;** except where the **&** is already followed by an **amp;**.
    ...
    ****E** {n,m}**Matches at least **n** and at most **m** occurrences of **E**.
    ...
    Note: Quantifiers are normally "greedy". They always match as much text as they can. For example, **0+** matches the first zero it finds and all the consecutive zeros after the first zero. Applied to \'20005\', it matches \'2 **000** 5\'. Quantifiers can be made non-greedy, see **setMinimal** ().
    ...
    **\\B** A non-word boundary. This assertion is true wherever **\\b** is false. For example if we searched for **\\Bon\\B** in "Left on" the match would fail (space and end of string aren\'t non-word boundaries), but it would match in "t **on** ne".
