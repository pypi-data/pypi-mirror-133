...
class QQuickItem(PySide2.QtCore.QObject, PySide2.QtQml.QQmlParserStatus):
    ...
    * The scene graph is invalidated; This can happen, for instance, if the
    window is hidden using **QQuickWindow::hide** (). If the item class
    ...
    to connecting to **QQuickWindow::sceneGraphInvalidated** (). The OpenGL
    context of this item\'s window will be bound when this slot is called. The
    only exception is if the native OpenGL has been destroyed outside Qt\'s
    control, for instance through `EGL_CONTEXT_LOST`.
      * The item is removed
    from the scene; If an item is taken out of the scene, for instance because
    it\'s parent was set to `null` or an item in another window, the
    **QQuickItem::releaseResources** () will be called on the GUI thread.
    **QQuickWindow::scheduleRenderJob** () should be used to schedule cleanup of
    rendering resources.
      * The item is deleted; When the destructor if an item
    runs, it should delete any graphics resources it has. If neither of the two
    conditions above were already met, the item will be part of a window and it
    is possible to use **QQuickWindow::scheduleRenderJob** () to have them
    cleaned up. If an implementation ignores the call to
    ...
    **Note:** Use of **QObject::deleteLater** () to clean up graphics resources
    is not recommended as this will run at an arbitrary time and it is unknown
    if there will be an OpenGL context bound when the deletion takes place.
    ...
    **Warning:** Using **QQuickPaintedItem**  uses an indirect 2D surface to
    render its content, either using software rasterization or using an OpenGL
    framebuffer object (FBO), so the rendering is a two-step operation. First
    rasterize the surface, then draw the surface. Using scene graph API directly
    is always significantly faster.
    ...
    **See also** **QQuickWindow**  and **QQuickPaintedItem** .
    ...
    def childMouseEventFilter(
        ...
        * item , QEvent * event )**
        ...
        Return true if the specified **event** should not be passed onto the
    ...
    def contains(self, point: PySide2.QtCore.QPointF) -> bool:
        ...
        **[virtual] bool QQuickItem::contains(const QPointF & point ) const**
        ...
        This function can be overwritten in order to handle point collisions in
        items with custom shapes. The default implementation checks if the point
        is inside the item's bounding rect.

        Note that this method is generally used to check whether the item is
        under the mouse cursor, and for that reason, the implementation of this
        function should be as light-weight as possible.
    ...
    def filtersChildMouseEvents(self) -> bool:
        ...
        **bool QQuickItem::filtersChildMouseEvents() const**
        ...
        Returns whether mouse and touch events of this item's children should be
    ...
    def forceActiveFocus(self, reason: PySide2.QtCore.Qt.FocusReason) -> None:
        """
        https://doc.qt.io/qt-5/qquickitem.html#forceActiveFocus-1
    ...
    def geometryChanged(
        ...
        **[virtual protected] void QQuickItem::geometryChanged(const QRectF &
        newGeometry , const QRectF & oldGeometry )**
    ...
    def grabMouse(self) -> None:
        ...
        **void QQuickItem::grabMouse()**
        ...
        This item will receive all mouse events until **ungrabMouse** () is
        called. Usually this function should not be called, since accepting for
        example a mouse press event makes sure that the following events are
        delivered to the item. If an item wants to take over mouse events from
        the current receiver, it needs to call this function.
    ...
    def grabTouchPoints(self, ids: typing.List) -> None:
        ...
        **void QQuickItem::grabTouchPoints(const QVector<int> & ids )**
        ...
        These touch points will be owned by the item until they are released.
        Alternatively, the grab can be stolen by a filtering item like
        Flickable. Use **setKeepTouchGrab** () to prevent the grab from being
        stolen.
    ...
    def setFiltersChildMouseEvents(self, filter: bool) -> None:
        ...
        **void QQuickItem::setFiltersChildMouseEvents(bool filter )**
        ...
        Sets whether mouse and touch events of this item's children should be
        filtered through this item.
    ...
    def ungrabMouse(self) -> None:
        ...
        **void QQuickItem::ungrabMouse()**
        ...
        Note that this function should only be called when the item wants to
        stop handling further events. There is no need to call this function
    ...
    def ungrabTouchPoints(self) -> None:
        ...
        **Note:** there is hardly any reason to call this function. It should
        only be called when an item does not want to receive any further events,
        so no move or release events will be delivered after calling this
        function.
    ...
    def updatePaintNode(
        ...
        **See also** **QSGMaterial** , **QSGSimpleMaterial** ,
        **QSGGeometryNode** , **QSGGeometry** , **QSGFlatColorMaterial** ,
        **QSGTextureMaterial** , **QSGNode::markDirty** (), and **Graphics
        Resource Handling** .
        """
        ...
