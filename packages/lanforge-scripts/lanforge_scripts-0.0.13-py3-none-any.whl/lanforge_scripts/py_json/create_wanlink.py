...
def main(args):
    ...
    lf_r = LFRequest(base_url+"/wl/list")
    print(lf_r.get_as_json())

    # remove old wanlinks
    if num_wanlinks > 0:
        lf_r = LFRequest(base_url+"/cli-json/rm_cx")
        lf_r.addPostData({
         'test_mgr': 'all',
         'cx_name': args['name']
        })
    ...
    try:
        ...
        lf_r = LFRequest(base_url+"/cli-json/rm_endp")
        lf_r.addPostData({
    ...
    lf_r = LFRequest(base_url+"/cli-json/add_wl_endp")
    lf_r.addPostData({
        'alias': args['name']+"-A",
        'shelf': 1,
        'resource': '1',
    ...
    # create cx
    lf_r = LFRequest(base_url+"/cli-json/add_cx")
    lf_r.addPostData({
       'alias': args['name'],
       'test_mgr': 'default_tm',
       'tx_endp': args['name']+"-A",
    ...
    lf_r = LFRequest(base_url+"/cli-json/set_wanlink_info")
    lf_r.addPostData({
        'name': args['name']+"-A",
    ...
    while seen < 1:
        sleep(1)
        lf_r = LFRequest(base_url+"/wl/"+args['name']+"?fields=name,state,_links")
        try:
            json_response = lf_r.getAsJson()
    ...
    # lf_r = LFRequest(base_url+"/cli-json/set_cx_state")
    # lf_r.addPostData({
    #    'test_mgr': 'all',
    #    'cx_name': args['name'],
    #    'cx_state': 'RUNNING'
