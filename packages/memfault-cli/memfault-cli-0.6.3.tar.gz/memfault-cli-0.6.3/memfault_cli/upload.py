...
class McuSdkElfSymbolUploader(Uploader):
    def can_upload_file(self) -> bool:
        with open(self.file_path, "rb") as file:
            ...
            if not self.ctx.software_info and mcu_build_id is None:
                LOG.error(
                    f"{self.file_path}: Build Id missing. Specify --software-version and --software-type options "
                    f"or add a Build Id (see https://mflt.io/symbol-file-build-ids)"
                )
                return False

        LOG.info(f"{self.file_path}: ELF file with .debug_info. Build Id: {mcu_build_id}")
        return True
