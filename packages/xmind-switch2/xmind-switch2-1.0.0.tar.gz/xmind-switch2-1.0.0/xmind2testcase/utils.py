...
def get_xmind_testsuites(xmind_file):
    ...
    workbook = xmind.load(xmind_file)   # 加载xmind文件
    xmind_content_dict = workbook.getData()  # 获取xmind画布数据
...
def get_xmind_testsuite_list(xmind_file):
    ...
    testsuite_list = get_xmind_testsuites(xmind_file)  # 获取test suites
    suite_data_list = []
...
def get_xmind_testcase_list(xmind_file):
    ...
    """
    xmind_file = get_absolute_path(xmind_file)   # 获取xmind 绝对路径
    logging.info('Start converting XMind file(%s) to testcases dict data...', xmind_file)   # 日志打印，xmind转换测试用例
    testsuites = get_xmind_testsuites(xmind_file)   # 获取xmind的testsuites(测试集合)
    testcases = []

    for testsuite in testsuites:
        product = testsuite.name
        version_mark = re.compile(r'[Vv]\d+\.\d+\.\d+').findall(product)  # 从主题中查找Vx.x.x格式的子字符串
        for suite in testsuite.sub_suites:
            for case in suite.testcase_list:
                ...
                if version_mark != []:
                    case_data['version'] = version_mark[0]
                # print(version_mark[0])
