	...
	def tlvRead(self,disp,df = None):
		...
		while True:
			try:
				...
				return self.list2df(self.v1df,self.v2df,self.v3df,self.v4df) if (df == 'DataFrame') else (self.v1,self.v2,self.v3,self.v4)
			#print(str(ch))
			if lstate == 'idle':
				...
				if idx == 32:   
					...
					try: 
						...
						return self.list2df(self.v1,self.v2,self.v3,self.v4) if (df == 'DataFrame') else (self.v1,self.v2,self.v3,self.v4)
				...
				if (idx%self.tlvLength == 0):
					...
					try:
						...
						if typeCnt != self.hdr.numTLVs:
							if self.dbg == True:
								print("({:}) -> (TL): typeCnt:{:}".format(lstate,typeCnt))
							lstate = 'TL'
						else:
							if self.dbg == True:
	...
	def typeFunc(self,typeT = None ,vUnit= None,objs= None,ldata = None, df = None):
		...
		if typeT == 'V1':
			for i in range(objs):
				...
				(dt,pt,xt,yt,zt) = struct.unpack('5h', ldata[start:end])
				d = dt * vUnit
				p = pt * vUnit
				x = xt * vUnit
				y = yt * vUnit
		...
		if typeT == 'V2':
			for i in range(objs):
				...
				(ccXt,ccYt,csXt,csYt) = struct.unpack('4h', ldata[start:end]) 
				ccX  =  vUnit * ccXt
				ccY  =  vUnit * ccYt
				csX  =  vUnit * csXt
				csY  =  vUnit * csYt
		...
		if typeT == 'V3':
			for i in range(objs):
				...
				(Xt,Yt,XDt,YDt,Xsizet,Ysizet) = struct.unpack('6h', ldata[start:end]) 
				X 	  =  vUnit * Xt
				Y     =  vUnit * Yt
				XD    =  vUnit * XDt
				YD    =  vUnit * YDt
