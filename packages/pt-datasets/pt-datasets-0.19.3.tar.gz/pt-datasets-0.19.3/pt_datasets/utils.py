...
def vectorize_text(
    ...
    assert vectorizer in supported_vectorizers or (
        isinstance(vectorizer, TfidfVectorizer)
        or isinstance(vectorizer, CountVectorizer)
    ), f"{vectorizer} is not supported."
...
def download_glove(dim: int) -> None:
    """
    Downloads the Magnitude file for GloVe embeddings.

    Parameters
    ----------
    dim: int
        The dimensionality of the GloVe to download.
    """
    supported_dims = (50, 100, 300)
    embeddings_meta = {
        50: (
        ...
        100: (
            "http://magnitude.plasticity.ai/glove/light/glove.6B.100d.magnitude",
            "glove.6B.100d.magnitude",
        ),
        300: (
            "http://magnitude.plasticity.ai/glove/light/glove.6B.300d.magnitude",
            "glove.6B.300d.magnitude",
        ),
    }
    assert dim in supported_dims
    path = os.path.join(str(Path.home()), ".magnitude")
    if not os.path.exists(path):
        os.mkdir(path)
    url = embeddings_meta.get(dim)[0]
    filename = embeddings_meta.get(dim)[1]
    download_script = f"wget --no-clobber -O {os.path.join(path, filename)} {url}"
    os.system(download_script)


def download_agnews_glove() -> None:
    agnews_300d_train = (
        "ag_news_train_300d.npy",
        "https://drive.google.com/uc?id=1KntNWoQrOWwO7Ld-SlnRsc1xLu-CIkRb",
    )
    agnews_300d_test = (
        "ag_news_test_300d.npy",
        "https://drive.google.com/uc?id=1KU2tDJ56osj_u0NosUrU2Mx19McF9hhO",
    )
    path = str(Path.home())
    path = os.path.join(path, "datasets")

    def download(filename: str, link: str) -> None:
        print(f"[INFO] Downloading {filename}...")
        download_script = f"""
        gdown {link} -O {filename}
        """
        os.system(download_script)

    download(os.path.join(path, agnews_300d_train[0]), agnews_300d_train[1])
    download(os.path.join(path, agnews_300d_test[0]), agnews_300d_test[1])
