...
def create_snapshot(
    ...
    if path != None:
        id_ = qumulo.rest.fs.get_file_attr(conninfo, credentials, path=path).lookup('file_number')
    if id_ != None:
...
def get_snapshot_tree_diff(
    ...
    method = 'GET'
    uri = UriBuilder(path='/v2/snapshots/{:d}/changes-since/{:d}'.format(newer_snap, older_snap))
...
def get_snapshot_file_diff_uri(
    ...
    return UriBuilder(
        path='/v2/snapshots/{:d}/changes-since/{:d}/files'.format(newer_snap, older_snap)
