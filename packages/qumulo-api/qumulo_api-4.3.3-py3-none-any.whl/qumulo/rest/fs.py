...
def read_entire_directory_and_ignore_not_found(
    ...
    try:
        for result in read_entire_directory(conninfo, credentials, page_size, path, id_, snapshot):
            yield result
            ...
            if 'files' in result.data:
                for f in result.data['files']:
                    ...
                    if f['type'] == 'FS_FILE_TYPE_DIRECTORY':
                        for ff in call_read_dir(conninfo, credentials, f['id'], max_depth):
                            yield ff

    result = get_file_attr(conninfo, credentials, path=path, snapshot=snapshot)
    yield result

    for f in call_read_dir(conninfo, credentials, result.data['id'], max_depth):
        yield f
    ...
    def call_read_dir(conninfo, credentials, id_):
        for result in read_entire_directory_and_ignore_not_found(
            ...
            if 'files' in result.data:
                for f in result.data['files']:
                    if f['type'] == 'FS_FILE_TYPE_DIRECTORY':
                        for ff in call_read_dir(conninfo, credentials, f['id']):
                            yield ff
                    yield request.RestResponse(f, result.etag)

    result = get_file_attr(conninfo, credentials, path=path, snapshot=snapshot)

    for f in call_read_dir(conninfo, credentials, result.data['id']):
        yield f
