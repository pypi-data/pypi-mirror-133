...
PRETTY_RIGHT_EXPLANATIONS = sorted(
    [(API_RIGHTS_TO_CLI[k], v) for k, v in RIGHT_EXPLANATIONS.items()]
)

PRETTY_RIGHTS_TABLE = sorted(
    [
        (k, sorted([API_RIGHTS_TO_CLI[v] for v in SHORTHAND_RIGHTS_TO_API[k]]))
        for k, v in SHORTHAND_RIGHTS_TO_API.items()
    ]
)
