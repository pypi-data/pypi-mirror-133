    ...
    def main(conninfo: Connection, credentials: Credentials, args: Namespace) -> None:
        ...
        try:
            ...
            if args.overwrite:
                ...
                raise ValueError(
                    'Target file {} exists. Use --overwrite ' 'to overwrite it.'.format(args.target)
                )
        ...
        with tqdm(
            ...
            for i, source_stream in enumerate(source_named_streams):
                copy_progress.set_description('Copying stream {}/{}'.format(2 + i, total_streams))
...
class ListWaitersByFileCommand(qumulo.lib.opts.Subcommand):
    ...
    def main(conninfo: Connection, credentials: Credentials, args: Namespace) -> None:
        if (args.protocol, args.lock_type) not in fs.VALID_WAITER_PROTO_TYPE_COMBINATIONS:
            raise ValueError(
                'Lock type {} not available for protocol {}'.format(args.lock_type, args.protocol)
            )
