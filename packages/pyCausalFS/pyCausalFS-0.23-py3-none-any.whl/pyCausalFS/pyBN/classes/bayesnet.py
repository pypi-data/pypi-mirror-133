...
import numpy as np
from pyCausalFS.pyBN.utils.class_equivalence import are_class_equivalent
from pyCausalFS.pyBN.utils.graph import topsort
...
class BayesNet(object):
    ...
    def flat_cpt(self, by_var=False, by_parents=False):
        ...
        if by_var:
            ...
            cpt = np.array([sum(self.cpt(rv)[i:(i + self.card(rv))]) for rv in self.nodes() for i in
                            range(len(self.cpt(rv)) / self.card(rv))])
        else:
