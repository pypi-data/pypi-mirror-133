...
from .confluent_configs import get_kafka_configs, init_consumer_configs, init_transactional_consumer_configs
...
def get_transactional_consumer(topics, schema_registry, default_schema=None, auto_subscribe=True, cluster=None):
    if isinstance(topics, str):
        topics = topics.split(',')
    LOGGER.debug('Setting up Kafka Transactional Consumer')
    consumer = DeserializingConsumer(init_transactional_consumer_configs(topics, schema_registry, get_kafka_configs()[0], cluster, default_schema))
    if auto_subscribe:
        consumer.subscribe(topics)  # in case multiple topics are read from
...
def poll_for_message(consumer, timeout=None):
    ...
    """
    message = consumer.poll(timeout if timeout else int(env_vars()['CONSUMER_POLL_TIMEOUT']))
    if message is None:
