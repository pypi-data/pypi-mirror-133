...
from .confluent_configs import get_kafka_configs, init_producer_configs, init_transactional_producer_configs, producer_serializers
...
def get_transactional_producer(topic_schema_dict, schema_registry, cluster=None):
    LOGGER.debug('Setting up Kafka Transactional Producer')
    producer = SerializingProducer(init_transactional_producer_configs(topic_schema_dict, schema_registry, get_kafka_configs()[0], cluster))
    LOGGER.debug('Producer setup complete.')
...
def produce_message(producer, producer_kwargs: dict, metrics_manager: MetricsManager = None,
    ...
    # single serialized producer management
    if hasattr(producer, 'schema_dict'):
